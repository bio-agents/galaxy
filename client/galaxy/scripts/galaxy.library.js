...
define([
  ...
  "mvc/library/library-libraryagentbar-view",
  "mvc/library/library-folderagentbar-view",
  ...
  function(mod_masthead,
    ...
    mod_libraryagentbar_view,
    mod_folderagentbar_view,
...
var GalaxyLibrary = Backbone.View.extend({
    ...
    libraryAgentbarView: null,
    libraryListView: null,
    library_router: null,
    libraryView: null,
    folderAgentbarView: null,
    folderListView: null,
    datasetView: null,
    ...
    initialize : function(){
      ...
      (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
        ...
        this.library_router.on( 'route:libraries', function() {
            if ( Galaxy.libraries.libraryAgentbarView){
                Galaxy.libraries.libraryAgentbarView.$el.unbind('click');
            }
            Galaxy.libraries.libraryAgentbarView = new mod_libraryagentbar_view.LibraryAgentbarView();
        ...
        this.library_router.on('route:libraries_page', function( show_page ) {
            if ( Galaxy.libraries.libraryAgentbarView === null ){
                Galaxy.libraries.libraryAgentbarView = new mod_libraryagentbar_view.LibraryAgentbarView();
        ...
        this.library_router.on( 'route:folder_content', function( id ) {
            if (Galaxy.libraries.folderAgentbarView){
                Galaxy.libraries.folderAgentbarView.$el.unbind( 'click' );
            }
            Galaxy.libraries.folderAgentbarView = new mod_folderagentbar_view.FolderAgentbarView( { id: id } );
        ...
        this.library_router.on( 'route:folder_page', function( id, show_page ) {
            if ( Galaxy.libraries.folderAgentbarView === null ){
                Galaxy.libraries.folderAgentbarView = new mod_folderagentbar_view.FolderAgentbarView( {id: id} );
        ...
        this.library_router.on( 'route:download', function( folder_id, format ) {
            if ( $( '#folder_list_body' ).find( ':checked' ).length === 0 ) {
                ...
                Galaxy.libraries.folderAgentbarView.download( folder_id, format );
        ...
        this.library_router.on( 'route:import_datasets', function( folder_id, source ){
            if ( Galaxy.libraries.folderAgentbarView && Galaxy.libraries.folderListView ){
                Galaxy.libraries.folderAgentbarView.showImportModal( { source:source } );
            } else {
                Galaxy.libraries.folderAgentbarView = new mod_folderagentbar_view.FolderAgentbarView( { id: folder_id } );
