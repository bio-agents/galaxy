...
TODO:
    ...
    make job data human readable (needs agent data)
        show only agent.inputs with labels (w/ job.params as values)
        input datasets are special
            they don't appear in job.params
            have to connect to datasets in the dag
                connect job.inputs to any agent.inputs by agent.input.name (in params)
...
//TODO: hack
    ...
    _createJobListItem : function( vertex ){
        ...
        var li = new view.JobItemClass({ model: job, agent: jobData.agent, jobData: jobData });
...
var HistoryStructureView = Backbone.View.extend( BASE_MVC.LoggableMixin ).extend({
    ...
    initialize : function( attributes ){
        ...
        this._processAgents( attributes.agents );
        this._processJobs( attributes.jobs );
        this._createDAG();
    },

    _processAgents : function( agents ){
        this.agents = agents || {};
        return this.agents;
    },
