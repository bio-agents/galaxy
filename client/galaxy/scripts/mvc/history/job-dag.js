...
//TODO: save these?
    ...
    self.dataKeys = [ 'jobs', 'historyContents', 'agents' ];
...
JobDAG.prototype._initFilters = function __initFilters(){
    ...
    if( self.options.excludeSetMetadata ){
        ...
        filters.push( function filterSetMetadata( jobData ){
            if( jobData.job.agent_id !== '__SET_METADATA__' ){ return true; }
...
JobDAG.prototype.read = function _read( data ){
    var self = this;
    if( _.has( data, 'historyContents' ) && _.has( data, 'jobs' ) && _.has( data, 'agents' ) ){
        // a job dag is composed of these three elements:
        //  clone the 3 data sources into the DAG, processing the jobs finally using the history and agents
        self.preprocessHistoryContents( data.historyContents || [] )
            .preprocessAgents( data.agents || {} )
...
/**  */
JobDAG.prototype.preprocessAgents = function _preprocessAgents( agents ){
    this.info( 'processing agents' );
    var self = this;
    self._agentMap = {};

    _.each( agents, function( agent, id ){
        self._agentMap[ id ] = _.clone( agent );
    });
    return self;
};

/** sort the cloned jobs, decorate with agent and history contents info, and store in prop array */
...
/** decorate with input/output datasets and agent */
JobDAG.prototype.preprocessJob = function _preprocessJob( job, index ){
    ...
    jobData.agent = self._agentMap[ job.agent_id ];
