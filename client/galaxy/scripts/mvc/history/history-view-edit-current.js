...
/** @lends CurrentHistoryView.prototype */{
    ...
    emptyMsg            : _l( "This history is empty. Click 'Get Data' on the left agent menu to start" ),
    ...
    /** In this override, add links to open data uploader or get data in the agents section */
    _renderEmptyMessage : function( $whereTo ){
        ...
        &&  ( Galaxy && Galaxy.upload && $agentMenu.size() ) ){
            $emptyMsg.empty();
            ...
            $emptyMsg.find( '.get-data-link' ).click( function( ev ){
                $agentMenu.parent().scrollTop( 0 );
                $agentMenu.find( 'span:contains("Get Data")' )
                    .click();
        ...
        this.listenTo( galaxy, 'galaxy_main:load', function( data ){
            var pathToMatch = data.fullpath,
                useToURLRegexMap = {
                    ...
                    'rerun'         : /agent_runner\/rerun\?id=([a-f0-9]+)/,
...
CurrentHistoryView.prototype.templates = (function(){
    ...
    var quotaMsgTemplate = BASE_MVC.wrapTemplate([
        '<div class="quota-message errormessage">',
            ...
            _l( 'Agent execution is on hold until your disk usage drops below your allocated quota' ), '.',
        '</div>'
    ], 'history' );
