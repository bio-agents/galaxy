...
var JobListItemView = _super.extend(/** @lends JobListItemView.prototype */{
    ...
    /** Return agent.inputs that should/can be safely displayed */
    _labelParamMap : function(){
        //ADAPTER
        var params = this.model.get( 'params' ),
            labelParamMap = {};
        _.each( this.agent.inputs, function( i ){
            //console.debug( i.label, i.model_class );
            if( i.label && i.model_class !== 'DataAgentParameter' ){
    ...
    _labelInputMap : function(){
        ...
        _.each( this.jobData.inputs, function( input ){
            var agentInput = view._findAgentInput( input.name );
            if( agentInput ){
                labelInputMap[ agentInput.label ] = input;
            }
        });
        return labelInputMap;
    },

    /** Return a agent.inputs object that matches (or partially matches) the given (job input) name */
    _findAgentInput : function( name ){
        //ADAPTER
        var agentInputs = this.agent.inputs,
            exactMatch = _.findWhere( agentInputs, { name : name });
        if( exactMatch ){ return exactMatch; }
        return this._findRepeatAgentInput( name, agentInputs );
    },

    /** Return a agent.inputs object that partially matches the given (job input) name (for repeat dataset inputs)*/
    _findRepeatAgentInput : function( name, agentInputs ){
        //ADAPTER
        agentInputs = agentInputs || this.agent.inputs;
        var partialMatch = _.find( agentInputs, function( i ){
...
//TODO: move to require text! plugin
    ...
    var titleBarTemplate = BASE_MVC.wrapTemplate([
        ...
        '<div class="title-bar clear" tabindex="0">',
            ...
            '<div class="title">',
                '<span class="name"><%- view.agent.name %></span>',
            '</div>',
            '<div class="subtitle">',
                '<span class="description"><%- view.agent.description %></span',
    ...
    var detailsTemplate = BASE_MVC.wrapTemplate([
        '<div class="details">',
            ...
            //    '<span class="value"><%- view.agent.version %></span>',
            //'</div>',
            '<div class="params">',
