...
define([
    ...
    'mvc/agent/agent-form-workflow',
    'mvc/ui/ui-misc',
    'utils/async-save-text',
    'libs/toastr',
    'ui/editable-text'
], function( Utils, Globals, Workflow, WorkflowCanvas, Node, AgentForm, Ui, async_save_text, Toastr ){

// Reset agent search to start state.
function reset_agent_search( initValue ) {
    // Function may be called in top frame or in agent_menu_frame;
    // in either case, get the agent menu frame.
    var agent_menu_frame = $("#galaxy_agents").contents();
    if (agent_menu_frame.length === 0) {
        agent_menu_frame = $(document);
    }

    // Remove classes that indicate searching is active.
    $(this).removeClass("search_active");
    agent_menu_frame.find(".agentTitle").removeClass("search_match");

    // Reset visibility of agents and labels.
    agent_menu_frame.find(".agentSectionBody").hide();
    agent_menu_frame.find(".agentTitle").show();
    agent_menu_frame.find(".agentPanelLabel").show();
    agent_menu_frame.find(".agentSectionWrapper").each( function() {
    ...
    agent_menu_frame.find("#search-no-results").hide();

    // Reset search input.
    agent_menu_frame.find("#search-spinner").hide();
    if (initValue) {
        var search_input = agent_menu_frame.find("#agent-search-query");
        search_input.val("search agents");
    }
}
...
EditorFormView = Backbone.View.extend({
        ...
        if (node && node.id != 'no-node') {
            ...
            ($el.find( 'form' ).length > 0) && $el.find( 'form' ).ajaxForm( {
                ...
                beforeSubmit: function( data ) {
                    ...
                    data.push( { name: 'agent_state', value: node.agent_state } );
    ...
    return Backbone.View.extend({
        initialize: function(options) {
            ...
            // Init searching.
            $("#agent-search-query").click( function (){
                $(this).focus();
                $(this).select();
            })
            .keyup( function () {
                ...
                if ( this.value.length < 3 ) {
                    ...
                    this.timer = setTimeout(function () {
                        $.get(self.urls.agent_search, { q: q }, function (data) {
                            // input.removeClass(config.loadingClass);
                            // Show live-search if results and search-term aren't empty
                            $("#search-no-results").hide();
                            // Hide all agent sections.
                            $(".agentSectionWrapper").hide();
                            // This hides all agents but not workflows link (which is in a .agentTitle div).
                            $(".agentSectionWrapper").find(".agentTitle").hide();
                            if ( data.length != 0 ) {
                                // Map agent ids to element ids and join them.
                                var s = $.map( data, function( n, i ) { return "link-" + n; } );
                                // First pass to show matching agents and their parents.
                                ...
                                $(".agentPanelLabel").each( function() {
                                   var this_label = $(this);
                                   var next = this_label.next();
                                   var no_visible_agents = true;
                                   // Look through agents following label and, if none are visible, hide label.
                                   while (next.length !== 0 && next.hasClass("agentTitle")) {
                                       if (next.is(":visible")) {
                                           no_visible_agents = false;
                                           break;
                                       } else {
                                           next = next.next();
                                       }
                                    }
                                    if (no_visible_agents) {
                                        this_label.hide();
                                    }
                                });
                            } else {
                ...
                success: function( data ) {
                     ...
                     if ( upgrade_message ) {
                        window.show_modal( "Workflow loaded with changes",
                                    "Problems were encountered loading this workflow (possibly a result of agent upgrades). Please review the following parameters and then save.<ul>" + upgrade_message + "</ul>",
                                    { "Continue" : hide_modal } );
                     } else {
                        hide_modal();
                     }
            ...
            function edit_workflow_outputs(){
                ...
                for (var node_key in self.workflow.nodes){
                    ...
                    if(['agent', 'subworkflow'].indexOf(node.type) >= 0){
                        new_content += "<div class='agentForm' style='margin-bottom:5px;'><div class='agentFormTitle'>Step " + node.id + " - " + node.name + "</div>";
            ...
            this.options.workflows.length > 0 && $( "#left" ).find( ".agentMenu" ).append( this._buildAgentPanelWorkflows() );

            // Agent menu
            $( "div.agentSectionBody" ).hide();
            $( "div.agentSectionTitle > span" ).wrap( "<a href='#'></a>" );
            var last_expanded = null;
            $( "div.agentSectionTitle" ).each( function() {
               var body = $(this).next( "div.agentSectionBody" );
               $(this).click( function() {
                   if ( body.is( ":hidden" ) ) {
        ...
        _buildAgentPanelWorkflows: function() {
            var self = this;
            var $section = $(   '<div class="agentSectionWrapper">' +
            ...
            _.each( this.options.workflows, function( workflow ) {
                if( workflow.id !== self.options.id ) {
                    var copy = new Ui.ButtonIcon({
                        ...
                        agenttip     : 'Copy and insert individual steps',
                        onclick     : function() {
                    ...
                    $section.find( '.agentSectionBg' ).append( $( '<div/>' ).addClass( 'agentTitle' ).append( $add ).append( copy.$el ) );
                }
            });
            return $section;
        },
        ...
        // Add a new step to the workflow by agent id
        add_node_for_agent: function ( id, title ) {
            node = this.workflow.create_node( 'agent', title, id );
            this._moduleInitAjax(node, { type: "agent", content_id: id, "_": "true" });
        },
        ...
        display_pja: function (pja, node) {
            ...
            $("#pja_container>.agentForm:last>.agentFormTitle>.buttons").click(function (){
                action_to_rem = $(this).closest(".agentForm", ".action_tag").children(".action_tag:first").text();
                $(this).closest(".agentForm").remove();
        ...
        showAgentForm: function ( text, node ) {
            ...
            // check if agent form already exists
            if ($container.find('#' + id).length == 0) {
                var $el = $('<div id="' + id + '" class="' + cls + '"/>');
                var formView = null;
                if (node.type == 'agent' && Utils.isJSON(text)) {
                    var options = JSON.parse(text);
                    options.node = node;
                    options.workflow = this.workflow;
                    options.datatypes = this.datatypes;
                    formView = new AgentForm.View(options);
                } else {
                    var options = {
                        html: text,
                        node: node,
        ...
        $newNodeElement: function(type, title_text) {
            var $f = $("<div class='agentForm agentFormInCanvas'></div>");
            var $title = $("<div class='agentFormTitle unselectable'><span class='nodeTitle'>" + title_text + "</div></div>" );
            add_node_icon($title.find('.nodeTitle'), type);
            $f.append( $title );
            $f.css( "left", $(window).scrollLeft() + 20 );
            $f.css( "top", $(window).scrollTop() + 20 );
            var $b = $("<div class='agentFormBody'></div>");
            $f.append($b);
            return $f
        },
        ...
        prebuildNode: function ( type, title_text, content_id ) {
            ...
            var tmp = "<div><img height='16' align='middle' src='" + Galaxy.root + "static/images/loading_small_white_bg.gif'/> loading agent info...</div>";
            $f.find(".agentFormBody").append(tmp);
            node.form_html = tmp;
            ...
            buttons.prependTo( $f.find(".agentFormTitle" ) );
