/**
    This is the run workflow agent form.
*/
define([ 'utils/utils', 'mvc/ui/ui-misc', 'mvc/form/form-view', 'mvc/form/form-data', 'mvc/agent/agent-form-base' ],
    function( Utils, Ui, Form, FormData, AgentFormBase ) {
    var View = Backbone.View.extend({
        initialize: function( options ) {
            ...
            _.each( options.steps, function( step, i ) {
                Galaxy.emit.debug( 'agent-form-composite::initialize()', i + ' : Preparing workflow step.' );
                step = Utils.merge( {
            ...
            _.each( this.steps, function( step, i ) {
                ...
                Galaxy.emit.debug( 'agent-form-composite::initialize()', i + ' : Workflow step state ready.', step );
            });
        ...
        */
        _execute: function() {
            ...
            if ( !validated ) {
                ...
                self._enabled( false );
                Galaxy.emit.debug( 'agents-form-composite::submit()', 'Validation complete.', job_def );
                Utils.request({
                    type    : 'POST',
                    url     : Galaxy.root + 'api/workflows/' + this.workflow_id + '/invocations',
                    data    : job_def,
                    success : function( response ) {
                        Galaxy.emit.debug( 'agent-form-composite::submit', 'Submission successful.', response );
                    ...
                    error   : function( response ) {
                        ...
                        if ( response && response.err_data ) {
                            ...
                            Galaxy.modal && Galaxy.modal.show({
                                ...
                                body    : ( response && response.err_msg ) || AgentTemplate.error( options.job_def ),
                                buttons : {
                                    'Close' : function() {
                                        Galaxy.modal.hide();
                                    }
