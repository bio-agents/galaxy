/**
 * Visualization and components for Sweepster, a visualization for exploring a agent's parameter space via
...
], function(_, d3, util, visualization, tracks, agents, data, config, mod_icon_btn) {

/**
 * A collection of agent input settings. Object is useful for keeping a list of settings
 * for future use without changing the input's value and for preserving inputs order.
 */
var AgentInputsSettings = Backbone.Model.extend({
    defaults: {
        inputs: null,
        values: null
    }
...
/**
 * Tree for a agent's parameters.
 */
var AgentParameterTree = Backbone.Model.extend({
    defaults: {
        agent: null,
        tree_data: null
    },
    ...
    initialize: function(options) {
        // Set up agent parameters to work with tree.
        var self = this;
        this.get('agent').get('inputs').each(function(input) {
        ...
        if (options.config) {
            _.each(options.config, function(input_config) {
                var input = self.get('agent').get('inputs').find(function(input) {
    ...
    /**
     * Sets tree data using agent's inputs.
     */
    ...
    get_tree_params: function() {
        ...
        return _(this.get('agent').get('inputs').where( {in_ptree: true} ))
    ...
    /**
     * Returns array of AgentInputsSettings objects based on a node and its subtree.
     */
    get_node_settings: function(target_node) {
        // -- Get fixed settings from agent and parent nodes.

        // Start with agent's settings.
        var fixed_settings = this.get('agent').get_inputs_dict();
        ...
        var self = this,
            get_settings = function(node, settings) {
                ...
                if (!node.children) {
                    ...
                    return new AgentInputsSettings({
                        inputs: self.get('agent').get('inputs'),
                        values: settings
                    });
                }
                else {
...
var SweepsterVisualization = visualization.Visualization.extend({
    ...
    initialize: function(options) {
        ...
        this.set('agent', new agents.Agent(options.agent));
        ...
        var agent_with_samplable_inputs = this.get('agent');
        this.set('agent_with_samplable_inputs', agent_with_samplable_inputs);
        // Remove complex parameters for now.
        agent_with_samplable_inputs.remove_inputs( [ 'data', 'hidden_data', 'conditional', 'text' ] );

        this.set('parameter_tree', new AgentParameterTree({
            agent: agent_with_samplable_inputs,
...
/**
 * Agent input (parameter) that enables both value and sweeping inputs. View is unusual as
 * it augments an existing input form row rather than creates a completely new HTML element.
 */
var AgentInputValOrSweepView = Backbone.View.extend({
...
var AgentParameterTreeDesignView = Backbone.View.extend({
    ...
    render: function() {
        // Start with agent form view.
        var agent_form_view = new agents.AgentFormView({
            model: this.model.get('agent')
        });
        agent_form_view.render();
        this.$el.append(agent_form_view.$el);

        // Set up views for each agent input.
        var self = this,
            inputs = self.model.get('agent').get('inputs');
        this.$el.find('.form-row').not('.form-actions').each(function(i) {
            var input_view = new AgentInputValOrSweepView({
                model: inputs.at(i),
                agent_row: $(this)
            });
        });
 ...
 */
var AgentParameterTreeView = Backbone.View.extend({
    className: 'agent-parameter-tree',
...
var SweepsterVisualizationView = Backbone.View.extend({
    ...
    helpText:
        ...
        '<ol><li>Create a parameter tree by using the icons next to the agent\'s parameter names to add or remove parameters.' +
        '<li>Adjust the tree by using parameter inputs to select min, max, and number of samples' +
        '<li>Run the agent with different settings by clicking on tree nodes' +
        '</ol></div>',

    initialize: function(options) {
        this.canvas_manager = new visualization.CanvasManager(this.$el.parents('body'));
        this.agent_param_tree_view = new AgentParameterTreeView({ model: this.model.get('parameter_tree') });
    ...
    render: function() {
        ...
        var tree_design_view = new AgentParameterTreeDesignView({
        ...
        // -- Render help and agent parameter tree in center panel. --
        ...
        // Parameter tree:
        this.agent_param_tree_view.render();
        $('#center').append(this.agent_param_tree_view.$el);
        ...
        var menu = mod_icon_btn.create_icon_buttons_menu(
            ...
            {
                agenttip_config: {placement: 'bottom'}
            });
    ...
    run_agent_on_dataset: function(settings) {
        var agent = this.model.get('agent'),
            agent_name = agent.get('name'),
            dataset = this.model.get('dataset');
        agent.set_input_values(settings.get('values'));
        $.when(agent.rerun(dataset)).then(function(outputs) {
            // TODO.
        });

        show_modal('Running ' + agent_name + ' on complete dataset',
                       agent_name + ' is running on dataset \'' +
    ...
    add_track: function(pm_track) {
        ...
        track_view.on('run_on_dataset', self.run_agent_on_dataset, self);
        ...
        track_view.$el.hover(function() {
            ...
            d3.select(self.agent_param_tree_view.$el[0]).selectAll("g.node")
            .filter(function(d) {
                return _.find(connected_node_ids, function(id) { return id === d.id; }) !== undefined;
            }).style('fill', '#f00');
        },
        function() {
            d3.select(self.agent_param_tree_view.$el[0]).selectAll("g.node").style('fill', '#000');
        });
        return pm_track;
    },

    /**
     * Sets up handling when tree nodes are clicked. When a node is clicked, the agent is run for each of
     * the settings defined by the node's subtree and tracks are added for each run.
     */
    handle_node_clicks: function() {
        // When node clicked in tree, run agent and add tracks to model.
        var self = this,
            param_tree = this.model.get('parameter_tree'),
            regions = this.model.get('regions'),
            node = d3.select(this.agent_param_tree_view.$el[0]).selectAll("g.node");
        node.on("click", function(d, i) {
            // Get all settings corresponding to node.
            var agent = self.model.get('agent'),
            ...
            if (all_settings.length >= 10) {
                show_modal("Whoa there cowboy!",
                            "You clicked on a node to try " + self.model.get('agent').get('name') +
            ...
            $.when(run_jobs_deferred).then(function(run_jobs) {
                ...
                // For each track, run agent using track's settings and update track.
                _.each(new_tracks, function(pm_track, index) {
                    setTimeout(function() {
                        // Set inputs and run agent.
                        agent.set_input_values(pm_track.get('settings').get('values'));
                        $.when(agent.rerun(dataset, regions)).then(function(output) {
                            ...
                            // Set dataset to be the agent's output.
                            track_config.dataset = dataset;
                            // Set agent to null so that it is not unpacked; unpacking it messes with
                            // the agent parameters and parameter tree.
                            track_config.agent = null;
