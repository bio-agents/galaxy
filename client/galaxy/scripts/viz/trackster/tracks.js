define([
    ...
    "utils/config",
    "ui/editable-text",
], function(_, visualization, viz_views, util, slotting, painters, filters_mod, data, agents_mod, config_mod) {
...
var
    ...
    DATA_CANNOT_RUN_TOOL = "Agent cannot be rerun: ",
...
Drawable.prototype.action_icons_def = [
    ...
    // Remove.
    {
        ...
        on_click_fn: function(drawable) {
            // Agenttip for remove icon must be deleted when drawable is deleted.
            $(".agenttip").remove();
            drawable.remove();
        }
    }
...
var TracksterView = Backbone.View.extend({
    ...
    render: function() {
        ...
        this.overview_close = $("<a/>").attr("title", "Close overview")
                                       .addClass("icon-button overview-close agenttip")
                                       .hide()
        ...
        this.location_span = $("<span/>").addClass("location").attr('title', 'Click to change location').agenttip( { placement: 'bottom' } ).appendTo(this.nav_controls);
        ...
        this.zo_link = $("<a/>").attr("id", "zoom-out").attr("title", "Zoom out").agenttip( {placement: 'bottom'} )
                                .click(function() { view.zoom_out(); }).appendTo(this.nav_controls);
        this.zi_link = $("<a/>").attr("id", "zoom-in").attr("title", "Zoom in").agenttip( {placement: 'bottom'} )
        ...
        */
        ...
        // Blur agent/filter inputs when user clicks on content div.
...
/**
 * Encapsulation of a agent that users can apply to tracks/datasets.
 */
var TracksterAgent = agents_mod.Agent.extend({
    defaults: {
        track: null
    },

    initialize: function(options) {
        agents_mod.Agent.prototype.initialize.call(this, options);

        // Restore agent visibility from state; default to hidden.
        var hidden = true;
        if (options.agent_state !== undefined && options.agent_state.hidden !== undefined) {
            hidden = options.agent_state.hidden;
        }
        this.set('hidden', hidden);

        // FIXME: need to restore agent values from options.agent_state
...
/**
 * View renders agent parameter HTML and updates parameter value as it is changed in the HTML.
 */
 var AgentParameterView = Backbone.View.extend({
...
/**
 * View for TracksterAgent.
 */
var TracksterAgentView = Backbone.View.extend({
    ...
    render: function() {
        var self = this;
            agent = this.model,
            parent_div = this.$el.addClass("dynamic-agent").hide();
        ...
        // Add name, inputs.
        var name_div = $("<div class='agent-name'>").appendTo(parent_div).text(agent.get('name'));
        agent.get('inputs').each(function(param) {
            // Render parameter.
            var param_view = new AgentParameterView({ model: param });
            param_view.render();
        ...
        var run_agent_row = $("<div>").addClass("param-row").appendTo(parent_div);
        var run_on_dataset_button = $("<input type='submit'>").attr("value", "Run on complete dataset").appendTo(run_agent_row);
        var run_on_region_button = $("<input type='submit'>").attr("value", "Run on visible region").appendTo(run_agent_row);
        run_on_region_button.click( function() {
            // Run agent to create new track.
            self.run_on_region();
        });
    ...
    /**
     * Show or hide agent depending on agent visibility state.
     */
    ...
    /**
     * Run agent on dataset. Output is placed in dataset's history and no changes to viz are made.
     */
    run_on_dataset: function() {
        ...
        this.run(
            ...
            // Success callback.
            function(track_data) {
                Galaxy.modal.show({title: agent.get('name') + " is Running", body: agent.get('name') + " is running on the complete dataset. Agent outputs are in dataset's history.", buttons : {'Close' : function() { Galaxy.modal.hide(); } } });
            }
        );
    },
    ...
    run_on_region: function() {
        //
        // Create track for agent's output immediately to provide user feedback.
        //
        var track = this.model.get('track'),
            ...
            current_track = track,
            // Set name of track to include agent name, parameters, and region used.
            track_name = agent.get('name') +
                         current_track.agent_region_and_parameters_str(region),
            container;
    ...
    /**
     * Run agent using a set of URL params and a success callback.
     */
    run: function(url_params, new_track, success_callback) {
        ...
        $.when(ss_deferred.go()).then(function(response) {
            if (response === "no converter") {
                // No converter available for input datasets, so cannot run agent.
    ...
    action_icons_def: [
        ...
        // Toggle track agent.
        {
            ...
            on_click_fn: function(track) {
                ...
                // Update track name.
                if (track.agent.is_visible()) {
                    track.set_name(track.config.get_value('name') + track.agent_region_and_parameters_str());
                }
                else {
                    track.revert_name();
                }
                // HACK: name change modifies icon placement, which leaves agenttip incorrectly placed.
                $(".agenttip").remove();
            }
        },
        // Go to parameter exploration visualization.
        {
            ...
            title: "Agent parameter space visualization",
            css_class: "arrow-split",
            on_click_fn: function(track) {
                var html =
                    '<strong>Agent</strong>:' + track.agent.get("name") + '<br/>' +
                ...
                // show dialog
                Galaxy.modal.show({title: "Visualize agent parameter space and output from different parameter settings?", body: html, buttons : {'No' : cancel_fn, 'Yes' : ok_fn } });
            }
        },
        // Remove track.
        Drawable.prototype.action_icons_def[2]
...
var TiledTrack = function(view, container, obj_dict) {
    ...
    this.agent = (obj_dict.agent ? new TracksterAgent( _.extend( obj_dict.agent, {
        'track': this,
        'agent_state': obj_dict.agent_state
    } ) )
    : null);
    ...
    if (this.header_div) {
        ...
        //
        // Create dynamic agent view.
        //
        if (this.agent) {
            var agent_view = new TracksterAgentView({ model: this.agent });
            agent_view.render();
            this.dynamic_agent_div = agent_view.$el;
            this.header_div.after(this.dynamic_agent_div);
        }
    }
...
extend(TiledTrack.prototype, Drawable.prototype, Track.prototype, {
    ...
    to_dict: function() {
        return {
            ...
            agent_state: (this.agent ? this.agent.state_dict() : {})
        };
    },
    ...
    update_icons: function() {
        ...
        //
        // Show/hide agent icons.
        //
        track.action_icons.agents_icon.toggle(track.agent !== null);
        track.action_icons.param_space_viz_icon.toggle(track.agent !== null);
    },
    ...
    /**
     * Utility function that creates a label string describing the region and parameters of a track's agent.
     */
    agent_region_and_parameters_str: function(region) {
        var track = this,
            region_str = (region !== undefined ? region.toString() : "all"),
            param_str = _.values( track.agent.get_inputs_dict()).join(', ');
    ...
    /**
     * Set up track to receive agent data.
     */
    init_for_agent_data: function() {
        ...
        //
        // Set up one-time, post-draw to clear agent execution settings.
        //
        /*
        ...
        this.postdraw_actions = function(tiles, width, w_scale, clear_after) {
            ...
            // Agent-execution specific post-draw init:
...
extend(CompositeTrack.prototype, TiledTrack.prototype, {
    ...
    update_icons: function() {
        // For now, hide filters and agent.
        this.action_icons.filters_icon.hide();
        this.action_icons.agents_icon.hide();
...
extend(VariantTrack.prototype, Drawable.prototype, TiledTrack.prototype, {
    ...
    get_canvas_height: function(result, mode, w_scale, canvas_width) {
        if (result.dataset_type === 'bigwig') {
            ...
            // dataset is complete (as is done when running agents). In that case, fall back on
