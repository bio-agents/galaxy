...
var Filter = function(obj_dict) {
    ...
    this.agent_id = obj_dict.agent_id;
    // Name to use for filter when building expression for agent.
    this.agent_exp_name = obj_dict.agent_exp_name;
};
...
extend(Filter.prototype, {
    ...
    to_dict: function() {
        return {
            ...
            agent_exp_name: this.agent_exp_name
        };
    } 
});
...
var create_action_icon =  function(title, css_class, on_click_fn) {
    return $("<a/>").attr("href", "javascript:void(0);").attr("title", title)
                    .addClass("icon-button").addClass(css_class).agenttip()
                    .click(on_click_fn);
};
...
extend(FiltersManager.prototype, {
    ...
    run_on_dataset: function() {
        ...
        // Filters with the same agent id are grouped.
        //
        var active_filters = {},
            filter, 
        ...
        for (var i = 0; i < this.filters.length; i++) {
            ...
            if (filter.agent_id) {
                ...
                if (filter.min !== filter.low) {
                    agent_filter_conditions = get_or_create_dict_item(active_filters, filter.agent_id, []);
                    agent_filter_conditions[agent_filter_conditions.length] = filter.agent_exp_name + " >= " + filter.low;
                }
                if (filter.max !== filter.high) {
                    agent_filter_conditions = get_or_create_dict_item(active_filters, filter.agent_id, []);
                    agent_filter_conditions[agent_filter_conditions.length] = filter.agent_exp_name + " <= " + filter.high;
                }
            }
        }
        
        //
        // Use agents to run filters.
        //
        
        // Create list of (agent_id, agent_filters) tuples.
        var active_filters_list = [];
        for (var agent_id in active_filters) {
            active_filters_list[active_filters_list.length] = [agent_id, active_filters[agent_id]];
        }
        ...
        (function run_filter(input_dataset_id, filters) {
            var 
                ...
                agent_id = filter_tuple[0],
                agent_filters = filter_tuple[1],
                agent_filter_str = "(" + agent_filters.join(") and (") + ")",
                url_params = {
                    cond: agent_filter_str,
            ...
            $.getJSON(run_agent_url, url_params, function(response) {
                if (response.error) {
                    // General error.
                    Galaxy.modal.show({
                        title: "Filter Dataset",
                        body : "Error running agent " + agent_id,
