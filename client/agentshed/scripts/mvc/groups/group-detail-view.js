                    ...
                    '<table class="grid table table-condensed">',
                        ...
                        '<tbody>',
                            ...
                            '<tr>',
                                ...
                                '</td>',
                                '<td>',
                                    '<a data-toggle="agenttip" data-placement="top" title="Repositories of <%= _.escape(member.username) %>" href="/repository/browse_repositories_by_user?user_id=<%= member.id %>&use_panels=true" id="<%= member.id %>"><%= member.user_repos_count %></a>',
                                '</td>',
                                '<td>',
                                    '<%= member.time_created %>',
                                '</td>',
                    ...
                    '<table class="grid table table-condensed">',
                        ...
                        '<tbody>',
                            '<% _.each(group.get("repositories"), function(repo) { %>',
                                '<tr>',
                                    '<td>',
                                        '<a data-toggle="agenttip" data-placement="top" title="Details of <%= _.escape(repo.name) %>" href="/view/<%= _.escape(repo.owner) %>/<%= _escape(repo.name) %>" id="<%= repo.id %>"><%= _.escape(repo.name) %></a>',
                                    '</td>',
                                    '<td>',
                                    ...
                                    '<td>',
                                        '<% _.each((repo.categories), function(cat) { %>',
                                            '<a data-toggle="agenttip" data-placement="top" title="Repositories in <%= cat.name %>" href="/repository/browse_repositories_in_category?id=<%= cat.id %>&use_panels=true"><%= cat.name %></a><br/>',
                                        // '<%= repo.categories %>',
                                        '<% }); %>',
                                    '</td>',
                                    '<td data-toggle="agenttip" data-placement="top" title="<%= repo.time_created_full %>">',
                                        '<%= repo.time_created %>',
                                    '</td>',
                                    '<td data-toggle="agenttip" data-placement="top" title="<%= repo.time_updated_full %>">',
                                        '<%= repo.time_updated %>',
                                    '</td>',
                                    '<td>',
                                        '<%= repo.ratings_mean %>',
