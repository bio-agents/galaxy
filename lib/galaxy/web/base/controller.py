...
from galaxy.workflow.modules import WorkflowModuleInjector, MissingAgentException
...
class BaseController( object ):
    ...
    def get_agentbox(self):
        """Returns the application agentbox"""
        return self.app.agentbox
...
class ExportsHistoryMixin:
    ...
    def queue_history_export( self, trans, history, gzip=True, include_hidden=False, include_deleted=False ):
        ...
        history_exp_agent = trans.app.agentbox.get_agent( '__EXPORT_HISTORY__' )
        params = {
        ...
        history_exp_agent.execute( trans, incoming=params, history=history, set_output_hid=True )
...
class ImportsHistoryMixin:
    ...
    def queue_history_import( self, trans, archive_type, archive_source ):
        ...
        history_imp_agent = trans.app.agentbox.get_agent( '__IMPORT_HISTORY__' )
        incoming = { '__ARCHIVE_SOURCE__' : archive_source, '__ARCHIVE_TYPE__' : archive_type }
        history_imp_agent.execute( trans, incoming=incoming )
...
class UsesVisualizationMixin( UsesLibraryMixinItems ):
        ...
        if vis.type == 'trackster':
            def unpack_track( track_dict ):
                ...
                return {
                    ...
                    "agent_state": track_dict['agent_state']
                }
    ...
    def get_agent_def( self, trans, hda ):
        """ Returns definition of an interactive agent for an HDA. """
        ...
        agent = trans.app.agentbox.get_agent( job.agent_id )
        if not agent:
            return None

        # Agent must have a Trackster configuration.
        if not agent.trackster_conf:
            return None

        # -- Get agent definition and add input values from job. --
        agent_dict = agent.to_dict( trans, io_details=True )
        agent_param_values = dict( [ ( p.name, p.value ) for p in job.parameters ] )
        agent_param_values = agent.params_from_strings( agent_param_values, trans.app, ignore_errors=True )

        # Only get values for simple inputs for now.
        inputs_dict = [ i for i in agent_dict[ 'inputs' ] if i[ 'type' ] not in [ 'data', 'hidden_data', 'conditional' ] ]
        for t_input in inputs_dict:
            # Add value to agent.
            if 'name' in t_input:
                name = t_input[ 'name' ]
                if name in agent_param_values:
                    value = agent_param_values[ name ]
        ...
        if visualization.type in [ 'trackster', 'genome' ]:
            ...
            def pack_track( track_dict ):
                ...
                return {
                    ...
                    "agent": self.get_agent_def( trans, dataset ),
                    "agent_state": track_dict.get( 'agent_state', {} )
                }
...
class UsesStoredWorkflowMixin( SharableItemSecurityMixin, UsesAnnotations ):
    ...
    def get_stored_workflow_steps( self, trans, stored_workflow ):
        ...
        for step in stored_workflow.latest_workflow.steps:
            try:
                ...
                step.upgrade_messages = "Unknown Agent ID"
    ...
    def _workflow_from_dict( self, trans, data, source=None, add_to_menu=False, publish=False ):
        ...
        return created_workflow.stored_workflow, created_workflow.missing_agents
