...
class Admin( object ):
    ...
    def index( self, trans, **kwd ):
        ...
        if trans.webapp.name == 'galaxy':
            is_repo_installed = trans.install_model.context.query( trans.install_model.AgentShedRepository ).first() is not None
            installing_repository_ids = suc.get_ids_of_agent_shed_repositories_being_installed( trans.app, as_string=True )
            ...
            return trans.fill_template( '/webapps/agent_shed/admin/index.mako',
                                        message=message,
                                        status=status )
    ...
    def center( self, trans, **kwd ):
        ...
        if trans.webapp.name == 'galaxy':
            ...
            return trans.fill_template( '/webapps/agent_shed/admin/center.mako',
                                        message=message,
                                        status=status )

    @web.expose
    @web.require_admin
    def package_agent( self, trans, **kwd ):
        params = util.Params( kwd )
        message = util.restore_text( params.get( 'message', ''  ) )
        agentbox = self.app.agentbox
        agent_id = None
        if params.get( 'package_agent_button', False ):
            agent_id = params.get('agent_id', None)
            try:
                agent_tarball = trans.app.agentbox.package_agent( trans, agent_id )
                trans.response.set_content_type( 'application/x-gzip' )
                download_file = open( agent_tarball )
                os.unlink( agent_tarball )
                tarball_path, filename = os.path.split( agent_tarball )
                trans.response.headers[ "Content-Disposition" ] = 'attachment; filename="%s.tgz"' % ( agent_id )
                return download_file
            except Exception:
                return trans.fill_template( '/admin/package_agent.mako',
                                            agent_id=agent_id,
                                            agentbox=agentbox,
                                            message=message,
                                            status='error' )
    ...
    @web.expose
    @web.require_admin
    def reload_agent( self, trans, **kwd ):
        params = util.Params( kwd )
        message = util.restore_text( params.get( 'message', ''  ) )
        status = params.get( 'status', 'done' )
        agentbox = self.app.agentbox
        agent_id = None
        if params.get( 'reload_agent_button', False ):
            agent_id = kwd.get( 'agent_id', None )
            galaxy.queue_worker.send_control_task(trans.app, 'reload_agent', noop_self=True, kwargs={'agent_id': agent_id} )
            message, status = trans.app.agentbox.reload_agent_by_id( agent_id)
        return trans.fill_template( '/admin/reload_agent.mako',
                                    agent_id=agent_id,
                                    agentbox=agentbox,
                                    message=message,
                                    status=status )
    ...
    @web.expose
    @web.require_admin
    def agent_versions( self, trans, **kwd ):
        if 'message' not in kwd or not kwd[ 'message' ]:
            kwd[ 'message' ] = 'Agent ids for agents that are currently loaded into the agent panel are highlighted in green (click to display).'
        return self.agent_version_list_grid( trans, **kwd )
    ...
    def roles( self, trans, **kwargs ):
        if 'operation' in kwargs:
            ...
            if operation == "manage role associations":
                # This is currently used only in the Agent Shed.
    ...
    @web.expose
    @web.require_admin
    def sanitize_whitelist( self, trans, submit_whitelist=False, agents_to_whitelist=[]):
        if submit_whitelist:
            ...
            with open(trans.app.config.sanitize_whitelist_file, 'wt') as f:
                if isinstance(agents_to_whitelist, basestring):
                    agents_to_whitelist = [agents_to_whitelist]
                new_whitelist = sorted([tid for tid in agents_to_whitelist if tid in trans.app.agentbox.agents_by_id])
                f.write("\n".join(new_whitelist))
        ...
        return trans.fill_template( '/webapps/galaxy/admin/sanitize_whitelist.mako',
                                    ...
                                    agents=trans.app.agentbox.agents_by_id )
