...
class FTPFileField(BaseField):
    ...
    def get_html( self, prefix="" ):
        ...
        rval += '<div class="agentParamHelp">This Galaxy server allows you to upload files via FTP.  To upload some files, log in to the FTP server at <strong>%s</strong> using your Galaxy credentials (email address and password).</div>' % self.ftp_site
        return rval
...
class SwitchingSelectField(BaseField):
    ...
    def get_html( self, prefix="", disabled=False ):
        ...
        html += ','.join( options )
        html += '};\n'
        html += 'if ( window.enhanced_galaxy_agents ) {\n'
        html += 'require( [ "galaxy.agents" ], function( mod_agents ) { new mod_agents.SwitchSelectView({\n'
                        ...
                        for step in workflow.latest_workflow.steps:
                            if step.type == 'data_input':
                                if step.agent_inputs and "name" in step.agent_inputs:
                                    workflow_inputs.append((step.agent_inputs['name'], TextField( '%s_%s' % (self.name, step.id), 20)))
