...
reloading the agentbox, etc., across multiple processes.
"""
...
# decorator).
def reload_agent(app, **kwargs):
    params = util.Params(kwargs)
    agent_id = params.get('agent_id', None)
    log.debug("Executing reload agent task for %s" % agent_id)
    if agent_id:
        app.agentbox.reload_agent_by_id( agent_id )
    else:
        log.error("Reload agent invoked without agent id.")
...
def reload_agent_data_tables(app, **kwargs):
    params = util.Params(kwargs)
    log.debug("Executing agent data table reload for %s" % params.get('table_names', 'all tables'))
    table_names = app.agent_data_tables.reload_tables( table_names=params.get('table_name', None))
...
control_message_to_task = { 'reload_agent': reload_agent,
                            'reload_display_application': reload_display_application,
                            'reload_agent_data_tables': reload_agent_data_tables,
...
class GalaxyQueueWorker(ConsumerMixin, threading.Thread):
    ...
    def __init__(self, app, queue=None, task_mapping=control_message_to_task, connection=None):
        ...
        # stale 'reload_agent', etc messages.  This can happen if, say, a web
