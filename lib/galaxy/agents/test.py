...
import os
import os.path
import galaxy.agents.parameters.basic
import galaxy.agents.parameters.grouping
...
@nottest
def parse_tests(agent, tests_source):
    """
    Build AgentTestBuilder objects for each "<test>" elements and
    ...
    for i, test_dict in enumerate(tests_dict.get('tests', [])):
        test = AgentTestBuilder( agent, test_dict, i, default_interactor=tests_default_interactor )
        tests.append( test )
    return tests


class AgentTestBuilder( object ):
    """
    Encapsulates information about a agent test, and allows creation of a
    ...
    def __init__( self, agent, test_dict, i, default_interactor ):
    ...
    def __matching_case_for_value( self, cond, declared_value ):
        ...
        if isinstance(test_param, galaxy.agents.parameters.basic.BooleanAgentParameter):
        ...
        elif isinstance(test_param, galaxy.agents.parameters.basic.SelectAgentParameter):
        ...
        # Check the agent's defined cases against predicate to determine
        # selected or default.
        for i, case in enumerate( cond.cases ):
            ...
            msg = msg_template % ( self.agent.id, declared_value, cond.name )
            log.info( msg )
    ...
    def __handle_test_dict( self, test_dict, i, default_interactor ):
        try:
            ...
            self.inputs = self.__process_raw_inputs( self.agent.inputs, test_dict["inputs"] )
    ...
    def __process_raw_inputs( self, agent_inputs, raw_inputs, parent_context=None ):
        """
        ...
        for key, value in agent_inputs.items():
            if isinstance( value, galaxy.agents.parameters.grouping.Conditional ):
                ...
                if case:
                    ...
                    if case_value is not None:
                        ...
                        # a default defined in agent). So we do not want to grow
            ...
            elif isinstance( value, galaxy.agents.parameters.grouping.Section ):
            ...
            elif isinstance( value, galaxy.agents.parameters.grouping.Repeat ):
                repeat_index = 0
                while True:
                ...
                if raw_input:
                    ...
                    if isinstance( value, galaxy.agents.parameters.basic.DataAgentParameter ):
                    ...
                    elif isinstance( value, galaxy.agents.parameters.basic.DataCollectionAgentParameter ):
...
def _process_simple_value( param, param_value ):
    if isinstance( param, galaxy.agents.parameters.basic.SelectAgentParameter ) and hasattr( param, 'static_options' ):
    ...
    elif isinstance( param, galaxy.agents.parameters.basic.BooleanAgentParameter ):
        # Like above, tests may use the agent define values of simply
        # true/false.
...
def _process_bool_param_value( param, param_value ):
    assert isinstance( param, galaxy.agents.parameters.basic.BooleanAgentParameter )
    was_list = False
    if isinstance( param_value, list ):
        was_list = True
