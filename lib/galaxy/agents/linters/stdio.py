...
def lint_stdio(agent_xml, lint_ctx):
    stdios = agent_xml.findall("./stdio")
    if not stdios:
        command = get_command(agent_xml)
        if command is None or not command.get("detect_errors"):
            lint_ctx.info("No stdio definition found, agent will determine an error from stderr.")
        return
