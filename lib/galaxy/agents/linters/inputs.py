...
def lint_inputs(agent_xml, lint_ctx):
    datasource = is_datasource(agent_xml)
    inputs = agent_xml.findall("./inputs//param")
    num_inputs = 0
    for param in inputs:
        num_inputs += 1
    ...
    conditional_selects = agent_xml.findall("./inputs//conditional")
...
def lint_repeats(agent_xml, lint_ctx):
    repeats = agent_xml.findall("./inputs//repeat")
