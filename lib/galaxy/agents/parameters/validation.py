...
class RegexValidator( Validator ):
    ...
    >>> from galaxy.agents.parameters.basic import AgentParameter
    >>> p = AgentParameter.build( None, XML( '''
...
class MetadataInFileColumnValidator( Validator ):
    ...
    def from_element( cls, param, elem ):
        ...
        if filename:
            filename = "%s/%s" % ( param.agent.app.config.agent_data_path, filename.strip() )
...
class MetadataInDataTableColumnValidator( Validator ):
    ...
    def from_element( cls, param, elem ):
        ...
        agent_data_table = param.agent.app.agent_data_tables[ table_name ]
        metadata_name = elem.get( "metadata_name", None )
        if metadata_name:
            metadata_name = metadata_name.strip()
        ...
        if line_startswith:
            line_startswith = line_startswith.strip()
        return cls( agent_data_table, metadata_name, metadata_column, message, line_startswith )

    def __init__( self, agent_data_table, metadata_name, metadata_column, message="Value for metadata not found.", line_startswith=None ):
        self.metadata_name = metadata_name
        self.message = message
        self.valid_values = []
        self._data_table_content_version = None
        self._agent_data_table = agent_data_table
        if isinstance( metadata_column, basestring ):
            metadata_column = agent_data_table.columns[ metadata_column ]
        self._metadata_column = metadata_column
        self._load_values()

    def _load_values( self ):
        self._data_table_content_version, data_fields = self._agent_data_table.get_version_fields()
        self.valid_values = []
    ...
    def validate( self, value, history=None ):
        if not value:
            return
        if hasattr( value, "metadata" ):
            if not self._agent_data_table.is_current_version( self._data_table_content_version ):
                log.debug( 'MetadataInDataTableColumnValidator values are out of sync with data table (%s), updating validator.', self._agent_data_table.name )
                self._load_values()
            if value.metadata.spec[self.metadata_name].param.to_string( value.metadata.get( self.metadata_name ) ) in self.valid_values:
                return
        raise ValueError( self.message )
