...
def expand_meta_parameters( trans, agent, incoming ):
    """
    Take in a dictionary of raw incoming parameters and expand to a list
    of expanded incoming parameters (one set of parameters per agent
    execution).
    """
...
def __expand_collection_parameter( trans, input_key, incoming_val, collections_to_match, linked=False ):
    ...
    if "|" in incoming_val:
        ...
        try:
            ...
            if src != "hdca":
                raise exceptions.AgentMetaParameterException( "Invalid dataset collection source type %s" % src )
