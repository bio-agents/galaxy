"""
Support for generating the options for a SelectAgentParameter dynamically (based
...
class DataMetaFilter( Filter ):
    ...
    def filter_options( self, options, trans, other_values ):
        ...
        is_data = isinstance( ref, galaxy.agents.wrappers.DatasetFilenameWrapper )
        is_data_list = isinstance( ref, galaxy.agents.wrappers.DatasetListWrapper ) or isinstance( ref, list )
        is_data_or_data_list = is_data or is_data_list
        if not isinstance( ref, self.dynamic_option.agent_param.agent.app.model.HistoryDatasetAssociation ) and not is_data_or_data_list:
    ...
    def filter_options( self, options, trans, other_values ):
        ...
        if value is None:
            if self.ref_name is not None:
                ...
                if not isinstance( data_ref, self.dynamic_option.agent_param.agent.app.model.HistoryDatasetAssociation ) and not isinstance( data_ref, galaxy.agents.wrappers.DatasetFilenameWrapper ):
...
class DynamicOptions( object ):
    """Handles dynamically generated SelectAgentParameter options"""
    def __init__( self, elem, agent_param  ):
        def load_from_parameter( from_parameter, transform_lines=None ):
            obj = self.agent_param
        ...
        agent_data_table_name = elem.get( 'from_data_table', None )
        # Options are defined from a data table loaded by the app
        self.agent_data_table = None
        self.missing_agent_data_table_name = None
        if agent_data_table_name:
            app = agent_param.agent.app
            if agent_data_table_name in app.agent_data_tables:
                self.agent_data_table = app.agent_data_tables[ agent_data_table_name ]
                ...
                if elem.find( "column" ) is not None:
                    ...
                    self.columns = self.agent_data_table.columns
                # Set self.missing_index_file if the index file to
                # which the agent_data_table refers does not exist.
                if self.agent_data_table.missing_index_file:
                    self.missing_index_file = self.agent_data_table.missing_index_file
            else:
                self.missing_agent_data_table_name = agent_data_table_name
                log.warn( "Data table named '%s' is required by agent but not configured" % agent_data_table_name )
        ...
        # on disk, a dataset, or the value of another parameter
            ...
            if data_file is not None:
                ...
                if not os.path.isabs( data_file ):
                    full_path = os.path.join( self.agent_param.agent.app.config.agent_data_path, data_file )
        ...
        for validator in elem.findall( 'validator' ):
            self.validators.append( validation.Validator.from_element( self.agent_param, validator ) )

        if self.dataset_ref_name:
            agent_param.data_ref = self.dataset_ref_name
    ...
    def get_fields( self, trans, other_values ):
        ...
        elif self.agent_data_table:
            options = self.agent_data_table.get_fields()
        else:
    ...
    def get_options( self, trans, other_values ):
        rval = []
        if self.file_fields is not None or self.agent_data_table is not None or self.dataset_ref_name is not None:
