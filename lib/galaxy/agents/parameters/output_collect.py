""" Code allowing agents to define extra files associated with an output datset.
"""
import os
import re
import operator
...
from galaxy.agents.parser.output_collection_def import DEFAULT_DATASET_COLLECTOR_DESCRIPTION
...
def collect_dynamic_collections(
    ...
    collections_service = agent.app.dataset_collections_service
    ...
    for name, has_collection in output_collections.items():
        if name not in agent.output_collections:
            continue
        output_collection_def = agent.output_collections[ name ]
...
class JobContext( object ):
    ...
    def __init__( self, agent, job, job_working_directory, inp_data ):
        self.inp_data = inp_data
        self.app = agent.app
        self.sa_session = agent.sa_session
        self.job = job
...
def collect_primary_datasets( agent, output, job_working_directory, input_ext ):
    app = agent.app
    sa_session = agent.sa_session
    ...
    for output_index, ( name, outdata ) in enumerate( output.items() ):
        dataset_collectors = map(dataset_collector, agent.outputs[ name ].dataset_collector_descriptions) if name in agent.outputs else [ DEFAULT_DATASET_COLLECTOR ]
        filenames = odict.odict()
        ...
        for filename_index, ( filename, extra_file_collector ) in enumerate( filenames.iteritems() ):
            ...
            # add agent/metadata provided information
    ...
    for extra_file_collector in extra_file_collectors:
        ...
        if extra_file_collector.directory:
            ...
            if not util.in_directory( directory, job_working_directory ):
                raise Exception( "Problem with agent configuration, attempting to pull in datasets from outside working directory." )
...
class DatasetCollector( object ):
    ...
    def __init__( self, dataset_collection_description ):
        ...
        # built from the agent parsing module - see galaxy.agents.parser.output_colleciton_def
