...
class AgentOutputBase( Dictifiable, object ):

    def __init__( self, name, label=None, filters=None, hidden=False ):
        super( AgentOutputBase, self ).__init__()
        self.name = name
        self.label = label
...
class AgentOutput( AgentOutputBase ):
    """
    Represents an output datasets produced by a agent. For backward
    ...
    def __init__( self, name, format=None, format_source=None, metadata_source=None,
                  ...
                  implicit=False ):
        super( AgentOutput, self ).__init__( name, label=label, filters=filters, hidden=hidden )
        self.format = format
    ...
    def to_dict( self, view='collection', value_mapper=None, app=None ):
        as_dict = super( AgentOutput, self ).to_dict( view=view, value_mapper=value_mapper )
        format = self.format
...
class AgentOutputCollection( AgentOutputBase ):
    """
    Represents a HistoryDatasetCollectionAssociation of output datasets produced
    by a agent.

    <outputs>
      <collection type="list" label="${agent.name} on ${on_string} fasta">
        <discover_datasets pattern="__name__" ext="fasta" visible="True" directory="outputFiles" />
      </collection>
      <collection type="paired" label="${agent.name} on ${on_string} paired reads">
    ...
    def __init__(
        ...
        super( AgentOutputCollection, self ).__init__( name, label=label, filters=filters, hidden=hidden )
    ...
    def known_outputs( self, inputs, type_registry ):
        ...
        if len( self.outputs ) > 1:
            output_parts = [AgentOutputCollectionPart(self, k, v) for k, v in self.outputs.items()]
        else:
            ...
            def prototype_dataset_element_to_output( element, parent_ids=[] ):
                ...
                return AgentOutputCollectionPart(
                    self,
...
class AgentOutputCollectionStructure( object ):
...
class AgentOutputCollectionPart( object ):
    ...
    def split_output_name( name ):
        assert AgentOutputCollectionPart.is_named_collection_part_name( name )
