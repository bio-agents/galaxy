...
workflow language reference implementation library cwlagent. These proxies
adapt cwlagent to Galaxy features and abstract the library away from the rest
of the framework.
"""
...
def agent_proxy(agent_path):
    """ Provide a proxy object to cwlagent data structures to just
    grab relevant data.
    """
    ensure_cwlagent_available()
    agent = to_cwl_agent_object(agent_path)
    return agent
...
def load_job_proxy(job_directory):
    ...
    agent_path = job_objects["agent_path"]
    job_inputs = job_objects["job_inputs"]
    cwl_agent = agent_proxy(agent_path)
    cwl_job = cwl_agent.job_proxy(job_inputs, job_directory=job_directory)
    return cwl_job


def to_cwl_agent_object(agent_path):
    if workflow is None:
        raise Exception("Using CWL agents requires cwlagent module.")
    proxy_class = None
    cwl_agent = None
    make_agent = workflow.defaultMakeAgent
    cwl_agent = main.load_agent(agent_path, False, False, make_agent, False)
    proxy_class = Draft2AgentProxy
    if proxy_class is None:
        raise Exception("Unsupported CWL object encountered.")
    proxy = proxy_class(cwl_agent, agent_path)
    return proxy
...
class AgentProxy( object ):
    ...
    def __init__(self, agent, agent_path):
        self._agent = agent
        self._agent_path = agent_path

    def job_proxy(self, input_dict, job_directory="."):
        """ Build a cwlagent.job.Job describing computation using a input_json
        Galaxy will generate mapping the Galaxy description of the inputs into
        a cwlagent compatible variant.
        """
    ...
    def docker_identifier(self):
        """ Return docker identifier for embedding in agent description. """

    @abstractmethod
    def description(self):
        """ Return description to agent. """


class Draft2AgentProxy(AgentProxy):

    def description(self):
        # Feels like I should be getting some abstract namespaced thing
        # not actually description, is this correct?
        return self._agent.agent.get('description')

    def input_instances(self):
        return self._find_inputs(self._agent.inputs_record_schema)
    ...
    def _find_inputs(self, schema):
        ...
        if isinstance(schema_type, list):
            ...
            if schema_type in self._agent.schemaDefs:
                schema = self._agent.schemaDefs[schema_type]

            if schema["type"] == "record":
                return map(_simple_field_to_input, schema["fields"])

    def output_instances(self):
        outputs_schema = self._agent.outputs_record_schema
    ...
    def docker_identifier(self):
        ...
        reqs_and_hints = agent.get("requirements", []) + agent.get("hints", [])
...
class JobProxy(object):
    ...
    def __init__(self, agent_proxy, input_dict, job_directory):
        self._agent_proxy = agent_proxy
        self._input_dict = input_dict
        self._job_directory = job_directory

    def cwl_job(self):
        return self._agent_proxy._agent.job(
            self._input_dict,
            self._job_directory,
    ...
    def save_job(self):
        ...
        job_objects = {
            "agent_path": os.path.abspath(self._agent_proxy._agent_path),
            "job_inputs": self._input_dict,
        }
        json.dump(job_objects, open(job_file, "w"))

    # @staticmethod
    # def load_job(agent_proxy, job_directory):
    #     job_file = JobProxy._job_file(job_directory)
    #     input_dict = json.load(open(job_file, "r"))
    #     return JobProxy(agent_proxy, input_dict, job_directory)
