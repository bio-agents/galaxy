...
from .resolver_mixins import UsesAgentDependencyDirMixin

import logging
log = logging.getLogger( __name__ )


class BaseGalaxyPackageDependencyResolver(DependencyResolver, UsesAgentDependencyDirMixin):
    dict_collection_visible_keys = DependencyResolver.dict_collection_visible_keys + ['base_path', 'versionless']

    def __init__(self, dependency_manager, **kwds):
        # Galaxy agent shed requires explicit versions on XML elements,
        # this in inconvient for testing or Galaxy instances not utilizing
        # the agent shed so allow a fallback version of the Galaxy package
