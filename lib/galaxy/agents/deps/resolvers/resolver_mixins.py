...
class UsesAgentDependencyDirMixin:
...
class UsesInstalledRepositoriesMixin:
    ...
    def _get_installed_dependency( self, name, type, version=None, **kwds ):
        installed_agent_dependencies = kwds.get("installed_agent_dependencies", [])
        for installed_agent_dependency in (installed_agent_dependencies or []):
            name_and_type_equal = installed_agent_dependency.name == name and installed_agent_dependency.type == type
            if version:
                if name_and_type_equal and installed_agent_dependency.version == version:
                    return installed_agent_dependency
            else:
