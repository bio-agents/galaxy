...
from ..requirements import AgentRequirement
...
class DependencyResolver(Dictifiable, object):
    ...
    # A "simple" dependency is one that does not depend on the the agent
    # resolving the dependency. Classic agent shed dependencies are non-simple
    # because the repository install context is used in dependency resolution
    # so the same requirement tags in different agents will have very different
    # resolution.
    ...
    def resolve( self, name, version, type, **kwds ):
        ...
        resolved agent dependency version (which may differ from requested
...
class ListableDependencyResolver:
    ...
    def _to_requirement(self, name, version=None):
        return AgentRequirement(name=name, type="package", version=version)
