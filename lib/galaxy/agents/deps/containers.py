...
class ContainerFinder(object):
    ...
    def find_container(self, agent_info, destination_info, job_info):
        ...
        # do not trust the containers annotated in agents.
        ...
        # Otherwise lets see if we can find container for the agent.

        # Exact matches first from explicitly listed containers in agents...
        for container_description in agent_info.container_descriptions:
        ...
        # is good enough.
        container_descriptions = self.container_registry.container_descriptions_for_requirements(agent_info.requirements)
    ...
    def __destination_container(self, container_id, container_type, agent_info, destination_info, job_info):
        ...
        # auto-fetched, etc....
        return CONTAINER_CLASSES[container_type](container_id, self.app_info, agent_info, destination_info, job_info)
...
class AgentInfo(object):
    # TODO: Introduce agent XML syntax to annotate the optional environment
...
class JobInfo(object):
    ...
    def __init__(self, working_directory, agent_directory, job_directory):
        self.working_directory = working_directory
        self.job_directory = job_directory
        # Agent files may be remote staged - so this is unintuitively a property
        # of the job not of the agent.
...
class Container( object ):
    ...
    def __init__(self, container_id, app_info, agent_info, destination_info, job_info):
...
class DockerContainer(Container):
    ...
    def containerize_command(self, command):
        ...
        env_directives = []
        for pass_through_var in self.agent_info.env_pass_through:
        ...
        # pass through only what agent needs however. (See todo in AgentInfo.)
    ...
    def __expand_str(self, value):
        ...
        add_var("agent_directory", self.job_info.agent_directory)
        ...
        if self.job_info.job_directory:
            ...
            defaults = "$job_directory:ro,$agent_directory:ro,$job_directory/outputs:rw,$working_directory:rw"
            ...
            defaults = "$galaxy_root:ro,$agent_directory:ro,$working_directory:rw,$default_file_path:ro"
        else:
            defaults = "$galaxy_root:ro,$agent_directory:ro,$working_directory:rw,$default_file_path:rw"
