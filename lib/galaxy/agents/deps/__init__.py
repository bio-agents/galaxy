...
from .resolvers.agent_shed_packages import AgentShedPackageDependencyResolver
...
def build_dependency_manager( config ):
    if getattr( config, "use_agent_dependencies", False ):
        dependency_manager_kwds = {
            'default_base_path': config.agent_dependency_dir,
...
class NullDependencyManager( object ):
    ...
    def uses_agent_shed_dependencies(self):
        return False
...
class DependencyManager( object ):
    ...
    def __init__( self, default_base_path, conf_file=None, **extra_config ):
        ...
        in `base_paths`.  The default base path is app.config.agent_dependency_dir.
        """
    ...
    def uses_agent_shed_dependencies(self):
        return any( map( lambda r: isinstance( r, AgentShedPackageDependencyResolver ), self.dependency_resolvers ) )
    ...
    def __default_dependency_resolvers( self ):
        return [
            AgentShedPackageDependencyResolver(self),
    ...
    def __resolvers_dict( self ):
        import galaxy.agents.deps.resolvers
        return plugin_config.plugins_dict( galaxy.agents.deps.resolvers, 'resolver_type' )
