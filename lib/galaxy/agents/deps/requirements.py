...
class AgentRequirement( object ):
    """
    Represents an external requirement that must be available for the agent to
    ...
    def from_dict( dict ):
        ...
        return AgentRequirement( name=name, type=type, version=version )
...
def parse_requirements_from_dict( root_dict ):
    ...
    return map(AgentRequirement.from_dict, requirements), map(ContainerDescription.from_dict, containers)
...
def parse_requirements_from_xml( xml_root ):
    ...
    ...     contents_document = '''<agent><requirements>%s</requirements></agent>'''
    ...
    for requirement_elem in requirement_elems:
        ...
        requirement = AgentRequirement( name=name, type=type, version=version )
