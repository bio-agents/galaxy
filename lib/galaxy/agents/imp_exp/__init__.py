...
from galaxy.agents.parameters.basic import UnvalidatedValue
            ...
            try:
                ...
                for dataset_attrs in datasets_attrs:
                    ...
                    if hda.extension == 'bam':
                        self.app.datatypes_registry.set_external_metadata_agent.agent_action.execute_via_app(
                            self.app.datatypes_registry.set_external_metadata_agent, self.app, jiha.job.session_id,
                ...
                for job_attrs in jobs_attrs:
                    ...
                    imported_job.agent_id = job_attrs[ 'agent_id' ]
                    imported_job.agent_version = job_attrs[ 'agent_version' ]
...
class JobExportHistoryArchiveWrapper( object, UsesAnnotations ):
    ...
    def setup_job( self, trans, jeha, include_hidden=False, include_deleted=False ):
        ...
        for id, job in jobs_dict.items():
            job_attrs = {}
            job_attrs[ 'agent_id' ] = job.agent_id
            job_attrs[ 'agent_version' ] = job.agent_version
        ...
        #
        # Create and return command line for running agent.
        #
        options = ""
        if jeha.compressed:
            options = "-G"
