    ...
    if uploaded_dataset.get( 'in_folder', False ):
        ...
        for name in uploaded_dataset.in_folder.split( os.path.sep ):
            ...
            if matches:
                ...
                new_folder = trans.app.model.LibraryFolder( name=name, description='Automatically created by upload agent' )
...
def create_paramfile( trans, uploaded_datasets ):
    """
    Create the upload agent's JSON "param" file.
    """
    def _chown( path ):
        try:
...
def create_job( trans, params, agent, json_file_path, data_list, folder=None, history=None, job_params=None ):
    """
    ...
    trans.sa_session.flush()
    log.info( 'agent %s created job id %d' % ( agent.id, job.id ) )
    trans.log_event( 'created job id %d' % job.id, agent_id=agent.id )

    for name, value in agent.params_to_strings( params, trans.app ).iteritems():
    ...
    trans.app.job_queue.put( job.id, job.agent_id )
