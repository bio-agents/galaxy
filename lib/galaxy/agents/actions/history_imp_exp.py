...
from galaxy.agents.actions import AgentAction
from galaxy.agents.imp_exp import JobExportHistoryArchiveWrapper
...
class ImportHistoryAgentAction( AgentAction ):
    """Agent action used for importing a history to an archive. """

    def execute( self, agent, trans, incoming={}, set_output_hid=False, overwrite=True, history=None, **kwargs ):
        #
        # Create job.
        #
        ...
        job.set_handler(agent.get_job_handler(None))
        trans.sa_session.flush()

        # Queue the job for execution
        trans.app.job_queue.put( job.id, agent.id )
        trans.log_event( "Added import history job to the job queue, id: %s" % str(job.id), agent_id=job.agent_id )

        return job, odict()


class ExportHistoryAgentAction( AgentAction ):
    """Agent action used for exporting a history to an archive. """
    ...
    def execute( self, agent, trans, incoming={}, set_output_hid=False, overwrite=True, history=None, **kwargs ):
        ...
        # Queue the job for execution
        trans.app.job_queue.put( job.id, agent.id )
        trans.log_event( "Added export history job to the job queue, id: %s" % str(job.id), agent_id=job.agent_id )
