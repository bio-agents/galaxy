...
from __init__ import AgentAction
...
class SetMetadataAgentAction( AgentAction ):
    """Agent action used for setting external metadata on an existing dataset"""

    def execute( self, agent, trans, incoming={}, set_output_hid=False, overwrite=True, history=None, job_params=None, **kwargs ):
        """
        Execute using a web transaction.
        """
        job, odict = self.execute_via_app( agent, trans.app, trans.get_galaxy_session().id,
        ...
        trans.log_event( "Added set external metadata job to the job queue, id: %s" % str(job.id), agent_id=job.agent_id )
        return job, odict

    def execute_via_app( self, agent, app, session_id, history_id, user=None,
        ...
        for name, value in agent.params_to_strings( incoming, app ).iteritems():
        ...
        app.job_queue.put( job.id, agent.id )
