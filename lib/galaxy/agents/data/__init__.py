"""
Manage agent data tables, which store (at the application level) data that is
used by agents, for example in the generation of dynamic options. Tables are
loaded and stored by names which agents use to refer to them. This allows
users to configure data tables for a local Galaxy instance without needing
to modify the agent configurations.
"""
...
class AgentDataTableManager( object ):
    """Manages a collection of agent data tables"""

    def __init__( self, agent_data_path, config_filename=None ):
        self.agent_data_path = agent_data_path
        # This stores all defined data table entries from both the agent_data_table_conf.xml file and the shed_agent_data_table_conf.xml file
        # at server startup. If agent shed repositories are installed that contain a valid file named agent_data_table_conf.xml.sample, entries
        ...
        for single_config_filename in util.listify( config_filename ):
            if not single_config_filename:
                continue
            self.load_from_config_file( single_config_filename, self.agent_data_path, from_shed_config=False )
    ...
    def load_from_config_file( self, config_filename, agent_data_path, from_shed_config=False ):
        """
        This method is called under 3 conditions:

        1. When the AgentDataTableManager is initialized (see __init__ above).
        2. Just after the AgentDataTableManager is initialized and the additional entries defined by shed_agent_data_table_conf.xml
           are being loaded into the AgentDataTableManager.data_tables.
        3. When a agent shed repository that includes a agent_data_table_conf.xml.sample file is being installed into a local
        ...
        for filename in config_filename:
            ...
            for table_elem in root.findall( 'table' ):
                table = AgentDataTable.from_elem( table_elem, agent_data_path, from_shed_config, filename=filename )
                table_elems.append( table_elem )
                if table.name not in self.data_tables:
                    self.data_tables[ table.name ] = table
                    log.debug( "Loaded agent data table '%s'", table.name )
                else:
                    log.debug( "Loading another instance of data table '%s', attempting to merge content.", table.name )
                    self.data_tables[ table.name ].merge_agent_data_table( table, allow_duplicates=False )  # only merge content, do not persist to disk, do not allow duplicate rows when merging
                    # FIXME: This does not account for an entry with the same unique build ID, but a different path.
        return table_elems

    def add_new_entries_from_config_file( self, config_filename, agent_data_path, shed_agent_data_table_config, persist=False ):
        """
        This method is called when a agent shed repository that includes a agent_data_table_conf.xml.sample file is being
        installed into a local galaxy instance.  We have 2 cases to handle, files whose root tag is <tables>, for example::
            ...
            <tables>
                ...
                <table name="tmap_indexes" comment_char="#">
                    ...
                    <file path="agent-data/tmap_index.loc" />
                </table>
            </tables>
        ...
        try:
            ...
            table_elems = []
        if persist:
            # Persist Galaxy's version of the changed agent_data_table_conf.xml file.
            self.to_xml_file( shed_agent_data_table_config, table_elems )
        return table_elems, error_message

    def to_xml_file( self, shed_agent_data_table_config, new_elems=None, remove_elems=None ):
        """
        Write the current in-memory version of the shed_agent_data_table_conf.xml file to disk.
        remove_elems are removed before new_elems are added.
        """
        if not ( new_elems or remove_elems ):
            log.debug( 'AgentDataTableManager.to_xml_file called without any elements to add or remove.' )
            return  # no changes provided, no need to persist any changes
        if not new_elems:
            new_elems = []
        if not remove_elems:
            remove_elems = []
        full_path = os.path.abspath( shed_agent_data_table_config )
        ...
        try:
            ...
            out_elems = []
            log.debug( 'Could not parse existing agent data table config, assume no existing elements: %s', e )
    ...
    def reload_tables( self, table_names=None ):
        ...
        for table_name in table_names:
            ...
            log.debug( "Reloaded agent data table '%s' from files.", table_name )
        return table_names


class AgentDataTable( object ):

    @classmethod
    def from_elem( cls, table_elem, agent_data_path, from_shed_config, filename ):
        table_type = table_elem.get( 'type', 'tabular' )
        assert table_type in agent_data_table_types, "Unknown data table type '%s'" % type
        return agent_data_table_types[ table_type ]( table_elem, agent_data_path, from_shed_config=from_shed_config, filename=filename )

    def __init__( self, config_element, agent_data_path, from_shed_config=False, filename=None ):
        ...
        self._load_info = ( [ config_element, agent_data_path ], { 'from_shed_config': from_shed_config } )
    ...
    def merge_agent_data_table( self, other_table, allow_duplicates=True, persist=False, persist_on_error=False, entry_source=None, **kwd ):
        raise NotImplementedError( "Abstract method" )
    ...
    def reload_from_files( self ):
        ...
        for ( agent_data_table_class, load_info ) in merged_info:
            self.merge_agent_data_table( agent_data_table_class( *load_info[0], **load_info[1] ), allow_duplicates=False )
        return self._update_version()


class TabularAgentDataTable( AgentDataTable, Dictifiable ):
    """
    ...
    def __init__( self, config_element, agent_data_path, from_shed_config=False, filename=None ):
        super( TabularAgentDataTable, self ).__init__( config_element, agent_data_path, from_shed_config, filename)
        self.config_element = config_element
        self.data = []
        self.configure_and_load( config_element, agent_data_path, from_shed_config)

    def configure_and_load( self, config_element, agent_data_path, from_shed_config=False, url_timeout=10 ):
        """
        ...
        repo_elem = config_element.find( 'agent_shed_repository' )
        if repo_elem is not None:
            repo_info = dict( agent_shed=repo_elem.find( 'agent_shed' ).text, name=repo_elem.find( 'repository_name' ).text,
        ...
        for file_element in config_element.findall( 'file' ):
            ...
            if file_path is None:
                log.debug( "Encountered a file element (%s) that does not contain a path value when loading agent data table '%s'.", util.xml_to_string( file_element ), self.name )
                continue
            ...
            # point directly to the desired file to load. Munging of the agent_data_tables_conf.xml.sample
            # can be done during installing / testing / metadata resetting with the creation of a proper
            # agent_data_tables_conf.xml file, containing correct <file path=> attributes. Allowing a
            # path.join with a different root should be allowed, but splitting should not be necessary.
            if agent_data_path and from_shed_config:
                # Must identify with from_shed_config as well, because the
                # regular galaxy app has and uses agent_data_path.
                # We're loading a agent in the agent shed, so we cannot use the Galaxy agent-data
                # directory which is hard-coded into the agent_data_table_conf.xml entries.
                filename = os.path.split( file_path )[ 1 ]
                filename = os.path.join( agent_data_path, filename )
            if os.path.exists( filename ):
                found = True
            else:
                # Since the path attribute can include a hard-coded path to a specific directory
                # (e.g., <file path="agent-data/cg_crr_files.loc" />) which may not be the same value
                # as self.agent_data_path, we'll parse the path to get the filename and see if it is
                # in self.agent_data_path.
                file_path, file_name = os.path.split( filename )
                if file_path and file_path != self.agent_data_path:
                    corrected_filename = os.path.join( self.agent_data_path, file_name )
            ...
            if found:
                ...
                log.warn( "Cannot find index file '%s' for agent data table '%s'" % ( filename, self.name ) )

            if filename not in self.filenames or not self.filenames[ filename ][ 'found' ]:
                self.filenames[ filename ] = dict( found=found, filename=filename, from_shed_config=from_shed_config, agent_data_path=agent_data_path,
                                                   config_element=config_element, agent_shed_repository=repo_info, errors=errors )
            else:
    ...
    def merge_agent_data_table( self, other_table, allow_duplicates=True, persist=False, persist_on_error=False, entry_source=None, **kwd ):
        ...
        if self.allow_duplicate_entries and not other_table.allow_duplicate_entries:
            log.debug( 'While attempting to merge agent data table "%s", the other instance of the table specified that duplicate entries are not allowed, now deduplicating all previous entries.', self.name )
        ...
        for i, line in enumerate( reader ):
            ...
            if line:
                ...
                if self.largest_index < len( fields ):
                    ...
                    line_error = "Line %i in agent data table '%s' is invalid (HINT: '%s' characters must be used to separate fields):\n%s" % ( ( i + 1 ), self.name, separator_char, line )
    ...
    def get_filename_for_source( self, source, default=None ):
        if source:
            ...
            if isinstance( source, dict ):
                ...
                source_repo_info = source.get_agent_shed_repository_info_dict()
        else:
            source_repo_info = None
        filename = default
        for name, value in self.filenames.iteritems():
            repo_info = value.get( 'agent_shed_repository', None )
    ...
    def _deduplicate_data( self ):
        ...
        for i, fields in enumerate( self.data ):
            ...
            if fields_hash in hash_list:
                dup_lines.append( i )
                log.debug( 'Found duplicate entry in agent data table "%s", but duplicates are not allowed, removing additional entry for: "%s"', self.name, fields )
            else:
    ...
    def to_dict(self, view='collection'):
        rval = super(TabularAgentDataTable, self).to_dict()
        if view == 'element':
...
class TabularAgentDataField(Dictifiable, object):
    ...
    def to_dict(self):
        rval = super(TabularAgentDataField, self).to_dict()
...
# Registry of agent data types by type_key
agent_data_table_types = dict( [ ( cls.type_key, cls ) for cls in [ TabularAgentDataTable ] ] )
