...
class AgentParameterValueWrapper( object ):
    """
    Base class for object that Wraps a Agent Parameter and Value.
    """
    ...
    def get_display_text( self, quote=True ):
        """
        Returns a string containing the value that would be displayed to the user in the agent interface.
        When quote is True (default), the string is escaped for e.g. command-line usage.
        """
        rval = self.input.value_to_display_text( self.value, self.input.agent.app ) or ''
        if quote:
            return pipes.quote( rval ) or "''"  # pipes.quote in Python < 2.7 returns an empty string instead of the expected quoted empty string
        return rval


class RawObjectWrapper( AgentParameterValueWrapper ):
    """
...
class InputValueWrapper( AgentParameterValueWrapper ):
    """
...
class SelectAgentParameterWrapper( AgentParameterValueWrapper ):
    """
    Wraps a SelectTooParameter so that __str__ returns the selected value, but all other
    attributes are accessible.
    """

    class SelectAgentParameterFieldWrapper:
        """
    ...
    def __init__( self, input, value, app, other_values={}, path_rewriter=None ):
        ...
        self.fields = self.SelectAgentParameterFieldWrapper( input, value, other_values, self._path_rewriter )
...
class DatasetFilenameWrapper( AgentParameterValueWrapper ):
    """
    ...
    def __init__( self, dataset, datatypes_registry=None, agent=None, name=None, dataset_path=None, identifier=None ):
        if not dataset:
            try:
                # TODO: allow this to work when working with grouping
                ext = agent.inputs[name].extensions[0]
            except:
                ext = 'data'
            self.dataset = wrap_with_safe_string( NoneDataset( datatypes_registry=datatypes_registry, ext=ext ), no_wrap_classes=AgentParameterValueWrapper )
        else:
            # Agent wrappers should not normally be accessing .dataset directly,
            # so we will wrap it and keep the original around for file paths
            # Should we name this .value to maintain consistency with most other AgentParameterValueWrapper?
            self.unsanitized = dataset
            self.dataset = wrap_with_safe_string( dataset, no_wrap_classes=AgentParameterValueWrapper )
...
class DatasetListWrapper( list, AgentParameterValueWrapper, HasDatasets ):
    """
    """
...
class DatasetCollectionWrapper( AgentParameterValueWrapper, HasDatasets ):
