"""
Classes encapsulating galaxy agents and agent configuration.
"""
...
from galaxy.agents.actions import DefaultAgentAction
from galaxy.agents.actions.upload import UploadAgentAction
from galaxy.agents.actions.data_source import DataSourceAgentAction
from galaxy.agents.actions.data_manager import DataManagerAgentAction
from galaxy.agents.parameters import params_to_incoming, check_param, params_from_strings, params_to_strings, visit_input_values
from galaxy.agents.parameters import output_collect
from galaxy.agents.parameters.basic import (BaseURLAgentParameter,
                                           DataAgentParameter, DataCollectionAgentParameter, HiddenAgentParameter,
                                           SelectAgentParameter, AgentParameter, UnvalidatedValue,
                                           contains_workflow_parameter)
from galaxy.agents.parameters.grouping import Conditional, ConditionalWhen, Repeat, Section, UploadDataset
from galaxy.agents.parameters.input_translation import AgentInputTranslator
from galaxy.agents.parameters.validation import LateValidationError
from galaxy.agents.test import parse_tests
from galaxy.agents.parser import get_agent_source
from galaxy.agents.parser.xml import XmlPageSource
from galaxy.agents.parser import AgentOutputCollectionPart
from galaxy.agents.agentbox import BaseGalaxyAgentBox
from galaxy.util import rst_to_html, string_as_bool
from galaxy.util import ExecutionTimer
from galaxy.util import listify
from galaxy.agents.parameters.meta import expand_meta_parameters
...
from .loader import template_macro_params, raw_agent_xml_tree, imported_macro_paths
...
global_agent_errors = AgentErrorLog()
...
class AgentBox( BaseGalaxyAgentBox ):
    """ A derivative of AbstractAgentBox with knowledge about Agent internals -
    how to construct them, action types, dependency management, etc....
    """

    def __init__( self, config_filenames, agent_root_dir, app ):
        super( AgentBox, self ).__init__(
    ...
    def create_agent( self, config_file, repository_id=None, guid=None, **kwds ):
        try:
            agent_source = get_agent_source( config_file, getattr( self.app.config, "enable_beta_agent_formats", False ) )
        except Exception, e:
            # capture and log parsing errors
            global_agent_errors.add_error(config_file, "Agent XML parsing", e)
            raise e
        # Allow specifying a different agent subclass to instantiate
        agent_module = agent_source.parse_agent_module()
        if agent_module is not None:
            module, cls = agent_module
            mod = __import__( module, globals(), locals(), [cls] )
            AgentClass = getattr( mod, cls )
        elif agent_source.parse_agent_type():
            agent_type = agent_source.parse_agent_type()
            AgentClass = agent_types.get( agent_type )
        else:
            # Normal agent - only insert dynamic resource parameters for these
            # agents.
            root = getattr( agent_source, "root", None )
            # TODO: mucking with the XML directly like this is terrible,
            # modify inputs directly post load if possible.
            if root is not None and hasattr( self.app, "job_config" ):  # agentshed may not have job_config?
                agent_id = root.get( 'id' )
                parameters = self.app.job_config.get_agent_resource_parameters( agent_id )
                if parameters:
                    inputs = root.find('inputs')
                    # If agent has not inputs, create some so we can insert conditional
                    if inputs is None:
            ...
            AgentClass = Agent
        agent = AgentClass( config_file, agent_source, self.app, guid=guid, repository_id=repository_id, **kwds )
        return agent

    def handle_datatypes_changed( self ):
        """ Refresh upload agents when new datatypes are added. """
        for agent_id in self._agents_by_id:
            agent = self._agents_by_id[ agent_id ]
            if isinstance( agent.agent_action, UploadAgentAction ):
                self.reload_agent_by_id( agent_id )

    def get_agent_components( self, agent_id, agent_version=None, get_loaded_agents_by_lineage=False, set_selected=False ):
        """
        Retrieve all loaded versions of a agent from the agentbox and return a select list enabling
        selection of a different version, the list of the agent's loaded versions, and the specified agent.
        """
        agentbox = self
        agent_version_select_field = None
        agents = []
        agent = None
        # Backwards compatibility for datasource agents that have default agent_id configured, but which
        # are now using only GALAXY_URL.
        agent_ids = listify( agent_id )
        for agent_id in agent_ids:
            if get_loaded_agents_by_lineage:
                agents = agentbox.get_loaded_agents_by_lineage( agent_id )
            else:
                agents = agentbox.get_agent( agent_id, agent_version=agent_version, get_all_versions=True )
            if agents:
                agent = agentbox.get_agent( agent_id, agent_version=agent_version, get_all_versions=False )
                if len( agents ) > 1:
                    agent_version_select_field = self.__build_agent_version_select_field( agents, agent.id, set_selected )
                break
        return agent_version_select_field, agents, agent

    def _get_agent_shed_repository( self, agent_shed, name, owner, installed_changeset_revision ):
        # Abstract agentbox doesn't have a dependency on the the database, so
        # override _get_agent_shed_repository here to provide this information.
    ...
    def __build_agent_version_select_field( self, agents, agent_id, set_selected ):
        """Build a SelectField whose options are the ids for the received list of agents."""
        options = []
        refresh_on_change_values = []
        for agent in agents:
            options.insert( 0, ( agent.version, agent.id ) )
            refresh_on_change_values.append( agent.id )
        select_field = SelectField( name='agent_id', refresh_on_change=True, refresh_on_change_values=refresh_on_change_values )
        for option_tup in options:
            selected = set_selected and option_tup[ 1 ] == agent_id
            if selected:
...
class DefaultAgentState( object ):
    """
    Keeps track of the state of a users interaction with a agent between
    requests. The default agent state keeps track of the current page (for
    multipage "wizard" agents) and the values of all
    """
    def __init__( self ):
        self.page = 0
    ...
    def encode( self, agent, app, secure=True ):
        """
        Convert the data to a string
        """
        # Convert parameters to a dictionary of strings, and save curent
        # page in that dict
        value = params_to_strings( agent.inputs, self.inputs, app )
        value["__page__"] = self.page
        value["__rerun_remap_job_id__"] = self.rerun_remap_job_id
        value = json.dumps( value )
        # Make it secure
        if secure:
            a = hmac_new( app.config.agent_secret, value )
            b = binascii.hexlify( value )
            return "%s:%s" % ( a, b )
        else:
            return value

    def decode( self, value, agent, app, secure=True ):
        """
        ...
        if secure:
            ...
            test = hmac_new( app.config.agent_secret, value )
            assert a == test
        # Restore from string
        ...
        if '__rerun_remap_job_id__' in values:
            ...
            self.rerun_remap_job_id = None
        self.inputs = params_from_strings( agent.inputs, values, app, ignore_errors=True )
...
class Agent( object, Dictifiable ):
    """
    Represents a computational agent that can be executed through Galaxy.
    """

    agent_type = 'default'
    requires_setting_metadata = True
    default_agent_action = DefaultAgentAction
    dict_collection_visible_keys = ( 'id', 'name', 'version', 'description', 'labels' )

    def __init__( self, config_file, agent_source, app, guid=None, repository_id=None, allow_code_files=True ):
        """Load a agent from the config named by `config_file`"""
        # Determine the full path of the directory where the agent config is
        self.config_file = config_file
        self.agent_dir = os.path.dirname( config_file )
        self.app = app
        ...
        self.action = '/agent_runner/index'
        ...
        # agent_data_table_conf.xml entries exist.
        self.input_params = []
        # Attributes of agents installed from Galaxy agent sheds.
        self.agent_shed = None
        self.repository_name = None
        self.repository_owner = None
        self.installed_changeset_revision = None
        # The agent.id value will be the value of guid, but we'll keep the
        # guid attribute since it is useful to have.
        self.guid = guid
        self.old_id = None
        self.version = None
        # Enable easy access to this agent's version lineage.
        self.lineage_ids = []
        # populate agentshed repository info, if available
        self.populate_agent_shed_info()
        # Parse XML element containing configuration
        try:
            self.parse( agent_source, guid=guid )
        except Exception, e:
            global_agent_errors.add_error(config_file, "Agent Loading", e)
            raise e
    ...
    def agent_version( self ):
        """Return a AgentVersion if one exists for our id"""
        return self.app.install_model.context.query( self.app.install_model.AgentVersion ) \
                                             .filter( self.app.install_model.AgentVersion.table.c.agent_id == self.id ) \
                                             .first()
    ...
    def agent_versions( self ):
        ...
        if agent_version:
            return agent_version.get_versions( self.app )
        return []

    @property
    def agent_shed_repository( self ):
        # If this agent is included in an installed agent shed repository, return it.
        if self.agent_shed:
            return suc.get_installed_repository( self.app,
                                                 agent_shed=self.agent_shed,
    ...
    def __get_job_agent_configuration(self, job_params=None):
        """Generalized method for getting this agent's job configuration.

        :type job_params: dict or None
        :returns: `galaxy.jobs.JobAgentConfiguration` -- JobAgentConfiguration that matches this `Agent` and the given `job_params`
        """
        rval = None
        if len(self.job_agent_configurations) == 1:
            # If there's only one config, use it rather than wasting time on comparisons
            rval = self.job_agent_configurations[0]
        elif job_params is None:
            for job_agent_config in self.job_agent_configurations:
                if not job_agent_config.params:
                    rval = job_agent_config
                    break
        else:
            for job_agent_config in self.job_agent_configurations:
                if job_agent_config.params:
                    # There are job params and this config has params defined
                    for param, value in job_params.items():
                        if param not in job_agent_config.params or job_agent_config.params[param] != job_params[param]:
                            break
                    else:
                    ...
                    rval = job_agent_config
        assert rval is not None, 'Could not get a job agent configuration for Agent %s with job_params %s, this is a bug' % (self.id, job_params)
        return rval

    def get_job_handler(self, job_params=None):
        """Get a suitable job handler for this `Agent` given the provided `job_params`.  If multiple handlers are valid for combination of `Agent` and `job_params` (e.g. the defined handler is a handler tag), one will be selected at random.

        :param job_params: Any params specific to this job (e.g. the job source)
        :type job_params: dict or None

        :returns: str -- The id of a job handler for a job run of this `Agent`
        """
        # convert tag to ID if necessary
        return self.app.job_config.get_handler(self.__get_job_agent_configuration(job_params=job_params).handler)
    ...
    def get_job_destination(self, job_params=None):
        ...
        """
        return self.app.job_config.get_destination(self.__get_job_agent_configuration(job_params=job_params).destination)

    def get_panel_section( self ):
        return self.app.agentbox.get_integrated_section_for_agent( self )

    def allow_user_access( self, user, attempting_access=True ):
        """
        :returns: bool -- Whether the user is allowed to access the agent.
        """
        return True

    def parse( self, agent_source, guid=None ):
        """
        Read agent configuration from the element `root` and fill in `self`.
        """
        # Get the UNIQUE id for the agent
        self.old_id = agent_source.parse_id()
        if guid is None:
            self.id = self.old_id
        else:
            self.id = guid
        if not self.id:
            raise Exception( "Missing agent 'id'" )

        # Get the (user visible) name of the agent
        self.name = agent_source.parse_name()
        if not self.name:
            raise Exception( "Missing agent 'name'" )

        self.version = agent_source.parse_version()
        if not self.version:
            # For backward compatibility, some agents may not have versions yet.
            self.version = "1.0.0"

        # Support multi-byte agents
        self.is_multi_byte = agent_source.parse_is_multi_byte()
        # Legacy feature, ignored by UI.
        self.force_history_refresh = False

        self.display_interface = agent_source.parse_display_interface( default=self.display_interface )

        self.require_login = agent_source.parse_require_login( self.require_login )

        request_param_translation_elem = agent_source.parse_request_param_translation_elem()
        if request_param_translation_elem is not None:
            # Load input translator, used by datasource agents to change names/values of incoming parameters
            self.input_translator = AgentInputTranslator.from_element( request_param_translation_elem )
        else:
            self.input_translator = None

        self.parse_command( agent_source )
        self.environment_variables = self.parse_environment_variables( agent_source )

        # Parameters used to build URL for redirection to external app
        redirect_url_params = agent_source.parse_redirect_url_params_elem()
        ...
        self.description = agent_source.parse_description()

        # Versioning for agents
        self.version_string_cmd = None
        version_command = agent_source.parse_version_command()
        if version_command is not None:
            self.version_string_cmd = version_command.strip()

            version_cmd_interpreter = agent_source.parse_version_command_interpreter()
            if version_cmd_interpreter:
                executable = self.version_string_cmd.split()[0]
                abs_executable = os.path.abspath(os.path.join(self.agent_dir, executable))
        ...
        # Parallelism for tasks, read from agent config.
        self.parallelism = agent_source.parse_parallelism()

        # Get JobAgentConfiguration(s) valid for this particular Agent.  At least
        ...
        if self.old_id != self.id:
            # Handle agentshed guids
            self_ids = [ self.id.lower(), self.id.lower().rsplit('/', 1)[0], self.old_id.lower() ]
        self.all_ids = self_ids
        # In the agentshed context, there is no job config.
        if 'job_config' in dir(self.app):
            self.job_agent_configurations = self.app.job_config.get_job_agent_configurations(self_ids)

        # Is this a 'hidden' agent (hidden in agent menu)
        self.hidden = agent_source.parse_hidden()

        self.__parse_legacy_features(agent_source)

        # Load any agent specific options (optional)
        self.options = dict( sanitize=True, refresh=False )
        self.__update_options_dict( agent_source )
        self.options = Bunch(** self.options)

        # Parse agent inputs (if there are any required)
        self.parse_inputs( agent_source )

        # Parse agent help
        self.parse_help( agent_source )

        # Description of outputs produced by an invocation of the agent
        self.parse_outputs( agent_source )

        # Parse result handling for agent exit codes and stdout/stderr messages:
        self.parse_stdio( agent_source )
        # Any extra generated config files for the agent
        self.__parse_config_files(agent_source)
        # Action
        action = agent_source.parse_action_module()
        if action is None:
            self.agent_action = self.default_agent_action()
        else:
            module, cls = action
            mod = __import__( module, globals(), locals(), [cls])
            self.agent_action = getattr( mod, cls )()
        # Tests
        self.__parse_tests(agent_source)

        # Requirements (dependencies)
        requirements, containers = agent_source.parse_requirements_and_containers()
        self.requirements = requirements
        self.containers = containers

        self.citations = self._parse_citations( agent_source )

        # Determine if this agent can be used in workflows
        self.is_workflow_compatible = self.check_workflow_compatible(agent_source)
        self.__parse_trackster_conf( agent_source )

    def __parse_legacy_features(self, agent_source):
        self.code_namespace = dict()
        self.hook_map = {}
        self.uihints = {}

        if not hasattr(agent_source, 'root'):
            return

        # TODO: Move following logic into XmlAgentSource.
        root = agent_source.root
        # Load any agent specific code (optional) Edit: INS 5/29/2007,
        # allow code files to have access to the individual agent's
        ...
        if self._allow_code_files:
            for code_elem in root.findall("code"):
                ...
                code_path = os.path.join( self.agent_dir, file_name )
    ...
    def __update_options_dict(self, agent_source):
        # TODO: Move following logic into AgentSource abstraction.
        if not hasattr(agent_source, 'root'):
            return
    ...
    def __parse_tests(self, agent_source):
        self.__tests_source = agent_source
        self.__tests_populated = False

    def __parse_config_files(self, agent_source):
        self.config_files = []
        if not hasattr(agent_source, 'root'):
            return
    ...
    def __parse_trackster_conf(self, agent_source):
        self.trackster_conf = None
        if not hasattr(agent_source, 'root'):
            return

        # Trackster configuration.
        trackster_conf = agent_source.root.find( "trackster_conf" )
        ...
        if not self.__tests_populated:
            ...
            if tests_source:
                try:
                    ...
                    self.__tests = None
                    log.exception( "Failed to parse agent tests" )
            else:
                self.__tests = None
            self.__tests_populated = True
        return self.__tests

    def parse_command( self, agent_source ):
        """
        """
        # Command line (template). Optional for agents that do not invoke a local program
        command = agent_source.parse_command()
        if command is not None:
            ...
            self.interpreter = agent_source.parse_interpreter()
        else:
            self.command = ''
            self.interpreter = None

    def parse_environment_variables( self, agent_source ):
        return agent_source.parse_environment_variables()

    def parse_inputs( self, agent_source ):
        """
        Parse the "<inputs>" element and create appropriate `AgentParameter`s.
        This implementation supports multiple pages and grouping constructs.
        """
        # Load parameters (optional)
        pages = agent_source.parse_input_pages()
        enctypes = set()
        if pages.inputs_defined:
            if hasattr(pages, "input_elem"):
                ...
                # nginx_upload_path. This logic is handled in the agent_form.mako
                # template.
                if self.nginx_upload and self.app.config.nginx_upload_path:
                    if '?' in urllib.unquote_plus( self.action ):
                        raise Exception( 'URL parameters in a non-default agent action can not be used '
        ...
        # Check if the agent either has no parameters or only hidden (and
        ...
        template_macros = {}
        if hasattr(agent_source, 'root'):
            template_macros = template_macro_params(agent_source.root)
        self.template_macro_params = template_macros
        for param in self.inputs.values():
            if not isinstance( param, ( HiddenAgentParameter, BaseURLAgentParameter ) ):
                self.input_required = True
                break

    def parse_help( self, agent_source ):
        """
        Parse the help text for the agent. Formatted in reStructuredText, but
        ...
        self.__help_source = agent_source

    def parse_outputs( self, agent_source ):
        """
        Parse <outputs> elements and fill in self.outputs (keyed by name)
        """
        self.outputs, self.output_collections = agent_source.parse_outputs(self)

    # TODO: Include the agent's name in any parsing warnings.
    def parse_stdio( self, agent_source ):
        """
        ...
        """
        exit_codes, regexes = agent_source.parse_stdio()
        self.stdio_exit_codes = exit_codes
        self.stdio_regexes = regexes

    def _parse_citations( self, agent_source ):
        # TODO: Move following logic into AgentSource abstraction.
        if not hasattr(agent_source, 'root'):
            return []
        ...
        for citation_elem in citations_elem:
            if citation_elem.tag != "citation":
                pass
            citation = self.app.citations_manager.parse_citation( citation_elem, self.agent_dir )
            if citation:
                ...
                if value_from:
                    ...
                    for (value, case_inputs_source) in input_source.parse_when_input_sources():
                        ...
                        try:
                            ...
                            log.warning( "Agent %s: a when tag has been defined for '%s (%s) --> %s', but does not appear to be selectable." %
                                         ( self.id, group.name, group.test_param.name, case.value ) )
                    for unspecified_case in possible_cases:
                        log.warning( "Agent %s: a when tag has not been defined for '%s (%s) --> %s', assuming empty inputs." %
    ...
    def parse_param_elem( self, input_source, enctypes, context ):
        """
        Parse a single "<param>" element and return a AgentParameter instance.
        Also, if the parameter has a 'required_enctype' add it to the set
        enctypes.
        """
        param = AgentParameter.build( self, input_source )
        ...
        for name in param.get_dependencies():
            ...
            if name not in context:
                log.error("Could not find dependency '%s' of parameter '%s' in agent %s" % (name, param.name, self.name) )
            context[ name ].refresh_on_change = True
        return param

    def populate_agent_shed_info( self ):
        if self.repository_id is not None and self.app.name == 'galaxy':
            repository_id = self.app.security.decode_id( self.repository_id )
            agent_shed_repository = self.app.install_model.context.query( self.app.install_model.AgentShedRepository ).get( repository_id )
            if agent_shed_repository:
                self.agent_shed = agent_shed_repository.agent_shed
                self.repository_name = agent_shed_repository.name
                self.repository_owner = agent_shed_repository.owner
                self.installed_changeset_revision = agent_shed_repository.installed_changeset_revision
    ...
    def __inititalize_help(self):
        agent_source = self.__help_source
        self.__help = None
        self.__help_by_page = []
        help_header = ""
        help_footer = ""
        help_text = agent_source.parse_help()
        if help_text is not None:
            if self.repository_id and help_text.find( '.. image:: ' ) >= 0:
                # Handle agent help image display for agents that are contained in repositories in the agent shed or installed into Galaxy.
                try:
            ...
            try:
                ...
                log.exception( "error in help for agent %s" % self.name )

            # Handle deprecated multi-page help text in XML case.
            if hasattr(agent_source, "root"):
                help_elem = agent_source.root.find("help")
                ...
                try:
                    ...
                    log.exception( "error in multi-page help for agent %s" % self.name )
    ...
    def find_output_def( self, name ):
        ...
        # up that stack.
        if AgentOutputCollectionPart.is_named_collection_part_name( name ):
            collection_name, part = AgentOutputCollectionPart.split_output_name( name )
    ...
    def check_workflow_compatible( self, agent_source ):
        """
        Determine if a agent can be used in workflows. External agents and the
        upload agent are currently not supported by workflows.
        """
        # Multiple page agents are not supported -- we're eliminating most
        # of these anyway
        if self.has_multiple_pages:
            return False
        # This is probably the best bet for detecting external web agents
        # right now
        if self.agent_type.startswith( 'data_source' ):
            return False
        ...
        # TODO: Anyway to capture agents that dynamically change their own
        #       outputs?
        return True

    def new_state( self, trans, history=None ):
        """
        Create a new `DefaultAgentState` for this agent. It will be initialized
    ...
    def fill_in_new_state( self, trans, inputs, state, context=None, history=None ):
        """
        Fill in a agent state dictionary with default values for all parameters
    ...
    def get_param_html_map( self, trans, page=0, other_values={} ):
        ...
        NOTE: This should be considered deprecated, it is only used for agents
              with `display` elements. These should be eliminated.
        """
        rval = dict()
        for key, param in self.inputs_by_page[page].iteritems():
            if not isinstance( param, AgentParameter ):
    ...
    def visit_inputs( self, value, callback ):
        """
        Call the function `callback` on each parameter of this agent. Visits
        ...
        for input in self.inputs.itervalues():
            if isinstance( input, AgentParameter ):
    ...
    def handle_input( self, trans, incoming, history=None, source='html' ):
        """
        Process incoming parameters for this agent from the dict `incoming`,
        update the agent state (or create if none existed), and either return
        to the form or execute the agent (only if 'execute' was clicked and
        ...
        if 'rerun_remap_job_id' in incoming:
            try:
                ...
                raise exceptions.MessageException( 'Failure executing agent (attempting to rerun invalid job).' )

        # Fixed set of input parameters may correspond to any number of jobs.
        # Expand these out to individual parameters for given jobs (agent executions).
        expanded_incomings, collection_info = expand_meta_parameters( trans, self, incoming )
        if not expanded_incomings:
            raise exceptions.MessageException( 'Agent execution failed, trying to run a agent over an empty collection.' )

        # Remapping a single job to many jobs doesn't make sense, so disable
        # remap if multi-runs of agents are being used.
        if rerun_remap_job_id and len( expanded_incomings ) > 1:
            raise exceptions.MessageException( 'Failure executing agent (cannot create multiple jobs when remapping existing job).' )
        ...
        for expanded_incoming in expanded_incomings:
            ...
            if not self.check_values:
                ...
                # If the agent provides a `validate_input` hook, call it.
                if validate_input:
                    validate_input( trans, errors, expanded_state.inputs, self.inputs )
                params = expanded_state.inputs
            all_errors.append( errors )
            all_params.append( params )
        log.debug("Validated and populated state for agent request %s" % validation_timer)
    ...
    def handle_single_execution( self, trans, rerun_remap_job_id, params, history, mapping_over_collection, execution_cache=None ):
        ...
        try:
            ...
            log.exception('Exception caught while attempting agent execution:')
            message = 'Error executing agent: %s' % str(e)
        ...
        if isinstance( out_data, odict ):
            ...
            if isinstance( out_data, str ):
                ...
                message = 'Failure executing agent (invalid data returned from agent execution)'
            return False, message
    ...
    def update_state( self, trans, inputs, state, incoming, source='html', prefix="", context=None,
        ...
        """
        Update the agent state in `state` using the user input in `incoming`.
                   ...
                   and update_only:
                    ...
                    try:
                        ...
                        if input.is_job_resource_conditional:
                            ...
                            # (e.g. from the run agent form) don't populate the
    ...
    def params_with_missing_data_table_entry( self ):
        ...
        options require an entry not currently in the agent_data_table_conf.xml file.
        """
        params = []
        for input_param in self.input_params:
            if isinstance( input_param, SelectAgentParameter ) and input_param.is_dynamic:
                options = input_param.options
                if options and options.missing_agent_data_table_name and input_param not in params:
    ...
    def get_static_param_values( self, trans ):
        """
        Returns a map of parameter names and values if the agent does not
        require any user input. Will raise an exception if any parameter
        does require input.
        """
        args = dict()
        for key, param in self.inputs.iteritems():
            # BaseURLAgentParameter is now a subclass of HiddenAgentParameter, so
            # we must check if param is a BaseURLAgentParameter first
            if isinstance( param, BaseURLAgentParameter ):
                args[key] = param.get_initial_value( trans, None )
            elif isinstance( param, HiddenAgentParameter ):
    ...
    def execute( self, trans, incoming={}, set_output_hid=True, history=None, **kwargs ):
        """
        Execute the agent using parameter values in `incoming`. This just
        dispatches to the `AgentAction` instance specified by
        `self.agent_action`. In general this will create a `Job` that
        when run will build the agent's outputs, e.g. `DefaultAgentAction`.
        """
        return self.agent_action.execute( self, trans, incoming=incoming, set_output_hid=set_output_hid, history=history, **kwargs )
        ...
        for input in inputs.itervalues():
            ...
            # Value, visit recursively as usual
                ...
                if isinstance( input, Repeat ):
                    ...
                    # Regular agent parameter, no recursion needed
                    try:
    ...
    def handle_unvalidated_param_values( self, input_values, app ):
        ...
        validate them (by calling `AgentParameter.from_html` and
        `AgentParameter.validate`).
        """
    ...
    def build_dependency_shell_commands( self, job_directory=None ):
        """Return a list of commands to be run to populate the current environment to include this agents requirements."""
        return self.app.agentbox.dependency_manager.dependency_shell_commands(
            self.requirements,
            installed_agent_dependencies=self.installed_agent_dependencies,
    ...
    def installed_agent_dependencies(self):
        if self.agent_shed_repository:
            installed_agent_dependencies = self.agent_shed_repository.agent_dependencies_installed_or_in_error
        else:
            installed_agent_dependencies = None
        return installed_agent_dependencies
    ...
    def parse_redirect_url( self, data, param_dict ):
        """
        Parse the REDIRECT_URL agent param. Agents that send data to an external
        application via a redirect must include the following 3 agent params:
        ...
        assert DATA_URL is not None, "DATA_URL parameter missing in agent config."
    ...
    def collect_primary_datasets( self, output, job_working_directory, input_ext ):
        """
        Find any additional datasets generated by a agent and attach (for
    ...
    def to_archive(self):
        ...
        tarball_files = []
        temp_files = []
        agent_xml = open( os.path.abspath( agent.config_file ), 'r' ).read()
        # Retrieve agent help images and rewrite the agent's xml into a temporary file with the path
        # modified to be relative to the repository root.
        image_found = False
        if agent.help is not None:
            agent_help = agent.help._source
            # Check each line of the rendered agent help for an image tag that points to a location under static/
            for help_line in agent_help.split( '\n' ):
                image_regex = re.compile( 'img alt="[^"]+" src="\${static_path}/([^"]+)"' )
                matches = re.search( image_regex, help_line )
                if matches is not None:
                    agent_help_image = matches.group(1)
                    tarball_path = agent_help_image
                    filesystem_path = os.path.abspath( os.path.join( self.app.config.root, 'static', agent_help_image ) )
                    if os.path.exists( filesystem_path ):
                        tarball_files.append( ( filesystem_path, tarball_path ) )
                        image_found = True
                        agent_xml = agent_xml.replace( '${static_path}/%s' % tarball_path, tarball_path )
        # If one or more agent help images were found, add the modified agent XML to the tarball instead of the original.
        if image_found:
            fd, new_agent_config = tempfile.mkstemp( suffix='.xml' )
            os.close( fd )
            open( new_agent_config, 'w' ).write( agent_xml )
            agent_tup = ( os.path.abspath( new_agent_config ), os.path.split( agent.config_file )[-1]  )
            temp_files.append( os.path.abspath( new_agent_config ) )
        else:
            agent_tup = ( os.path.abspath( agent.config_file ), os.path.split( agent.config_file )[-1]  )
        tarball_files.append( agent_tup )
        # TODO: This feels hacky.
        agent_command = agent.command.strip().split()[0]
        agent_path = os.path.dirname( os.path.abspath( agent.config_file ) )
        # Add the agent XML to the tuple that will be used to populate the tarball.
        if os.path.exists( os.path.join( agent_path, agent_command ) ):
            tarball_files.append( ( os.path.join( agent_path, agent_command ), agent_command ) )
        # Find and add macros and code files.
        for external_file in agent.get_externally_referenced_paths( os.path.abspath( agent.config_file ) ):
            external_file_abspath = os.path.abspath( os.path.join( agent_path, external_file ) )
            tarball_files.append( ( external_file_abspath, external_file ) )
        if os.path.exists( os.path.join( agent_path, "Dockerfile" ) ):
            tarball_files.append( ( os.path.join( agent_path, "Dockerfile" ), "Dockerfile" ) )
            ...
            for test in tests:
                ...
                for label, filename, _ in test.outputs:
                    ...
                    if os.path.exists( output_filepath ):
                        ...
                        tarball_files.append( td_tup )
        for param in agent.input_params:
            # Check for agent data table definitions.
            if hasattr( param, 'options' ):
                if hasattr( param.options, 'agent_data_table' ):
                    data_table = param.options.agent_data_table
                    if hasattr( data_table, 'filenames' ):
                        ...
                        for data_table_filename in data_table.filenames:
                            ...
                            if not data_table.filenames[ data_table_filename ][ 'from_shed_config' ]:
                                ...
                                sample_file = os.path.join( data_table.filenames[ data_table_filename ][ 'agent_data_path' ],
                                                            tar_file )
                                # Use the .sample file, if one exists. If not, skip this data table.
                                if os.path.exists( sample_file ):
                                    tarfile_path, tarfile_name = os.path.split( tar_file )
                                    tarfile_path = os.path.join( 'agent-data', tarfile_name )
                        ...
                        if len( data_table_definitions ) > 0:
                            ...
                            tarball_files.append( ( table_conf, os.path.join( 'agent-data', 'agent_data_table_conf.xml.sample' ) ) )
    ...
    def to_dict( self, trans, link_details=False, io_details=False ):
        """ Returns dict of agent. """

        # Basic information
        agent_dict = super( Agent, self ).to_dict()

        # If an admin user, expose the path to the actual agent config XML file.
        if trans.user_is_admin():
            agent_dict['config_file'] = os.path.abspath(self.config_file)

        # Add link details.
        if link_details:
            # Add details for creating a hyperlink to the agent.
            if not isinstance( self, DataSourceAgent ):
                link = url_for( controller='agent_runner', agent_id=self.id )
            else:
                link = url_for( controller='agent_runner', action='data_source_redirect', agent_id=self.id )

            # Basic information
            agent_dict.update( { 'link': link,
                                'min_width': self.uihints.get( 'minwidth', -1 ),
                                'target': self.target } )

        # Add input and output details.
        if io_details:
            agent_dict[ 'inputs' ] = [ input.to_dict( trans ) for input in self.inputs.values() ]
            agent_dict[ 'outputs' ] = [ output.to_dict( app=trans.app ) for output in self.outputs.values() ]

        agent_dict[ 'panel_section_id' ], agent_dict[ 'panel_section_name' ] = self.get_panel_section()

        return agent_dict

    def to_json(self, trans, kwd={}, job=None, workflow_mode=False):
        """
        Recursively creates a agent dictionary containing repeats, dynamic options and updated states.
        """
        ...
        # set workflow mode ( TODO: Should be revised/parsed without trans to agent parameters (basic.py) )
        ...
        if job:
            try:
                ...
                agent_message = self._compare_agent_version(trans, job)
        ...
        def check_state(trans, input, value, context):
            ...
            if isinstance(value, galaxy.agents.parameters.basic.DummyDataset):
                return [ None, None ]
            elif isinstance(value, galaxy.agents.parameters.basic.RuntimeValue):
        ...
        # builds agent model including all attributes
        def iterate(group_inputs, inputs, state_inputs, other_values=None):
            ...
            for input_index, input in enumerate( inputs.itervalues() ):
                ...
                if input.type == 'repeat':
                    agent_dict = input.to_dict(trans)
                    group_cache = agent_dict['cache'] = {}
                    for i in range( len( group_state ) ):
                        group_cache[i] = {}
                        iterate( group_cache[i], input.inputs, group_state[i], other_values )
                elif input.type == 'conditional':
                    agent_dict = input.to_dict(trans)
                    if 'test_param' in agent_dict:
                        test_param = agent_dict['test_param']
                        ...
                        for i in range(len( agent_dict['cases'] ) ):
                            current_state = {}
                            if i == group_state.get('__current_case__', None):
                                current_state = group_state
                            iterate(agent_dict['cases'][i]['inputs'], input.cases[i].inputs, current_state, other_values)
                elif input.type == 'section':
                    agent_dict = input.to_dict(trans)
                    iterate( agent_dict['inputs'], input.inputs, group_state, other_values )
                else:
                    # expand input dictionary, resolve dynamic parameters
                    try:
                        agent_dict = input.to_dict(trans, other_values=other_values)
                    except Exception:
                        agent_dict = input.to_dict(trans)
                        log.exception('agents::to_json() - Skipping parameter expansion for %s.' % input.name)
                        pass

                    # backup default value
                    try:
                        agent_dict['default_value'] = input.get_initial_value(trans, other_values, history=history)
                    except Exception:
                        agent_dict['default_value'] = None
                        log.exception('agents::to_json() - Getting initial value failed %s.' % input.name)
                        pass

                    # update input value from agent state
                    agent_dict['value'] = state_inputs.get(input.name, agent_dict['default_value'])

                    # add text value
                    agent_dict[ 'text_value' ] = input.value_to_display_text( agent_dict[ 'value' ], trans.app )

                    # sanitize values
                    sanitize(agent_dict, 'value')
                    sanitize(agent_dict, 'default_value')

                # backup final input dictionary
                group_inputs[input_index] = agent_dict

        # sanatization for the final agent state
        ...
        # expand incoming parameters (parameters might trigger multiple agent executions,
        ...
        # initialize and populate agent state
        state_inputs = {}
        state_errors = {}
        populate_state(trans, self.inputs, state_inputs, state_errors, params.__dict__)

        # create basic agent model
        agent_model = self.to_dict(trans)
        agent_model['inputs'] = {}

        # build agent model and agent state
        iterate(agent_model['inputs'], self.inputs, state_inputs, '')
        ...
        agent_help = ''
        if self.help:
            agent_help = self.help
            agent_help = agent_help.render( static_path=url_for( '/static' ), host_url=url_for('/', qualified=True) )
            if type( agent_help ) is not unicode:
                agent_help = unicode( agent_help, 'utf-8')

        # check if citations exist
        agent_citations = False
        if self.citations:
            agent_citations = True

        # get agent versions
        agent_versions = []
        agents = self.app.agentbox.get_loaded_agents_by_lineage(self.id)
        for t in agents:
            if t.version not in agent_versions:
                agent_versions.append(t.version)
        ...
        sharable_url = None
        if self.agent_shed_repository:
            sharable_url = self.agent_shed_repository.get_sharable_url( trans.app )
    ...
    def _map_source_to_history(self, trans, agent_inputs, params, history):
        ...
        def mapping_callback( input, value, prefixed_name, prefixed_label ):
            ...
            if isinstance( input, DataAgentParameter ):
                ...
                return map_to_history( value )
        visit_input_values( agent_inputs, params, mapping_callback )

    def _compare_agent_version( self, trans, job ):
        """
        Compares a agent version with the agent version from a job (from AgentRunner).
        """
        agent_id = job.agent_id
        agent_version = job.agent_version
        message = ''
        try:
            select_field, agents, agent = self.app.agentbox.get_agent_components( agent_id, agent_version=agent_version, get_loaded_agents_by_lineage=False, set_selected=True )
            if agent is None:
                raise exceptions.MessageException( 'This dataset was created by an obsolete agent (%s). Can\'t re-run.' % agent_id )
            if ( self.id != agent_id and self.old_id != agent_id ) or self.version != agent_version:
                if self.id == agent_id:
                    if agent_version is None:
                        # for some reason jobs don't always keep track of the agent version.
                        message = ''
                    else:
                        message = 'This job was run with agent version "%s", which is not available.  ' % agent_version
                        if len( agents ) > 1:
                            message += 'You can re-run the job with the selected agent or choose another version of the agent.'
                        else:
                            message += 'You can re-run the job with this agent version, which is a different version of the original agent.'
                else:
                    new_agent_shed_url = agent.agent_shed_repository.get_sharable_url( agent.app ) + '/%s/' % agent.agent_shed_repository.changeset_revision
                    old_agent_shed = agent_id.split( "/repos/" )[0]
                    old_agent_shed_url = common_util.get_agent_shed_url_from_agent_shed_registry( self.app, old_agent_shed )
                    old_agent_shed_url = old_agent_shed_url + "/view/%s/%s/" % (agent.repository_owner, agent.repository_name)
                    message = 'This job was run with <a href=\"%s\" target=\"_blank\">agent id \"%s\"</a>, version "%s", which is not available.  ' % (old_agent_shed_url, agent_id, agent_version)
                    if len( agents ) > 1:
                        message += 'You can re-run the job with the selected <a href=\"%s\" target=\"_blank\">agent id \"%s\"</a> or choose another derivation of the agent.' % (new_agent_shed_url, self.id)
                    else:
                        message += 'You can re-run the job with <a href=\"%s\" target=\"_blank\">agent id \"%s\"</a>, which is a derivation of the original agent.' % (new_agent_shed_url, self.id)
        except Exception, e:
            raise exceptions.MessageException( str( e ) )
        return message
    ...
    def get_externally_referenced_paths( self, path ):
        """ Return relative paths to externally referenced files by the agent
        described by file at `path`. External components should not assume things
        about the structure of agent xml files (this is the agent's responsibility).
        """
        tree = raw_agent_xml_tree(path)
        root = tree.getroot()
        external_paths = []
        for code_elem in root.findall( 'code' ):
...
class OutputParameterJSONAgent( Agent ):
    """
    Alternate implementation of Agent that provides parameters and other values
    JSONified within the contents of an output dataset
    """
    agent_type = 'output_parameter_json'
...
class DataSourceAgent( OutputParameterJSONAgent ):
    """
    Alternate implementation of Agent for data_source agents -- those that
    allow the user to query and extract data from another web site.
    """
    agent_type = 'data_source'
    default_agent_action = DataSourceAgentAction

    def _build_GALAXY_URL_parameter( self ):
        return AgentParameter.build( self, ElementTree.XML( '<param name="GALAXY_URL" type="baseurl" value="/agent_runner?agent_id=%s" />' % self.id ) )

    def parse_inputs( self, agent_source ):
        super( DataSourceAgent, self ).parse_inputs( agent_source )
        # Open all data_source agents in _top.
        self.target = '_top'
...
class AsyncDataSourceAgent( DataSourceAgent ):
    agent_type = 'data_source_async'

    def _build_GALAXY_URL_parameter( self ):
        return AgentParameter.build( self, ElementTree.XML( '<param name="GALAXY_URL" type="baseurl" value="/async/%s" />' % self.id ) )


class DataDestinationAgent( Agent ):
    agent_type = 'data_destination'


class SetMetadataAgent( Agent ):
    """
    Agent implementation for special agent that sets metadata on an existing
    dataset.
    """
    agent_type = 'set_metadata'
...
class ExportHistoryAgent( Agent ):
    agent_type = 'export_history'


class ImportHistoryAgent( Agent ):
    agent_type = 'import_history'


class DataManagerAgent( OutputParameterJSONAgent ):
    agent_type = 'manage_data'
    default_agent_action = DataManagerAgentAction

    def __init__( self, config_file, root, app, guid=None, data_manager_id=None, **kwds ):
        self.data_manager_id = data_manager_id
        super( DataManagerAgent, self ).__init__( config_file, root, app, guid=guid, **kwds )
        if self.data_manager_id is None:
            self.data_manager_id = self.id

    def exec_after_process( self, app, inp_data, out_data, param_dict, job=None, **kwds ):
        assert self.allow_user_access( job.user ), "You must be an admin to access this agent."
        # run original exec_after_process
        super( DataManagerAgent, self ).exec_after_process( app, inp_data, out_data, param_dict, job=job, **kwds )
        # process results of agent
    ...
    def get_default_history_by_trans( self, trans, create=False ):
        def _create_data_manager_history( user ):
            ...
            return history
        user = trans.user
        assert user, 'You must be logged in to use this agent.'
        assert self.allow_user_access( user ), "You must be an admin to access this agent."
    ...
    def allow_user_access( self, user, attempting_access=True ):
        ...
        :param attempting_access: is the user attempting to do something with the
                               the agent (set false for incidental checks like agentbox
                               listing)
        :type attempting_access:  bool

        :returns: bool -- Whether the user is allowed to access the agent.
        Data Manager agents are only accessible to admins.
        """
        if super( DataManagerAgent, self ).allow_user_access( user ) and self.app.config.is_admin_user( user ):
            return True
        ...
        if attempting_access:
            if user:
                user = user.id
            log.debug( "User (%s) attempted to access a data manager agent (%s), but is not an admin.", user, self.id )
        return False

# Populate agent_type to AgentClass mappings
agent_types = {}
for agent_class in [ Agent, SetMetadataAgent, OutputParameterJSONAgent,
                    DataManagerAgent, DataSourceAgent, AsyncDataSourceAgent,
                    DataDestinationAgent ]:
    agent_types[ agent_class.agent_type ] = agent_class
