...
from galaxy.agents.parameters import wrapped_json
from galaxy.agents.parameters.grouping import Conditional, Repeat, Section
...
class AgentEvaluator( object ):
    """ An abstraction linking together a agent and a job runtime to evaluate
    agent inputs in an isolated, testable manner.
    """

    def __init__( self, app, agent, job, local_working_directory ):
        self.app = app
        self.job = job
        self.agent = agent
    ...
    def set_compute_environment( self, compute_environment, get_special=None ):
        ...
        incoming = self.agent.params_from_strings( incoming, self.app )
        # Do any validation that could not be done at job creation
        self.agent.handle_unvalidated_param_values( incoming, self.app )
        ...
        # Certain agents require tasks to be completed prior to job execution
        # ( this used to be performed in the "exec_before_job" hook, but hooks are deprecated ).
        self.agent.exec_before_job( self.app, inp_data, out_data, param_dict )
        # Run the before queue ("exec_before_job") hook
        self.agent.call_hook( 'exec_before_job', self.app, inp_data=inp_data,
                             out_data=out_data, agent=self.agent, param_dict=incoming)
    ...
    def build_param_dict( self, incoming, input_datasets, output_datasets, output_collections, output_paths, job_working_directory, input_paths=[] ):
        ...
        param_dict.update(self.agent.template_macro_params)
    ...
    def __populate_wrappers(self, param_dict, input_dataset_paths):
        ...
        def wrap_input( input_values, input ):
            if isinstance( input, DataAgentParameter ) and input.multiple:
                ...
                for conversion_name, conversion_extensions, conversion_datatypes in input.conversions:
                    ...
                    if input_values[ input.name ] and not converted_dataset:
                        ...
                        # None) without actually being a agent parameter
                        input_values[ conversion_name ] = \
                            DatasetFilenameWrapper( converted_dataset,
                                                    datatypes_registry=self.app.datatypes_registry,
                                                    agent=Bunch( conversion_name=Bunch( extensions=conv_ext ) ),
                ...
                input_values[ input.name ] = SelectAgentParameterWrapper(
        ...
        #       agents where the inputs don't even get passed through. These
        #       agents (e.g. UCSC) should really be handled in a special way.
        if self.agent.check_values:
            self.__walk_inputs( self.agent.inputs, param_dict, wrap_input )
    ...
    def __populate_input_dataset_wrappers(self, param_dict, input_datasets, input_dataset_paths):
        ...
        #   agent?: can this be abstracted out as part of being a datasouce agent?)
    ...
    def __populate_output_collection_wrappers(self, param_dict, output_collections, output_paths, job_working_directory):
        ...
        for name, out_collection in output_collections.items():
            if name not in agent.output_collections:
                continue
                # message_template = "Name [%s] not found in agent.output_collections %s"
                # message = message_template % ( name, agent.output_collections )
            ...
            output_def = agent.output_collections[ name ]
    ...
    def __populate_output_dataset_wrappers(self, param_dict, output_datasets, output_paths, job_working_directory):
        ...
        for name, hda in output_datasets.items():
            ...
            for child in hda.children:
                param_dict[ "_CHILD___%s___%s" % ( name, child.designation ) ] = DatasetFilenameWrapper( child )
        for out_name, output in self.agent.outputs.iteritems():
            if out_name not in param_dict and output.filters:
                # Assume the reason we lack this output is because a filter
                # failed to pass; for agent writing convienence, provide a
                # NoneDataset
    ...
    def __populate_non_job_params(self, param_dict):
        ...
        def get_data_table_entry(table_name, query_attr, query_val, return_attr):
            ...
            if table_name in self.app.agent_data_tables:
                return self.app.agent_data_tables[ table_name ].get_entry( query_attr, query_val, return_attr )

        param_dict['__agent_directory__'] = self.compute_environment.agent_directory()
        ...
        param_dict['__agent_data_path__'] = param_dict['GALAXY_DATA_INDEX_DIR'] = self.app.config.agent_data_path
        # For the upload agent, we need to know the root directory and the
    ...
    def __populate_unstructured_path_rewrites(self, param_dict):
        ...
        def rewrite_unstructured_paths( input_values, input ):
            if isinstance( input, SelectAgentParameter ):
                input_values[ input.name ] = SelectAgentParameterWrapper(
                    input, input_values[ input.name ], self.app, other_values=param_dict, path_rewriter=self.unstructured_path_rewriter )

        if not self.agent.check_values and self.unstructured_path_rewriter:
            # The agents weren't "wrapped" yet, but need to be in order to get
            # the paths rewritten.
            self.__walk_inputs( self.agent.inputs, param_dict, rewrite_unstructured_paths )

    def __sanitize_param_dict( self, param_dict ):
        """
        Sanitize all values that will be substituted on the command line, with the exception of AgentParameterValueWrappers,
        ...
        skip = [ 'chromInfo' ] + self.agent.template_macro_params.keys()
        if not self.agent or not self.agent.options or self.agent.options.sanitize:
            for key, value in param_dict.items():
                if key not in skip:
                    # Remove key so that new wrapped object will occupy key slot
                    del param_dict[key]
                    # And replace with new wrapped key
                    param_dict[ wrap_with_safe_string( key, no_wrap_classes=AgentParameterValueWrapper ) ] = wrap_with_safe_string( value, no_wrap_classes=AgentParameterValueWrapper )

    def build( self ):
        """
        Build runtime description of job to execute, evaluate command and
        config templates corresponding to this agent with these inputs on this
        compute environment.
        """
        ...
        try:
            ...
            global_agent_errors.add_error(self.agent.config_file, "Building Config Files", e)
            raise e
        try:
            self.__build_param_file( )
        except Exception, e:
            # capture and log parsing errors
            global_agent_errors.add_error(self.agent.config_file, "Building Param File", e)
            raise e
        try:
            self.__build_command_line( )
        except Exception, e:
            # capture and log parsing errors
            global_agent_errors.add_error(self.agent.config_file, "Building Command Line", e)
            raise e
        try:
            self.__build_environment_variables()
        except Exception, e:
            global_agent_errors.add_error(self.agent.config_file, "Building Environment Variables", e)
            raise e
    ...
    def __build_command_line( self ):
        """
        Build command line to invoke this agent given a populated param_dict
        """
        ...
        if interpreter:
            ...
            agent_dir = os.path.abspath( self.agent.agent_dir )
            abs_executable = os.path.join( agent_dir, executable )
    ...
    def __build_config_files( self ):
        ...
        param_dict = self.param_dict
        config_filenames = []
        for name, filename, content in self.agent.config_files:
    ...
    def __build_environment_variables( self ):
        ...
        for environment_variable_def in self.agent.environment_variables:
            directory = self.local_working_directory
    ...
    def __build_config_file_text( self, content ):
        ...
        return json.dumps(wrapped_json.json_wrap(self.agent.inputs, self.param_dict)), False
