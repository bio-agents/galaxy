...
class Bam( Binary ):
    ...
    def _get_samagents_version( self ):
        # Determine the version of samagents being used.  Wouldn't it be nice if
        # samagents provided a version flag to make this much simpler?
        version = '0.0.0'
        samagents_exec = which('samagents')
        if not samagents_exec:
            message = 'Attempting to use functionality requiring samagents, but it cannot be located on Galaxy\'s PATH.'
            raise Exception(message)

        output = subprocess.Popen( [ 'samagents' ], stderr=subprocess.PIPE, stdout=subprocess.PIPE ).communicate()[1]
        lines = output.split( '\n' )
        for line in lines:
    ...
    def merge(split_files, output_file):
        ...
        command = ["samagents", "merge", "-f", output_file] + split_files
    ...
    def _is_coordinate_sorted( self, file_name ):
        ...
        params = [ "samagents", "view", "-H", file_name ]
    ...
    def dataset_content_needs_grooming( self, file_name ):
        ...
        version = self._get_samagents_version()
        if version < '0.1.13':
            return not self._is_coordinate_sorted( file_name )
        else:
            # Samagents version 0.1.13 or newer produces an error condition when attempting to index an
            # unsorted bam file - see http://biostar.stackexchange.com/questions/5273/is-my-bam-file-sorted.
            # So when using a newer version of samagents, we'll first check if the input BAM file is sorted
            ...
            # upload agent / framework to allow setting metadata from directly within the agent itself, it should be
            # done generically so that all agents will have the ability.  In testing, a 6.6 gb BAM file took 128
            # seconds to index with samagents, and 45 minutes to sort, so indexing is relatively inexpensive.
            if self._is_coordinate_sorted( file_name ):
                return False
            index_name = tempfile.NamedTemporaryFile( prefix="bam_index" ).name
            stderr_name = tempfile.NamedTemporaryFile( prefix="bam_index_stderr" ).name
            command = 'samagents index %s %s' % ( file_name, index_name )
    ...
    def groom_dataset_content( self, file_name ):
        ...
        """
        # Use samagents to sort the Bam file
        # $ samagents sort
        # Usage: samagents sort [-on] [-m <maxMem>] <in.bam> <out.prefix>
        ...
        samagents_created_sorted_file_name = "%s.bam" % tmp_sorted_dataset_file_name_prefix  # samagents accepts a prefix, not a filename, it always adds .bam to the prefix
        command = "samagents sort %s %s" % ( file_name, tmp_sorted_dataset_file_name_prefix )
        ...
        if stderr:
            if exit_code != 0:
                ...
                print stderr
        # Move samagents_created_sorted_file_name to our output dataset location
        shutil.move( samagents_created_sorted_file_name, file_name )
    ...
    def set_meta( self, dataset, overwrite=True, **kwd ):
        ...
        # $ samagents index
        # Usage: samagents index <in.bam> [<out.index>]
        stderr_name = tempfile.NamedTemporaryFile( prefix="bam_index_stderr" ).name
        command = [ 'samagents', 'index', dataset.file_name, index_file.file_name ]
        exit_code = subprocess.call( args=command, stderr=open( stderr_name, 'wb' ) )
        # Did index succeed?
        if exit_code == -6:
            # SIGABRT, most likely samagents 1.0+ which does not accept the index name parameter.
            ...
            try:
                command = [ 'samagents', 'index', dataset_symlink ]
                exit_code = subprocess.call( args=command, stderr=open( stderr_name, 'wb' ) )
                shutil.move( dataset_symlink + '.bai', index_file.file_name )
            except Exception as e:
                open( stderr_name, 'ab+' ).write( 'Galaxy attempted to build the BAM index with samagents 1.0+ but failed: %s\n' % e)
    ...
    # TODO: incorporate samagents options to control output: regions first, then flags, etc.
    @dataproviders.decorators.dataprovider_factory( 'line', dataproviders.line.FilteredLineDataProvider.settings )
    def line_dataprovider( self, dataset, **settings ):
        samagents_source = dataproviders.dataset.SamagentsDataProvider( dataset )
        settings[ 'comment_char' ] = '@'
        return dataproviders.line.FilteredLineDataProvider( samagents_source, **settings )
    ...
    def regex_line_dataprovider( self, dataset, **settings ):
        samagents_source = dataproviders.dataset.SamagentsDataProvider( dataset )
        settings[ 'comment_char' ] = '@'
        return dataproviders.line.RegexLineDataProvider( samagents_source, **settings )
    ...
    def column_dataprovider( self, dataset, **settings ):
        samagents_source = dataproviders.dataset.SamagentsDataProvider( dataset )
        settings[ 'comment_char' ] = '@'
        return dataproviders.column.ColumnarDataProvider( samagents_source, **settings )
    ...
    def dict_dataprovider( self, dataset, **settings ):
        samagents_source = dataproviders.dataset.SamagentsDataProvider( dataset )
        settings[ 'comment_char' ] = '@'
        return dataproviders.column.DictDataProvider( samagents_source, **settings )
    ...
    def header_dataprovider( self, dataset, **settings ):
        # in this case we can use an option of samagents view to provide just what we need (w/o regex)
        samagents_source = dataproviders.dataset.SamagentsDataProvider( dataset, '-H' )
        return dataproviders.line.RegexLineDataProvider( samagents_source, **settings )
    ...
    def genomic_region_dataprovider( self, dataset, **settings ):
        ...
        # samagents_source = dataproviders.dataset.SamagentsDataProvider( dataset )
        # return dataproviders.dataset.GenomicRegionDataProvider( samagents_source, metadata_source=dataset,
        #                                                        2, 3, 3, **settings )
    ...
    @dataproviders.decorators.dataprovider_factory( 'samagents' )
    def samagents_dataprovider( self, dataset, **settings ):
        """Generic samagents interface - all options available through settings."""
        dataset_source = dataproviders.dataset.DatasetDataProvider( dataset )
        return dataproviders.dataset.SamagentsDataProvider( dataset_source, **settings )
...
class CRAM( Binary ):
    ...
    def set_index_file(self, dataset, index_file):
        try:
            ...
            # xref: https://github.com/samagents/samagents/issues/199
...
class Bcf( Binary):
    ...
    def set_meta( self, dataset, overwrite=True, **kwd ):
        ...
        command = [ 'bcfagents', 'index', dataset_symlink ]
        try:
            subprocess.check_call( args=command, stderr=open( stderr_name, 'wb' ) )
            shutil.move( dataset_symlink + '.csi', index_file.file_name )  # this will fail if bcfagents < 1.0 is used, because it creates a .bci index file instead of .csi
        except Exception as e:
            stderr = open( stderr_name ).read().strip()
...
class Sra( Binary ):
    """ Sequence Read Archive (SRA) datatype originally from mdshw5/sra-agents-galaxy"""
    file_ext = 'sra'
