...
class Fasta( Sequence ):
    ...
    def split(cls, input_datasets, subdir_generator_function, split_params):
        ...
        if 'split_mode' not in split_params:
            raise Exception('Agent does not define a split mode')
...
class Maf( Alignment ):
    ...
    # Readonly and optional, users can't unset it, but if it is not set, we are generally ok; if required use a metadata validator in the agent definition
    ...
    def set_meta( self, dataset, overwrite=True, **kwd ):
        ...
        from galaxy.agents.util.maf_utilities import build_maf_index_species_chromosomes
...
class Axt( data.Text ):
    ...
    # gvk- 11/19/09 - This is really an alignment, but we no longer have agents that use this data type, and it is
    ...
    def sniff( self, filename ):
        ...
        axt alignment files are produced from Blastz, an alignment agent available from Webb Miller's lab
