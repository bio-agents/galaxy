...
class Registry( object ):
    ...
    def __init__( self ):
        ...
        self.converters = []
        # Converters defined in datatypes_conf.xml included in installed agent shed repositories.
        self.proprietary_converters = []
        self.converter_deps = {}
        self.available_tracks = []
        self.set_external_metadata_agent = None
        self.sniff_order = []
        ...
        # agent shed repositories that contain display applications.
    ...
    def load_datatypes( self, root_dir=None, config=None, deactivate=False, override=True ):
        ...
        an installed Agent Shed repository.  If deactivate is True, an installed Agent Shed repository that includes custom datatypes
        is being deactivated or uninstalled, so appropriate loaded datatypes will be removed from the registry.  The value of
        override will be False when a Agent Shed repository is being installed.  Since installation is occurring after the datatypes
        ...
        if root_dir and config:
            ...
            for elem in registration.findall( 'datatype' ):
                ...
                if edam_format and not make_subclass:
                    ...
                    continue
                # Proprietary datatypes included in installed agent shed repositories will include two special attributes
                # (proprietary_path and proprietary_datatype_module) if they depend on proprietary datatypes classes.
                # The value of proprietary_path is the path to the cloned location of the agent shed repository's contained
                ...
                if proprietary_path is not None or proprietary_datatype_module is not None and not handling_proprietary_datatypes:
                    handling_proprietary_datatypes = True
                if deactivate:
                    # We are deactivating or uninstalling an installed agent shed repository, so eliminate the datatype
                    ...
                    if can_process_datatype:
                        ...
                        if ok:
                            if not deactivate:
                                # A new agent shed repository that contains custom datatypes is being installed, and since installation is
                                ...
                                for converter in elem.findall( 'converter' ):
                                    # Build the list of datatype converters which will later be loaded into the calling app's agentbox.
            ...
            # Persist the xml form of the registry into a temporary file so that it can be loaded from the command line by agents and
    ...
    def load_datatype_sniffers( self, root, deactivate=False, handling_proprietary_datatypes=False, override=False ):
        ...
        distributed config) or contained within an installed Agent Shed repository.  If deactivate is True, an installed Agent
        Shed repository that includes custom sniffers is being deactivated or uninstalled, so appropriate loaded sniffers will
        be removed from the registry.  The value of override will be False when a Agent Shed repository is being installed.
                ...
                if dtype is not None:
                    ...
                    if ok:
                        ...
                        if ok:
                            ...
                            if ok:
                                if deactivate:
                                    # We are deactivating or uninstalling an installed Agent Shed repository, so eliminate the appropriate sniffers.
    ...
    def load_datatype_converters( self, agentbox, installed_repository_dict=None, deactivate=False ):
        """
        If deactivate is False, add datatype converters from self.converters or self.proprietary_converters
        to the calling app's agentbox.  If deactivate is True, eliminates relevant converters from the calling
        app's agentbox.
        """
        if installed_repository_dict:
            # Load converters defined by datatypes_conf.xml included in installed agent shed repository.
        ...
        for elem in converters:
            ...
            try:
                config_path = os.path.join( converter_path, agent_config )
                converter = agentbox.load_agent( config_path )
                if installed_repository_dict:
                    # If the converter is included in an installed agent shed repository, set the agent
                    # shed related agent attributes.
                    converter.agent_shed = installed_repository_dict[ 'agent_shed' ]
                    ...
                    # The converter should be included in the list of agents defined in agent_dicts.
                    agent_dicts = installed_repository_dict[ 'agent_dicts' ]
                    for agent_dict in agent_dicts:
                        if agent_dict[ 'id' ] == converter.id:
                            converter.guid = agent_dict[ 'guid' ]
                            converter.id = agent_dict[ 'guid' ]
                            break
                if deactivate:
                    agentbox.remove_agent_by_id( converter.id, remove_from_panel=False )
                    ...
                    agentbox.register_agent( converter )
    ...
    def load_display_applications( self, app, installed_repository_dict=None, deactivate=False ):
        ...
        """
        if installed_repository_dict:
            # Load display applications defined by datatypes_conf.xml included in installed agent shed repository.
                ...
                try:
                    ...
                    if display_app:
                        if display_app.id in self.display_applications:
                            ...
                            # If the display application is included in an installed agent shed repository,
                            # set the agent shed related agent attributes.
                            display_app.agent_shed = installed_repository_dict[ 'agent_shed' ]
                            ...
                            # The display application should be included in the list of agents defined in agent_dicts.
                            agent_dicts = installed_repository_dict[ 'agent_dicts' ]
                            for agent_dict in agent_dicts:
                                if agent_dict[ 'id' ] == display_app.id:
                                    display_app.guid = agent_dict[ 'guid' ]
                                    display_app.id = agent_dict[ 'guid' ]
                                    break
                        if deactivate:
                            if display_app.id in self.display_applications:
                                del self.display_applications[ display_app.id ]
    ...
    def load_external_metadata_agent( self, agentbox ):
        """Adds a agent which is used to set external metadata"""
        ...
        # agent.  We'll load a special agent to be used for Auto-Detecting metadata; this is less than ideal, but effective
        # Properly building a agent without relying on parsing an XML file is near difficult...so we bundle with Galaxy.
        set_meta_agent = agentbox.load_hidden_lib_agent( "galaxy/datatypes/set_metadata_agent.xml" )
        self.set_external_metadata_agent = set_meta_agent
        self.log.debug( "Loaded external metadata agent: %s", self.set_external_metadata_agent.id )
    ...
    def get_upload_metadata_params( self, context, group, agent ):
        """Returns dict of case value:inputs for metadata conditional for upload agent"""
        rval = {}
