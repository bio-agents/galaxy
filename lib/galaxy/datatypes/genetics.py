...
class rgTabList(Tabular):
    """
    for sampleid and for featureid lists of exclusions or inclusions in the clean agent
...
class rgSampleList(rgTabList):
    """
    for sampleid exclusions or inclusions in the clean agent
...
class rgFeatureList( rgTabList ):
    """
    for featureid lists of exclusions or inclusions in the clean agent
...
class Rgenetics(Html):
    ...
    def set_meta( self, dataset, **kwd ):
        ...
        if not dataset.blurb:
            dataset.blurb = 'Composite file - Rgenetics Galaxy agentkit'
        return True
...
class ldIndep(Rgenetics):
    ...
    This is really a plink binary, but some agents work better with less redundancy so are constrained to
    these files
    """
    file_ext = "ldreduced"
...
class RexpBase( Html ):
    ...
    def set_meta( self, dataset, **kwd ):
        ...
        if not dataset.blurb:
            dataset.blurb = 'R loadable BioC expression object for the Rexpression Galaxy agentkit'
        return True
