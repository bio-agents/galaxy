    ...
    def run_job( self, job ):
        ...
        if job.params[ 'type' ] == 'finish_transfer':
            ...
            try:
                ...
                self.app.datatypes_registry.set_external_metadata_agent.agent_action.execute( self.app.datatypes_registry.set_external_metadata_agent,
                                                                                            FakeTrans( self.app,
    ...
    def _execute_workflow( self, sample):
        ...
        for step in workflow.steps:
            ...
            if step.type == 'agent' or step.type is None:
                # Restore the agent state for the step
        ...
        for i, step in enumerate( workflow.steps ):
            job = None
            if step.type == 'agent' or step.type is None:
                agent = self.app.agentbox.get_agent( step.agent_id )
                ...
                job, out_data = agent.execute( fk_trans, step.state.inputs, history=sample.history)
