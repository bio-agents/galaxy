...
def build_command(
    ...
    # log.debug( 'Agent evaluation for job (%s) produced command-line: %s' % ( job_id, base_command_line ) )
    if not base_command_line:
        raise Exception("Attempting to run a agent with empty command definition.")
...
def __externalize_commands(job_wrapper, shell, commands_builder, remote_command_params, script_name="agent_script.sh"):
    local_container_script = join( job_wrapper.working_directory, script_name )
    agent_commands = commands_builder.build()
    config = job_wrapper.app.config
    integrity_injection = ""
    if check_script_integrity(config):
        integrity_injection = INTEGRITY_INJECTION
    script_contents = u"#!%s\n%s%s" % (shell, integrity_injection, agent_commands)
    ...
    log.info("Built script [%s] for agent command[%s]" % (local_container_script, agent_commands))
    return commands
...
class CommandsBuilder(object):
    ...
    def __init__(self, initial_command=u''):
        ...
        # agent command. If these are used capture the return code and ensure
