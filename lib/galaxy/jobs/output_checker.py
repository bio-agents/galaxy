...
def check_output( agent, stdout, stderr, agent_exit_code, job ):
    """
    Check the output of a agent - given the stdout, stderr, and the agent's
    exit code, return True if the agent exited succesfully and False
    ...
    Note that, if the agent did not define any exit code handling or
    ...
    """
    # By default, the agent succeeded. This covers the case where the code
    # has a bug but the agent was ok, and it lets a workflow continue.
    success = True

    try:
        # Check exit codes and match regular expressions against stdout and
        # stderr if this agent was configured to do so.
        # If there is a regular expression for scanning stdout/stderr,
        # then we assume that the agent writer overwrote the default
        # behavior of just setting an error if there is *anything* on
        # stderr.
        if ( len( agent.stdio_regexes ) > 0 or
             len( agent.stdio_exit_codes ) > 0 ):
            ...
            if agent_exit_code is not None:
                for stdio_exit_code in agent.stdio_exit_codes:
                    if ( agent_exit_code >= stdio_exit_code.range_start and
                         agent_exit_code <= stdio_exit_code.range_end ):
                        ...
                        if ( None is code_desc ):
                            code_desc = ""
                        agent_msg = ( "%s: Exit code %d (%s)" % (
                                     StdioErrorLevel.desc( stdio_exit_code.error_level ),
                                     agent_exit_code,
                                     code_desc ) )
                        log.info( "Job %s: %s" % (job.get_id_tag(), agent_msg) )
                        stderr = agent_msg + "\n" + stderr
            ...
            if max_error_level < StdioErrorLevel.FATAL:
                ...
                # a fatal error yet
                for regex in agent.stdio_regexes:
            ...
            if max_error_level >= StdioErrorLevel.FATAL:
                log.debug("Agent exit code indicates an error, failing job.")
                success = False
            else:
                success = True
        ...
        # the job has an error, and the job is ok otherwise.
            ...
            # TODO: Add in the agent and job id:
            # log.debug( "Agent did not define exit code or stdio handling; "
            #          + "checking stderr for success" )
            if stderr:
                peak = stderr[0:250]
                log.debug("Agent produced standard error failing job - [%s]" % peak)
                success = False
            else:
                success = True
    ...
    # On any exception, return True.
        ...
        log.warning( "Agent check encountered unexpected exception; " +
                     "assuming agent was successful: " + tb )
        success = True
...
def __regex_err_msg( match, regex ):
    """
    Return a message about the match on agent output using the given
    AgentStdioRegex regex object. The regex_match is a MatchObject
