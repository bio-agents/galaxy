"""
Support for running a agent in Galaxy via an internal job management system
"""
...
# This interface is currently experimental, is only used by the upload agent,
...
class JobAgentConfiguration( Bunch ):
    """
    Provides details on what handler and destination a agent should use

    A JobAgentConfiguration will have the required attribute 'id' and optional
    ...
    def __init__(self, **kwds):
        ...
        self['params'] = dict()
        super(JobAgentConfiguration, self).__init__(**kwds)
...
class JobConfiguration( object ):
    ...
    def __parse_job_conf_xml(self, tree):
        ...
        if agents is not None:
            for agent in self.__findall_with_required(agents, 'agent'):
                # There can be multiple definitions with identical ids, but different params
                id = agent.get('id').lower().rstrip('/')
                if id not in self.agents:
                    self.agents[id] = list()
                self.agents[id].append(JobAgentConfiguration(**dict(agent.items())))
                self.agents[id][-1]['params'] = self.__get_params(agent)
    ...
    def __parse_job_conf_legacy(self):
        ...
        # Set agent handler configs
        for id, agent_handlers in self.app.config.agent_handlers.items():
            self.agents[id] = list()
            for handler_config in agent_handlers:
                # rename the 'name' key to 'handler'
                handler_config['handler'] = handler_config.pop('name')
                self.agents[id].append(JobAgentConfiguration(**handler_config))

        # Set agent runner configs
        for id, agent_runners in self.app.config.agent_runners.items():
            # Might have been created in the handler parsing above
            if id not in self.agents:
                self.agents[id] = list()
            for runner_config in agent_runners:
                ...
                for agent_conf in self.agents[id]:
                    if agent_conf.params == runner_config.get('params', {}):
                        agent_conf['destination'] = url
                        break
                else:
                    ...
                    self.agents[id].append(JobAgentConfiguration(**runner_config))
    ...
    def get_agent_resource_parameters( self, agent_id ):
        """ Given a agent id, return XML elements describing parameters to
        insert into job resources.

        :agent id: A agent ID (a string)
        ...
        if agent_id in self.agents:
            resource_group = self.agents[ agent_id ][ 0 ].get_resource_group()
    ...
    @property
    def default_job_agent_configuration(self):
        """
        The default JobAgentConfiguration, used if a agent does not have an
        explicit defintion in the configuration.  It consists of a reference to
        the default handler and default destination.

        :returns: JobAgentConfiguration -- a representation of a <agent> element that uses the default handler and destination
        """
        return JobAgentConfiguration(id='default', handler=self.default_handler_id, destination=self.default_destination_id)

    # Called upon instantiation of a Agent object
    def get_job_agent_configurations(self, ids):
        """
        Get all configured JobAgentConfigurations for a agent ID, or, if given
        a list of IDs, the JobAgentConfigurations for the first id in ``ids``
        matching a agent definition.

        .. note::

            You should not mix agent shed agent IDs, versionless agent shed IDs, and agent config agent IDs that refer to the same agent.

        :param ids: Agent ID or IDs to fetch the JobAgentConfiguration of.
        :type ids: list or str.
        :returns: list -- JobAgentConfiguration Bunches representing <agent> elements matching the specified ID(s).

        Example agent ID strings include:

        * Full agent shed id: ``agentshed.example.org/repos/nate/filter_agent_repo/filter_agent/1.0.0``
        * Agent shed id less version: ``agentshed.example.org/repos/nate/filter_agent_repo/filter_agent``
        * Agent config agent id: ``filter_agent``
        """
        rval = []
        ...
        for id in ids:
            if id in self.agents:
                # If a agent has definitions that include job params but not a
                # definition for jobs without params, include the default
                # config
                for job_agent_configuration in self.agents[id]:
                    if not job_agent_configuration.params:
                        break
                else:
                    rval.append(self.default_job_agent_configuration)
                rval.extend(self.agents[id])
                break
        else:
            rval.append(self.default_job_agent_configuration)
        return rval
    ...
    # This is called by Agent.get_job_handler()
...
class JobWrapper( object ):
    ...
    def __init__( self, job, queue, use_persisted_destination=False ):
        ...
        self.agent = queue.app.agentbox.get_agent( job.agent_id, job.agent_version, exact=True )
        self.queue = queue
        self.app = queue.app
        self.sa_session = self.app.model.context
        self.extra_filenames = []
        self.command_line = None
        # Agent versioning variables
        ...
        self.agent_provided_job_metadata = None
    ...
    def can_split( self ):
        ...
        return self.app.config.use_tasked_jobs and self.agent.parallelism
    ...
    def get_param_dict( self ):
        ...
        param_dict = self.agent.params_from_strings( param_dict, self.app )
        return param_dict
    ...
    def prepare( self, compute_environment=None ):
        ...
        def get_special( ):
            ...
            if not special:
                special = self.sa_session.query( model.GenomeIndexAgentData ).filter_by( job=job ).first()
            return special

        agent_evaluator = self._get_agent_evaluator( job )
        compute_environment = compute_environment or self.default_compute_environment( job )
        agent_evaluator.set_compute_environment( compute_environment, get_special=get_special )

        self.sa_session.flush()

        self.command_line, self.extra_filenames, self.environment_variables = agent_evaluator.build()
        # Ensure galaxy_lib_dir is set in case there are any later chdirs
        self.galaxy_lib_dir
        # Shell fragment to inject dependencies
        self.dependency_shell_commands = self.agent.build_dependency_shell_commands(job_directory=self.working_directory)
        ...
        self.param_dict = agent_evaluator.param_dict
        version_string_cmd = self.agent.version_string_cmd
    ...
    def _get_agent_evaluator( self, job ):
        # Hacky way to avoid cirular import for now.
        # Placing AgentEvaluator in either jobs or agents
        # result in ciruclar dependency.
        from galaxy.agents.evaluation import AgentEvaluator
    ...
    def fail( self, message, exception=False, stdout="", stderr="", exit_code=None ):
        ...
        if not job.state == job.states.DELETED:
            ...
            if exception:
                ...
                # Get the exception and let the agent attempt to generate
                # a better message
                etype, evalue, tb = sys.exc_info()
                m = self.agent.handle_job_failure_exception( evalue )
                if m:
                    message = m
            if self.app.config.outputs_to_working_directory:
        ...
        # If the job was deleted, call agent specific fail actions (used for e.g. external metadata) and clean up
        if self.agent:
            self.agent.job_failed( self, message, exception )
    ...
    def finish( self, stdout, stderr, agent_exit_code=None, remote_working_directory=None ):
        """
        ...
        try:
            ...
            return self.fail( job.info, stdout=stdout, stderr=stderr, exit_code=agent_exit_code )
        ...
        if job.state == job.states.DELETED or job.state == job.states.ERROR:
            ...
            return self.fail( job.info, stderr=stderr, stdout=stdout, exit_code=agent_exit_code )

        # Check the agent's stdout, stderr, and exit code for errors, but only
        ...
        if ( self.check_agent_output( stdout, stderr, agent_exit_code, job ) ):
            final_job_state = job.states.OK
        else:
            final_job_state = job.states.ERROR

        if self.agent.version_string_cmd:
        ...
        for dataset_assoc in job.output_datasets + job.output_library_datasets:
            ...
            for dataset in dataset_assoc.dataset.dataset.history_associations + dataset_assoc.dataset.dataset.library_associations:
                ...
                dataset.agent_version = self.version_string
                dataset.set_size()
                if 'uuid' in context:
                ...
                if job.states.ERROR == final_job_state:
                    ...
                    # If the agent was expected to set the extension, attempt to retrieve it
                    if dataset.ext == 'auto':
                    ...
                    try:
                        ...
                        if ( not dataset.datatype.composite_type and dataset.dataset.is_multi_byte() ) or self.agent.is_multi_byte:
                        ...
                        if ( not dataset.datatype.composite_type and dataset.dataset.is_multi_byte() ) or self.agent.is_multi_byte:
                            ...
                            dataset.set_peek()
                    try:
                        # set the name if provided by the agent
        ...
        # TODO: eliminate overlap with agents/evaluation.py
        ...
        for _, data in inp_data.items():
            # For loop odd, but sort simulating behavior in galaxy.agents.actions
            if not data:
                continue
            input_ext = data.ext
        # why not re-use self.param_dict here? ##dunno...probably should, this causes
        # agents.parameters.basic.UnvalidatedValue to be used in following methods
        # instead of validated and transformed values during i.e. running workflows
        param_dict = dict( [ ( p.name, p.value ) for p in job.parameters ] )
        param_dict = self.agent.params_from_strings( param_dict, self.app )
        # Create generated output children and primary datasets and add to param_dict
        collected_datasets = {
            'children': self.agent.collect_child_datasets(out_data, self.working_directory),
            'primary': self.agent.collect_primary_datasets(out_data, self.working_directory, input_ext)
        }
        self.agent.collect_dynamic_collections(
            out_collections,
        ...
        # Certain agents require tasks to be completed after job execution
        # ( this used to be performed in the "exec_after_process" hook, but hooks are deprecated ).
        self.agent.exec_after_process( self.queue.app, inp_data, out_data, param_dict, job=job )
        # Call 'exec_after_process' hook
        self.agent.call_hook( 'exec_after_process', self.queue.app, inp_data=inp_data,
                             out_data=out_data, param_dict=param_dict,
                             agent=self.agent, stdout=job.stdout, stderr=job.stderr )
        job.command_line = unicodify(self.command_line)
    ...
    def check_agent_output( self, stdout, stderr, agent_exit_code, job ):
        return check_output( self.agent, stdout, stderr, agent_exit_code, job )

    def cleanup( self, delete_files=True ):
        # At least one of these agent cleanup actions (job import), is needed
        # for theagent to work properly, that is why one might want to run
        ...
        try:
            ...
            galaxy.agents.imp_exp.JobExportHistoryArchiveWrapper( self.job_id ).cleanup_after_job( self.sa_session )
            galaxy.agents.imp_exp.JobImportHistoryArchiveWrapper( self.app, self.job_id ).cleanup_after_job()
            if delete_files:
    ...
    def get_agent_provided_job_metadata( self ):
        if self.agent_provided_job_metadata is not None:
            return self.agent_provided_job_metadata

        # Look for JSONified job metadata
        self.agent_provided_job_metadata = []
        ...
        if os.path.exists( meta_file ):
            for line in open( meta_file, 'r' ):
                try:
                    ...
                    log.exception( '(%s) Got JSON data from agent, but data is improperly formatted or no "type" key in data' % self.job_id )
                ...
                # the finish method, so if a agent writes out metadata for a
                ...
                if line['type'] == 'dataset' and 'dataset_id' not in line:
                    try:
                        ...
                        log.warning( '(%s) Agent provided job dataset-specific metadata without specifying a dataset' % self.job_id )
                        continue
                self.agent_provided_job_metadata.append( line )
        return self.agent_provided_job_metadata

    def get_dataset_finish_context( self, job_context, dataset ):
        for meta in self.get_agent_provided_job_metadata():
    ...
    def setup_external_metadata( self, exec_dir=None, tmp_dir=None,
        ...
        # extension could still be 'auto' if this is the upload agent.
        job = self.get_job()
        if set_extension:
        ...
        if resolve_metadata_dependencies:
            metadata_agent = self.app.agentbox.get_agent("__SET_METADATA__")
            dependency_shell_commands = metadata_agent.build_dependency_shell_commands(job_directory=self.working_directory)
    ...
    def __link_file_check( self ):
        ...
        specific to the upload agent and relies on an injected job param.  This
        ...
        return self.agent.id == 'upload1' and param_dict.get( 'link_data_only', None ) == 'link_to_files'
    ...
    def requires_setting_metadata( self ):
        if self.agent:
            return self.agent.requires_setting_metadata
        return False
...
class TaskWrapper(JobWrapper):
    ...
    def prepare( self, compute_environment=None ):
        ...
        agent_evaluator = self._get_agent_evaluator( job )
        compute_environment = compute_environment or self.default_compute_environment( job )
        agent_evaluator.set_compute_environment( compute_environment )
    ...
    def finish( self, stdout, stderr, agent_exit_code=None ):
        ...
        log.debug( 'task %s for job %d ended; exit code: %d'
                   % (self.task_id, self.job_id,
                      agent_exit_code if agent_exit_code is not None else -256 ) )
        ...
        # Check what the agent returned. If the stdout or stderr matched
        # regular expressions that indicate errors, then set an error.
        # The same goes if the agent's exit code was in a given range.
        if ( self.check_agent_output( stdout, stderr, agent_exit_code, task ) ):
        ...
        task.exit_code = agent_exit_code
        task.command_line = self.command_line
        self.sa_session.flush()
...
class ComputeEnvironment( object ):
    ...
    def agent_directory( self ):
        """ Absolute path to agent files for this job on compute server. """

    @abstractmethod
    def version_path( self ):
        """ Location of the version file for the underlying agent. """
...
class SharedComputeEnvironment( SimpleComputeEnvironment ):
    ...
    to AgentEvaluator - valid when Galaxy and compute share all the relevant
    file systems.
    """
    ...
    def agent_directory( self ):
        return os.path.abspath(self.job_wrapper.agent.agent_dir)
...
class ParallelismInfo(object):
    """
    Stores the information (if any) for running multiple instances of the agent in parallel
