...
class JobRunnerMapper( object ):
    ...
    def __invoke_expand_function( self, expand_function, destination_params ):
        ...
        possible_args = {
            ...
            "agent": self.job_wrapper.agent,
            "agent_id": self.job_wrapper.agent.id,
    ...
    def __determine_expand_function_name( self, destination ):
        # default look for function with name matching an id of agent, unless one specified
        expand_function_name = destination.params.get('function', None)
        if not expand_function_name:
            for agent_id in self.job_wrapper.agent.all_ids:
                if self.__last_rule_module_with_function( agent_id ):
                    expand_function_name = agent_id
                    break
    ...
    def __cache_job_destination( self, params, raw_job_destination=None ):
        if raw_job_destination is None:
            raw_job_destination = self.job_wrapper.agent.get_job_destination( params )
