...
class BaseJobRunner( object ):
    ...
    def get_work_dir_outputs( self, job_wrapper, job_working_directory=None ):
        ...
        job = job_wrapper.get_job()
        job_agent = job_wrapper.agent
        for (joda, dataset) in self._walk_dataset_outputs( job ):
            if joda and job_agent:
                hda_agent_output = job_agent.find_output_def( joda.name )
                if hda_agent_output and hda_agent_output.from_work_dir:
                    # Copy from working dir to HDA.
                    # TODO: move instead of copy to save time?
                    source_file = os.path.join( job_working_directory, hda_agent_output.from_work_dir )
    ...
    def _handle_metadata_externally( self, job_wrapper, resolve_requirements=False ):
        ...
        if job_wrapper.get_state() not in [ model.Job.states.ERROR, model.Job.states.DELETED ] and job_wrapper.output_paths:
            ...
            if resolve_requirements:
                dependency_shell_commands = self.app.datatypes_registry.set_external_metadata_agent.build_dependency_shell_commands(job_directory=job_wrapper.working_directory)
    ...
    def _find_container(
        ...
        compute_agent_directory=None,
        compute_job_directory=None
    ):
        if not compute_working_directory:
            compute_working_directory = job_wrapper.working_directory

        if not compute_agent_directory:
            compute_agent_directory = job_wrapper.agent.agent_dir

        agent = job_wrapper.agent
        from galaxy.agents.deps import containers
        agent_info = containers.AgentInfo(agent.containers, agent.requirements)
        job_info = containers.JobInfo(compute_working_directory, compute_agent_directory, compute_job_directory)
...
class JobState( object ):
    ...
    def set_defaults( self, files_dir ):
        if self.job_wrapper is not None:
            ...
            job_name = 'g%s' % id_tag
            if self.job_wrapper.agent.old_id:
                job_name += '_%s' % self.job_wrapper.agent.old_id
