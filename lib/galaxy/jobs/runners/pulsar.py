...
class PulsarJobRunner( AsynchronousJobRunner ):
    ...
    def __prepare_job(self, job_wrapper, job_destination):
        ...
        try:
            ...
            agent = job_wrapper.agent
            remote_job_config = client.setup(agent.id, agent.version)
            ...
            remote_agent_directory = os.path.abspath(os.path.join(remote_job_directory, "agent_files"))
            container = self._find_container(
                job_wrapper,
                compute_working_directory=remote_working_directory,
                compute_agent_directory=remote_agent_directory,
    ...
    def __dependencies_description( pulsar_client, job_wrapper ):
        ...
        if not remote_dependency_resolution:
            return None
        requirements = job_wrapper.agent.requirements or []
        installed_agent_dependencies = job_wrapper.agent.installed_agent_dependencies or []
...
class PulsarComputeEnvironment( ComputeEnvironment ):
    ...
    def __init__( self, pulsar_client, job_wrapper, remote_job_config ):
        ...
        self._agent_dir = remote_job_config[ "agents_directory" ]
