...
class JobHandlerQueue( object ):
    ...
    def __check_jobs_at_startup( self ):
        ...
        for job in jobs_at_startup:
            if not self.app.agentbox.has_agent( job.agent_id, job.agent_version, exact=True ):
                log.warning( "(%s) Agent '%s' removed from agent config, unable to recover job" % ( job.id, job.agent_id ) )
                self.job_wrapper( job ).fail( 'This agent was disabled before the job completed.  Please contact your Galaxy administrator.' )
                ...
                if self.track_jobs_in_database:
                    ...
                    self.queue.put( ( job.id, job.agent_id ) )
    ...
    def __verify_job_ready( self, job, job_wrapper ):
        ...
        try:
            assert job_wrapper.agent is not None, 'This agent was disabled before the job completed.  Please contact your Galaxy administrator.'
            # Cause the job_destination to be set and cached by the mapper
            job_destination = job_wrapper.job_destination
        except AssertionError as e:
            log.warning( "(%s) Agent '%s' removed from agent config, unable to run job" % ( job.id, job.agent_id ) )
            job_wrapper.fail( e )
    ...
    def __verify_in_memory_job_inputs( self, job ):
        ...
        for dataset_assoc in job.input_datasets + job.input_library_datasets:
            ...
            # an error in the input data causes us to bail immediately
                ...
                return JOB_INPUT_ERROR
            elif idata.state != idata.states.OK and not ( idata.state == idata.states.SETTING_METADATA and job.agent_id is not None and job.agent_id == self.app.datatypes_registry.set_external_metadata_agent.id ):
                # need to requeue
                return JOB_WAIT
    ...
    def put( self, job_id, agent_id ):
        """Add a job to the queue (by job identifier)"""
        if not self.track_jobs_in_database:
            self.queue.put( ( job_id, agent_id ) )
            self.sleeper.wake()
