...
class Genomes( object ):
    ...
    def reload_genomes( self ):
        ...
        for table_name in self._table_versions.keys():
            table = self.app.agent_data_tables.get( table_name, None )
            if table is not None:
                self._table_versions[ table_name ] = table._loaded_content_version

        twobit_table = self.app.agent_data_tables.get( 'twobit', None )
        twobit_fields = {}
        if twobit_table is None:
            # Add genome data (twobit files) to genomes, directly from twobit.loc
            try:
                for line in open( os.path.join( self.app.config.agent_data_path, "twobit.loc" ) ):
                    if line.startswith("#"):
                        continue
                    val = line.split()
                    if len( val ) == 2:
