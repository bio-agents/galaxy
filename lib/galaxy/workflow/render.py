...
class WorkflowCanvas( object ):
    ...
    def add_steps( self, highlight_errors=False ):
        # Only highlight missing agents if displaying in the agent shed.
        for step_dict in self.data:
            agent_unavailable = step_dict.get( 'agent_errors', False )
            if highlight_errors and agent_unavailable:
                fill = "#EBBCB2"
            else:
                fill = "#EBD9B2"
    ...
    def populate_data_for_step( self, step, module_name, module_data_inputs, module_data_outputs, agent_errors=None ):
        step_dict = {
            'id': step.order_index,
        ...
        if agent_errors:
            step_dict[ 'agent_errors' ] = agent_errors
