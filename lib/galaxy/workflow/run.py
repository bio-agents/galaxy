...
BASIC_WORKFLOW_STEP_TYPES = [ None, "agent", "data_input", "data_collection_input" ]
...
def force_queue( trans, workflow ):
    ...
    for step in workflow.steps:
        ...
        if step.type == "data_collection_input" and force_for_collection:
            ...
            return True
        if step.type == "agent" and has_module and step.module.agent.produces_collections_with_unknown_structure:
...
def __invoke( trans, workflow, workflow_run_config, workflow_invocation=None, populate_state=False ):
    ...
    """
    if populate_state:
        modules.populate_module_and_state( trans, workflow, workflow_run_config.param_map, allow_agent_state_corrections=workflow_run_config.allow_agent_state_corrections )
...
class WorkflowProgress( object ):
    ...
    def replacement_for_agent_input( self, step, input, prefixed_name ):
        """ For given workflow 'step' that has had input_connections_by_name
        populated fetch the actual runtime input for the given agent 'input'.
        """
        ...
        if prefixed_name in step.input_connections_by_name:
            ...
            if input.type == "data" and input.multiple:
                ...
                # If replacement is just one dataset collection, replace agent
                # input with dataset collection - agent framework will extract
                # datasets properly.
