...
def extract_steps( trans, history=None, job_ids=None, dataset_ids=None, dataset_collection_ids=None ):
    ...
    for hid in dataset_ids:
        ...
        step.agent_inputs = dict( name="Input Dataset" )
    ...
    for hid in dataset_collection_ids:
        ...
        step.agent_inputs = dict( name="Input Dataset Collection", collection_type=collection_type )
        hid_to_output_pair[ hid ] = ( step, 'output' )
        steps.append( step )
    # Agent steps
    for job_id in job_ids:
        ...
        agent_inputs, associations = step_inputs( trans, job )
        step = model.WorkflowStep()
        step.type = 'agent'
        step.agent_id = job.agent_id
        step.agent_version = job.agent_version
        step.agent_inputs = agent_inputs
        ...
        for assoc in (job.output_datasets + job.output_dataset_collection_instances):
            ...
            if AgentOutputCollectionPart.is_named_collection_part_name( assoc_name ):
                continue
            if job in summary.implicit_map_jobs:
                hid = None
...
def step_inputs( trans, job ):
    ...
    param_values = job.get_param_values( trans.app, ignore_errors=True )  # If a agent was updated and e.g. had a text value changed to an integer, we don't want a traceback here
    associations = __cleanup_param_values( agent.inputs, param_values )
    agent_inputs = agent.params_to_strings( param_values, trans.app )
    return agent_inputs, associations
...
def __cleanup_param_values( inputs, values ):
    ...
    def cleanup( prefix, inputs, values ):
        for key, input in inputs.items():
            if isinstance( input, ( SelectAgentParameter, DrillDownSelectAgentParameter ) ):
