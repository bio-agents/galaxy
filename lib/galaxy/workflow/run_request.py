...
class WorkflowRunConfig( object ):
    ...
    :param param_map: Override step parameters - should be dict with step id keys and
                      agent param name-value dicts as values.
    :type param_map: dict
    """

    def __init__( self, target_history, replacement_dict, copy_inputs_to_history=False, inputs={}, param_map={}, allow_agent_state_corrections=False ):
    ...
    for step in steps:
        ...
        for inputs_by_el in inputs_by.split("|"):
            if inputs_by_el == "step_id":
                ...
                possible_input_keys.append(step.agent_inputs.get( 'name', None ))
            else:
...
def normalize_step_parameters(steps, param_map, legacy=False):
    ...
    per tep fashion or by agent id and normalize the parameters so
...
def _step_parameters(step, param_map, legacy=False):
    ...
    in which case PARAM_DICT affects all steps with the given agent id.
    If both by-agent-id and by-step-id specifications are used, the
    ...
    """
    param_dict = param_map.get(step.agent_id, {}).copy()
    if legacy:
    ...
    # Inputs can be nested dict, but Galaxy agent code wants nesting of keys (e.g.
...
def _flatten_step_params( param_dict, prefix="" ):
    # TODO: Temporary work around until agent code can process nested data
    # structures. This should really happen in there so the agents API gets
    # this functionality for free and so that repeats can be handled
    # properly. Also the agent code walks the agent inputs so it nows what is
...
def build_workflow_run_config( trans, workflow, payload ):
    ...
    allow_agent_state_corrections = payload.get( 'allow_agent_state_corrections', False )
    ...
    run_config = WorkflowRunConfig(
        ...
        param_map=param_map,
        allow_agent_state_corrections=allow_agent_state_corrections
    )
    return run_config
    ...
    for step in workflow.steps:
        ...
        if step.type == "subworkflow":
            subworkflow_run_config = WorkflowRunConfig(
                ...
                inputs={},
                param_map={},
                allow_agent_state_corrections=run_config.allow_agent_state_corrections
            )
