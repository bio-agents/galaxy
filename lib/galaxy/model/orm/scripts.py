...
DATABASE = {
    ...
    "agent_shed":
        {
            'repo': 'lib/galaxy/webapps/agent_shed/model/migrate',
            'config_file': 'config/agent_shed.ini',
            'old_config_file': 'agent_shed_wsgi.ini',
            'default_sqlite_file': './database/community.sqlite',
            'config_override': 'TOOL_SHED_CONFIG_',
        },
    "install":
        {
            'repo': 'lib/galaxy/model/agent_shed_install/migrate',
            'old_config_file': 'universe_wsgi.ini',
...
def get_config( argv, cwd=None ):
    ...
    >>> write_ini('agent_shed.ini', 'database_connection', 'sqlite:///pg/testdb1')
    >>> config = get_config(['manage_db.py', 'agent_shed'], cwd=config_dir)
    >>> config['repo']
    'lib/galaxy/webapps/agent_shed/model/migrate'
    >>> config['db_url']
    'sqlite:///pg/testdb1'
    ...
    if argv and (argv[-1] in DATABASE):
        database = argv.pop()  # database name agent_shed, galaxy, or install.
    else:
        database = 'galaxy'
