...
from iteragents import ifilter, imap
...
class Job( object, JobLike, Dictifiable ):
    ...
    """
    A job represents a request to run a agent given input datasets, agent
    ...
    def set_agent_version( self, agent_version ):
    ...
    def get_param_values( self, app, ignore_errors=False ):
        ...
        dict of agent parameter values.
        """
        param_dict = self.raw_param_dict()
        agent = app.agentbox.get_agent( self.agent_id )
        param_dict = agent.params_from_strings( param_dict, app, ignore_errors=ignore_errors )
        return param_dict
...
class Task( object, JobLike ):
    ...
    def get_param_values( self, app ):
        ...
        agent = app.agentbox.get_agent( self.agent_id )
        param_dict = agent.params_from_strings( param_dict, app )
        return param_dict
...
# agent output (if mapping over an input collection).
...
# A DatasetCollection will be mapped to at most one job per agent output
...
class GenomeIndexAgentData( object ):
...
class DatasetInstance( object ):
    ...
    def __init__( self, id=None, hid=None, name=None, info=None, blurb=None, peek=None, agent_version=None, extension=None,
    ...
    def get_metadata_dataset( self, dataset_ext ):
        ...
        for name, value in self.metadata.items():
            ...
            if dataset_ext == 'bai' and name == 'bam_index' and isinstance( value, MetadataFile ):
                # HACK: MetadataFile objects cannot be used by agents, so return
...
class WorkflowStepConnection( object ):
    ...
    # index data before a normal agent runs or for workflows that manage data
    # outside of Galaxy.
...
class WorkflowInvocation( object, Dictifiable ):
    ...
    def to_dict( self, view='collection', value_mapper=None, step_details=False ):
        ...
        if view == 'element':
            ...
            inputs = {}
            for step in self.steps:
                if step.workflow_step.type == 'agent':
...
class AgentTagAssociation( ItemTagAssociation ):
    def __init__( self, id=None, user=None, agent_id=None, tag_id=None, user_tname=None, value=None ):
        self.id = id
        self.user = user
        self.agent_id = agent_id
        self.tag_id = tag_id
