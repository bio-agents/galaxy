...
model.Job.table = Table(
    ...
    Column( "agent_version", TEXT, default="1.0.0" ),
...
model.GenomeIndexAgentData.table = Table(
    "genome_index_agent_data", metadata,
    Column( "id", Integer, primary_key=True ),
    Column( "job_id", Integer, ForeignKey( "job.id" ), index=True ),
...
model.WorkflowStep.table = Table(
    ...
    # Reserved for future
    Column( "agent_version", TEXT ),
    Column( "agent_inputs", JSONType ),
    Column( "agent_errors", JSONType ),
...
model.AgentTagAssociation.table = Table(
    "agent_tag_association", metadata,
    Column( "id", Integer, primary_key=True ),
...
mapper( model.GenomeIndexAgentData, model.GenomeIndexAgentData.table, properties=dict(
...
tag_mapping( model.AgentTagAssociation, "tagged_agents" )
...
def init( file_path, url, engine_options={}, create_tables=False, map_install_models=False,
    ...
    if map_install_models:
        import galaxy.model.agent_shed_install.mapping  # noqa
        from galaxy.model import agent_shed_install
        model_modules.append(agent_shed_install)
