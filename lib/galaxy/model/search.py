...
# Agent Searching
##################
class AgentView(ViewQueryBaseClass):
    VIEW_NAME = "agent"
    FIELDS = {
        'agent_id': ViewField('name', sqlalchemy_field=(AgentVersion, "agent_id")),
        'id': ViewField('id', sqlalchemy_field=(AgentVersion, "id")),
    }

    def search(self, trans):
        self.query = trans.install_model.context.query( AgentVersion )
...
class JobView(ViewQueryBaseClass):
    DOMAIN = "job"
    FIELDS = {
        'agent_name': ViewField('agent_name', sqlalchemy_field=(Job, "agent_id")),
