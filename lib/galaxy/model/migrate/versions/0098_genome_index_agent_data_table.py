"""
Migration script to create the genome_index_agent_data table.
"""
import datetime
import logging
import sys
...
GenomeIndexAgentData_table = Table( "genome_index_agent_data", metadata,
...
def upgrade(migrate_engine):
    ...
    metadata.reflect()
    try:
        GenomeIndexAgentData_table.create()
    except Exception, e:
        log.debug( "Creating genome_index_agent_data table failed: %s" % str( e ) )
...
def downgrade(migrate_engine):
    ...
    metadata.reflect()
    try:
        GenomeIndexAgentData_table.drop()
    except Exception, e:
        log.debug( "Dropping genome_index_agent_data table failed: %s" % str( e ) )
