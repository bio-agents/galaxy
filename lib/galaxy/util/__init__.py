...
class Params( object ):
    ...
    # is NEVER_SANITIZE required now that sanitizing for agent parameters can be controlled on a per parameter basis and occurs via InputValueWrappers?
...
def read_dbnames(filename):
    ...
    try:
        ...
        name_to_db_base = {}
        if filename is None:
            # Should only be happening with the galaxy.agents.parameters.basic:GenomeBuildParameter docstring unit test
            filename = os.path.join( 'agent-data', 'shared', 'ucsc', 'builds.txt.sample' )
