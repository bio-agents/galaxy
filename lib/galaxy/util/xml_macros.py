...
def load(path):
    """
    Loads agent from file system and preprocesses agent macros.
    """
...
def raw_agent_xml_tree(path):
    """ Load raw (no macro expansion) tree representation of agent represented
...
def _import_macros(root, path):
    ...
    if macros_el is not None:
        macro_els = _load_macros(macros_el, agent_dir)
...
def _load_macros(macros_el, agent_dir):
    macros = []
    # Import macros from external files.
    macros.extend(_load_imported_macros(macros_el, agent_dir))
    # Load all directly defined macros.
    macros.extend(_load_embedded_macros(macros_el, agent_dir))
    return macros


def _load_embedded_macros(macros_el, agent_dir):
    macros = []
...
def _load_imported_macros(macros_el, agent_dir):
    macros = []

    for agent_relative_import_path in _imported_macro_paths_from_el(macros_el):
        import_path = \
            os.path.join(agent_dir, agent_relative_import_path)
        file_macros = _load_macro_file(import_path, agent_dir)
...
def _imported_macro_paths_from_el(macros_el):
    ...
    for macro_import_el in macro_import_els:
        ...
        agent_relative_import_path = \
            os.path.basename(raw_import_path)  # Sanitize this
        imported_macro_paths.append( agent_relative_import_path )
    return imported_macro_paths


def _load_macro_file(path, agent_dir):
    tree = _parse_xml(path)
    root = tree.getroot()
    return _load_macros(root, agent_dir)
