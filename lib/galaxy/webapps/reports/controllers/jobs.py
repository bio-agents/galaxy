...
class SpecifiedDateListGrid( grids.Grid ):
    ...
    class AgentColumn( grids.TextColumn ):
        ...
        def filter( self, trans, user, query, column_filter ):
            if column_filter is not None:
                query = query.filter( model.Job.table.c.agent_id == column_filter )
    ...
    columns = [
        ...
        AgentColumn( "Agent Id",
                    key="agent_id",
                    link=( lambda item: dict( operation="agent_per_month", id=item.id, webapp="reports" ) ),
                    attach_popup=False ),
...
class Jobs( BaseUIController, ReportQueryBuilder ):
        ...
        if 'operation' in kwd:
            ...
            if operation == "job_info":
                return trans.response.send_redirect( web.url_for( controller='jobs',
                                                                  action='job_info',
                                                                  **kwd ) )
            elif operation == "agent_for_month":
                kwd[ 'f-agent_id' ] = kwd[ 'agent_id' ]
            elif operation == "agent_per_month":
                # The received id is the job id, so we need to get the job's agent_id.
                job_id = kwd.get( 'id', None )
                job = get_job( trans, job_id )
                kwd[ 'agent_id' ] = job.agent_id
                return trans.response.send_redirect( web.url_for( controller='jobs',
                                                                  action='agent_per_month',
                                                                  **kwd ) )
            ...
            elif operation == "specified_agent_in_error":
                kwd[ 'f-state' ] = 'error'
                kwd[ 'f-agent_id' ] = kwd[ 'agent_id' ]
    ...
    @web.expose
    def per_agent( self, trans, **kwd ):
        message = ''
        PageSpec = namedtuple('PageSpec', ['entries', 'offset', 'page', 'pages_found'])

        params = util.Params( kwd )
        monitor_email = params.get( 'monitor_email', 'monitor@bx.psu.edu' )
        specs = sorter( 'agent_id', kwd )
        sort_id = specs.sort_id
        order = specs.order
        arrow = specs.arrow
        _order = specs.exc_order
        ...
        jobs = []
        q = sa.select( ( model.Job.table.c.agent_id.label( 'agent_id' ),
                         sa.func.count( model.Job.table.c.id ).label( 'total_jobs' ) ),
                       whereclause=model.Job.table.c.user_id != monitor_user_id,
                       from_obj=[ model.Job.table ],
                       group_by=[ 'agent_id' ],
        ...
        all_jobs_per_agent = sa.select( ( model.Job.table.c.agent_id.label( 'agent_id' ),
                                        model.Job.table.c.id.label( 'id' ),
        ...
        currday = date.today()
        trends = dict()
        for job in all_jobs_per_agent.execute():
            curr_agent = re.sub(r'\W+', '', str(job.agent_id))
            try:
                day = currday - job.date
            except TypeError:
                day = currday - datetime.date(job.date)
            ...
            try:
                if container < spark_limit:
                    trends[curr_agent][container] += 1
            except KeyError:
                trends[curr_agent] = [0] * spark_limit
                if container < spark_limit:
                    trends[curr_agent][container] += 1

        for row in q.execute():
            jobs.append( ( row.agent_id,
                           row.total_jobs ) )

        pages_found = ceil(len(jobs) / float(entries))
        page_specs = PageSpec(entries, offset, page, pages_found)

        return trans.fill_template( '/webapps/reports/jobs_per_agent.mako',
                                    order=order,
                                    arrow=arrow,
                                    sort_id=sort_id,
                                    spark_limit=spark_limit,
    ...
    @web.expose
    def errors_per_agent( self, trans, **kwd ):
        """
        Queries the DB for user jobs in error. Filters out monitor jobs.
        """
        ...
        jobs_in_error_per_agent = sa.select( ( model.Job.table.c.agent_id.label( 'agent_id' ),
                                              sa.func.count( model.Job.table.c.id ).label( 'total_jobs' ) ),
                                            whereclause=sa.and_( model.Job.table.c.state == 'error',
                                                                 model.Job.table.c.user_id != monitor_user_id ),
                                            from_obj=[ model.Job.table ],
        ...
        all_jobs_per_agent_errors = sa.select( ( self.select_day( model.Job.table.c.create_time ).label( 'date' ),
                                              model.Job.table.c.id.label( 'id' ),
                                              model.Job.table.c.agent_id.label( 'agent_id' ) ),
                                              whereclause=sa.and_( model.Job.table.c.state == 'error',
                                                                   model.Job.table.c.user_id != monitor_user_id ),
                                              from_obj=[ model.Job.table ]
                                              )
        ...
        currday = date.today()
        trends = dict()
        for job in all_jobs_per_agent_errors.execute():
            curr_agent = re.sub(r'\W+', '', str(job.agent_id))
            try:
                day = currday - job.date
            except TypeError:
            ...
            try:
                ...
                if day < spark_limit:
                    trends[curr_agent][container] += 1
        jobs = []
        for row in jobs_in_error_per_agent.execute():
            jobs.append( ( row.total_jobs, row.agent_id ) )

        pages_found = ceil(len(jobs) / float(entries))
        page_specs = PageSpec(entries, offset, page, pages_found)

        return trans.fill_template( '/webapps/reports/jobs_errors_per_agent.mako',
                                    order=order,
                                    arrow=arrow,
                                    sort_id=sort_id,
                                    spark_limit=spark_limit,
    ...
    @web.expose
    def agent_per_month( self, trans, **kwd ):
        message = ''
        ...
        agent_id = params.get( 'agent_id', 'Add a column1' )
        specified_date = params.get( 'specified_date', datetime.utcnow().strftime( "%Y-%m-%d" ) )
        q = sa.select( ( self.select_month( model.Job.table.c.create_time ).label( 'date' ),
                         sa.func.count( model.Job.table.c.id ).label( 'total_jobs' ) ),
                       whereclause=sa.and_( model.Job.table.c.agent_id == agent_id,
                                            model.Job.table.c.user_id != monitor_user_id ),
                       from_obj=[ model.Job.table ],
                       group_by=self.group_by_month( model.Job.table.c.create_time ),
                       order_by=[ _order ] )
        ...
        # Use to make sparkline
        all_jobs_for_agent = sa.select( ( self.select_month(model.Job.table.c.create_time).label('month'),
                             ...
                             from_obj=[ model.Job.table ] )
        trends = dict()
        for job in all_jobs_for_agent.execute():
        ...
        for row in q.execute():
            jobs.append( ( row.date.strftime( "%Y-%m" ),
                           ...
                           row.date.strftime( "%B" ),
                           row.date.strftime( "%Y" ) ) )
        return trans.fill_template( '/webapps/reports/jobs_agent_per_month.mako',
                                    order=order,
                                    arrow=arrow,
                                    sort_id=sort_id,
