...
class ASync( BaseUIController ):
    ...
    @web.expose
    def default(self, trans, agent_id=None, data_id=None, data_secret=None, **kwd):
        """Catches the agent id and redirects as needed"""
        return self.index( trans, agent_id=agent_id, data_id=data_id, data_secret=data_secret, **kwd)

    @web.expose
    def index(self, trans, agent_id=None, data_secret=None, **kwd):
        """Manages ascynchronous connections"""

        if agent_id is None:
            return "agent_id argument is required"
        ...
        agent = agentbox.get_agent( agent_id )
        if not agent:
            return "Agent with id %s not found" % agent_id
        ...
        if data_id:
            ...
            if STATUS == 'OK':
                key = hmac_new( trans.app.config.agent_secret, "%d:%d" % ( data.id, data.history_id ) )
                if key != data_secret:
                    return "You do not have permission to alter data %s." % data_id
                # push the job into the queue
                data.state = data.blurb = data.states.RUNNING
                log.debug('executing agent %s' % agent.id)
                trans.log_event( 'Async executing agent %s' % agent.id, agent_id=agent.id )
                galaxy_url = trans.request.base + '/async/%s/%s/%s' % ( agent_id, data.id, key )
                ...
                params[agent.outputs.keys()[0]] = data.id
                agent.execute( trans, incoming=params )
            else:
            ...
            if params.data_type:
                ...
                GALAXY_TYPE = params.GALAXY_TYPE or agent.outputs.values()[0].format

            GALAXY_NAME = params.name or params.GALAXY_NAME or '%s query' % agent.name
            ...
            trans.sa_session.flush()
            trans.log_event( "Added dataset %d to history %d" % (data.id, trans.history.id ), agent_id=agent_id )
            ...
            try:
                ...
                # Use provided URL or fallback to agent action
                url = URL or agent.action
        ...
        return trans.fill_template( 'root/agent_runner.mako', out_data={}, num_jobs=1, job_errors=[] )
