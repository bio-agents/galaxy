...
class RootController( controller.JSAppLauncher, UsesAnnotations ):
    ...
    def _get_extended_config( self, trans ):
        ...
        config = {
            ...
            'search_url'                    : web.url_for( controller='root', action='agent_search' ),
            # TODO: next two should be redundant - why can't we build one from the other?
            'agentbox'                       : app.agentbox.to_dict( trans, in_panel=False ),
            'agentbox_in_panel'              : app.agentbox.to_dict( trans ),
    ...
    @web.expose
    def index( self, trans, agent_id=None, workflow_id=None, history_id=None, m_c=None, m_a=None, **kwd ):
        """
        Root and entry point for client-side web app.

        :type       agent_id: str or None
        :param      agent_id: load center panel with given agent if not None
    ...
    @web.json
    def agent_search( self, trans, **kwd ):
        """Searches the agent database and returns data for any agent
        ...
        trans.log_action( trans.get_user(), "agent_search.search", "", { "query" : query, "tags" : tags } )
        results = []
        if tags:
            tags = trans.sa_session.query( trans.app.model.Tag ).filter( trans.app.model.Tag.name.in_( tags ) ).all()
            for tagged_agent_il in [ tag.tagged_agents for tag in tags ]:
                for tagged_agent in tagged_agent_il:
                    if tagged_agent.agent_id not in results:
                        results.append( tagged_agent.agent_id )
            if trans.user:
                trans.user.preferences['selected_agent_tags'] = ','.join( [ tag.name for tag in tags ] )
                trans.sa_session.flush()
        elif trans.user:
            trans.user.preferences['selected_agent_tags'] = ''
            trans.sa_session.flush()
        if len( query ) > 2:
            search_results = trans.app.agentbox_search.search( query )
            if 'tags[]' in kwd:
    ...
    @web.expose
    def agent_help( self, trans, id ):
        """Return help page for agent identified by 'id' if available
        """
        agentbox = self.get_agentbox()
        agent = agentbox.get_agent( id )
        yield "<html><body>"
        if not agent:
            # TODO: arent agent ids strings now?
            yield "Unknown agent id '%d'" % id
        elif agent.help:
            yield agent.help
        else:
            yield "No additional help available for agent '%s'" % agent.name
        yield "</body></html>"
    ...
    def display( self, trans, id=None, hid=None, tofile=None, toext=".txt", encoded_id=None, **kwd ):
        ...
        Used by the twill agent test driver - used anywhere else? Would like to drop hid
