...
class User( BaseUIController, UsesFormDefinitionsMixin, CreatesUsersMixin, CreatesApiKeysMixin ):
        ...
        if user:
            ...
            if trans.webapp.name == 'galaxy' and not refresh_frames:
                if trans.app.config.require_login:
                    refresh_frames = [ 'masthead', 'history', 'agents' ]
                else:
        ...
        if not user and trans.app.config.require_login:
            if trans.app.config.allow_user_creation:
                ...
                if trans.webapp.name == 'galaxy':
                    ...
                    header = REQUIRE_LOGIN_TEMPLATE % ( "Galaxy agent shed", create_account_str )
            else:
                if trans.webapp.name == 'galaxy':
                    header = REQUIRE_LOGIN_TEMPLATE % ( "Galaxy instance", "" )
                else:
                    header = REQUIRE_LOGIN_TEMPLATE % ( "Galaxy agent shed", "" )
    ...
    def manage_user_info( self, trans, cntrller, **kwd ):
        ...
        if trans.webapp.name == 'galaxy':
            ...
            return trans.fill_template( '/webapps/agent_shed/user/manage_info.mako',
                                        cntrller=cntrller,
                                        user=user,
                                        email=email,
                                        username=username,
    ...
    def __validate( self, trans, params, email, password, confirm, username ):
        # If coming from the agent shed webapp, we'll require a public user name
        if trans.webapp.name == 'agent_shed':
            if not username:
                return "A public user name is required in the agent shed."
            if username in [ 'repos' ]:
                return "The term <b>%s</b> is a reserved word in the agent shed, so it cannot be used as a public user name." % escape( username )
    ...
    @web.expose
    @web.require_login()
    def agentbox_filters( self, trans, cntrller, **kwd ):
        """
            Sets the user's default filters for the agentbox.
            Agentbox filters are specified in galaxy.ini.
        ...
        def get_filter_mapping( db_filters, config_filters ):
            ...
            for filter_name in config_filters:
                if ":" in filter_name:
                    ...
                    module_name = 'galaxy.agents.filters.%s' % module_name.strip()
                    ...
                    module = __import__( 'galaxy.agents.filters', globals(), fromlist=['temp_module'] )
        ...
        if user:
            ...
            for name, value in user.preferences.items():
                if name == 'agentbox_agent_filters':
                    saved_user_agent_filters = listify( value, do_strip=True )
            ...
            agent_filters = get_filter_mapping( saved_user_agent_filters, trans.app.config.user_agent_filters )
            ...
            return trans.fill_template( 'user/agentbox_filters.mako',
                                        cntrller=cntrller,
                                        message=message,
            ...
            # User not logged in, history group must be only public
            return trans.show_error_message( "You must be logged in to change private agentbox filters." )

    @web.expose
    @web.require_login( "to change the private agentbox filters" )
    def edit_agentbox_filters( self, trans, cntrller, **kwd ):
        params = util.Params( kwd )
        message = util.restore_text( params.get( 'message', '' ) )
        user_id = params.get( 'user_id', False )
        if not user_id:
            # User must be logged in to create a new address
            return trans.show_error_message( "You must be logged in to change the AgentBox filters." )

        user = trans.sa_session.query( trans.app.model.User ).get( trans.security.decode_id( user_id ) )

        if params.get( 'edit_agentbox_filter_button', False ):
            agent_filters = list()
            ...
            for name, state in params.flatten():
                if state == 'on':
                    if name.startswith('t_'):
                        agent_filters.append( name[2:] )
                    elif name.startswith('l_'):
                        label_filters.append( name[2:] )
                    elif name.startswith('s_'):
                        section_filters.append( name[2:] )
            user.preferences['agentbox_agent_filters'] = ','.join( agent_filters )
            user.preferences['agentbox_section_filters'] = ','.join( section_filters )
            user.preferences['agentbox_label_filters'] = ','.join( label_filters )

            trans.sa_session.add( user )
            trans.sa_session.flush()
            message = 'AgentBox filters has been updated.'
            kwd = dict( message=message, status='done' )

        # Display the AgentBox filters form with the current values filled in
        return self.agentbox_filters( trans, cntrller, **kwd )

    @web.expose
    @web.require_login( "to get most recently used agent" )
    @web.json_pretty
    def get_most_recently_used_agent_async( self, trans ):
        """ Returns information about the most recently used agent. """

        # Get most recently used agent.
        query = trans.sa_session.query( self.app.model.Job.agent_id ).join( self.app.model.History ) \
                                .filter( self.app.model.History.user == trans.user ) \
                                .order_by( self.app.model.Job.create_time.desc() ).limit(1)
        agent_id = query[0][0]  # Get first element in first row of query.
        agent = self.get_agentbox().get_agent( agent_id )

        # Return agent info.
        agent_info = {"id": agent.id,
                     "link": url_for( controller='agent_runner', agent_id=agent.id ),
                     "target": agent.target,
                     "name": agent.name,  # TODO: translate this using _()
                     "minsizehint": agent.uihints.get( 'minwidth', -1 ),
