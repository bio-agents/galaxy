...
from galaxy.agents.errors import EmailErrorReporter
...
class DatasetInterface( BaseUIController, UsesAnnotations, UsesItemRatings, UsesExtendedMetadataMixin ):
    ...
    def report_error( self, trans, id, email='', message="", **kwd ):
        ...
        if biostar_report:
            return trans.response.send_redirect( url_for( controller='biostar', action='biostar_agent_bug_report', hda=id, email=email, message=message ) )
        try:
    ...
    def transfer_status(self, trans, dataset_id, filename=None):
        ...
        # through agent's output page anyhow so tying a JavaScript event that will
            ...
            if params.change:
                ...
                if data.datatype.allow_datatype_change and trans.app.datatypes_registry.get_datatype_by_extension( params.datatype ).allow_datatype_change:
                    ...
                    if not __ok_to_edit_metadata( data.id ):
                        ...
                        trans.app.datatypes_registry.set_external_metadata_agent.agent_action.execute( trans.app.datatypes_registry.set_external_metadata_agent, trans, incoming={ 'input1': data }, overwrite=False )  # overwrite is False as per existing behavior
...
#                        status = 'done'
                ...
                if not __ok_to_edit_metadata( data.id ):
                    ...
                    trans.app.datatypes_registry.set_external_metadata_agent.agent_action.execute( trans.app.datatypes_registry.set_external_metadata_agent, trans, incoming={ 'input1': data } )
                    trans.sa_session.flush()
                    refresh_frames = ['history']
        ...
        if job_dataset_association.creating_job_associations:
            ...
            if job:
                ...
                try:
                    ...
                    agent = agentbox.get_agent( job.agent_id )
                    assert agent is not None, 'Requested agent has not been loaded.'
                    ...
                    try:
                        ...
                        upgrade_messages = agent.check_and_update_param_values( job.get_param_values( trans.app, ignore_errors=True ),
                                                                               trans,
                                                                               update_values=False )
