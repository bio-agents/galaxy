...
from galaxy.agents.actions import upload_common
...
# Whoosh is compatible with Python 2.5+ Try to import Whoosh and set flag to indicate whether agent search is enabled.
try:
    import whoosh.index
...
class LibraryCommon( BaseUIController, UsesFormDefinitionsMixin, UsesExtendedMetadataMixin, UsesLibraryMixinItems ):
    ...
    def ldda_edit_info( self, trans, cntrller, library_id, folder_id, id, **kwd ):
        ...
        if kwd.get( 'change', False ):
            ...
            if __ok_to_edit_metadata( ldda.id ):
                ...
                status = 'done'
                trans.app.datatypes_registry.set_external_metadata_agent.agent_action.execute( trans.app.datatypes_registry.set_external_metadata_agent, trans, incoming={ 'input1': ldda } )
            else:
    ...
    def upload_library_dataset( self, trans, cntrller, library_id, folder_id, **kwd ):
        ...
        if kwd.get( 'runagent_btn', False ) or kwd.get( 'ajax_upload', False ) or cntrller == 'api':
            error = False
    ...
    def upload_dataset( self, trans, cntrller, library_id, folder_id, replace_dataset=None, **kwd ):
        # Set up the traditional agent state/params
        agent_id = 'upload1'
        agent = trans.app.agentbox.get_agent( agent_id )
        state = agent.new_state( trans )
        agent.populate_state( trans, agent.inputs, state.inputs, kwd )
        agent_params = state.inputs
        dataset_upload_inputs = []
        for input_name, input in agent.inputs.iteritems():
        ...
        if response_code == 200:
            precreated_datasets = upload_common.get_precreated_datasets( trans, agent_params, trans.app.model.LibraryDatasetDatasetAssociation, controller=cntrller )
            if upload_option == 'upload_file':
                agent_params = upload_common.persist_uploads( agent_params )
                uploaded_datasets = upload_common.get_uploaded_datasets( trans, cntrller, agent_params, precreated_datasets, dataset_upload_inputs, library_bunch=library_bunch )
        ...
        job, output = upload_common.create_job( trans, agent_params, agent, json_file_path, data_list, folder=library_bunch.folder, job_params=job_params )
