...
class HistoryController( BaseUIController, SharableMixin, UsesAnnotations, UsesItemRatings,
    ...
    def display_structured( self, trans, id=None ):
        ...
        for hda in history.active_datasets:
            ...
            # before jobs were tracked, or from the upload agent before it
    ...
    def structure( self, trans, id=None, **kwargs ):
        ...
        agents = {}
        for agent_id in set( map( lambda j: j[ 'agent_id' ], jobs ) ):
            unquoted_id = urllib.unquote_plus( agent_id )
            agent = self.app.agentbox.get_agent( unquoted_id )
            if not agent:
                raise exceptions.ObjectNotFound( "Could not find agent with id '%s'" % agent_id )
                # TODO: some fallback for agent information
            agents[ agent_id ] = agent.to_dict( trans, io_details=True, link_details=True )

        return trans.fill_template( "history/structure.mako", historyId=history_dictionary[ 'id' ],
            history=history_dictionary, contents=contents, jobs=jobs, agents=agents, **kwargs )
