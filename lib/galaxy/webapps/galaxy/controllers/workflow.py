...
from galaxy.workflow.modules import MissingAgentException
...
class WorkflowController( BaseUIController, SharableMixin, UsesStoredWorkflowMixin, UsesItemRatings ):
    ...
    @web.json
    def editor_form_post( self, trans, type='agent', content_id=None, annotation=None, label=None, **incoming ):
        """
        Accepts a agent state and incoming values, and generates a new agent
        ...
        is selected.
        """
        agent_state = incoming.pop('agent_state', None)
        ...
        if type == 'agent':
            return {
                'label': module.label,
                'agent_state': module.get_state(),
                'data_inputs': module.get_data_inputs(),
                'data_outputs': module.get_data_outputs(),
                'agent_errors': module.get_errors(),
    ...
    def get_new_module_info( self, trans, type, **kwargs ):
        ...
        of the initial form, and the initial agent state (with default values).
        ...
        return {
            ...
            'agenttip': module.get_agenttip( static_path=url_for( '/static' ) ),
            'data_inputs': module.get_data_inputs(),
            'data_outputs': module.get_data_outputs(),
            'form_html': module.get_config_form(),
            'annotation': ""
    ...
    def save_workflow( self, trans, id, workflow_data ):
        ...
        except workflows.MissingAgentsException as e:
            return dict(
                name=e.workflow.name,
                message="This workflow includes missing or invalid agents. "
                        "It cannot be saved until the following steps are removed or the missing agents are enabled.",
                errors=e.errors,
            )
    ...
    def export_to_file( self, trans, id ):
        ...
        if not stored_dict:
            # This workflow has a agent that's missing from the distribution
            trans.response.status = 400
            return "Workflow cannot be exported due to missing agents."
    ...
    def import_workflow( self, trans, cntrller='workflow', **kwd ):
        ...
        # id being imported from a Galaxy agent shed repository.
        ...
        # from a Galaxy agent shed, in which case the value was encoded.
        ...
        # from a agent shed repository installed locally or from the API.
        ...
        if installed_repository_file and not import_button:
            ...
            workflow_file.close()
            import_button = True
        if agent_shed_url and not import_button:
            # Use urllib (send another request to the agent shed) to retrieve the workflow.
            params = dict( repository_metadata_id=repository_metadata_id,
                           workflow_name=encoding_util.agent_shed_encode( workflow_name ),
                           open_for_url=True )
            pathspec = [ 'workflow', 'import_workflow' ]
            workflow_text = common_util.agent_shed_get( trans.app, agent_shed_url, pathspec=pathspec, params=params )
            import_button = True
        if import_button:
            workflow_data = None
            if url:
                ...
                # This case occurs when the workflow_text was sent via http from the agent shed.
            ...
            if workflow_data:
                ...
                if data:
                    # Create workflow if possible.  If a required agent is not available in the local
                    # Galaxy instance, the agent information will be available in the step_dict.
                    src = None
                    if cntrller != 'api':
                        src = workflow_source
                    workflow, missing_agent_tups = self._workflow_from_dict( trans, data, source=src, add_to_menu=add_to_menu )
                    ...
                    if missing_agent_tups:
                        if trans.user_is_admin():
                            # A required agent is not available in the local Galaxy instance.
                            # TODO: It would sure be nice to be able to redirect to a mako template here that displays a nice
                            # page including the links to the configured agent sheds instead of this message, but trying
                            ...
                            message += "The workflow requires the following agents that are not available in this Galaxy instance."
                            message += "You can likely install the required agents from one of the Galaxy agent sheds listed below.<br/>"
                            for missing_agent_tup in missing_agent_tups:
                                missing_agent_id, missing_agent_name, missing_agent_version = missing_agent_tup
                                message += "<b>Agent name</b> %s, <b>id</b> %s, <b>version</b> %s<br/>" % (
                                           escape( missing_agent_name ),
                                           escape( missing_agent_id ),
                                           escape( missing_agent_version ) )
                            message += "<br/>"
                            for shed_name, shed_url in trans.app.agent_shed_registry.agent_sheds.items():
                                if shed_url.endswith( '/' ):
                                    shed_url = shed_url.rstrip( '/' )
                                    url = '%s/repository/find_agents?galaxy_url=%s' % ( shed_url, url_for( '/', qualified=True ) )
                                    if missing_agent_tups:
                                        url += '&agent_id='
                                    for missing_agent_tup in missing_agent_tups:
                                        missing_agent_id = missing_agent_tup[0]
                                        url += '%s,' % escape( missing_agent_id )
                                message += '<a href="%s">%s</a><br/>' % ( url, shed_name )
                                status = 'error'
                            if installed_repository_file or agent_shed_url:
                            ...
                            pass
                    if agent_shed_url:
                        # We've received the textual representation of a workflow from a Galaxy agent shed.
                        message = "Workflow <b>%s</b> imported successfully." % escape( workflow.name )
                        url = '%s/workflow/view_workflow?repository_metadata_id=%s&workflow_name=%s&message=%s' % \
                            ( agent_shed_url, repository_metadata_id, encoding_util.agent_shed_encode( workflow_name ), message )
                        return trans.response.send_redirect( url )
                    elif installed_repository_file:
                        # The workflow was read from a file included with an installed agent shed repository.
                        message = "Workflow <b>%s</b> imported successfully." % escape( workflow.name )
                        if cntrller == 'api':
