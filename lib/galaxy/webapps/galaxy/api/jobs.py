...
class JobController( BaseAPIController, UsesLibraryMixinItems ):
    ...
    def index( self, trans, **kwd ):
        """
        index( trans, state=None, agent_id=None, history_id=None, date_range_min=None, date_range_max=None, user_details=False )
        * GET /api/jobs:
        ...
        :type   agent_id: string or list
        :param  agent_id: limit listing of jobs to those that match one of the included agent_ids. If none, all are returned.
        ...
        query = build_and_apply_filters( query, kwd.get( 'agent_id', None ), lambda t: trans.app.model.Job.agent_id == t )
        query = build_and_apply_filters( query, kwd.get( 'agent_id_like', None ), lambda t: trans.app.model.Job.agent_id.like(t) )
    ...
    def build_for_rerun( self, trans, id, **kwd ):
        ...
        * GET /api/jobs/{id}/build_for_rerun
            returns a agent input/param template prepopulated with this job's
        ...
        agent = self.app.agentbox.get_agent( job.agent_id, job.agent_version )
        if not agent.is_workflow_compatible:
            raise exceptions.ConfigDoesNotAllowException( "Agent '%s' cannot be rerun." % ( job.agent_id ) )
        return agent.to_json(trans, {}, job=job)
    ...
    def create( self, trans, payload, **kwd ):
        """ See the create method in agents.py in order to submit a job. """
        raise NotImplementedError( 'Please POST to /api/agents instead.' )
    ...
    def search( self, trans, payload, **kwd ):
        ...
        * POST /api/jobs/search:
        ...
        :param  payload: Dictionary containing description of requested job. This is in the same format as
            a request to POST /apt/agents would take to initiate a job
        ...
        if agent_id is None:
            raise exceptions.ObjectAttributeMissingException( "No agent id" )

        agent = trans.app.agentbox.get_agent( agent_id )
        if agent is None:
            raise exceptions.ObjectNotFound( "Requested agent not found" )
        ...
        query = trans.sa_session.query( trans.app.model.Job ).filter(
            trans.app.model.Job.agent_id == agent_id,
