...
class ConfigurationController( BaseAPIController ):
    ...
    @expose_api
    @require_admin
    def dynamic_agent_confs(self, trans):
        confs = self.app.agentbox.dynamic_confs(include_migrated_agent_conf=True)
        return map(_agent_conf_to_dict, confs)

    @expose_api
    @require_admin
    def agent_lineages(self, trans):
        rval = []
        for id, agent in self.app.agentbox.agents():
    ...
    @expose_api
    @require_admin
    def reload_agentbox(self, trans):
        """
        PUT /api/configuration/agentbox
        Reload the Galaxy agentbox (but not individual agents).
        """
        self.app.reload_agentbox()


def _agent_conf_to_dict(conf):
    return dict(
        config_filename=conf['config_filename'],
        agent_path=conf['agent_path'],
    )
