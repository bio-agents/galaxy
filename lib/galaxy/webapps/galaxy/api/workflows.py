...
class WorkflowsAPIController(BaseAPIController, UsesStoredWorkflowMixin, UsesAnnotations, SharableMixin):
    ...
    def create(self, trans, payload, **kwd):
        ...
        :param  allow_agent_state_corrections:  If set to True, any Agent parameter changes will not prevent running workflow, defaults to False
        :type   allow_agent_state_corrections:  bool
        """
        ways_to_create = set( [
            'workflow_id',
        ...
        # invoke may throw MessageExceptions on agent erors, failure
    ...
    def workflow_dict( self, trans, workflow_id, **kwd ):
        ...
        if not ret_dict:
            ...
            message = "Workflow cannot be exported due to missing agents."
    ...
    def build_module( self, trans, payload={}):
        ...
        Builds module details including a agent model for the workflow editor.
        """
        agent_id = payload.get( 'agent_id', None )
        agent_version = payload.get( 'agent_version', None )
        agent_inputs = payload.get( 'inputs', {} )
        annotation = payload.get( 'annotation', agent_inputs.get('annotation', '') )

        # load agent
        agent = self._get_agent( agent_id, agent_version=agent_version, user=trans.user )
        ...
        # create agent model and default agent state (if missing)
        agent_model = module.agent.to_json(trans, agent_inputs, workflow_mode=True)
        module.state.inputs = copy.deepcopy(agent_model['state_inputs'])
        return {
            ...
            'post_job_actions'  : module.get_post_job_actions(agent_inputs)
        }
    ...
    def __api_import_new_workflow( self, trans, payload, **kwd ):
        ...
        workflow, missing_agent_tups = self._workflow_from_dict( trans, data, **from_dict_kwds )
