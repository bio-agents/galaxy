...
def paste_app_factory( global_conf, **kwargs ):
    ...
    webapp.add_route( '/async/{agent_id}/{data_id}/{data_secret}', controller='async', action='index', agent_id=None, data_id=None, data_secret=None )
    ...
    try:
        ...
        if galaxy.model.agent_shed_install.mapping.metadata.bind:
            galaxy.model.agent_shed_install.mapping.metadata.bind.dispose()
    except:
        log.exception("Unable to dispose of pooled agentshed install model database connections.")
...
def populate_api_routes( webapp, app ):
    ...
    webapp.mapper.resource( 'agent_data', 'agent_data', path_prefix='/api' )
    webapp.mapper.connect( '/api/agent_data/{id:.+?}/fields/{value:.+?}/files/{path:.+?}', action='download_field_file', controller="agent_data" )
    webapp.mapper.connect( '/api/agent_data/{id:.+?}/fields/{value:.+?}', action='show_field', controller="agent_data" )
    webapp.mapper.connect( '/api/agent_data/{id:.+?}/reload', action='reload', controller="agent_data" )
    ...
    webapp.mapper.connect( '/api/agents/{id:.+?}/build', action='build', controller="agents" )
    webapp.mapper.connect( '/api/agents/{id:.+?}/reload', action='reload', controller="agents" )
    webapp.mapper.connect( '/api/agents/{id:.+?}/diagnostics', action='diagnostics', controller="agents" )
    webapp.mapper.connect( '/api/agents/{id:.+?}/citations', action='citations', controller="agents" )
    webapp.mapper.connect( '/api/agents/{id:.+?}/download', action='download', controller="agents" )
    webapp.mapper.connect( '/api/agents/{id:.+?}', action='show', controller="agents" )
    webapp.mapper.resource( 'agent', 'agents', path_prefix='/api' )

    webapp.mapper.connect( '/api/dependency_resolvers/dependency', action="manager_dependency", controller="agent_dependencies" )
    webapp.mapper.connect( '/api/dependency_resolvers/requirements', action="manager_requirements", controller="agent_dependencies" )
    webapp.mapper.connect( '/api/dependency_resolvers/{id}/dependency', action="resolver_dependency", controller="agent_dependencies", method=['GET'] )
    webapp.mapper.connect( '/api/dependency_resolvers/{id}/dependency', action="install_dependency", controller="agent_dependencies", method=['POST'] )
    webapp.mapper.connect( '/api/dependency_resolvers/{id}/requirements', action="resolver_requirements", controller="agent_dependencies" )
    webapp.mapper.resource( 'dependency_resolver', 'dependency_resolvers', controller="agent_dependencies", path_prefix='api' )
    ...
    webapp.mapper.connect(
        'dynamic_agent_confs',
        '/api/configuration/dynamic_agent_confs',
        controller="configuration",
        action="dynamic_agent_confs"
    )
    webapp.mapper.connect(
        'agent_lineages',
        '/api/configuration/agent_lineages',
        controller="configuration",
        action="agent_lineages"
    )
    webapp.mapper.connect(
        '/api/configuration/agentbox',
        controller="configuration",
        action="reload_agentbox",
    ...
    # Handle displaying agent help images and README file images contained in repositories installed from the agent shed.
    webapp.add_route( '/admin_agentshed/static/images/{repository_id}/{image_file:.+?}',
                      controller='admin_agentshed',
                      action='display_image_in_repository',
                      repository_id=None,
                      image_file=None )

    # Galaxy API for agent shed features.
    webapp.mapper.resource( 'agent_shed_repository',
                            ...
                            controller='agent_shed_repositories',
                            name_prefix='agent_shed_repository_',
                            path_prefix='/api',
                            new={ 'install_repository_revision': 'POST' },
                            parent_resources=dict( member_name='agent_shed_repository', collection_name='agent_shed_repositories' ) )
