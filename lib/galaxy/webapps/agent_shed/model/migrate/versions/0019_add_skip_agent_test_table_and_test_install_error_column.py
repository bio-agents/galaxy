"""
Migration script to add the skip_agent_test table and add the test_install_error column to the repository_metadata table.
"""
import datetime
import logging
import sys
...
SkipAgentTest_table = Table( "skip_agent_test", metadata,
...
def upgrade( migrate_engine ):
    ...
    # Create skip_agent_test table.
    try:
        SkipAgentTest_table.create()
    except Exception, e:
        print "Creating the skip_agent_test table failed: %s" % str( e )
...
def downgrade( migrate_engine ):
    ...
    # Drop the skip_agent_test table.
    try:
        SkipAgentTest_table.drop()
    except Exception, e:
        print "Dropping the skip_agent_test table failed: %s" % str( e )
