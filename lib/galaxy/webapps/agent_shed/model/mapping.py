...
import galaxy.webapps.agent_shed.util.shed_statistics as shed_statistics
from galaxy.model.base import ModelMapping
from galaxy.model.custom_types import JSONType, TrimmedString
from galaxy.model.orm.engine_factory import build_engine
from galaxy.model.orm.now import now
from galaxy.webapps.agent_shed.model import APIKeys, Category, Component, ComponentReview, GalaxySession, Group, GroupRoleAssociation, PasswordResetToken, Repository, RepositoryCategoryAssociation, RepositoryMetadata, RepositoryRatingAssociation, RepositoryReview, RepositoryRoleAssociation, Role, SkipAgentTest, Tag, User, UserGroupAssociation, UserRoleAssociation
from galaxy.webapps.agent_shed.security import CommunityRBACAgent
...
RepositoryMetadata.table = Table( "repository_metadata", metadata,
                                  ...
                                  Column( "agent_versions", JSONType, nullable=True ),
                                  Column( "malicious", Boolean, default=False ),
                                  Column( "downloadable", Boolean, default=True ),
                                  Column( "agents_functionally_correct", Boolean, default=False, index=True ),
                                  ...
                                  Column( "agent_test_results", JSONType, nullable=True ),
                                  ...
                                  Column( "includes_agents", Boolean, default=False, index=True ),
                                  Column( "includes_agent_dependencies", Boolean, default=False, index=True ),
                                  Column( "includes_workflows", Boolean, default=False, index=True ) )

SkipAgentTest.table = Table( "skip_agent_test", metadata,
                            Column( "id", Integer, primary_key=True ),
                            Column( "create_time", DateTime, default=now ),
                            Column( "update_time", DateTime, default=now, onupdate=now ),
...
mapper( SkipAgentTest, SkipAgentTest.table,
        properties=dict( repository_revision=relation( RepositoryMetadata,
                                                       backref='skip_agent_tests' ) ) )
...
def init( file_path, url, engine_options={}, create_tables=False ):
    ...
    result = ModelMapping([galaxy.webapps.agent_shed.model], engine=engine)
    ...
    # Load local agent shed security policy
    result.security_agent = CommunityRBACAgent( result )
    result.shed_counter = shed_statistics.ShedCounter( result )
    result.hgweb_config_manager = galaxy.webapps.agent_shed.util.hgweb_config.HgWebConfigManager()
    return result
