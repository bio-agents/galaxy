...
class Hg( object ):
    ...
    def __init__( self, app, config ):
        ...
        self.authentication = AuthBasicAuthenticator( 'hgweb in the agent shed', self.__basic_authentication )
                ...
                if changeset_groups:
                    ...
                    if 'PATH_INFO' in environ:
                        ...
                        if repository_type == rt_util.REPOSITORY_SUITE_DEFINITION:
                            ...
                            # Handle repositories of type agent_dependency_definition, which can only contain a single
                            # file named agent_dependencies.xml.
                            for entry in changeset_groups:
                                if len( entry ) == 2:
                                    # We possibly found an altered file entry.
                                    filename, change_list = entry
                                    if filename and isinstance( filename, str ):
                                        if filename == rt_util.TOOL_DEPENDENCY_DEFINITION_FILENAME:
                                            ...
                                            msg = "Only a single file named agent_dependencies.xml can be pushed to a repository "
                                            msg += "of type 'Agent dependency definition'."
                                            log.debug( msg )
                                            return self.__display_exception_remotely( start_response, msg )
                        else:
                            # If the changeset includes changes to dependency definition files, make sure tag sets
                            # are not missing "agentshed" or "changeset_revision" attributes since automatically populating
                            # them is not supported when pushing from the command line.  These attributes are automatically
                            # populated only when using the agent shed upload utility.
                            for entry in changeset_groups:
                                if len( entry ) == 2:
                                    # We possibly found an altered file entry.
                                    filename, change_list = entry
                                    if filename and isinstance( filename, str ):
                                        if filename in [ rt_util.REPOSITORY_DEPENDENCY_DEFINITION_FILENAME,
                                            ...
                                            # We check both files since agent dependency definitions files can contain complex
    ...
    def repository_tag_is_valid( self, filename, line ):
        ...
        Agent Shed from the command line to ensure that all required attributes exist.
        """
        required_attributes = [ 'agentshed', 'name', 'owner', 'changeset_revision' ]
        ...
        for required_attribute in required_attributes:
            ...
            if not defined:
                ...
                error_msg += 'the Agent Shed upload utility.  '
    ...
    def repository_tags_are_valid( self, filename, change_list ):
        ...
        changes to the agent shed on the command line.
        """
        tag = '<repository'
