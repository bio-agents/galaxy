        ...
        if 'HTTP_REMOTE_USER' in environ and environ[ 'HTTP_REMOTE_USER' ] != '(null)':
            if not environ[ 'HTTP_REMOTE_USER' ].count( '@' ):
                if self.maildomain is not None:
                    ...
                    title = "Access to this Galaxy agent shed is denied"
                    message = """
                        This Galaxy agent shed is configured to authenticate users via an external
                        method (such as HTTP authentication in Apache), but only a username (not
                        an email address) was provided by the upstream (proxy) server.  Since agent
                        ...
                        can access this agent shed.  Contact your local agent shed administrator.
                    """
                    return self.error( start_response, title, message )
            return self.app( environ, start_response )
            ...
            message = """
                ...
                <p>Contact your local Galaxy agent shed administrator.
            """
            return self.error( start_response, title, message )

    def error( self, start_response, title="Access denied", message="Contact your local Galaxy agent shed administrator." ):
