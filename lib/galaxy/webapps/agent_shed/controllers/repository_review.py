...
import agent_shed.grids.repository_review_grids as repository_review_grids
...
from agent_shed.util import review_util
from agent_shed.util import shed_util_common as suc
from agent_shed.util.container_util import STRSEP
...
class RepositoryReviewController( BaseUIController, ratings_util.ItemRatings ):
    ...
    repositories_with_no_agent_tests_grid = repository_review_grids.RepositoriesWithNoAgentTestsGrid()
    ...
    def browse_review( self, trans, **kwd ):
        ...
        return trans.fill_template( '/webapps/agent_shed/repository_review/browse_review.mako',
        ...
        if kwd.get( 'create_component_button', False ):
            if not name or not description:
                ...
                trans.response.send_redirect( web.url_for( controller='repository_review',
                                                           ...
                                                           message=message,
                                                           status=status ) )
        return trans.fill_template( '/webapps/agent_shed/repository_review/create_component.mako',
                                    name=name,
        ...
        if kwd.get( 'edit_component_button', False ):
            ...
            if component.description != new_description:
                ...
                return trans.response.send_redirect( web.url_for( controller='repository_review',
                                                                  ...
                                                                  message=message,
                                                                  status=status ) )
        return trans.fill_template( '/webapps/agent_shed/repository_review/edit_component.mako',
                                    component=component,
                                    message=message,
                                    status=status )
    ...
    def edit_review( self, trans, **kwd ):
        ...
        revision_approved_select_field = grids_util.build_approved_select_field( trans,
                                                                                 ...
                                                                                 for_component=False )
        rev, changeset_revision_label = hg_util.get_rev_label_from_changeset_revision( repo, review.changeset_revision )
        return trans.fill_template( '/webapps/agent_shed/repository_review/edit_review.mako',
                                    repository=repository,
                                    review=review,
                                    changeset_revision_label=changeset_revision_label,
    ...
    def manage_repositories_ready_for_review( self, trans, **kwd ):
        ...
        1) It contains no agents
        2) It contains agents the agents_functionally_correct flag is set to True.  This implies that the repository metadata revision was installed and tested
           by the Agent Shed's install and test framework.
        """
        if 'operation' in kwd:
            operation = kwd['operation'].lower()
            ...
            if operation == "view_or_manage_repository":
                return trans.response.send_redirect( web.url_for( controller='repository_review',
                                                                  action='view_or_manage_repository',
                                                                  **kwd ) )
        message = 'Any of these repositories that contain agents have been installed into Galaxy and proven to be functionally correct by executing the tests defined '
        message += 'for each agent.  Repositories that do not contain agents have not been installed into Galaxy. '
        kwd[ 'message' ] = message
        kwd[ 'status' ] = 'warning'
    ...
    def manage_repositories_with_invalid_tests( self, trans, **kwd ):
        """
        Display a list of repositories that contain agents, have not yet been reviewed, and have invalid functional tests.  Tests are defined as
        invalid if they are missing from the agent config or if defined test data is not included in the repository.
        """
        if 'operation' in kwd:
            operation = kwd['operation'].lower()
            if operation == "inspect repository revisions":
            ...
            if operation == "view_or_manage_repository":
                return trans.response.send_redirect( web.url_for( controller='repository_review',
                                                                  action='view_or_manage_repository',
                                                                  **kwd ) )
        message = 'These repositories contain agents with missing functional tests or test data.  '
        kwd[ 'message' ] = message
        kwd[ 'status' ] = 'warning'
        return self.repositories_with_no_agent_tests_grid( trans, **kwd )
    ...
    def manage_repository_reviews( self, trans, mine=False, **kwd ):
        ...
        return trans.fill_template( '/webapps/agent_shed/repository_review/reviews_of_repository.mako',
                                    repository=repository,
    ...
    def manage_repository_reviews_of_revision( self, trans, **kwd ):
        ...
        reviews = review_util.get_reviews_by_repository_id_changeset_revision( trans.app,
                                                                               repository_id,
                                                                               changeset_revision )
        return trans.fill_template( '/webapps/agent_shed/repository_review/reviews_of_changeset_revision.mako',
                                    repository=repository,
                                    changeset_revision=changeset_revision,
                                    changeset_revision_label=changeset_revision_label,
                                    reviews=reviews,
    ...
    def select_previous_review( self, trans, **kwd ):
        ...
        message = escape( kwd.get( 'message', '' ) )
        status = kwd.get( 'status', 'done' )
        repository = suc.get_repository_in_agent_shed( trans.app, kwd[ 'id' ] )
        changeset_revision = kwd.get( 'changeset_revision', None )
        repo = hg_util.get_repo_for_repository( trans.app, repository=repository, repo_path=None, create=False )
        previous_reviews_dict = review_util.get_previous_repository_reviews( trans.app,
                                                                             ...
                                                                             changeset_revision )
        rev, changeset_revision_label = hg_util.get_rev_label_from_changeset_revision( repo, changeset_revision )
        return trans.fill_template( '/webapps/agent_shed/repository_review/select_previous_review.mako',
                                    repository=repository,
                                    changeset_revision=changeset_revision,
                                    changeset_revision_label=changeset_revision_label,
