    ...
    def upload( self, trans, **kwd ):
        ...
        if kwd.get( 'upload_button', False ):
            ...
            if uploaded_file or uploaded_directory:
                ...
                tdtm = data_table_manager.AgentDataTableManager( trans.app )
                ok = True
                isgzip = False
                isbz2 = False
                if uploaded_file:
                ...
                if istar:
                    ...
                    if repository.type == rt_util.REPOSITORY_SUITE_DEFINITION and \
                        ...
                        ok = False
                        message = 'Repositories of type <b>Agent dependency definition</b> can only contain a single file named '
                        message += '<b>agent_dependencies.xml</b>.'
                    if ok:
                        ...
                        if ok:
                            ...
                            if full_path.endswith( 'agent_data_table_conf.xml.sample' ):
                                # Handle the special case where a agent_data_table_conf.xml.sample file is being uploaded
                                # by parsing the file and adding new entries to the in-memory trans.app.agent_data_tables
                                # dictionary.
                                error, error_message = tdtm.handle_sample_agent_data_table_conf_file( full_path, persist=False )
                                if error:
                ...
                if ok:
                    ...
                    if str( repository.type ) not in [ rt_util.REPOSITORY_SUITE_DEFINITION,
                        ...
                        if change_repository_type_message:
                            ...
                            # Provide a warning message if a agent_dependencies.xml file is provided, but agent dependencies
                            # weren't loaded due to a requirement tag mismatch or some other problem.  Agent dependency
                            # definitions can define orphan agent dependencies (no relationship to any agents contained in the
                            ...
                            # provided incorrect information (agent shed, name owner, changeset_revision) for the definition.
                            orphan_message = dd.generate_message_for_orphan_agent_dependencies( repository, metadata_dict )
                            if orphan_message:
                                message += orphan_message
                                status = 'warning'
                    # Handle messaging for invalid agent dependencies.
                    invalid_agent_dependencies_message = dd.generate_message_for_invalid_agent_dependencies( metadata_dict )
                    if invalid_agent_dependencies_message:
                        message += invalid_agent_dependencies_message
                        status = 'error'
                ...
                # Reset the agent_data_tables by loading the empty agent_data_table_conf.xml file.
                tdtm.reset_agent_data_tables()
        return trans.fill_template( '/webapps/agent_shed/repository/upload.mako',
                                    repository=repository,
