...
class Configuration( object ):
    ...
    def __init__( self, **kwargs ):
        ...
        # Repository and Agent search API
        self.agentshed_search_on = string_as_bool( kwargs.get( "agentshed_search_on", True ) )
        self.whoosh_index_dir = kwargs.get( "whoosh_index_dir", 'database/agentshed_whoosh_indexes' )
        ...
        self.agent_name_boost = kwargs.get( "agent_name_boost", 1.2 )
        self.agent_description_boost = kwargs.get( "agent_description_boost", 0.6 )
        self.agent_help_boost = kwargs.get( "agent_help_boost", 0.4 )
        self.agent_repo_owner_username = kwargs.get( "agent_repo_owner_username", 0.3 )
        # Analytics
        ...
        self.agent_data_path = resolve_path( kwargs.get( "agent_data_path", "shed-agent-data" ), os.getcwd() )
        ...
        # Install and test framework for testing agents contained in repositories.
        self.display_legacy_test_results = string_as_bool( kwargs.get( 'display_legacy_test_results', True ) )
        self.num_agent_test_results_saved = kwargs.get( 'num_agent_test_results_saved', 5 )
        self.update_integrated_agent_panel = False
        ...
        # Where the agent shed hgweb.config file is stored - the default is the Galaxy installation directory.
        ...
        self.citation_cache_data_dir = resolve_path( kwargs.get( "citation_cache_data_dir", "database/agent_shed_citations/data" ), self.root )
        self.citation_cache_lock_dir = resolve_path( kwargs.get( "citation_cache_lock_dir", "database/agent_shed_citations/locks" ), self.root )

    @property
    def shed_agent_data_path( self ):
        return self.agent_data_path
    ...
    def sentry_dsn_public( self ):
        ...
        """
        # TODO refactor this to a common place between agentshed/galaxy config, along
    ...
    def check( self ):
        ...
        paths_to_check = [ self.root, self.file_path, self.hgweb_config_dir, self.agent_data_path, self.template_path ]
        ...
        for path in self.file_path, \
            self.template_cache, \
                os.path.join( self.agent_data_path, 'shared', 'jars' ):
            if path not in [ None, False ] and not os.path.isdir( path ):
                try:
                    os.makedirs( path )
                except Exception, e:
