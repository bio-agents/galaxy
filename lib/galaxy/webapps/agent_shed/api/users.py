...
class UsersController( BaseAPIController ):
    """RESTful controller for interactions with users in the Agent Shed."""
...
:       param key: the current Galaxy admin user's API key
        ...
        user_dict = dict( message='',
                          status='ok' )
        # Make sure the current user's API key proves he is an admin user in this Agent Shed.
        if trans.user_is_admin():
            ...
            user_dict[ 'message' ] = 'You are not authorized to create a user in this Agent Shed.'
            user_dict[ 'status' ] = 'error'
        return user_dict
    ...
    def __validate( self, trans, email, password, confirm, username ):
        ...
        if username in [ 'repos' ]:
            return "The term <b>%s</b> is a reserved word in the Agent Shed, so it cannot be used as a public user name." % username
