...
from galaxy.workflow.modules import module_factory, is_agent_module_type, AgentModule
from galaxy.agents.parameters.basic import DataAgentParameter, DataCollectionAgentParameter
from galaxy.agents.parameters import visit_input_values
...
CreatedWorkflow = namedtuple("CreatedWorkflow", ["stored_workflow", "workflow", "missing_agents"])
...
class WorkflowContentsManager(UsesAnnotations):
    ...
    def build_workflow_from_dict(
        ...
        # the local Galaxy instance.  Each tuple in the list of missing_agent_tups
        ...
        for step_dict in self.__walk_step_dicts( data ):
            ...
            if module.type == 'agent' and module.agent is None:
                ...
                if 'content_id' in step_dict:
                    agent_id = step_dict[ 'content_id' ]
                else:
                    # Support legacy workflows... (created pre 16.01)
                    agent_id = step_dict[ 'agent_id' ]
                missing_agent_tup = ( agent_id, step_dict[ 'name' ], step_dict[ 'agent_version' ])
        ...
        return CreatedWorkflow(
            ...
            workflow=workflow,
            missing_agents=missing_agent_tups
        )
    ...
    def update_workflow_from_dict(self, trans, stored_workflow, workflow_data, from_editor=False):
        ...
        for key, step_dict in data['steps'].iteritems():
            is_agent = is_agent_module_type( step_dict[ 'type' ] )
            agent_id = step_dict.get('content_id', step_dict.get('agent_id', None))
            if is_agent and agent_id is None:
                raise exceptions.RequestParameterInvalidException("No agent_id could be located for for step [%s]" % step_dict)
            if is_agent and not trans.app.agentbox.has_agent( agent_id, exact=True ):
                errors.append("Step %s requires agent '%s'." % (step_dict['id'], step_dict['agent_id']))
        if errors:
            raise MissingAgentsException(workflow, errors)
    ...
    def _workflow_to_dict_editor(self, trans, stored):
        ...
        for step in workflow.steps:
            ...
            if not module:
                ...
                invalid_agent_form_html = """<div class="agentForm agent-node-error">
                                            <div class="agentFormTitle form-row-error">Unrecognized Agent: %s</div>
                                            <div class="agentFormBody"><div class="form-row">
                                            The agent id '%s' for this agent is unrecognized.<br/><br/>
                                            To save this workflow, you will need to delete this step or enable the agent.
                                            </div></div></div>""" % (step.agent_id, step.agent_id)
                step_dict = {
                    'id': step.order_index,
                    'type': 'invalid',
                    'content_id': step.content_id,
                    'name': 'Unrecognized Agent: %s' % step.agent_id,
                    'agent_state': None,
                    'agenttip': None,
                    'agent_errors': ["Unrecognized Agent Id: %s" % step.agent_id],
                    'data_inputs': [],
                    'data_outputs': [],
                    'form_html': invalid_agent_form_html,
            ...
            if (hasattr(module, "version_changes")) and (module.version_changes):
                if step.order_index in data['upgrade_messages']:
                    data['upgrade_messages'][step.order_index][module.agent.name] = "\n".join(module.version_changes)
                else:
                    data['upgrade_messages'][step.order_index] = {module.agent.name: "\n".join(module.version_changes)}
            # Get user annotation.
            step_annotation = self.get_item_annotation_obj( trans.sa_session, trans.user, step )
            ...
            if step.type is None or step.type == 'agent':
                ...
                visit_input_values( module.agent.inputs, module.state.inputs, callback )
                # Filter
    ...
    def _workflow_to_dict_export( self, trans, stored=None, workflow=None ):
        ...
        for step in workflow.steps:
            ...
            step_dict = {
                ...
                'agent_id': content_id,  # For worklfows exported to older Galaxies,
                                        # eliminate after a few years...
                'agent_version': step.agent_version,
                'name': module.get_name(),
                'agent_state': module.get_state( secure=False ),
                'agent_errors': module.get_errors(),
                'uuid': str(step.uuid),
                'label': step.label or None,
                # 'data_inputs': module.get_data_inputs(),
            ...
            if module.type == 'subworkflow':
                ...
                del step_dict['agent_version']
                del step_dict['agent_state']
                del step_dict['agent_errors']
            ...
            # agents. This should be removed at some point. Mirrored
    ...
    def _workflow_to_dict_instance(self, stored, legacy=True):
        ...
        for step in workflow.input_steps:
            step_type = step.type
            if step.agent_inputs and "name" in step.agent_inputs:
                label = step.agent_inputs['name']
        ...
        for step in workflow.steps:
            ...
            if step_type == 'subworkflow':
                del step_dict['agent_id']
                del step_dict['agent_version']
                del step_dict['agent_inputs']
...
class MissingAgentsException(object):
