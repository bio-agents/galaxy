...
class _UnflattenedMetadataDatasetAssociationSerializer( base.ModelSerializer,
    ...
    def add_serializers( self ):
        ...
        self.serializable_keyset.update([ 'name', 'state', 'agent_version', 'extension', 'visible', 'dbkey' ])
    ...
    def serialize_rerunnable( self, dataset, key, **context ):
        """
        Return False if this agent that created this dataset can't be re-run
        (e.g. upload).
        """
        if dataset.creating_job:
            agent = self.app.agentbox.get_agent( dataset.creating_job.agent_id, dataset.creating_job.agent_version )
            if agent and agent.is_workflow_compatible:
                return True
        return False
