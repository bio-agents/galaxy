...
class CitationsManager( object ):
    ...
    def citations_for_agent( self, agent ):
        return agent.citations

    def citations_for_agent_ids( self, agent_ids ):
        citation_collection = CitationCollection()
        for agent_id in agent_ids:
            agent = self._get_agent( agent_id )
            for citation in self.citations_for_agent( agent ):
                citation_collection.add( citation )
        return citation_collection.citations

    def parse_citation( self, citation_elem, agent_directory ):
        return parse_citation( citation_elem, agent_directory, self )

    def _get_agent( self, agent_id ):
        agent = self.app.agentbox.get_agent( agent_id )
        return agent
...
class DoiCache( object ):
    ...
    def get_bibtex( self, doi ):
        createfunc = funcagents.partial(self._raw_get_bibtex, doi)
