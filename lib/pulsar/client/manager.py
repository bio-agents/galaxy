...
class MessageQueueClientManager(object):
    ...
    def ensure_has_status_update_callback(self, callback):
        with self.callback_lock:
            ...
            callback_wrapper = funcagents.partial(self.callback_wrapper, callback)
            run = funcagents.partial(self.callback_consumer, callback_wrapper)
    ...
    def ensure_has_ack_consumers(self):
        with self.callback_lock:
            for name in ('setup', 'kill'):
                ...
                run = funcagents.partial(self.ack_consumer, name)
                thread = threading.Thread(
