...
A Galaxy agent can be configured to be executed remotely via Pulsar by
adding a line to the ``galaxy.ini`` file under the ``galaxy:agent_runners``
section with the format::

    <agent_id> = pulsar://http://<pulsar_host>:<pulsar_port>

As an example, if a host named remotehost is running the Pulsar server
application on port ``8913``, then the agent with id ``test_agent`` can
be configured to run remotely on remotehost by adding the following
line to ``galaxy.ini``::

    test_agent = pulsar://http://remotehost:8913

Remember this must be added after the ``[galaxy:agent_runners]`` header
