...
class FileStager(object):
    ...
    **Parameters**
    ...
    def __init__(self, client, client_job_description, job_config):
        ...
        if client_job_description.agent is not None:
            self.agent_id = client_job_description.agent.id
            self.agent_version = client_job_description.agent.version
            self.agent_dir = abspath(client_job_description.agent.agent_dir)
        else:
            self.agent_id = None
        ...
        self.__initialize_referenced_agent_files()
        if self.rewrite_paths:
            self.__initialize_referenced_arbitrary_files()

        self.__upload_agent_files()
    ...
    def __handle_setup(self, job_config):
        if not job_config:
            job_config = self.client.setup(self.agent_id, self.agent_version)
    ...
    def __initialize_referenced_agent_files(self):
        self.referenced_agent_files = self.job_inputs.find_referenced_subfiles(self.agent_dir)
    ...
    def __upload_agent_files(self):
        for referenced_agent_file in self.referenced_agent_files:
            self.transfer_tracker.handle_transfer(referenced_agent_file, path_type.TOOL)
...
class TransferTracker(object):
    ...
    def handle_transfer(self, path, type, name=None, contents=None):
        ...
        if action.staging_needed:
            ...
            register = self.rewrite_paths or type == 'agent'  # Even if inputs not rewritten, agent must be.
            if register:
...
def _read(path):
    """
    Utility method to quickly read small files (config files and agent
