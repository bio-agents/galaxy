...
class ClientJobDescription(object):
    ...
    **Parameters**
    ...
    agent_dir : str
        Directory containing agent to execute (if a wrapper is used, it will
        be transferred to remote server).
    working_directory : str
        Local path created by Galaxy for running this job.
    dependencies_description : list
        galaxy.agents.deps.dependencies.DependencyDescription object describing
        agent dependency context for remote depenency resolution.
    env: list
    ...
    arbitrary_files : dict()
        Additional non-input, non-agent, non-config, non-working directory files
    ...
    def agent_dependencies(self):
        if not self.remote_dependency_resolution:
            return None
        return dict(
            requirements=(self.agent.requirements or []),
            installed_agent_dependencies=(self.agent.installed_agent_dependencies or [])
        )
