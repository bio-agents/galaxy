...
class BaseJobClient(object):
    ...
    def setup(self, agent_id=None, agent_version=None):
        """
        Setup remote Pulsar server to run this job.
        """
        setup_args = {"job_id": self.job_id}
        if agent_id:
            setup_args["agent_id"] = agent_id
        if agent_version:
            setup_args["agent_version"] = agent_version
...
def _setup_params_from_job_config(job_config):
    ...
    agent_id = job_config.get("agent_id", None)
    agent_version = job_config.get("agent_version", None)
    return dict(
        job_id=job_id,
        agent_id=agent_id,
