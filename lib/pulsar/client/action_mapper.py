...
path_type = Bunch(
    ...
    CONFIG="config",
    # Files from agent's agent_dir (for now just wrapper if available).
    TOOL="agent",
    ...
    # Other fixed agent parameter paths (likely coming from agent data, but not
...
class FileActionMapper(object):
    ...
    >>> action = mapper.action('/opt/galaxy/agents/filters/catWrapper.py', 'input')
