...
from agent_shed.dependencies.repository.relation_builder import RelationBuilder
from agent_shed.dependencies import attribute_handlers
from agent_shed.galaxy_install.repository_dependencies.repository_dependency_manager import RepositoryDependencyInstallManager
...
class ExportRepositoryManager( object ):
    ...
    def __init__( self, app, user, agent_shed_url, repository, changeset_revision, export_repository_dependencies, using_api ):
        self.app = app
        ...
        self.agent_shed_url = agent_shed_url
        self.user = user
    ...
    def export_repository( self ):
        ...
        try:
            ...
            for repository_id, ordered_repository, ordered_changeset_revision in zip( ordered_repository_ids,
                                                                                      ordered_repositories,
                                                                                      ordered_changeset_revisions ):
                work_dir = tempfile.mkdtemp( prefix="tmp-agentshed-export-er" )
        ...
        if self.using_api:
            encoded_repositories_archive_name = encoding_util.agent_shed_encode( repositories_archive_filename )
            ...
            download_url = common_util.url_join( agent_shed_url, pathspec=pathspec, params=params )
    ...
    def generate_export_elem( self ):
        ...
        sub_elements[ 'agent_shed' ] = str( self.agent_shed_url.rstrip( '/' ) )
    ...
    def generate_repository_archive( self, repository, changeset_revision, work_dir ):
        ...
        tdah = attribute_handlers.AgentDependencyAttributeHandler( self.app, unpopulate=True )
            ...
            if root.find( '.hg' ) < 0 and root.find( 'hgrc' ) < 0:
                ...
                for name in files:
                    ...
                    if name == rt_util.REPOSITORY_DEPENDENCY_DEFINITION_FILENAME:
                        # Eliminate the agentshed, and changeset_revision attributes from all <repository> tags.
    ...
    def generate_repository_archive_filename( self, use_tmp_archive_dir=False ):
        agent_shed = self.remove_protocol_from_agent_shed_url()
        ...
        if use_tmp_archive_dir:
            tmp_archive_dir = tempfile.mkdtemp( prefix="tmp-agentshed-arcdir" )
    ...
    def get_components_from_repo_info_dict( self, repo_info_dict ):
        ...
        for repository_name, repo_info_tup in repo_info_dict.items():
            ...
            description, repository_clone_url, changeset_revision, ctx_rev, \
                repository_owner, repository_dependencies, agent_dependencies = \
    ...
    def get_repo_info_dicts( self ):
        ...
        repository = suc.get_repository_in_agent_shed( self.app, self.repository_id )
        ...
        rb = RelationBuilder( self.app, repository, repository_metadata, agentshed_base_url )
        ...
        repo_info_dict[ str( repository.name ) ] = ( str( self.repository.description ),
                                                     common_util.generate_clone_url_for_repository_in_agent_shed( self.user,
                                                                                                                 self.repository ),
                                                     ...
                                                     None )
        all_required_repo_info_dict = rdim.get_required_repo_info_dicts( self.agent_shed_url, [ repo_info_dict ] )
    ...
    def get_repository_attributes_and_sub_elements( self, repository, archive_name ):
        ...
        # We have to associate the public username since the user_id will be different between agent sheds.
    ...
    def remove_protocol_from_agent_shed_url( self ):
        protocol, base = self.agent_shed_url.split( '://' )
...
class ImportRepositoryManager( object ):
    ...
    def create_repository_and_import_archive( self, repository_archive_dict, import_results_tups ):
        """
        Create a new repository in the agent shed and populate it with the contents of a gzip compressed
        ...
        if name is None or username is None:
            ...
            if status is None:
                # The repository does not yet exist in this Agent Shed and the current user is authorized to import
                ...
                if user is None:
                    ok = False
                    results_message += 'Import failed: repository owner <b>%s</b> does not have an account in this Agent Shed.' % \
                        str( username )
                    import_results_tups.append( ( ok, ( str( name ), str( username ) ), results_message ) )
                else:
                    user_id = user.id
                    # The categories entry in the repository_archive_dict is a list of category names.  If a name does not
                    # exist in the current Agent Shed, the category will not be created, so it will not be associated with
                    # the repository.
                    category_ids = []
                    ...
                    for category_name in category_names:
                        ...
                        if category is None:
                            results_message += 'This Agent Shed does not have the category <b>%s</b> so it ' % str( category_name )
                ...
                # The repository either already exists in this Agent Shed or the current user is not authorized to create it.
                ok = True
                results_message += 'Import not necessary: repository status for this Agent Shed is: %s.' % str( status )
                import_results_tups.append( ( ok, ( str( name ), str( username ) ), results_message ) )
    ...
    def extract_capsule_files( self, **kwd ):
        ...
        if tar_archive is not None and capsule_file_name is not None:
            ...
            return_dict[ 'encoded_file_path' ] = encoding_util.agent_shed_encode( file_path )
    ...
    def get_archives_from_manifest( self, manifest_file_path ):
        ...
        for elem in manifest_root:
            # <repository name="package_lapack_3_4" type="agent_dependency_definition" username="test">
    ...
    def get_dependent_downloadable_revisions( self, repository_metadata ):
        ...
        """
        # This method is called only from the agent shed.
        sa_session = self.app.model.context.current
    ...
    def get_export_info_dict( self, export_info_file_path ):
        ...
        for elem in export_info_root:
            ...
            elif elem.tag == 'agent_shed':
                export_info_dict[ 'agent_shed' ] = elem.text
    ...
    def get_repository_status_from_agent_shed( self, repository_info_dicts ):
        """
        For each exported repository archive contained in the capsule, inspect the Agent Shed to
    ...
    def import_repository_archive( self, repository, repository_archive_dict ):
        ...
        tdah = attribute_handlers.AgentDependencyAttributeHandler( self.app, unpopulate=False )
        ...
        if ok:
            ...
            for filename in filenames_in_archive:
                ...
                if os.path.split( uploaded_file_name )[ -1 ] == rt_util.REPOSITORY_DEPENDENCY_DEFINITION_FILENAME:
                    # Inspect the contents of the file to see if agentshed or changeset_revision attributes
                    ...
                    # Inspect the contents of the file to see if agentshed or changeset_revision
            ...
            # Send email notification to those that have registered to receive alerts for new repositories in this Agent Shed.
            new_repo_alert = True
