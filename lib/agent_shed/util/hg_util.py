...
def get_file_context_from_ctx( ctx, filename ):
    ...
    # ['tmap_wrapper_0.0.19/agent_data_table_conf.xml.sample', 'tmap_wrapper_0.3.3/agent_data_table_conf.xml.sample'].
    ...
    for ctx_file in ctx.files():
        ...
        if filename == ctx_file_name:
            try:
                ...
                continue
            if fctx:
                fh = tempfile.NamedTemporaryFile( 'wb', prefix="tmp-agentshed-gntfc", dir=dir )
...
def unpack_chunks( hg_unbundle10_obj ):
    ...
    is created when a changeset that is pushed to a Agent Shed repository using hg push from the command line
...
def unpack_groups( hg_unbundle10_obj ):
    ...
    created when a changeset that is pushed to a Agent Shed repository using hg push from the command
