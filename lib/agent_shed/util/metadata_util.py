...
from agent_shed.util import common_util, hg_util, shed_util_common as suc
...
def get_repository_dependency_tups_from_repository_metadata( app, repository_metadata, deprecated_only=False ):
    ...
    list defines the entire repository dependency tree.  This method is called only from the Agent Shed.
    """
    dependency_tups = []
        ...
        if metadata:
            ...
            if repository_dependencies_dict is not None:
                ...
                if repository_dependency_tups is not None:
                    ...
                    # ['http://localhost:9009', 'package_samagents_0_1_18', 'devteam', 'ef37fc635cb9', 'False', 'False']
                    for repository_dependency_tup in repository_dependency_tups:
                        agentshed, name, owner, changeset_revision, pir, oicct = \
...
def get_repository_metadata_by_repository_id_changeset_revision( app, id, changeset_revision, metadata_only=False ):
    """Get a specified metadata record for a specified repository in the agent shed."""
    if metadata_only:
...
def is_downloadable( metadata_dict ):
    ...
    if 'agent_dependencies' in metadata_dict:
        # We have agent dependencies, and perhaps only agent dependencies!
        return True
