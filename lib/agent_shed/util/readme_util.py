...
from agent_shed.util import basic_util, common_util, hg_util

log = logging.getLogger( __name__ )


def build_readme_files_dict( app, repository, changeset_revision, metadata, agent_path=None ):
    """
    ...
    file contents may not be available on disk.  This method is used by both Galaxy and the Agent Shed.
    """
    if app.name == 'galaxy':
        ...
        if 'readme_files' in metadata:
            for relative_path_to_readme_file in metadata[ 'readme_files' ]:
                ...
                if can_use_disk_files:
                    if agent_path:
                        full_path_to_readme_file = os.path.abspath( os.path.join( agent_path, relative_path_to_readme_file ) )
                    else:
                    ...
                    if text:
                        ...
                        if text_of_reasonable_length.find( '.. image:: ' ) >= 0:
                            # Handle image display for README files that are contained in repositories in the agent shed or installed into Galaxy.
                            lock = threading.Lock()
                            lock.acquire( True )
                            try:
                    ...
                    # We must be in the agent shed and have an old changeset_revision, so we need to retrieve the file contents from the repository manifest.
...
def get_readme_files_dict_for_display( app, agent_shed_url, repo_info_dict ):
    """
    Return a dictionary of README files contained in the single repository being installed so they can be displayed on the agent panel section
    selection page.
    """
