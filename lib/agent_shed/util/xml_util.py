...
def create_and_write_tmp_file( elems, use_indent=False ):
    ...
    fh = tempfile.NamedTemporaryFile( 'wb', prefix="tmp-agentshed-cawrf"  )
...
def indent( elem, level=0 ):
    ...
    shed agent config (e.g., shed_agent_conf.xml from the in-memory list of config_elems because each config_elem
...
def xml_to_string( elem, encoding='utf-8', use_indent=False, level=0 ):
    if elem is not None:
        if use_indent:
            # We were called from AgentPanelManager.config_elems_to_xml_file(), so
            # set the level to 1 since level 0 is the <agentbox> tag set.
