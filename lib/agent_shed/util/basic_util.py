...
RUN install-repository "--url ${agent_shed_url} -o ${repository_owner} --name ${repository_name}"
'''
...
def evaluate_template( text, install_environment ):
    ...
    agent dependency.  The value of the received install_dir is the root installation directory of
    the agent_dependency.
    """
...
def get_env_var_values( install_environment ):
    ...
    install_environment.agent_shed_repository_install_dir is the root installation directory of the repository
    that contains the agent dependency being installed, install_environment.install_dir is the root
    installation directory of the agent dependency, and install_environment.tmp_work_dir is the
    temporary directory where the agent dependency compilation/installation is being processed.
    """
    env_var_dict = {}
    env_var_dict[ 'REPOSITORY_INSTALL_DIR' ] = install_environment.agent_shed_repository_install_dir
