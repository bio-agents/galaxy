...
UNDESIRABLE_FILES = [ '.hg_archival.txt', 'hgrc', '.DS_Store', 'agent_test_output.html', 'agent_test_output.json' ]
...
def check_archive( repository, archive ):
    for member in archive.getmembers():
        ...
        if repository.type == rt_util.TOOL_DEPENDENCY_DEFINITION and member.name != rt_util.TOOL_DEPENDENCY_DEFINITION_FILENAME:
            message = 'Repositories of type <b>Agent dependency definition</b> can contain only a single file named <b>agent_dependencies.xml</b>.'
            message += 'This archive contains a member named %s.' % str( member.name )
            return False, message
    return True, ''
...
def get_change_lines_in_file_for_tag( tag, change_dict ):
    ...
    changeset being pushed to the Agent Shed from the command line. This method cleans
...
def handle_directory_changes( app, host, username, repository, full_path, filenames_in_archive, remove_repo_files_not_in_tar,
    ...
    for filename_in_archive in filenames_in_archive:
        ...
        if filename_in_archive.endswith( 'agent_data_table_conf.xml.sample' ):
            # Handle the special case where a agent_data_table_conf.xml.sample file is being uploaded
            # by parsing the file and adding new entries to the in-memory app.agent_data_tables
            # dictionary.
            tdtm = data_table_manager.AgentDataTableManager( app )
            error, message = tdtm.handle_sample_agent_data_table_conf_file( filename_in_archive, persist=False )
            if error:
