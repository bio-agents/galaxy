...
def build_agent_dependencies_select_field( app, agent_shed_repository, name, multiple=True, display='checkboxes',
                                          uninstalled_only=False ):
    """
    Generate a SelectField consisting of the current list of agent dependency ids
    for an installed agent shed repository.
    """
    agent_dependencies_select_field = SelectField( name=name, multiple=multiple, display=display )
    for agent_dependency in agent_shed_repository.agent_dependencies:
        if uninstalled_only:
            if agent_dependency.status not in [ app.install_model.AgentDependency.installation_status.NEVER_INSTALLED,
                                               app.install_model.AgentDependency.installation_status.UNINSTALLED ]:
                continue
        else:
            if agent_dependency.status in [ app.install_model.AgentDependency.installation_status.NEVER_INSTALLED,
                                           app.install_model.AgentDependency.installation_status.UNINSTALLED ]:
                continue
        option_label = '%s version %s' % ( str( agent_dependency.name ), str( agent_dependency.version ) )
        option_value = app.security.encode_id( agent_dependency.id )
        agent_dependencies_select_field.add_option( option_label, option_value )
    return agent_dependencies_select_field


def create_or_update_agent_dependency( app, agent_shed_repository, name, version, type, status, set_status=True ):
    """Create or update a agent_dependency record in the Galaxy database."""
    # Called from Galaxy (never the agent shed) when a new repository is being installed or when an uninstalled
    # repository is being reinstalled.
    context = app.install_model.context
    # First see if an appropriate agent_dependency record exists for the received agent_shed_repository.
    if version:
        agent_dependency = get_agent_dependency_by_name_version_type_repository( app, agent_shed_repository, name, version, type )
    else:
        agent_dependency = get_agent_dependency_by_name_type_repository( app, agent_shed_repository, name, type )
    if agent_dependency:
        # In some cases we should not override the current status of an existing agent_dependency, so do so only
        # if set_status is True.
        if set_status:
            set_agent_dependency_attributes(app, agent_dependency=agent_dependency, status=status)
    else:
        # Create a new agent_dependency record for the agent_shed_repository.
        debug_msg = 'Creating a new record for version %s of agent dependency %s for revision %s of repository %s.  ' % \
            ( str( version ), str( name ), str( agent_shed_repository.changeset_revision ), str( agent_shed_repository.name ) )
        debug_msg += 'The status is being set to %s.' % str( status )
        log.debug( debug_msg )
        agent_dependency = app.install_model.AgentDependency( agent_shed_repository.id, name, version, type, status )
        context.add( agent_dependency )
        context.flush()
    return agent_dependency


def create_agent_dependency_objects( app, agent_shed_repository, relative_install_dir, set_status=True ):
    """
    Create or update a AgentDependency for each entry in agent_dependencies_config.  This method is called when
    installing a new agent_shed_repository.
    """
    agent_dependency_objects = []
    shed_config_dict = agent_shed_repository.get_shed_config_dict( app )
    if shed_config_dict.get( 'agent_path' ):
        relative_install_dir = os.path.join( shed_config_dict.get( 'agent_path' ), relative_install_dir )
    # Get the agent_dependencies.xml file from the repository.
    agent_dependencies_config = hg_util.get_config_from_disk( 'agent_dependencies.xml', relative_install_dir )
    tree, error_message = xml_util.parse_xml( agent_dependencies_config )
    if tree is None:
        return agent_dependency_objects
    root = tree.getroot()
    for elem in root:
        agent_dependency_type = elem.tag
        if agent_dependency_type == 'package':
            name = elem.get( 'name', None )
            version = elem.get( 'version', None )
            if name and version:
                status = app.install_model.AgentDependency.installation_status.NEVER_INSTALLED
                agent_dependency = create_or_update_agent_dependency( app,
                                                                    agent_shed_repository,
                                                                    name=name,
                                                                    version=version,
                                                                    type=agent_dependency_type,
                                                                    status=status,
                                                                    set_status=set_status )
                agent_dependency_objects.append( agent_dependency )
        elif agent_dependency_type == 'set_environment':
...
def get_agent_dependency( app, id ):
    """Get a agent_dependency from the database via id"""
    return app.install_model.context.query( app.install_model.AgentDependency ).get( app.security.decode_id( id ) )


def get_agent_dependency_by_name_type_repository( app, repository, name, type ):
    context = app.install_model.context
    return context.query( app.install_model.AgentDependency ) \
                  .filter( and_( app.install_model.AgentDependency.table.c.agent_shed_repository_id == repository.id,
                                 app.install_model.AgentDependency.table.c.name == name,
                                 app.install_model.AgentDependency.table.c.type == type ) ) \
                  .first()


def get_agent_dependency_by_name_version_type( app, name, version, type ):
    context = app.install_model.context
    return context.query( app.install_model.AgentDependency ) \
                  .filter( and_( app.install_model.AgentDependency.table.c.name == name,
                                 app.install_model.AgentDependency.table.c.version == version,
                                 app.install_model.AgentDependency.table.c.type == type ) ) \
                  .first()


def get_agent_dependency_by_name_version_type_repository( app, repository, name, version, type ):
    context = app.install_model.context
    return context.query( app.install_model.AgentDependency ) \
                  .filter( and_( app.install_model.AgentDependency.table.c.agent_shed_repository_id == repository.id,
                                 app.install_model.AgentDependency.table.c.name == name,
...
def get_agent_dependency_ids( as_string=False, **kwd ):
    agent_dependency_id = kwd.get( 'agent_dependency_id', None )
    if 'agent_dependency_ids' in kwd:
        agent_dependency_ids = util.listify( kwd[ 'agent_dependency_ids' ] )
    elif 'id' in kwd:
        agent_dependency_ids = util.listify( kwd[ 'id' ] )
    elif 'inst_td_ids' in kwd:
        agent_dependency_ids = util.listify( kwd[ 'inst_td_ids' ] )
    elif 'uninstalled_agent_dependency_ids' in kwd:
        agent_dependency_ids = util.listify( kwd[ 'uninstalled_agent_dependency_ids' ] )
    else:
        agent_dependency_ids = []
    if agent_dependency_id and agent_dependency_id not in agent_dependency_ids:
        agent_dependency_ids.append( agent_dependency_id )
    if as_string:
        return ','.join( agent_dependency_ids )
    return agent_dependency_ids


def get_agent_dependency_install_dir( app, repository_name, repository_owner, repository_changeset_revision, agent_dependency_type,
                                     agent_dependency_name, agent_dependency_version ):
    if agent_dependency_type == 'package':
        return os.path.abspath( os.path.join( app.config.agent_dependency_dir,
                                              agent_dependency_name,
                                              agent_dependency_version,
                                              repository_owner,
                                              repository_name,
                                              repository_changeset_revision ) )
    if agent_dependency_type == 'set_environment':
        return os.path.abspath( os.path.join( app.config.agent_dependency_dir,
...
def parse_package_elem( package_elem, platform_info_dict=None, include_after_install_actions=True ):
    """
    Parse a <package> element within a agent dependency definition and return a list of action tuples.
    This method is called when setting metadata on a repository that includes a agent_dependencies.xml
    file or when installing a repository that includes a agent_dependencies.xml file.  If installing,
    ...
    # tag set) to be processed in the order they are defined in the agent_dependencies.xml file.
    actions_elem_tuples = []
...
def remove_agent_dependency( app, agent_dependency ):
    """The received agent_dependency must be in an error state."""
    context = app.install_model.context
    dependency_install_dir = agent_dependency.installation_directory( app )
    removed, error_message = remove_agent_dependency_installation_directory( dependency_install_dir )
    if removed:
        agent_dependency.status = app.install_model.AgentDependency.installation_status.UNINSTALLED
        agent_dependency.error_message = None
        context.add( agent_dependency )
        context.flush()
        # Since the received agent_dependency is in an error state, nothing will need to be changed in any
...
def remove_agent_dependency_installation_directory( dependency_install_dir ):
    if os.path.exists( dependency_install_dir ):
        try:
            shutil.rmtree( dependency_install_dir )
            removed = True
            error_message = ''
            log.debug( "Removed agent dependency installation directory: %s" % str( dependency_install_dir ) )
        except Exception, e:
            removed = False
            error_message = "Error removing agent dependency installation directory %s: %s" % ( str( dependency_install_dir ), str( e ) )
...
def set_agent_dependency_attributes( app, agent_dependency, status, error_message=None ):
    sa_session = app.install_model.context
    if status == app.install_model.AgentDependency.installation_status.UNINSTALLED:
        installation_directory = agent_dependency.installation_directory( app )
        remove_agent_dependency_installation_directory( installation_directory )
    agent_dependency.error_message = error_message
    if str( agent_dependency.status ) != str( status ):
        agent_shed_repository = agent_dependency.agent_shed_repository
        debug_msg = 'Updating an existing record for version %s of agent dependency %s for revision %s of repository %s ' % \
            ( str( agent_dependency.version ), str( agent_dependency.name ), str( agent_shed_repository.changeset_revision ), str( agent_shed_repository.name ) )
        debug_msg += 'by updating the status from %s to %s.' % ( str( agent_dependency.status ), str( status ) )
        log.debug( debug_msg )
    agent_dependency.status = status
