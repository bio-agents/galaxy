...
TOOL_DEPENDENCY_DEFINITION = 'agent_dependency_definition'
TOOL_DEPENDENCY_DEFINITION_FILENAME = 'agent_dependencies.xml'
...
def build_repository_type_select_field( trans, repository=None, name='repository_type' ):
    """Called from the Agent Shed to generate the current list of supported repository types."""
    if repository:
...
def generate_message_for_repository_type_change( app, repository ):
    ...
    if repository.can_change_type_to( app, REPOSITORY_SUITE_DEFINITION ):
        ...
        agent_dependency_definition_type_class = \
        ...
        message += "not be added to this repository, consider setting its type to <b>%s</b>.<br/>" % \
            agent_dependency_definition_type_class.label
    return message
