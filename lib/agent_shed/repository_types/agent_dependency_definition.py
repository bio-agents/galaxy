...
class AgentDependencyDefinition( TipOnly ):

    def __init__( self ):
        self.type = rt_util.TOOL_DEPENDENCY_DEFINITION
        self.label = 'Agent dependency definition'
        self.valid_file_names = [ 'agent_dependencies.xml' ]
    ...
    def is_valid_for_type( self, app, repository, revisions_to_check=None ):
        ...
        for changeset in changeset_revisions:
            ...
            # Inspect all files in the changeset (in sorted order) to make sure there is only one and it is named agent_dependencies.xml.
