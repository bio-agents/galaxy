...
from agent_shed.util import hg_util, metadata_util, shed_util_common as suc
...
def filter_by_latest_downloadable_changeset_revision_that_has_failing_agent_tests( trans, repository ):
    """
    Inspect the latest downloadable changeset revision for the received repository to see if it
    includes at least 1 agent that has at least 1 failing test.  This will filter out repositories
    of type repository_suite_definition and agent_dependency_definition.
    """
    repository_metadata = get_latest_downloadable_repository_metadata_if_it_includes_agents( trans, repository )
    if repository_metadata is not None \
            and has_been_tested( repository_metadata ) \
            and not repository_metadata.missing_test_components \
            and not repository_metadata.agents_functionally_correct \
            and not repository_metadata.test_install_error:
        return repository_metadata.changeset_revision
    return None


def filter_by_latest_downloadable_changeset_revision_that_has_missing_agent_test_components( trans, repository ):
    """
    Inspect the latest downloadable changeset revision for the received repository to see if it
    includes agents that are either missing functional tests or functional test data.  If the
    changset revision includes agents but is missing agent test components, return the changeset
...
def filter_by_latest_downloadable_changeset_revision_that_has_no_failing_agent_tests( trans, repository ):
    """
    Inspect the latest downloadable changeset revision for the received repository to see if it
    includes agents with no failing tests.  This will filter out repositories of type repository_suite_definition
    and agent_dependency_definition.
    """
    repository_metadata = get_latest_downloadable_repository_metadata_if_it_includes_agents( trans, repository )
    if repository_metadata is not None \
            and has_been_tested( repository_metadata ) \
            and not repository_metadata.missing_test_components \
            and repository_metadata.agents_functionally_correct:
        return repository_metadata.changeset_revision
    return None


def filter_by_latest_metadata_changeset_revision_that_has_invalid_agents( trans, repository ):
    """
    Inspect the latest changeset revision with associated metadata for the received repository
    to see if it has invalid agents.  This will filter out repositories of type repository_suite_definition
    and agent_dependency_definition.
    """
    repository_metadata = get_latest_repository_metadata_if_it_includes_invalid_agents( trans, repository )
    if repository_metadata is not None:
        return repository_metadata.changeset_revision
    return None
...
def filter_by_latest_downloadable_changeset_revision_that_has_test_install_errors( trans, repository ):
    ...
    it has agent test installation errors.  This will return repositories of type unrestricted
    as well as types repository_suite_definition and agent_dependency_definition.
    """
...
def filter_by_latest_downloadable_changeset_revision_with_skip_tests_checked( trans, repository ):
    ...
    # The skip_agent_tests attribute is a SkipAgentTest table mapping backref to the RepositoryMetadata table.
    if repository_metadata is not None and repository_metadata.skip_agent_tests:
        return repository_metadata.changeset_revision
    return None
...
def get_latest_downloadable_repository_metadata_if_it_includes_agents( trans, repository ):
    """
    Return the latest downloadable repository_metadata record for the received repository if its
    includes_agents attribute is True.  This will filter out repositories of type repository_suite_definition
    and agent_dependency_definition.
    """
    repository_metadata = get_latest_downloadable_repository_metadata( trans, repository )
    if repository_metadata is not None and repository_metadata.includes_agents:
        return repository_metadata
    return None
...
def get_latest_downloadable_repository_metadata_if_it_has_test_install_errors( trans, repository ):
    ...
    well as types repository_suite_definition and agent_dependency_definition.
    """
    repository_metadata = get_latest_downloadable_repository_metadata( trans, repository )
    if repository_metadata is not None \
            and has_been_tested( repository_metadata ) \
...
def get_latest_repository_metadata_if_it_includes_invalid_agents( trans, repository ):
    """
    Return the latest repository_metadata record for the received repository that contains invalid
    agents if one exists.  This will filter out repositories of type repository_suite_definition and
    agent_dependency_definition.
    """
    repository_metadata = get_latest_repository_metadata( trans, repository )
    if repository_metadata is not None:
        metadata = repository_metadata.metadata
        if metadata is not None and 'invalid_agents' in metadata:
            return repository_metadata
    return None
...
def has_been_tested( repository_metadata ):
    """
    Return True if the received repository_metadata record'd agent_test_results column was populated by
    the Agent Shed's install and test framework.
    """
    agent_test_results = repository_metadata.agent_test_results
    if agent_test_results is None:
        return False
    # The install and test framework's preparation scripts will populate the agent_test_results column
    # with something like this:
    # [{"test_environment":
    #    {"time_tested": "2014-05-15 16:15:18",
    #     "agent_shed_database_version": 22,
    #     "agent_shed_mercurial_version": "2.2.3",
    #     "agent_shed_revision": "13459:9a1415f8108f"}
    # }]
    agent_test_results = listify( agent_test_results )
    for test_results_dict in agent_test_results:
