...
from agent_shed.util import hg_util, metadata_util
...
class CategoryGrid( grids.Grid ):
    ...
    template = '/webapps/agent_shed/category/grid.mako'
...
class RepositoryGrid( grids.Grid ):
    ...
    class MetadataRevisionColumn( grids.GridColumn ):
        ...
        def get_value( self, trans, grid, repository ):
            ...
            # A repository's metadata revisions may not all be installable, as some may contain only invalid agents.
    ...
    class AgentsFunctionallyCorrectColumn( grids.BooleanColumn ):
            ...
            if repository.type == rt_util.UNRESTRICTED:
                try:
                    if len( repository.metadata_revisions ) > 0:
                        ...
                        if displayed_metadata_revision.includes_agents:
                            if displayed_metadata_revision.agents_functionally_correct:
                                return 'yes'
                            else:
                                return 'no'
                    return 'n/a'
                ...
                try:
                    if len( repository.metadata_revisions ) > 0:
                        ...
                        if displayed_metadata_revision.test_install_error:
                            return 'no'
                        agent_test_results = listify( displayed_metadata_revision.agent_test_results )
                        if len( agent_test_results ) > 0:
                            last_agent_test_result = agent_test_results[ 0 ]
                            installation_error_dict = last_agent_test_result.get( 'installation_errors', {} )
    ...
    title = "Repositories"
    model_class = model.Repository
    template = '/webapps/agent_shed/repository/grid.mako'
    default_sort_key = "name"
    ...
    columns = [
        ...
        AgentsFunctionallyCorrectColumn( "Agents or<br/>Package<br/>Verified" ),
        UserColumn( "Owner",
...
class MyWritableRepositoriesGrid( RepositoryGrid ):
    ...
    columns = [
        ...
        RepositoryGrid.AgentsFunctionallyCorrectColumn( "Agents or<br/>Package<br/>Verified" ),
        RepositoryGrid.UserColumn( "Owner",
                                   model_class=model.User,
                                   link=( lambda item: dict( operation="repositories_by_user", id=item.id ) ),
                                   attach_popup=False,
...
class RepositoriesMissingAgentTestComponentsGrid( RepositoryGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories with missing agent test components"
    columns = [
        RepositoryGrid.NameColumn( "Name",
                                   key="name",
                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
    ...
    def build_initial_query( self, trans, **kwd ):
        # Filter by latest installable revisions that contain agents with missing agent test components.
        revision_clause_list = []
        for repository in trans.sa_session.query( model.Repository ) \
                                          .filter( and_( model.Repository.table.c.deprecated == false(),
                                                         model.Repository.table.c.deleted == false() ) ):
            changeset_revision = \
                grids_util.filter_by_latest_downloadable_changeset_revision_that_has_missing_agent_test_components( trans, repository )
...
class MyWritableRepositoriesMissingAgentTestComponentsGrid( RepositoriesMissingAgentTestComponentsGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories I can change with missing agent test components"
    columns = [ col for col in RepositoriesMissingAgentTestComponentsGrid.columns ]
    operations = []
    use_paging = False
    ...
    def build_initial_query( self, trans, **kwd ):
        ...
        if user_clause_list:
            ...
            # further by latest installable revisions that contain agents with missing agent test components.
            revision_clause_list = []
            for repository in trans.sa_session.query( model.Repository ) \
                                              .filter( and_( model.Repository.table.c.deprecated == false(),
                                                             model.Repository.table.c.deleted == false() ) ) \
...
class RepositoriesWithTestInstallErrorsGrid( RepositoryGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories with agent test installation errors"
    columns = [
        RepositoryGrid.NameColumn( "Name",
                                   key="name",
                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
...
class MyWritableRepositoriesWithTestInstallErrorsGrid( RepositoriesWithTestInstallErrorsGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories I can change with agent test installation errors"
...
class RepositoriesWithSkipTestsCheckedGrid( RepositoryGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories with skip agent tests checked"
    columns = [
        RepositoryGrid.NameColumn( "Name",
                                   key="name",
                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
...
class MyWritableRepositoriesWithSkipTestsCheckedGrid( RepositoriesWithSkipTestsCheckedGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories I can change with skip agent tests checked"
...
class RepositoriesWithFailingAgentTestsGrid( RepositoryGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories with failing agent tests"
    columns = [
        RepositoryGrid.NameColumn( "Name",
                                   key="name",
                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
    ...
    def build_initial_query( self, trans, **kwd ):
        # Filter by latest installable revisions that contain agents with at least 1 failing agent test.
        revision_clause_list = []
        for repository in trans.sa_session.query( model.Repository ) \
                                          .filter( and_( model.Repository.table.c.deprecated == false(),
                                                         model.Repository.table.c.deleted == false() ) ):
            changeset_revision = \
                grids_util.filter_by_latest_downloadable_changeset_revision_that_has_failing_agent_tests( trans, repository )
            if changeset_revision:
                revision_clause_list.append( model.RepositoryMetadata.table.c.changeset_revision == changeset_revision )
        if revision_clause_list:
            return trans.sa_session.query( model.Repository ) \
...
class MyWritableRepositoriesWithFailingAgentTestsGrid( RepositoriesWithFailingAgentTestsGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories I can change with failing agent tests"
    columns = [ col for col in RepositoriesWithFailingAgentTestsGrid.columns ]
    operations = []
    use_paging = False
    ...
    def build_initial_query( self, trans, **kwd ):
        ...
        if user_clause_list:
            # We have the list of repositories that the current user is authorized to update, so filter
            # further by latest installable revisions that contain agents with at least 1 failing agent test.
            revision_clause_list = []
            for repository in trans.sa_session.query( model.Repository ) \
                                              .filter( and_( model.Repository.table.c.deprecated == false(),
                                                             model.Repository.table.c.deleted == false() ) ) \
...
class RepositoriesWithNoFailingAgentTestsGrid( RepositoryGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories with no failing agent tests"
    columns = [
        RepositoryGrid.NameColumn( "Name",
                                   key="name",
                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
    ...
    def build_initial_query( self, trans, **kwd ):
        ...
        for repository in trans.sa_session.query( model.Repository ) \
                                          .filter( and_( model.Repository.table.c.deprecated == false(),
                                                         model.Repository.table.c.deleted == false() ) ):
            changeset_revision = \
                grids_util.filter_by_latest_downloadable_changeset_revision_that_has_no_failing_agent_tests( trans, repository )
            if changeset_revision:
                revision_clause_list.append( model.RepositoryMetadata.table.c.changeset_revision == changeset_revision )
        if revision_clause_list:
            return trans.sa_session.query( model.Repository ) \
...
class MyWritableRepositoriesWithNoFailingAgentTestsGrid( RepositoriesWithNoFailingAgentTestsGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories I can change with no failing agent tests"
    columns = [ col for col in RepositoriesWithNoFailingAgentTestsGrid.columns ]
    operations = []
    use_paging = False
    ...
    def build_initial_query( self, trans, **kwd ):
        ...
        if user_clause_list:
            # We have the list of repositories that the current user is authorized to update, so filter
            # further by latest installable revisions that contain at least 1 agent, no missing agent test
            # components, and no failing agent tests.
            revision_clause_list = []
            for repository in trans.sa_session.query( model.Repository ) \
                                              .filter( and_( model.Repository.table.c.deprecated == false(),
                                                             model.Repository.table.c.deleted == false() ) ) \
...
class RepositoriesWithInvalidAgentsGrid( RepositoryGrid ):
    # This grid displays only the latest installable revision of each repository.

    class InvalidAgentConfigColumn( grids.GridColumn ):

        def __init__( self, col_name ):
            grids.GridColumn.__init__( self, col_name )

        def get_value( self, trans, grid, repository ):
            # At the time this grid is displayed we know that the received repository will have invalid agents in its latest changeset revision
            # that has associated metadata.
            val = ''
            repository_metadata = \
                grids_util.get_latest_repository_metadata_if_it_includes_invalid_agents( trans, repository )
            metadata = repository_metadata.metadata
            invalid_agents = metadata.get( 'invalid_agents', [] )
            if invalid_agents:
                for invalid_agent_config in invalid_agents:
                    href_str = '<a href="load_invalid_agent?repository_id=%s&agent_config=%s&changeset_revision=%s">%s</a>' % \
                        ( trans.security.encode_id( repository.id ), invalid_agent_config, repository_metadata.changeset_revision, invalid_agent_config )
                    val += href_str
                    val += '<br/>'
                val = val.rstrip( '<br/>' )
            return val

    title = "Repositories with invalid agents"
    columns = [
        InvalidAgentConfigColumn( "Agent config" ),
        RepositoryGrid.NameColumn( "Name",
                                   key="name",
                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
                                   attach_popup=False ),
    ...
    def build_initial_query( self, trans, **kwd ):
        # Filter by latest metadata revisions that contain invalid agents.
        revision_clause_list = []
        for repository in trans.sa_session.query( model.Repository ) \
                                          .filter( and_( model.Repository.table.c.deprecated == false(),
                                                         model.Repository.table.c.deleted == false() ) ):
            changeset_revision = \
                grids_util.filter_by_latest_metadata_changeset_revision_that_has_invalid_agents( trans, repository )
            if changeset_revision:
                revision_clause_list.append( model.RepositoryMetadata.table.c.changeset_revision == changeset_revision )
        if revision_clause_list:
            return trans.sa_session.query( model.Repository ) \
...
class MyWritableRepositoriesWithInvalidAgentsGrid( RepositoriesWithInvalidAgentsGrid ):
    # This grid displays only the latest installable revision of each repository.
    title = "Repositories I can change with invalid agents"
    columns = [ col for col in RepositoriesWithInvalidAgentsGrid.columns ]
    operations = []
    use_paging = False
    ...
    def build_initial_query( self, trans, **kwd ):
        ...
        if user_clause_list:
            # We have the list of repositories that the current user is authorized to update, so filter
            # further by latest metadata revisions that contain invalid agents.
            revision_clause_list = []
            for repository in trans.sa_session.query( model.Repository ) \
                                              .filter( and_( model.Repository.table.c.deprecated == false(),
                                                             model.Repository.table.c.deleted == false() ) ) \
...
class RepositoryMetadataGrid( grids.Grid ):
    ...
    class IncludesAgentsColumn( grids.BooleanColumn ):
        def get_value( self, trans, grid, repository_metadata ):
            if repository_metadata.includes_agents:
                return 'yes'
            return ''

    class IncludesAgentDependenciesColumn( grids.BooleanColumn ):
        def get_value( self, trans, grid, repository_metadata ):
            if repository_metadata.includes_agent_dependencies:
                return 'yes'
            return ''
...
class RepositoryDependenciesGrid( RepositoryMetadataGrid ):
    ...
    title = "Valid repository dependency definitions in this agent shed"
    default_sort_key = "Repository.name"
    columns = [
...
class DatatypesGrid( RepositoryMetadataGrid ):
    ...
    title = "Custom datatypes in this agent shed"
    default_sort_key = "Repository.name"
    columns = [
...
class AgentDependenciesGrid( RepositoryMetadataGrid ):

    class AgentDependencyColumn( grids.TextColumn ):
        ...
        def get_value( self, trans, grid, repository_metadata ):
            ...
            if repository_metadata:
                metadata = repository_metadata.metadata
                if metadata:
                    tds_dict = metadata.get( 'agent_dependencies', {} )
                    if tds_dict:
    ...
    title = "Agent dependency definitions in this agent shed"
    default_sort_key = "Repository.name"
    columns = [
        AgentDependencyColumn( "Agent dependency",
                              attach_popup=False ),
        RepositoryMetadataGrid.RepositoryNameColumn( "Repository name",
                                                     model_class=model.Repository,
                                                     link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
    ...
    def build_initial_query( self, trans, **kwd ):
        return trans.sa_session.query( model.RepositoryMetadata ) \
                               .join( model.Repository ) \
                               .filter( and_( model.RepositoryMetadata.table.c.includes_agent_dependencies == true(),
                                              model.Repository.table.c.deleted == false(),
                                              model.Repository.table.c.deprecated == false() ) ) \
                               .join( model.User.table )


class AgentsGrid( RepositoryMetadataGrid ):

    class AgentsColumn( grids.TextColumn ):
        ...
        def get_value( self, trans, grid, repository_metadata ):
            ...
            if repository_metadata:
                metadata = repository_metadata.metadata
                if metadata:
                    agent_dicts = metadata.get( 'agents', [] )
                    if agent_dicts:
                        # Create tuples of the attributes we want so we can sort them by extension.
                        agent_tups = []
                        for agent_dict in agent_dicts:
                            ...
                            # For now we'll just display agent id and version.
                            if agent_id and version:
                                agent_tups.append( ( agent_id, version ) )
                        sorted_agent_tups = sorted( agent_tups, key=lambda agent_tup: agent_tup[ 0 ] )
                        for agent_tup in sorted_agent_tups:
                            agent_id, version = agent_tup[ :2 ]
                            agent_str = '<a href="browse_datatypes?operation=view_or_manage_repository&id=%s">' % trans.security.encode_id( repository_metadata.id )
                            agent_str += '<b>%s:</b> %s' % ( escape_html( agent_id ), escape_html( version ) )
                            agent_str += '</a>'
                            agent_line.append( agent_str )
            return '<br />'.join( agent_line )

    title = "Valid agents in this agent shed"
    default_sort_key = "Repository.name"
    columns = [
        AgentsColumn( "Agent id and version",
                     attach_popup=False ),
        RepositoryMetadataGrid.RepositoryNameColumn( "Repository name",
                                                     model_class=model.Repository,
                                                     link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
    ...
    def build_initial_query( self, trans, **kwd ):
        return trans.sa_session.query( model.RepositoryMetadata ) \
                               .join( model.Repository ) \
                               .filter( and_( model.RepositoryMetadata.table.c.includes_agents == true(),
                                              model.Repository.table.c.deleted == false(),
                                              model.Repository.table.c.deprecated == false() ) ) \
                               .join( model.User.table )
...
class ValidCategoryGrid( CategoryGrid ):
    ...
    model_class = model.Category
    template = '/webapps/agent_shed/category/valid_grid.mako'
    default_sort_key = "name"
    columns = [
