...
class RepositoryGridFilterManager( object ):
    """Provides filtered views of the many Agent SHed repository grids."""
    ...
    def get_filter( self, trans ):
        filter = trans.get_cookie( name='agentshedrepogridfilter' )
    ...
    def set_filter( self, trans, **kwd ):
        ...
        if filter is not None and self.is_valid_filter( filter ):
            trans.set_cookie( value=filter, name='agentshedrepogridfilter' )
        # if the filter is not valid, expire the cookie.
        trans.set_cookie( value=filter, name='agentshedrepogridfilter', age=-1 )
