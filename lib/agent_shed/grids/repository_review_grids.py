...
from agent_shed.grids.repository_grids import RepositoryGrid
...
class ComponentGrid( grids.Grid ):
    ...
    template = '/webapps/agent_shed/repository_review/grid.mako'
...
class RepositoriesReadyForReviewGrid( RepositoriesWithoutReviewsGrid ):
    ...
    # 1) Have no agents
    # 2) Have agents that have been proven to be functionally correct within Galaxy.
    # This grid filters out repositories that have been marked as either deprecated or deleted.
    ...
    def build_initial_query( self, trans, **kwd ):
        return trans.sa_session.query( model.Repository ) \
                               ...
                               .filter( and_( model.RepositoryMetadata.table.c.downloadable == true(),
                                              or_( model.RepositoryMetadata.table.c.includes_agents == false(),
                                                   and_( model.RepositoryMetadata.table.c.includes_agents == true(),
                                                         model.RepositoryMetadata.table.c.agents_functionally_correct == true() ) ) ) ) \
                               .join( model.User.table )
...
class RepositoriesWithNoAgentTestsGrid( RepositoriesWithoutReviewsGrid ):
    # Repositories that are ready for human review are those that either:
    # 1) Have no agents
    # 2) Have agents that have been proven to be functionally correct within Galaxy.
    # This grid filters out repositories that have been marked as either deprecated or deleted.
    title = "Repositories that contain agents with no tests or test data"
    columns = [
        RepositoriesWithoutReviewsGrid.NameColumn( "Repository name",
                                                   key="name",
                                                   link=( lambda item: dict( operation="view_or_manage_repository", id=item.id ) ),
    ...
    def build_initial_query( self, trans, **kwd ):
        return trans.sa_session.query( model.Repository ) \
                               ...
                               .join( model.RepositoryMetadata.table ) \
                               .filter( and_( model.RepositoryMetadata.table.c.downloadable == true(),
                                              model.RepositoryMetadata.table.c.includes_agents == true(),
                                              model.RepositoryMetadata.table.c.agents_functionally_correct == false() ) ) \
                               .join( model.User.table )
