    ...
    def get_certified_level_one_tuple( self, repository ):
        ...
        if latest_installable_changeset_revision not in [ None, hg_util.INITIAL_CHANGELOG_HASH ]:
            ...
            if repository_metadata:
                ...
                if repository_metadata.time_last_tested is not None and repository_metadata.agent_test_results is not None:
                    if repository.type in [ rt_util.REPOSITORY_SUITE_DEFINITION, rt_util.TOOL_DEPENDENCY_DEFINITION ]:
                        # Look in the agent_test_results dictionary for installation errors.
                        try:
                            agent_test_results_dict = repository_metadata.agent_test_results[ 0 ]
                        except Exception, e:
                        ...
                        if 'installation_errors' in agent_test_results_dict:
                            return ( latest_installable_changeset_revision, False )
                        ...
                        if repository_metadata.includes_agents:
                            if repository_metadata.agents_functionally_correct:
                                return ( latest_installable_changeset_revision, True )
                            return ( latest_installable_changeset_revision, False )
                        else:
                            # Look in the agent_test_results dictionary for installation errors.
