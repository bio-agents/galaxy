...
into Galaxy from the Agent Shed.
"""
import json
import logging
import os
...
class RepositoryDependencyInstallManager( object ):
    ...
    def build_repository_dependency_relationships( self, repo_info_dicts, agent_shed_repositories ):
        """
        Build relationships between installed agent shed repositories and other installed
        agent shed repositories upon which they depend.  These relationships are defined in
        the repository_dependencies entry for each dictionary in the received list of repo_info_dicts.
        Each of these dictionaries is associated with a repository in the received agent_shed_repositories
        list.
        """
            ...
            for name, repo_info_tuple in repo_info_dict.items():
                ...
                if repository_dependencies:
                    for key, val in repository_dependencies.items():
                        ...
                        d_agentshed, d_name, d_owner, d_changeset_revision = components_list[ 0:4 ]
                        for tsr in agent_shed_repositories:
                            # Get the the agent_shed_repository defined by name, owner and changeset_revision.  This is
                            # the repository that will be dependent upon each of the agent shed repositories contained in
                            # val.  We'll need to check agent_shed_repository.agent_shed as well if/when repository dependencies
                            # across agent sheds is supported.
                        ...
                        if d_repository is None:
                            ...
                            d_repository = self.get_or_create_agent_shed_repository( d_agentshed,
                                                                                    d_name,
                                                                                    d_owner,
                        ...
                        for repository_dependency_components_list in val:
                            ...
                            # Get the the agent_shed_repository defined by rd_name, rd_owner and rd_changeset_revision.  This
                            # is the repository that will be required by the current d_repository.
                            # TODO: Check agent_shed_repository.agent_shed as well when repository dependencies across agent sheds is supported.
                            for tsr in agent_shed_repositories:
                            ...
                            if required_repository is None:
                                ...
                                required_repository = self.get_or_create_agent_shed_repository( rd_agentshed,
                                                                                               rd_name,
                                                                                               rd_owner,
                                                                                               rd_changeset_revision )
                            ...
                            if not rrda:
                                ...
                                if not repository_dependency:
                                    ...
                                    repository_dependency = install_model.RepositoryDependency( agent_shed_repository_id=required_repository.id )
                                ...
                                rrda = install_model.RepositoryRepositoryDependencyAssociation( agent_shed_repository_id=d_repository.id,
                                                                                                repository_dependency_id=repository_dependency.id )
                                install_model.context.add( rrda )
                                install_model.context.flush()

    def create_repository_dependency_objects( self, agent_path, agent_shed_url, repo_info_dicts, install_repository_dependencies=False,
                                              no_changes_checked=False, agent_panel_section_id=None, new_agent_panel_section_label=None ):
        """
        Discover all repository dependencies and make sure all agent_shed_repository and
        ...
        # or updated agent shed repositories, including repository dependencies that may not
        # be installed.
        all_created_or_updated_agent_shed_repositories = []
        # There will be a one-to-one mapping between items in 3 lists:
        # created_or_updated_agent_shed_repositories, agent_panel_section_keys
        # and filtered_repo_info_dicts.  The 3 lists will filter out repository
        # dependencies that are not to be installed.
        created_or_updated_agent_shed_repositories = []
        ...
        all_required_repo_info_dict = self.get_required_repo_info_dicts( agent_shed_url, repo_info_dicts )
            ...
            if self.is_in_repo_info_dicts( repo_info_dict, repo_info_dicts ) or install_repository_dependencies:
                for name, repo_info_tuple in repo_info_dict.items():
                    ...
                    repository_db_record, installed_changeset_revision = \
                        suc.repository_was_previously_installed( self.app, agent_shed_url, name, repo_info_tuple, from_tip=False )
                    if repository_db_record:
                        if repository_db_record.status in [ install_model.AgentShedRepository.installation_status.INSTALLED,
                                                            install_model.AgentShedRepository.installation_status.CLONING,
                                                            install_model.AgentShedRepository.installation_status.SETTING_TOOL_VERSIONS,
                                                            install_model.AgentShedRepository.installation_status.INSTALLING_REPOSITORY_DEPENDENCIES,
                                                            install_model.AgentShedRepository.installation_status.INSTALLING_TOOL_DEPENDENCIES,
                                                            install_model.AgentShedRepository.installation_status.LOADING_PROPRIETARY_DATATYPES ]:
                            ...
                            if repository_db_record.status in [ install_model.AgentShedRepository.installation_status.ERROR,
                                                                install_model.AgentShedRepository.installation_status.NEW,
                                                                install_model.AgentShedRepository.installation_status.UNINSTALLED ]:
                                # The current agent shed repository is not currently installed, so we can update its
                            ...
                            elif repository_db_record.status in [ install_model.AgentShedRepository.installation_status.DEACTIVATED ]:
                                # The current agent shed repository is deactivated, so updating its database record
                                # is not necessary - just activate it.
                                log.debug( "Reactivating deactivated agent_shed_repository '%s'." % str( repository_db_record.name ) )
                                ...
                                can_update_db_record = False
                            elif repository_db_record.status not in [ install_model.AgentShedRepository.installation_status.NEW ]:
                                # Set changeset_revision here so suc.create_or_update_agent_shed_repository will find
                    ...
                    if can_update_db_record:
                        # The database record for the agent shed repository currently being processed can be updated.
                        # Get the repository metadata to see where it was previously located in the agent panel.
                        ...
                        if repository_db_record and repository_db_record.metadata:
                            _, agent_panel_section_key = \
                                tpm.handle_agent_panel_selection( agentbox=self.app.agentbox,
                            ...
                            # We're installing a new agent shed repository that does not yet have a database record.
                            agent_panel_section_key, _ = \
                                tpm.handle_agent_panel_section( self.app.agentbox,
                                                               agent_panel_section_id=agent_panel_section_id,
                                                               new_agent_panel_section_label=new_agent_panel_section_label )
                        agent_shed_repository = \
                            suc.create_or_update_agent_shed_repository( app=self.app,
                                                                       name=name,
                                                                       ...
                                                                       metadata_dict={},
                                                                       status=install_model.AgentShedRepository.installation_status.NEW,
                                                                       current_changeset_revision=changeset_revision,
                                                                       owner=repository_owner,
                                                                       dist_to_shed=False )
                        if agent_shed_repository not in all_created_or_updated_agent_shed_repositories:
                            all_created_or_updated_agent_shed_repositories.append( agent_shed_repository )
                        # Only append the agent shed repository to the list of created_or_updated_agent_shed_repositories if
                        ...
                        if install_repository_dependencies or self.is_in_repo_info_dicts( repo_info_dict, repo_info_dicts ):
                            if agent_shed_repository not in created_or_updated_agent_shed_repositories:
                                # Keep the one-to-one mapping between items in 3 lists.
                                created_or_updated_agent_shed_repositories.append( agent_shed_repository )
        ...
        self.build_repository_dependency_relationships( all_repo_info_dicts, all_created_or_updated_agent_shed_repositories )
        return created_or_updated_agent_shed_repositories, agent_panel_section_keys, all_repo_info_dicts, filtered_repo_info_dicts

    def get_or_create_agent_shed_repository( self, agent_shed, name, owner, changeset_revision ):
        """
        Return a agent shed repository database record defined by the combination of
        agent shed, repository name, repository owner and changeset_revision or
        installed_changeset_revision.  A new agent shed repository record will be
        ...
        repository = suc.get_repository_for_dependency_relationship( self.app, agent_shed, name, owner, changeset_revision )
        if not repository:
            agent_shed_url = common_util.get_agent_shed_url_from_agent_shed_registry( self.app, agent_shed )
            repository_clone_url = os.path.join( agent_shed_url, 'repos', owner, name )
            ctx_rev = suc.get_ctx_rev( self.app, agent_shed_url, name, owner, changeset_revision )
            repository = suc.create_or_update_agent_shed_repository( app=self.app,
                                                                    name=name,
                                                                    description=None,
                                                                    installed_changeset_revision=changeset_revision,
                                                                    ctx_rev=ctx_rev,
    ...
    def get_repository_dependencies_for_installed_agent_shed_repository( self, app, repository ):
        """
        Send a request to the appropriate agent shed to retrieve the dictionary of repository dependencies defined
        ...
        try:
            raw_text = common_util.agent_shed_get( app, agent_shed_url, pathspec=pathspec, params=params )
        except Exception, e:
            log.error("The URL\n%s\nraised the exception:\n%s\n", common_util.url_join( agent_shed_url, pathspec=pathspec, params=params ), str( e ) )
            return ''
        if len( raw_text ) > 2:
    ...
    def get_repository_dependency_by_repository_id( self, install_model, decoded_repository_id ):
        return install_model.context.query( install_model.RepositoryDependency ) \
                                    .filter( install_model.RepositoryDependency.table.c.agent_shed_repository_id == decoded_repository_id ) \
                                    .first()

    def get_required_repo_info_dicts( self, agent_shed_url, repo_info_dicts ):
        """
        ...
        all_repo_info_dicts = []
        if repo_info_dicts:
            # We'll send tuples of ( agent_shed, repository_name, repository_owner, changeset_revision ) to the agent
            ...
            for repo_info_dict in repo_info_dicts:
                ...
                for repository_name, repo_info_tup in repo_info_dict.items():
                    ...
                    if repository_dependencies:
                        for key, val in repository_dependencies.items():
                            ...
                            repository_components_tuple = container_util.get_components_from_key( key )
                            components_list = suc.extract_components_from_tuple( repository_components_tuple )
                            # Skip listing a repository dependency if it is required only to compile a agent dependency
                            # defined for the dependent repository since in this case, the repository dependency is really
                            # a dependency of the dependent repository's contained agent dependency, and only if that
                            # agent dependency requires compilation.
                        ...
                        components_list = [ agent_shed_url, repository_name, repository_owner, changeset_revision ]
                        required_repository_tups.append( components_list )
                if required_repository_tups:
                    ...
                    encoded_required_repository_str = encoding_util.agent_shed_encode( encoded_required_repository_str )
                    if suc.is_agent_shed_client( self.app ):
                        # Handle secure / insecure Agent Shed URL protocol changes and port changes.
                    ...
                    url = common_util.url_join( agent_shed_url, pathspec=pathspec )
                        ...
                        for k, v in required_repo_info_dict.items():
                            if k == 'repo_info_dicts':
                                ...
                                for encoded_dict_str in encoded_dict_strings:
                                    decoded_dict = encoding_util.agent_shed_decode( encoded_dict_str )
                            ...
                            if required_repo_info_dicts:
                                for required_repo_info_dict in required_repo_info_dicts:
                                    ...
                                    if required_repo_info_dict_key not in all_repo_info_dicts_keys:
                                        ...
                                        # check that the agent id is already present. If it is not, we are dealing with the same repo name,
                                        # but a different owner/changeset revision or version and we add the repo to the list of repos to be installed.
                                        agent_id = required_repo_info_dict[ required_repo_info_dict_key ][ 1 ]
                                        is_present = False
                                        for repo_info_dict in all_repo_info_dicts:
    ...
    def reset_previously_installed_repository( self, repository ):
        """
        Reset the attributes of a agent_shed_repository that was previously installed.
        ...
        freshly installed.
        """
        debug_msg = "Resetting agent_shed_repository '%s' for installation.\n" % str( repository.name )
        debug_msg += "The current state of the agent_shed_repository is:\n"
        debug_msg += "deleted: %s\n" % str( repository.deleted )
        debug_msg += "agent_shed_status: %s\n" % str( repository.agent_shed_status )
        ...
        repository.status = self.app.install_model.AgentShedRepository.installation_status.NEW
