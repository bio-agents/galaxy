...
class RecipeStep( object ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
                      install_environment, work_dir, current_dir=None, initial_download=False ):
        raise "Unimplemented Method"

    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        raise "Unimplemented Method"
...
class AssertDirectoryExecutable( RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        return agent_dependency, None, None
...
class Autoconf( RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        with settings( warn_only=True ):
            ...
            install_environment.handle_command( agent_dependency=agent_dependency,
                                                cmd=cmd,
                                                return_output=False,
                                                job_name=package_name )
            # The caller should check the status of the returned agent_dependency since this function
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        if dir_valid( target_dir ):
            ...
            if dir_valid( target_dir ):
                ...
                dir = current_dir
        return agent_dependency, None, dir
...
class DownloadBinary( Download, RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        # any errors in the move step are correctly sent to the agent dependency error handler.
        ...
        dir = None
        if initial_download:
            return agent_dependency, filtered_actions, dir
        return agent_dependency, None, None

    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        platform_info_dict = agent_dependency_util.get_platform_info_dict()
        platform_info_dict[ 'name' ] = str( agent_dependency.name )
        platform_info_dict[ 'version' ] = str( agent_dependency.version )
        ...
        if len( url_template_elems ) > 1:
            ...
            url_template_elem = agent_dependency_util.get_download_url_for_platform( url_template_elems, platform_info_dict )
        else:
...
class DownloadByUrl( Download, RecipeStep ):
    ...
    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        ...
        #     http://sourceforge.net/projects/samagents/files/samagents/0.1.18/samagents-0.1.18.tar.bz2
        # </action>
        #
...
class SetEnvironment( RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        root_dir_dict = dict( action='set_to',
                              name='%s_ROOT_DIR' % re.sub( r"[^A-Z0-9_]", "_", agent_dependency.name.upper() ),
        ...
        # The caller should check the status of the returned agent_dependency since return_code is not
        # returned by this function.
        return agent_dependency, None, None

    def handle_environment_variables( self, install_environment, agent_dependency, env_var_dict,
                                      set_prior_environment_commands ):
        """
        This method works with a combination of three agent dependency definition tag sets, which are defined in
        the agent_dependencies.xml file in the order discussed here.  The example for this discussion is the
        agent_dependencies.xml file contained in the osra repository, which is available at:

        https://testagentshed.g2.bx.psu.edu/view/bgruening/osra

        The first tag set defines a complex repository dependency like this.  This tag set ensures that changeset
        revision XXX of the repository named package_graphicsmagick_1_3 owned by YYY in the agent shed ZZZ has been
        previously installed.

        <agent_dependency>
            <package name="graphicsmagick" version="1.3.18">
                <repository changeset_revision="XXX" name="package_graphicsmagick_1_3" owner="YYY" prior_installation_required="True" agentshed="ZZZ" />
            </package>
            ...
        ...
        * By the way, there is an env.sh file associated with version 1.3.18 of the graphicsmagick package which looks
        ...
        ----
        GRAPHICSMAGICK_ROOT_DIR=/<my configured agent dependency path>/graphicsmagick/1.3.18/YYY/package_graphicsmagick_1_3/XXX/gmagick;
        ...
        tag set discussed above) and compiled, where the compiled dependency is needed by the agent dependency currently
        ...
        <action type="set_environment_for_install">
            <repository changeset_revision="XXX" name="package_graphicsmagick_1_3" owner="YYY" agentshed="ZZZ">
                <package name="graphicsmagick" version="1.3.18" />
            </repository>
        </action>

        The third tag set enables discovery of the same required package dependency discussed above for correctly compiling
        the osra version 2.0.0 package, but in this case the package can be discovered at agent execution time.  Using the
        ...
        file associated with the osra version 2.0.0 agent dependency when it is installed into Galaxy.  * Refer to where we
        ...
        ----
        LD_LIBRARY_PATH=/<my configured agent dependency path>/graphicsmagick/1.3.18/YYY/package_graphicsmagick_1_3/XXX/gmagick/lib/:$LD_LIBRARY_PATH;
        export LD_LIBRARY_PATH
        LD_LIBRARY_PATH=/<my configured agent dependency path>/osra/1.4.0/YYY/depends_on/XXX/potrace/build/lib/:$LD_LIBRARY_PATH;
        export LD_LIBRARY_PATH
        PATH=/<my configured agent dependency path>/osra/1.4.0/YYY/depends_on/XXX/bin:$PATH;
        export PATH
        OSRA_DATA_FILES=/<my configured agent dependency path>/osra/1.4.0/YYY/depends_on/XXX/share;
        ...
        if '$ENV[' in env_var_value and ']' in env_var_value:
            ...
            for env_cmd in set_prior_environment_commands:
                # LD_LIBRARY_PATH=/<my configured agent dependency path>/<some path>; export LD_LIBRARY_PATH
                if env_cmd.startswith( inherited_env_var_name ):
                    env_val = env_cmd.split( '=' )[1]
                    # /<my configured agent dependency path>/<some path>; export LD_LIBRARY_PATH
                    env_val = env_val.split( ';' )[0]
                    # /<my configured agent dependency path>/<some path>
    ...
    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        # This function is only called for set environment actions as defined above, not within a <set_environment> agent
...
class SetEnvironmentForInstall( RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        """
        # Currently the only action supported in this category is a list of paths to one or more agent
        ...
        # <action type="set_environment_for_install"> tag set in the agent_dependencies.xml file.
    ...
    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        ...
        #    </repository>
        # </action>
        # This action type allows for defining an environment that will properly compile a agent dependency.
        ...
        # the current agent dependency package.  See the package_matplotlib_1_2 repository in the test agent
...
class SetupPerlEnvironment( Download, RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        if env_shell_file_paths is None:
            ...
            install_environment.add_env_shell_file_paths( env_shell_file_paths )
        log.debug( 'Handling setup_perl_environment for agent dependency %s with install_environment.env_shell_file_paths:\n%s' %
                   ( str( agent_dependency.name ), str( install_environment.env_shell_file_paths ) ) )
        dir = os.path.curdir
                ...
                for perl_package_dict in perl_packages:
                    ...
                    if perl_package.find( '://' ) != -1:
                        ...
                        with lcd( tmp_work_dir ):
                            ...
                            return_code = install_environment.handle_command( agent_dependency=agent_dependency,
                                                                              cmd=cmd,
                                                                              return_output=False,
                                                                              job_name=package_name)
                            if return_code:
...
class SetupREnvironment( Download, RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        if env_shell_file_paths is None:
            ...
            install_environment.add_env_shell_file_paths( env_shell_file_paths )
        log.debug( 'Handling setup_r_environment for agent dependency %s with install_environment.env_shell_file_paths:\n%s' %
                   ( str( agent_dependency.name ), str( install_environment.env_shell_file_paths ) ) )
        tarball_names = []
...
class SetupRubyEnvironment( Download, RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        if env_shell_file_paths is None:
            ...
            install_environment.add_env_shell_file_paths( env_shell_file_paths )
        log.debug( 'Handling setup_ruby_environment for agent dependency %s with install_environment.env_shell_file_paths:\n%s' %
                   ( str( agent_dependency.name ), str( install_environment.env_shell_file_paths ) ) )
        dir = os.path.curdir
        current_dir = os.path.abspath( os.path.join( work_dir, dir ) )
        with lcd( current_dir ):
...
class SetupPythonEnvironment( Download, RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        if env_shell_file_paths is None:
            ...
            install_environment.add_env_shell_file_paths( env_shell_file_paths )
        log.debug( 'Handling setup_python_environment for agent dependency %s with install_environment.env_shell_file_paths:\n%s' %
                   ( str( agent_dependency.name ), str( install_environment.env_shell_file_paths ) ) )
        dir = os.path.curdir
        current_dir = os.path.abspath( os.path.join( work_dir, dir ) )
        with lcd( current_dir ):
            with settings( warn_only=True ):
                ...
                for python_package_tup_dict in python_package_tups:
                    ...
                    package_path = os.path.join( install_environment.agent_shed_repository_install_dir, package )
...
class SetupVirtualEnv( Download, RecipeStep ):
    ...
    def execute_step( self, agent_dependency, package_name, actions, action_dict, filtered_actions, env_file_builder,
        ...
        venv_src_directory = os.path.abspath( os.path.join( self.app.config.agent_dependency_dir, '__virtualenv_src' ) )
        ...
        # The caller should check the status of the returned agent_dependency since this function does nothing
    ...
    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        ...
        # <action type="setup_virtualenv">agents/requirements.txt</action>
    ...
    def __get_site_packages_directory( self, install_environment, app, agent_dependency, python_cmd, venv_directory ):
        ...
        for site_packages_command in [ r"""%s -c 'import site; site.getsitepackages()[0]'""" %
                                       ...
                                       os.path.join( venv_directory, "bin", "python" ) ]:
            output = install_environment.handle_command( agent_dependency=agent_dependency,
...
class TemplateCommand( RecipeStep ):
    ...
    def prepare_step( self, agent_dependency, action_elem, action_dict, install_environment, is_binary_download ):
        ...
        if language == 'cheetah':
            ...
            raise Exception( "Unsupported template language '%s' in agent dependency definition." % str( language ) )
        return action_dict
