...
from agent_shed.util import common_util, container_util, hg_util, metadata_util
...
class RelationBuilder( object ):
    ...
    def __init__( self, app, repository, repository_metadata, agent_shed_url ):
    ...
    def get_prior_installation_required_and_only_if_compiling_contained_td( self ):
        """
        This method is called from the agent shed and never Galaxy.  If self.all_repository_dependencies
        ...
        """
        cleaned_agentshed_base_url = common_util.remove_protocol_from_agent_shed_url( self.agent_shed_url )
        if self.all_repository_dependencies:
            for rd_key, rd_tups in self.all_repository_dependencies.items():
                if rd_key in [ 'root_key', 'description' ]:
                    continue
                for rd_tup in rd_tups:
                    rd_agentshed, rd_name, rd_owner, rd_changeset_revision, \
                        rd_prior_installation_required, \
                        rd_only_if_compiling_contained_td = \
                        common_util.parse_repository_dependency_tuple( rd_tup )
                    cleaned_rd_agentshed = common_util.remove_protocol_from_agent_shed_url( rd_agentshed )
                    if cleaned_rd_agentshed == cleaned_agentshed_base_url and \
            ...
            # Get the list of changeset revisions from the agent shed to which self.repository may be updated.
    ...
    def get_key_for_repository_changeset_revision( self ):
        # The received agentshed_base_url must include the port, but doesn't have to include the protocol.
        ...
        key = container_util.generate_repository_dependencies_key_for_repository( self.agent_shed_url,
    ...
    def get_repository_dependencies_for_changeset_revision( self ):
        ...
        self.agent_shed_url/repository_name/repository_owner/changeset_revision
        ...
        metadata = self.repository_metadata.metadata
        if metadata:
            # The value of self.agent_shed_url must include the port, but doesn't have to include
            # the protocol.
    ...
    def get_repository_dependency_as_key( self, repository_dependency ):
        ...
        return container_util.generate_repository_dependencies_key_for_repository( agent_shed,
                                                                                   name,
                                                                                   owner,
                                                                                   changeset_revision,
                                                                                   prior_installation_required,
    ...
    def get_updated_changeset_revisions_for_repository_dependencies( self, key_rd_dicts ):
        ...
        for key_rd_dict in key_rd_dicts:
            ...
            rd_agentshed, rd_name, rd_owner, rd_changeset_revision, \
                ...
                rd_only_if_compiling_contained_td = \
                common_util.parse_repository_dependency_tuple( repository_dependency )
            if suc.agent_shed_is_this_agent_shed( rd_agentshed ):
                ...
                if repository:
                    ...
                    if repository_metadata:
                        ...
                        if repository_metadata:
                            ...
                            agentshed, repository_name, repository_owner, repository_changeset_revision = components_list[ 0:4 ]
    ...
    def handle_key_rd_dicts_for_repository( self, current_repository_key, repository_key_rd_dicts ):
        ...
        if suc.agent_shed_is_this_agent_shed( agentshed ):
            ...
            # The repository is in a different agent shed, so build an url and send a request.
            error_message = "Repository dependencies are currently supported only within the same Agent Shed.  "
            error_message += "Ignoring repository dependency definition for agent shed "
            error_message += "%s, name %s, owner %s, changeset revision %s" % ( agentshed, name, owner, changeset_revision )
    ...
    def prune_invalid_repository_dependencies( self, repository_dependencies ):
        ...
        combination of agent shed, name , owner, changeset_revision is used and a repository_metadata
        record is not found.
        """
    ...
    def remove_repository_dependency_reference_to_self( self, key_rd_dicts ):
        ...
        for key_rd_dict in key_rd_dicts:
            ...
            agentshed, name, owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td = \
                common_util.parse_repository_dependency_tuple( repository_dependency )
            cleaned_agentshed = common_util.remove_protocol_from_agent_shed_url( agentshed )
            if cleaned_rd_agentshed == cleaned_agentshed and rd_name == name and rd_owner == owner:
