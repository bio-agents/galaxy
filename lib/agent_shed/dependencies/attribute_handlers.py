...
from agent_shed.dependencies.agent import tag_attribute_handler
from agent_shed.repository_types.util import REPOSITORY_DEPENDENCY_DEFINITION_FILENAME
from agent_shed.repository_types.util import TOOL_DEPENDENCY_DEFINITION_FILENAME
...
class RepositoryDependencyAttributeHandler( object ):
    ...
    def handle_complex_dependency_elem( self, parent_elem, elem_index, elem ):
        """
        Populate or unpopulate the agentshed and changeset_revision attributes of a
    ...
    def handle_elem( self, elem ):
        """Populate or unpopulate the changeset_revision and agentshed attributes of repository tags."""
        # <repository name="molecule_datatypes" owner="test" changeset_revision="1a070566e9c6" />
        # <repository changeset_revision="xxx" name="package_xorg_macros_1_17_1" owner="test" agentshed="yyy">
        ...
        if len( sub_elems ) > 0:
            ...
            sub_elements = None
        if self.unpopulate:
            # We're exporting the repository, so eliminate all agentshed and changeset_revision attributes
            # from the <repository> tag.
            if agentshed or changeset_revision:
                attributes = odict()
                ...
                altered = True
            return altered, new_elem, error_message
        # From here on we're populating the agentshed and changeset_revision attributes if necessary.
        if not agentshed:
            # Default the setting to the current agent shed.
            agentshed = str( url_for( '/', qualified=True ) ).rstrip( '/' )
            elem.attrib[ 'agentshed' ] = agentshed
            altered = True
    ...
    def handle_sub_elem( self, parent_elem, elem_index, elem ):
        """
        Populate or unpopulate the agentshed and changeset_revision attributes for each of
...
class AgentDependencyAttributeHandler( object ):
    ...
    def handle_tag_attributes( self, agent_dependencies_config ):
        """
        Populate or unpopulate the tooshed and changeset_revision attributes of each <repository>
        tag defined within a agent_dependencies.xml file.
        """
        ...
        altered = False
        error_message = ''
        # Make sure we're looking at a valid agent_dependencies.xml file.
