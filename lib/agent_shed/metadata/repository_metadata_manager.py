...
from agent_shed.repository_types.metadata import TipOnly
...
class RepositoryMetadataManager( metadata_generator.MetadataGenerator ):
    ...
    def add_agent_versions( self, id, repository_metadata, changeset_revisions ):
        # Build a dictionary of { 'agent id' : 'parent agent id' } pairs for each agent in repository_metadata.
        metadata = repository_metadata.metadata
        agent_versions_dict = {}
        for agent_dict in metadata.get( 'agents', [] ):
            # We have at least 2 changeset revisions to compare agent guids and agent ids.
            parent_id = self.get_parent_id( id,
                                            agent_dict[ 'id' ],
                                            agent_dict[ 'version' ],
                                            agent_dict[ 'guid' ],
                                            changeset_revisions )
            agent_versions_dict[ agent_dict[ 'guid' ] ] = parent_id
        if agent_versions_dict:
            repository_metadata.agent_versions = agent_versions_dict
    ...
    def compare_changeset_revisions( self, ancestor_changeset_revision, ancestor_metadata_dict ):
        ...
        ancestor_agents = ancestor_metadata_dict.get( 'agents', [] )
        ancestor_guids = [ agent_dict[ 'guid' ] for agent_dict in ancestor_agents ]
        ancestor_guids.sort()
        ...
        ancestor_agent_dependencies = ancestor_metadata_dict.get( 'agent_dependencies', {} )
        ...
        current_agents = self.metadata_dict.get( 'agents', [] )
        current_guids = [ agent_dict[ 'guid' ] for agent_dict in current_agents ]
        current_guids.sort()
        ...
        current_agent_dependencies = self.metadata_dict.get( 'agent_dependencies', {} )
        ...
        no_agent_dependencies = not ancestor_agent_dependencies and not current_agent_dependencies
        no_agents = not ancestor_guids and not current_guids
        ...
        if no_datatypes and no_readme_files and no_repository_dependencies and \
                no_agent_dependencies and no_agents and no_workflows and \
                no_data_manager:
        ...
        agent_dependency_comparison = self.compare_agent_dependencies( ancestor_agent_dependencies,
                                                                     current_agent_dependencies )
        ...
        if ancestor_guids == current_guids and \
                ...
                agent_dependency_comparison == self.EQUAL and \
        ...
        agent_dependency_is_subset = agent_dependency_comparison in self.SUBSET_VALUES
        ...
        if repository_dependency_is_subset and agent_dependency_is_subset and \
    ...
    def compare_data_manager( self, ancestor_metadata, current_metadata ):
        ...
        def __data_manager_dict_to_tuple_list( metadata_dict ):
            # we do not check agent_guid or agent conf file name
    ...
    def compare_readme_files( self, ancestor_readme_files, current_readme_files ):
        ...
        # NOTE: Although repository README files are considered a Galaxy utility similar to agents,
        ...
        # 1. Upload the filtering agent to a new repository - this will result in installable revision 0.
    ...
    def compare_repository_dependencies( self, ancestor_repository_dependencies, current_repository_dependencies ):
        ...
        if len( ancestor_repository_dependencies ) <= len( current_repository_dependencies ):
            for ancestor_tup in ancestor_repository_dependencies:
                a_agent_shed, a_repo_name, a_repo_owner, a_changeset_revision, \
                    a_prior_installation_required, \
                    a_only_if_compiling_contained_td = ancestor_tup
                cleaned_a_agent_shed = common_util.remove_protocol_from_agent_shed_url( a_agent_shed )
                found_in_current = False
                for current_tup in current_repository_dependencies:
                    c_agent_shed, c_repo_name, c_repo_owner, \
                        c_changeset_revision, c_prior_installation_required, \
                        c_only_if_compiling_contained_td = current_tup
                    cleaned_c_agent_shed = common_util.remove_protocol_from_agent_shed_url( c_agent_shed )
                    if cleaned_c_agent_shed == cleaned_a_agent_shed and \
    ...
    def compare_agent_dependencies( self, ancestor_agent_dependencies, current_agent_dependencies ):
        """
        Determine if ancestor_agent_dependencies is the same as or a subset of current_agent_dependencies.
        """
        # The agent_dependencies dictionary looks something like:
        # {'bwa/0.5.9': {'readme': 'some string', 'version': '0.5.9', 'type': 'package', 'name': 'bwa'}}
        if len( ancestor_agent_dependencies ) <= len( current_agent_dependencies ):
            for ancestor_td_key, ancestor_requirements_dict in ancestor_agent_dependencies.items():
                if ancestor_td_key in current_agent_dependencies:
                    ...
                    return self.NOT_EQUAL_AND_NOT_SUBSET
            # At this point we know that ancestor_agent_dependencies is at least a subset of current_agent_dependencies.
            if len( ancestor_agent_dependencies ) == len( current_agent_dependencies ):
                return self.EQUAL
            else:
                return self.SUBSET
    ...
    def create_or_update_repository_metadata( self, changeset_revision, metadata_dict ):
        """Create or update a repository_metadatqa record in the agent shed."""
        ...
        if metadata_dict:
            ...
            if 'agent_dependencies' in metadata_dict:
        ...
        if has_repository_dependencies or \
                ...
                includes_datatypes or includes_agents or \
                includes_agent_dependencies or includes_workflows:
            downloadable = True
        else:
            downloadable = False
        ...
        if repository_metadata:
            ...
            # so we don't need to check the skip_agent_test table.
            check_skip_agent_test = False
            ...
            repository_metadata.includes_agents = includes_agents
            repository_metadata.includes_agent_dependencies = includes_agent_dependencies
            ...
            # need to update the skip_agent_test table.
            check_skip_agent_test = True
            repository_metadata = \
        ...
        self.sa_session.add( repository_metadata )
        self.sa_session.flush()
        if check_skip_agent_test:
            # Since we created a new repository_metadata record, we may need to update the
            # skip_agent_test table to point to it.  Inspect each changeset revision in the
            # received repository's changelog (up to the received changeset revision) to see
            # if it is contained in the skip_agent_test table.  If it is, but is not associated
            # with a repository_metadata record, reset that skip_agent_test record to the newly
            ...
            for changeset in repo.changelog:
                ...
                skip_agent_test = self.get_skip_agent_test_by_changeset_revision( changeset_hash )
                if skip_agent_test:
                    # We found a skip_agent_test record associated with the changeset_revision,
                    ...
                    if repository_revision:
                        # The skip_agent_test record is associated with a valid repository_metadata
                        # record, so proceed.
                        continue
                    # We found a skip_agent_test record that is associated with an invalid
                    ...
                    # skip_agent_test records that require updating, so we won't break here, we'll
                    # continue to inspect the rest of the changelog up to the received changeset_revision.
                    skip_agent_test.repository_metadata_id = repository_metadata.id
                    self.sa_session.add( skip_agent_test )
                    self.sa_session.flush()
    ...
    def different_revision_defines_tip_only_repository_dependency( self, rd_tup, repository_dependencies ):
        ...
        """
        rd_agent_shed, rd_name, rd_owner, rd_changeset_revision, rd_prior_installation_required, rd_only_if_compiling_contained_td = \
            common_util.parse_repository_dependency_tuple( rd_tup )
        cleaned_rd_agent_shed = common_util.remove_protocol_from_agent_shed_url( rd_agent_shed )
        for repository_dependency in repository_dependencies:
            ...
            cleaned_agent_shed = common_util.remove_protocol_from_agent_shed_url( agent_shed )
            if cleaned_rd_agent_shed == cleaned_agent_shed and rd_name == name and rd_owner == owner:
    ...
    def get_parent_id( self, id, old_id, version, guid, changeset_revisions ):
        ...
        for changeset_revision in changeset_revisions:
            ...
            metadata = repository_metadata.metadata
            agents_dicts = metadata.get( 'agents', [] )
            for agent_dict in agents_dicts:
                if agent_dict[ 'guid' ] == guid:
                    # The agent has not changed between the compared changeset revisions.
                    continue
                if agent_dict[ 'id' ] == old_id and agent_dict[ 'version' ] != version:
                    # The agent version is different, so we've found the parent.
                    return agent_dict[ 'guid' ]
        if parent_id is None:
            # The agent did not change through all of the changeset revisions.
            return old_id
    ...
    def get_query_for_setting_metadata_on_repositories( self, my_writable=False, order=True ):
        ...
        a page.  When called from the Agent Shed API, order is False.
        """
        # When called from the Agent Shed API, the metadata is reset on all repositories of types
        # repository_suite_definition and agent_dependency_definition in addition to other selected
        # repositories.
        if my_writable:
    ...
    def get_skip_agent_test_by_changeset_revision( self, changeset_revision ):
        """
        Return a skip_agent_test record whose initial_changeset_revision is the received
        changeset_revision.
        """
        # There should only be one, but we'll use first() so callers won't have to handle exceptions.
        return self.sa_session.query( self.app.model.SkipAgentTest ) \
                              .filter( self.app.model.SkipAgentTest.table.c.initial_changeset_revision == changeset_revision ) \
                              .first()
    ...
    def new_metadata_required_for_utilities( self ):
        ...
        agents_required = self.new_agent_metadata_required( repository_metadata )
        agent_dependencies_required = self.new_agent_dependency_metadata_required( repository_metadata )
        ...
        if datatypes_required or repository_dependencies_required or \
                agents_required or agent_dependencies_required or workflows_required:
            return True
        return False
    ...
    def new_agent_metadata_required( self, repository_metadata ):
        """
        Compare the last saved metadata for each agent in the repository with the new metadata in
            ...
            if repository_metadata:
                ...
                if metadata:
                    if 'agents' in metadata:
                        saved_agent_ids = []
                        # The metadata for one or more agents was successfully generated in the past
                        # for this repository, so we first compare the version string for each agent id
                        ...
                        for new_agent_metadata_dict in self.metadata_dict[ 'agents' ]:
                            for saved_agent_metadata_dict in metadata[ 'agents' ]:
                                if saved_agent_metadata_dict[ 'id' ] not in saved_agent_ids:
                                    saved_agent_ids.append( saved_agent_metadata_dict[ 'id' ] )
                                if new_agent_metadata_dict[ 'id' ] == saved_agent_metadata_dict[ 'id' ]:
                                    if new_agent_metadata_dict[ 'version' ] != saved_agent_metadata_dict[ 'version' ]:
                                        return True
                        # So far, a new metadata record is not required, but we still have to check to see if
                        # any new agent ids exist in self.metadata_dict that are not in the saved metadata.  We do
                        # this because if a new tarball was uploaded to a repository that included agents, it
                        # may have removed existing agent files if they were not included in the uploaded tarball.
                        for new_agent_metadata_dict in self.metadata_dict[ 'agents' ]:
                            if new_agent_metadata_dict[ 'id' ] not in saved_agent_ids:
                                return True
                        return False
                    else:
                        # The new metadata includes agents, but the stored metadata does not, so we can
                ...
                # table record.
                return True
        # self.metadata_dict includes no metadata for agents, so a new repository_metadata table
        # record is not needed.
        return False

    def new_agent_dependency_metadata_required( self, repository_metadata ):
        """
        Compare the last saved metadata for each agent dependency in the repository with the new
        metadata in self.metadata_dict to determine if a new repository_metadata table record is
        required or if the last saved metadata record can be updated for agent_dependencies instead.
        """
        if repository_metadata:
            metadata = repository_metadata.metadata
            if metadata:
                if 'agent_dependencies' in metadata:
                    saved_agent_dependencies = metadata[ 'agent_dependencies' ]
                    new_agent_dependencies = self.metadata_dict.get( 'agent_dependencies', None )
                    if new_agent_dependencies:
                        # TODO: We used to include the following here to handle the case where
                        # agent dependency definitions were deleted.  However, this erroneously
                        ...
                        # agent dependency definitions so that we can guarantee reproducibility,
                        # but we need to do it in a way that is better than the following.
                        # for new_agent_dependency in new_agent_dependencies:
                        #     if new_agent_dependency not in saved_agent_dependencies:
                        #         return True
                        # The saved metadata must be a subset of the new metadata.
                        for saved_agent_dependency in saved_agent_dependencies:
                            if saved_agent_dependency not in new_agent_dependencies:
                                return True
                        return False
                    else:
                        # The agent_dependencies.xml file must have been deleted, so create a new
                ...
                # We have repository metadata that does not include metadata for any agent dependencies
                # in the repository, so we can update the existing repository metadata.
                return False
        else:
            if 'agent_dependencies' in self.metadata_dict:
                # There is no saved repository metadata, so we need to create a new repository_metadata
                # record.
                return True
            else:
                # self.metadata_dict includes no metadata for agent dependencies, so a new repository_metadata
                # record is not needed.
                return False
    ...
    def reset_all_metadata_on_repository_in_agent_shed( self ):
        """Reset all metadata on a single repository in a agent shed."""
        ...
        for changeset in self.repository.get_changesets_for_setting_metadata( self.app ):
            work_dir = tempfile.mkdtemp( prefix="tmp-agentshed-ramorits" )
        ...
        # Set agent version information for all downloadable changeset revisions.  Get the list of changeset
        # revisions from the changelog.
        self.reset_all_agent_versions( repo )
        # Reset the agent_data_tables by loading the empty agent_data_table_conf.xml file.
        self.app.agent_data_tables.data_tables = {}

    def reset_all_agent_versions( self, repo ):
        """Reset agent version lineage for those changeset revisions that include valid agents."""
        encoded_repository_id = self.app.security.encode_id( self.repository.id )
        changeset_revisions_that_contain_agents = []
        for changeset in repo.changelog:
            ...
            if repository_metadata:
                metadata = repository_metadata.metadata
                if metadata:
                    if metadata.get( 'agents', None ):
                        changeset_revisions_that_contain_agents.append( changeset_revision )
        # The list of changeset_revisions_that_contain_agents is now filtered to contain only those that
        # are downloadable and contain agents.  If a repository includes agents, build a dictionary of
        # { 'agent id' : 'parent agent id' } pairs for each agent in each changeset revision.
        for index, changeset_revision in enumerate( changeset_revisions_that_contain_agents ):
            agent_versions_dict = {}
            repository_metadata = suc.get_repository_metadata_by_changeset_revision( self.app,
                                                                                     encoded_repository_id,
                                                                                     changeset_revision )
            ...
            if index == 0:
                ...
                # changeset_revisions in which to match agents.  The parent agent id for agents in the
                # first changeset_revision will be the "old_id" in the agent config.
                for agent_dict in agent_dicts:
                    agent_versions_dict[ agent_dict[ 'guid' ] ] = agent_dict[ 'id' ]
            else:
                for agent_dict in agent_dicts:
                    parent_id = self.get_parent_id( encoded_repository_id,
                                                    ...
                                                    agent_dict[ 'version' ],
                                                    agent_dict[ 'guid' ],
                                                    changeset_revisions_that_contain_agents[ 0:index ] )
                    agent_versions_dict[ agent_dict[ 'guid' ] ] = parent_id
            if agent_versions_dict:
                repository_metadata.agent_versions = agent_versions_dict
                self.sa_session.add( repository_metadata )
        ...
        if repository_ids:
            ...
            for repository_id in repository_ids:
                try:
                    ...
                    self.reset_all_metadata_on_repository_in_agent_shed()
    ...
    def set_repository( self, repository ):
        ...
        self.repository_clone_url = common_util.generate_clone_url_for_repository_in_agent_shed( self.user, repository )
        ...
        if self.metadata_dict:
            ...
            if not tip_only and self.new_metadata_required_for_utilities():
                ...
                if repository_metadata:
                    ...
                    # compiling a agent dependency defined for the dependent repository.
                    ...
                    repository_metadata.has_repository_dependencies = has_repository_dependencies
                    if 'agent_dependencies' in self.metadata_dict:
                        repository_metadata.includes_agent_dependencies = True
                    else:
                        repository_metadata.includes_agent_dependencies = False
                    if 'agents' in self.metadata_dict:
                        repository_metadata.includes_agents = True
                    else:
                        repository_metadata.includes_agents = False
                    if 'workflows' in self.metadata_dict:
                    ...
                    repository_metadata = self.create_or_update_repository_metadata( self.repository.tip( self.app ),
                                                                                     self.metadata_dict )
            if 'agents' in self.metadata_dict and repository_metadata and status != 'error':
                # Set agent versions on the new downloadable change set.  The order of the list of changesets is
                ...
                self.add_agent_versions( encoded_id, repository_metadata, changeset_revisions )
    ...
    def set_repository_metadata_due_to_new_tip( self, host, content_alert_str=None, **kwd ):
        """Set metadata on the tip of self.repository in the agent shed."""
