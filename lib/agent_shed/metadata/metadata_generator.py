...
from galaxy.agents.data_manager.manager import DataManager
from galaxy.agents.loader_directory import looks_like_a_agent
from galaxy.agents.parser.interface import TestCollectionDef
...
class MetadataGenerator( object ):
    ...
    def __init__( self, app, repository=None, changeset_revision=None, repository_clone_url=None,
        ...
        if self.app.name == 'galaxy':
            ...
            if relative_install_dir is None and self.repository is not None:
                agent_path, relative_install_dir = self.repository.get_agent_relative_path( self.app )
            ...
            if metadata_dict is None:
                # Shed related agent panel configs are only relevant to Galaxy.
            ...
            if repository_clone_url is None and self.repository is not None:
                self.repository_clone_url = \
                    common_util.generate_clone_url_for_repository_in_agent_shed( self.user, self.repository )
            else:
                self.repository_clone_url = repository_clone_url
            if shed_config_dict is None:
                self.shed_config_dict = {}
    ...
    def generate_data_manager_metadata( self, repo_dir, data_manager_config_filename, metadata_dict,
        ...
        agent_path = None
        if shed_config_dict:
            agent_path = shed_config_dict.get( 'agent_path', None )
        agents = {}
        for agent in metadata_dict.get( 'agents', [] ):
            agent_conf_name = agent[ 'agent_config' ]
            if agent_path:
                agent_conf_name = os.path.join( agent_path, agent_conf_name )
            agents[ agent_conf_name ] = agent
        root = tree.getroot()
        data_manager_agent_path = root.get( 'agent_path', None )
        if data_manager_agent_path:
            relative_data_manager_dir = os.path.join( relative_data_manager_dir, data_manager_agent_path )
        for i, data_manager_elem in enumerate( root.findall( 'data_manager' ) ):
            agent_file = data_manager_elem.get( 'agent_file', None )
            ...
            if data_manager_id is None:
                ...
                continue
            # FIXME: default behavior is to fall back to agent.name.
            ...
            if agent_file is None:
                log.error( 'Data Manager entry is missing agent_file attribute in "%s".' % ( data_manager_config_filename ) )
                invalid_data_managers.append( { 'index': i,
                                                'error_message': 'Data Manager entry is missing agent_file attribute' } )
                continue
            else:
                ...
                if bad_data_table:
                    continue
            data_manager_metadata_agent_file = os.path.normpath( os.path.join( relative_data_manager_dir, agent_file ) )
            agent_metadata_agent_file = os.path.join( repo_files_directory, data_manager_metadata_agent_file )
            agent = agents.get( agent_metadata_agent_file, None )
            if agent is None:
                log.error( "Unable to determine agents metadata for '%s'." % ( data_manager_metadata_agent_file ) )
                invalid_data_managers.append( { 'index': i,
                                                'error_message': 'Unable to determine agents metadata' } )
                continue
            data_managers[ data_manager_id ] = { 'id': data_manager_id,
                                                 'name': data_manager_name,
                                                 'guid': guid,
                                                 'version': version,
                                                 'agent_config_file': data_manager_metadata_agent_file,
                                                 'data_tables': data_tables,
                                                 'agent_guid': agent[ 'guid' ] }
            log.debug( 'Loaded Data Manager agent_files: %s' % ( agent_file ) )
        return metadata_dict
            ...
            for elem in registration.findall( 'datatype' ):
                ...
                for sub_elem in elem:
                    if sub_elem.tag == 'converter':
                        ...
                        agent_config = sub_elem.attrib[ 'file' ]
                        target_datatype = sub_elem.attrib[ 'target_datatype' ]
                        # Parse the agent_config to get the guid.
                        agent_config_path = hg_util.get_config_from_disk( agent_config, repository_files_dir )
                        full_path = os.path.abspath( agent_config_path )
                        agent, valid, error_message = \
                            tv.load_agent_from_config( self.app.security.encode_id( self.repository.id ), full_path )
                        if agent is None:
                            guid = None
                        else:
                            guid = suc.generate_agent_guid( self.repository_clone_url, agent )
                        converter_dict = dict( agent_config=agent_config,
                                               guid=guid,
    ...
    def generate_environment_dependency_metadata( self, elem, valid_agent_dependencies_dict ):
        """
        The value of env_var_name must match the value of the "set_environment" type
        in the agent config's <requirements> tag set, or the agent dependency will be
        ...
        for env_elem in elem:
            ...
            if env_name:
                ...
                if 'set_environment' in valid_agent_dependencies_dict:
                    valid_agent_dependencies_dict[ 'set_environment' ].append( requirements_dict )
                else:
                    valid_agent_dependencies_dict[ 'set_environment' ] = [ requirements_dict ]
        return valid_agent_dependencies_dict
    ...
    def generate_metadata_for_changeset_revision( self ):
        ...
        agent_data_table_conf.xml.sample file, in which case the entries should ultimately be
        persisted to the file referred to by self.app.config.shed_agent_data_table_config.
        """
        tv = agent_validator.AgentValidator( self.app )
        if self.shed_config_dict is None:
            self.shed_config_dict = {}
        if self.updating_installed_repository:
            # Keep the original agent shed repository metadata if setting metadata on a repository
        ...
        agent_dependencies_config = None
        original_agent_data_path = self.app.config.agent_data_path
        original_agent_data_table_config_path = self.app.config.agent_data_table_config_path
        if self.resetting_all_metadata_on_repository:
            ...
            self.app.config.agent_data_path = self.repository_files_dir
            self.app.config.agent_data_table_config_path = self.repository_files_dir
        else:
            # Use a temporary working directory to copy all sample files.
            work_dir = tempfile.mkdtemp( prefix="tmp-agentshed-gmfcr" )
            ...
            if self.shed_config_dict.get( 'agent_path' ):
                files_dir = os.path.join( self.shed_config_dict[ 'agent_path' ], files_dir )
            self.app.config.agent_data_path = work_dir  # FIXME: Thread safe?
            self.app.config.agent_data_table_config_path = work_dir
        ...
        sample_file_metadata_paths, sample_file_copy_paths = \
            self.get_sample_files_from_disk( repository_files_dir=files_dir,
                                             agent_path=self.shed_config_dict.get( 'agent_path' ),
        ...
        # can load agents that depend on them.
        for sample_file in sample_file_copy_paths:
            agent_util.copy_sample_file( self.app, sample_file, dest_path=work_dir )
            # If the list of sample files includes a agent_data_table_conf.xml.sample file, load
            # its table elements into memory.
            relative_path, filename = os.path.split( sample_file )
            if filename == 'agent_data_table_conf.xml.sample':
                new_table_elems, error_message = \
                    self.app.agent_data_tables.add_new_entries_from_config_file( config_filename=sample_file,
                                                                                agent_data_path=self.app.config.agent_data_path,
        ...
        for root, dirs, files in os.walk( files_dir ):
            if root.find( '.hg' ) < 0 and root.find( 'hgrc' ) < 0:
                ...
                for name in files:
                    ...
                    # See if we have a agent config.
                    elif looks_like_a_agent(os.path.join( root, name ), invalid_names=self.NOT_TOOL_CONFIGS ):
                        ...
                        if element_tree is None:
                            ...
                            is_agent = element_tree_root.tag == 'agent'
                        if is_agent:
                            agent, valid, error_message = \
                                tv.load_agent_from_config( self.app.security.encode_id( self.repository.id ),
                                                          full_path )
                            if agent is None:
                                if not valid:
                                    invalid_agent_configs.append( name )
                                    self.invalid_file_tups.append( ( name, error_message ) )
                            else:
                                invalid_files_and_errors_tups = \
                                    tv.check_agent_input_params( files_dir,
                                                                name,
                                                                agent,
                                ...
                                for tup in invalid_files_and_errors_tups:
                                    if name in tup:
                                        ...
                                        break
                                if can_set_metadata:
                                    relative_path_to_agent_config = \
                                        self.get_relative_path_to_repository_file( root,
                                                                                   name,
                                                                                   self.relative_install_dir,
                                                                                   work_dir,
                                                                                   self.shed_config_dict )
                                    metadata_dict = self.generate_agent_metadata( relative_path_to_agent_config,
                                                                                 agent,
                                                                                 metadata_dict )
                                else:
                                    for tup in invalid_files_and_errors_tups:
        ...
        # This step must be done after metadata for agents has been defined.
        agent_dependencies_config = hg_util.get_config_from_disk( rt_util.TOOL_DEPENDENCY_DEFINITION_FILENAME, files_dir )
        if agent_dependencies_config:
            metadata_dict, error_message = \
                self.generate_agent_dependency_metadata( agent_dependencies_config,
                                                        metadata_dict,
        ...
        if invalid_agent_configs:
            metadata_dict[ 'invalid_agents' ] = invalid_agent_configs
        self.metadata_dict = metadata_dict
        # Reset the value of the app's agent_data_path  and agent_data_table_config_path to their respective original values.
        self.app.config.agent_data_path = original_agent_data_path
        self.app.config.agent_data_table_config_path = original_agent_data_table_config_path
        basic_util.remove_dir( work_dir )

    def generate_package_dependency_metadata( self, elem, valid_agent_dependencies_dict, invalid_agent_dependencies_dict ):
        """
        Generate the metadata for a agent dependencies package defined for a repository.  The
        value of package_name must match the value of the "package" type in the agent config's
        ...
        if package_name and package_version:
            ...
            for sub_elem in elem:
                if sub_elem.tag == 'readme':
                    ...
                    # is True, the agent dependency definition will be set as invalid.  This is currently the only case
                    # where a agent dependency definition is considered invalid.
                    ...
                    if package_install_version == '1.0':
                        ...
                        # in agent_dependency_util.parse_package_elem().
                        actions_elem_tuples = agent_dependency_util.parse_package_elem( sub_elem,
                            ...
                            if in_actions_group:
                                ...
                                for action_elem in actions_elem:
                                    if action_elem.tag == 'package':
                                        ...
                                        #    <repository changeset_revision="b107b91b3574" name="package_readline_6_2" owner="devteam" prior_installation_required="True" agentshed="http://localhost:9009">
        ...
        if requirements_dict:
            ...
            if repository_dependency_is_valid:
                valid_agent_dependencies_dict[ dependency_key ] = requirements_dict
            else:
                # Append the error message to the requirements_dict.
                requirements_dict[ 'error' ] = error_message
                invalid_agent_dependencies_dict[ dependency_key ] = requirements_dict
        return valid_agent_dependencies_dict, \
            invalid_agent_dependencies_dict, \
            repository_dependency_tup, \
    ...
    def generate_repository_dependency_metadata( self, repository_dependencies_config, metadata_dict ):
        ...
        repository_dependencies_config.  This method is called from the agent shed as well as from Galaxy.
        """
        error_message = ''
            ...
            for repository_elem in root.findall( 'repository' ):
                ...
                if repository_dependency_is_valid:
                    ...
                    agentshed, name, owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td = \
                        repository_dependency_tup
                    repository_dependency_tup = ( agentshed,
                                                  name,
                                                  owner,
                                                  changeset_revision,
    ...
    def generate_agent_metadata( self, agent_config, agent, metadata_dict ):
        """Update the received metadata_dict with changes that have been applied to the received agent."""
        # Generate the guid.
        guid = suc.generate_agent_guid( self.repository_clone_url, agent )
        # Handle agent.requirements.
        agent_requirements = []
        for agent_requirement in agent.requirements:
            name = str( agent_requirement.name )
            agent_type = str( agent_requirement.type )
            version = str( agent_requirement.version ) if agent_requirement.version else None
            requirement_dict = dict( name=name,
                                     type=agent_type,
                                     version=version )
            agent_requirements.append( requirement_dict )
        # Handle agent.tests.
        agent_tests = []
        if agent.tests:
            for ttb in agent.tests:
                required_files = []
                ...
                for param_name, values in ttb.inputs.iteritems():
                    ...
                    if param_name is not None:
                        if values in [ None, False ]:
                            # An example is the third test in http://testagentshed.g2.bx.psu.edu/view/devteam/samagents_rmdup
                            # which is defined as:
                            # <test>
                            ...
                            if isinstance( values, TestCollectionDef ):
                                ...
                                try:
                                    ...
                                    log.exception( 'Expected a list of values for agent "%s" parameter "%s", got %s: %s', agent.id, param_name, type( values ), values )
                                inputs.append( ( param_name, values ) )
                outputs = []
                ...
                test_dict = dict( name=str( ttb.name ),
                                  ...
                                  inputs=inputs,
                                  outputs=outputs )
                agent_tests.append( test_dict )
        # Determine if the agent should be loaded into the agent panel.  Examples of valid agents that
        # should not be displayed in the agent panel are datatypes converters and DataManager agents
        # (which are of type 'manage_data').
        datatypes = metadata_dict.get( 'datatypes', None )
        add_to_agent_panel_attribute = self.set_add_to_agent_panel_attribute_for_agent( agent=agent,
                                                                                     guid=guid,
                                                                                     datatypes=datatypes )
        agent_dict = dict( id=agent.id,
                          guid=guid,
                          name=agent.name,
                          version=agent.version,
                          description=agent.description,
                          version_string_cmd=agent.version_string_cmd,
                          agent_config=agent_config,
                          agent_type=agent.agent_type,
                          requirements=agent_requirements,
                          tests=agent_tests,
                          add_to_agent_panel=add_to_agent_panel_attribute )
        if 'agents' in metadata_dict:
            metadata_dict[ 'agents' ].append( agent_dict )
        else:
            metadata_dict[ 'agents' ] = [ agent_dict ]
        return metadata_dict

    def generate_agent_dependency_metadata( self, agent_dependencies_config, metadata_dict, original_repository_metadata=None ):
        """
        If the combination of name, version and type of each element is defined in the <requirement> tag for
        at least one agent in self.repository, then update the received metadata_dict with information from the
        parsed agent_dependencies_config.
        """
        error_message = ''
        if original_repository_metadata:
            # Keep a copy of the original agent dependencies dictionary and the list of agent
            # dictionaries in the metadata.
            original_valid_agent_dependencies_dict = original_repository_metadata.get( 'agent_dependencies', None )
        else:
            original_valid_agent_dependencies_dict = None
        ...
        rvs.valid_agent_dependencies_dict = {}
        rvs.invalid_agent_dependencies_dict = {}
        valid_repository_dependency_tups = []
        invalid_repository_dependency_tups = []
        ...
        def _check_elem_for_dep( elems ):
            ...
            for elem in elems:
                if elem.tag == 'package':
                    rvs.valid_agent_dependencies_dict, rvs.invalid_agent_dependencies_dict, \
                    ...
                    if repository_dependency_is_valid:
                        ...
                        if repository_dependency_tup and repository_dependency_tup not in invalid_repository_dependency_tups:
                            # We have an invalid complex repository dependency, so mark the agent dependency as invalid.
                    ...
                    rvs.valid_agent_dependencies_dict = \
                        self.generate_environment_dependency_metadata( elem, rvs.valid_agent_dependencies_dict )
                error_messages += _check_elem_for_dep( elem )
            return error_messages
        error_message = "\n".join([error_message] + _check_elem_for_dep( root ))
        if rvs.valid_agent_dependencies_dict:
            if original_valid_agent_dependencies_dict:
                # We're generating metadata on an update pulled to a agent shed repository installed
                # into a Galaxy instance, so handle changes to agent dependencies appropriately.
                irm = self.app.installed_repository_manager
                updated_agent_dependency_names, deleted_agent_dependency_names = \
                    irm.handle_existing_agent_dependencies_that_changed_in_update( self.repository,
                                                                                  original_valid_agent_dependencies_dict,
                                                                                  rvs.valid_agent_dependencies_dict )
            metadata_dict[ 'agent_dependencies' ] = rvs.valid_agent_dependencies_dict
        if rvs.invalid_agent_dependencies_dict:
            metadata_dict[ 'invalid_agent_dependencies' ] = rvs.invalid_agent_dependencies_dict
        if valid_repository_dependency_tups:
            metadata_dict = \
    ...
    def get_relative_path_to_repository_file( self, root, name, relative_install_dir, work_dir, shed_config_dict ):
        if self.resetting_all_metadata_on_repository:
            ...
            if relative_install_dir and \
                shed_config_dict.get( 'agent_path' ) and \
                    relative_path_to_file.startswith( os.path.join( shed_config_dict.get( 'agent_path' ), relative_install_dir ) ):
                relative_path_to_file = relative_path_to_file[ len( shed_config_dict.get( 'agent_path' ) ) + 1: ]
        return relative_path_to_file

    def get_sample_files_from_disk( self, repository_files_dir, agent_path=None, relative_install_dir=None ):
        if self.resetting_all_metadata_on_repository:
                ...
                for name in files:
                    if name.endswith( '.sample' ):
                        if self.resetting_all_metadata_on_repository:
                            ...
                            sample_file_copy_paths.append( relative_path_to_sample_file )
                            if agent_path and relative_install_dir:
                                if relative_path_to_sample_file.startswith( os.path.join( agent_path, relative_install_dir ) ):
                                    relative_path_to_sample_file = relative_path_to_sample_file[ len( agent_path ) + 1 :]
    ...
    def handle_repository_elem( self, repository_elem, only_if_compiling_contained_td=False ):
        ...
        repository_dependencies.xml file or a agent_dependencies.xml file.  If the former,
        we're generating repository dependencies metadata for a repository in the Agent Shed.
        If the latter, we're generating package dependency metadata within Galaxy or the
        Agent Shed.
        """
        is_valid = True
        error_message = ''
        agentshed = repository_elem.get( 'agentshed', None )
        ...
        if self.app.name == 'galaxy':
            if self.updating_installed_repository:
                ...
                if agentshed is None or name is None or owner is None or changeset_revision is None:
                    # Several packages exist in the Agent Shed that contain invalid repository
                    ...
                    # installation from proceeding.  Reaching here implies a bug in the Agent Shed
                    # framework.
        ...
        if not agentshed:
            # Default to the current agent shed.
            agentshed = str( url_for( '/', qualified=True ) ).rstrip( '/' )
            repository_dependency_tup[0] = agentshed
        user = None
        repository = None
        agentshed = common_util.remove_protocol_from_agent_shed_url( agentshed )
        if self.app.name == 'galaxy':
            # We're in Galaxy.  We reach here when we're generating the metadata for a agent
            ...
            if repository:
                ...
                # Send a request to the agent shed to retrieve appropriate additional changeset
                # revisions with which the repository
                # may have been installed.
                text = suc.get_updated_changeset_revisions_from_agent_shed( self.app,
                                                                           agentshed,
                                                                           name,
                                                                           owner,
                                                                           changeset_revision )
                ...
                # only if compiling the dependent repository's agent dependency.
                if not only_if_compiling_contained_td:
                    # We'll currently default to setting the repository dependency definition as invalid
                    # if an installed repository cannot be found.  This may not be ideal because the agent
                    # shed may have simply been inaccessible when metadata was being generated for the
                    # installed agent shed repository.
                    error_message = "Ignoring invalid repository dependency definition for agent shed %s, name %s, owner %s, " % \
                        ( agentshed, name, owner )
            ...
            # We're in the agent shed.
            if suc.agent_shed_is_this_agent_shed( agentshed ):
                try:
                    user = self.sa_session.query( self.app.model.User ) \
                                          .filter( self.app.model.User.table.c.username == owner ) \
                                          .one()
                except Exception:
                    error_message = "Ignoring repository dependency definition for agent shed %s, name %s, owner %s, " % \
                        ( agentshed, name, owner )
                ...
                # Repository dependencies are currently supported within a single agent shed.
                error_message = "Repository dependencies are currently supported only within the same agent shed.  Ignoring "
                error_message += "repository dependency definition  for agent shed %s, name %s, owner %s, changeset revision %s.  " % \
                    ( agentshed, name, owner, changeset_revision )
                log.debug( error_message )
                is_valid = False
                return repository_dependency_tup, is_valid, error_message
    ...
    def set_add_to_agent_panel_attribute_for_agent( self, agent, guid, datatypes ):
        """
        Determine if a agent should be loaded into the Galaxy agent panel.  Examples of valid agents that
        should not be displayed in the agent panel are datatypes converters and DataManager agents.
        """
        if hasattr( agent, 'agent_type' ):
            if agent.agent_type in [ 'manage_data' ]:
                # We have a DataManager agent.
                return False
        if datatypes:
            for datatype_dict in datatypes:
                ...
                # [{"converters":
                #    [{"target_datatype": "gff",
                #      "agent_config": "bed_to_gff_converter.xml",
    ...
    def update_repository_dependencies_metadata( self, metadata, repository_dependency_tups, is_valid, description ):
        ...
        for repository_dependency_tup in repository_dependency_tups:
            if is_valid:
                agent_shed, name, owner, changeset_revision, \
                    prior_installation_required, \
