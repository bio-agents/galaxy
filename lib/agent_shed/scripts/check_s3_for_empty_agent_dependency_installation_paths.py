...
from agent_shed.util.basic_util import INSTALLATION_LOG
...
class BucketList( object ):
    ...
    def __init__( self, amazon_id, amazon_secret, bucket ):
        ...
        self.install_dirs = self.get_agent_dependency_install_paths()
        self.empty_installation_paths = self.check_for_empty_agent_dependency_installation_paths()
    ...
    def get_agent_dependency_install_paths( self ):
        found_paths = []
        for item in self.bucket.list():
            name = str( item.name )
            # Skip environment_settings and __virtualenv_src, since these directories do not contain package agent dependencies.
            ...
            # present, we need to check the contents of that path. If not, then the agent dependency was completely
            # uninstalled.
            if len( paths ) >= 5:
    ...
    def check_for_empty_agent_dependency_installation_paths( self ):
        empty_directories = []
        for item in self.install_dirs:
            # Get all entries under the path for this agent dependency.
            contents = self.bucket.list( prefix=item )
            agent_dependency_path_contents = []
            ...
            for item in contents:
                agent_dependency_path_contents.append( item )
                # If there are more than two items in the path, we cannot safely assume that the dependency failed to
                # install correctly.
                if len( agent_dependency_path_contents ) > 2:
                    break
            # If the root directory is the only entry in the path, we have an empty agent dependency installation path.
            if len( agent_dependency_path_contents ) == 1:
                empty_directories.append( agent_dependency_path_contents[ 0 ] )
            ...
            elif len( agent_dependency_path_contents ) == 2 and \
                    agent_dependency_path_contents[1].name.endswith( INSTALLATION_LOG ):
                empty_directories.append( agent_dependency_path_contents[ 0 ] )
...
def main( args ):
    ...
    if len( dependency_cleaner.empty_installation_paths ) == 0:
        ...
        return 0
    print 'The following %d agent dependency installation paths were found to be empty or contain only the file %s.' % \
...
if __name__ == '__main__':
    description = 'Determine if there are any agent dependency installation paths that should be removed. Remove them if '
    ...
    parser.add_argument( '--bucket',
                         ...
                         action='store',
                         metavar='name',
                         help='The S3 bucket where agent dependencies are installed.' )
