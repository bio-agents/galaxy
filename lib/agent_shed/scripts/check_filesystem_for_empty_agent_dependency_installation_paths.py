...
def main( args ):
    ...
    if not os.path.exists( args.basepath ):
        print 'Agent dependency path %s does not exist.' % str( args.basepath )
        return 1
    print 'Checking path %s for empty agent dependency installation directories.' % args.basepath
    for root, dirs, files in os.walk( args.basepath ):
        ...
        if len( path_parts ) != 5:
            continue
        # We have a agent dependency installation path.
        no_dirs = False
        no_files = False
        if len( dirs ) == 0:
            no_dirs = True
    ...
    if len( empty_installation_paths ) > 0:
        print 'The following %d agent dependency installation directories were found to be empty or contain only the file %s.' % \
        ...
        print 'No empty agent dependency installation directories found.'
    return 0
