...
dependencies from the agent shed into a compressed archive.

Here is a working example of how to use this script to export a repository from the agent shed.
./export.py --url http://testagentshed.g2.bx.psu.edu --name chemicalagentbox --owner bgruening --revision 4133dbf7ff4d --export_repository_dependencies True --download_dir /tmp
"""
...
def generate_repository_archive_filename( agent_shed_url, name, owner, changeset_revision, file_type,
...
def remove_protocol_from_agent_shed_url( agent_shed_url ):
    protocol, base = agent_shed_url.split( '://' )
...
def main( options ):
    """Collect all user data and export the repository via the Agent Shed API."""
    base_agent_shed_url = options.agent_shed_url.rstrip( '/' )
    repositories_url = '%s/api/repositories' % base_agent_shed_url
    data = {}
    ...
    if repository_id:
        ...
        file_type = 'gz'
        url = '%s%s' % ( base_agent_shed_url, '/api/repository_revisions/%s/export' % str( repository_id ) )
        ...
        if error_messages:
            ...
            repositories_archive_filename = \
                generate_repository_archive_filename( base_agent_shed_url,
                                                      options.name,
                                                      options.owner,
        ...
        print "Invalid agent_shed / name / owner ."
