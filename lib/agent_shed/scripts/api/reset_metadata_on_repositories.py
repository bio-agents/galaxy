#!/usr/bin/env python
"""
Script to reset metadata on certain repositories in the Agent Shed.  If the received API key is associated
with an admin user in the Agent Shed, setting the my_writable param value to True will restrict resetting
metadata to only repositories that are writable by the user in addition to those repositories of type
agent_dependency_definition.  The my_writable param is ignored if the current user is not an admin user,
in which case this same restriction is automatic.

usage: reset_metadata_on_repositories.py key <my_writable>

Here is a working example of how to use this script to reset metadata on certain repositories in a specified Agent Shed.
...
def main( options ):
    ...
    if string_as_bool( one_per_request ):
        url = '%s/api/repositories/repository_ids_for_setting_metadata?key=%s&my_writable=%s' % ( base_agent_shed_url, api_key, str( my_writable ) )
        ...
        for repository_id in repository_ids:
            if repository_id in encoded_ids_to_skip:
                ...
                url = '%s/api/repositories/reset_metadata_on_repository' % base_agent_shed_url
                try:
        ...
        url = '%s/api/repositories/reset_metadata_on_repositories' % base_agent_shed_url
        try:
            submit( url, data, options.api )
        except Exception, e:
            log.exception( str( e ) )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser( description='Reset metadata on certain repositories in the Agent Shed via the Agent Shed API.' )
