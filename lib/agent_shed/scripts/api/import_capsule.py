#!/usr/bin/env python
"""
Import the contents of a repository capsule exported from a Agent Shed into another Agent Shed.  For each exported repository
archive contained in the capsule, inspect the Agent Shed to see if that repository already exists or if the current user is
...
Repositories are also associated with 1 or more categories in the Agent Shed from which the capsule was exported.  If any of
these categories are not contained in the Agent Shed to which the capsule is being imported, they will NOT be created by this
...
def main( options ):
    ...
    url = '%s/api/repositories/new/import_capsule' % base_agent_shed_url
    try:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser( description='Import the contents of a repository capsule via the Agent Shed API.' )
