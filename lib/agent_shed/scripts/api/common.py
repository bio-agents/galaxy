...
from agent_shed.util import common_util, hg_util
...
def delete( api_key, url, data, return_formatted=True ):
    ...
    'data' will become the JSON payload read by the Agent Shed.
    """
    try:
...
def get_api_url( base, parts=[], params=None ):
    """Compose and return a URL for the Agent Shed API."""
...
def get_repository_dict( url, repository_dict ):
    """
    Send a request to the Agent Shed to get additional information about the repository defined
...
def json_from_url( url ):
    """Send a request to the Agent Shed via the Agent Shed API and handle the response."""
    error_message = ''
