...
from agent_shed.util.common_util import url_join
...
def main():
    ...
    try:
        ...
        sys.exit( "Usage: python %s <agent shed .ini file> [options]" % sys.argv[ 0 ] )
...
def send_mail_to_owner( app, name, owner, email, repositories_deprecated, days=14 ):
    ...
    # Since there is no way to programmatically determine the URL for the agent shed from the .ini file, this method requires that
    # an environment variable named TOOL_SHED_CANONICAL_URL be set, pointing to the agent shed that is being checked.
    ...
    if None in [ smtp_server, from_address ]:
        ...
        return
    subject = "Regarding your agent shed repositories at %s" % url
    message_body_template = 'The agent shed automated repository checker has discovered that one or more of your repositories hosted ' + \
        'at this agent shed url ${url} have remained empty for over ${days} days, so they have been marked as deprecated. If you have plans ' + \
...
class DeprecateRepositoriesApplication( object ):
    ...
    def __init__( self, config ):
        ...
        self.model = galaxy.webapps.agent_shed.model.mapping.init( config.file_path, config.database_connection, engine_options={}, create_tables=False )
        self.config = config
