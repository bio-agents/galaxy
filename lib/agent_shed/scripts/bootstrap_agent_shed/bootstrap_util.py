...
import galaxy.webapps.agent_shed.model.mapping as agent_shed_model
...
def check_db( config_parser ):
    ...
    if config_parser.has_option( 'app:main', 'database_connection' ):
        ...
        sys.exit('The database configuration setting is missing from the agent_shed.ini file.  Add this setting before attempting to bootstrap.')

    sa_session = None

    database_exists_message = 'The database configured for this Agent Shed is not new, so bootstrapping is not allowed.  '
    database_exists_message += 'Create a new database that has not been migrated before attempting to bootstrap.'

    try:
        model = agent_shed_model.init( config_parser.get( 'app:main', 'file_path' ), dburi, engine_options={}, create_tables=False )
    ...
    if config_parser.has_option( 'app:main', 'hgweb_config_dir' ):
        ...
        if len( configured_repos ) >= 1:
            message = "This Agent Shed's hgweb.config file contains entries, so bootstrapping is not allowed.  Delete"
...
def admin_user_info( ):
    user_info_config = os.path.abspath( os.path.join( os.getcwd(), 'lib/agent_shed/scripts/bootstrap_agent_shed', 'user_info.xml' ) )
...
def get_local_agent_shed_url( config_parser ):
    port = '9009'
...
def main( args ):
    ...
    if args.method == 'check_db':
        ...
        return get_local_agent_shed_url( config_parser )
    else:
        return 1

parser = argparse.ArgumentParser()
parser.add_argument( '-c', '--config_file', dest='config', action='store', default='config/agent_shed.ini.sample' )
