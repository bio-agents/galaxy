...
class Folder( object ):
    ...
    def __init__( self, id=None, key=None, label=None, parent=None ):
        ...
        self.invalid_agent_dependencies = []
        self.invalid_agents = []
        ...
        self.agent_dependency_installation_errors = []
        self.agent_dependency_successful_installations = []
        self.valid_agents = []
...
class InvalidAgent( object ):
    """Invalid agent object"""

    def __init__( self, id=None, agent_config=None, repository_id=None, changeset_revision=None, repository_installation_status=None ):
        self.id = id
...
class RepositoryDependency( object ):
    ...
    def __init__( self, id=None, agentshed=None, repository_name=None, repository_owner=None, changeset_revision=None, prior_installation_required=False,
                  only_if_compiling_contained_td=False, installation_status=None, agent_shed_repository_id=None ):
        self.id = id
...
class Agent( object ):
    ...
    def __init__( self, id=None, agent_config=None, agent_id=None, name=None, description=None, version=None, requirements=None,
...
class AgentDependency( object ):
    ...
    def __init__( self, id=None, name=None, version=None, type=None, readme=None, installation_status=None, repository_id=None,
        ...
        self.installation_status = installation_status
        self.repository_id = repository_id
        self.agent_dependency_id = agent_dependency_id
...
class Workflow( object ):
    ...
    def __init__( self, id=None, workflow_name=None, steps=None, format_version=None, annotation=None,
        ...
        """
        When rendered in the agent shed, repository_metadata_id will have a value and repository_id will
...
class UtilityContainerManager( object ):
    ...
    def build_invalid_agents_folder( self, folder_id, invalid_agent_configs, changeset_revision, repository=None,
                                    label='Invalid agents' ):
        """Return a folder hierarchy containing invalid agents."""
        # TODO: Should we display invalid agents on the agent panel selection page when installing the
        # repository into Galaxy?
        if invalid_agent_configs:
            invalid_agent_id = 0
            folder_id += 1
            invalid_agents_root_folder = Folder( id=folder_id, key='root', label='root', parent=None )
            folder_id += 1
            folder = Folder( id=folder_id, key='invalid_agents', label=label, parent=invalid_agents_root_folder )
            invalid_agents_root_folder.folders.append( folder )
            for invalid_agent_config in invalid_agent_configs:
                ...
                if repository:
                    ...
                    repository_id = None
                    repository_installation_status = None
                invalid_agent = InvalidAgent( id=invalid_agent_id,
                                            agent_config=invalid_agent_config,
                                            repository_id=repository_id,
                                            changeset_revision=changeset_revision,
                                            repository_installation_status=repository_installation_status )
                folder.invalid_agents.append( invalid_agent )
        else:
            invalid_agents_root_folder = None
        return folder_id, invalid_agents_root_folder
    ...
    def build_agents_folder( self, folder_id, agent_dicts, repository, changeset_revision, valid=True, label='Valid agents' ):
        """Return a folder hierarchy containing valid agents."""
        if agent_dicts:
            container_object_agent_id = 0
            folder_id += 1
            agents_root_folder = Folder( id=folder_id, key='root', label='root', parent=None )
            folder_id += 1
            folder = Folder( id=folder_id, key='agents', label=label, parent=agents_root_folder )
            if self.app.name == 'galaxy':
                folder.description = 'click the name to inspect the agent metadata'
            agents_root_folder.folders.append( folder )
            # Insert a header row.
            container_object_agent_id += 1
            agent = Agent( id=container_object_agent_id,
                         agent_config='',
                         agent_id='',
                         name='Name',
                         ...
                         repository_id='',
                         changeset_revision='' )
            folder.valid_agents.append( agent )
            if repository:
                repository_id = repository.id
                if self.app.name == 'galaxy':
                    repository_installation_status = repository.status
            ...
            for agent_dict in agent_dicts:
                if not isinstance( agent_dict, dict ):
                    # Due to some previous bug (hopefully not current) invalid agent strings may be included in the received
                    # list of agent_dicts.  For example, the picard repository metadata has 2 invalid agents in the received
                    # list of supposedly valid agents:  'rgPicardASMetrics.xml', 'rgPicardGCBiasMetrics.xml'.
                    continue
                container_object_agent_id += 1
                requirements = agent_dict.get( 'requirements', None )
                ...
                try:
                    agent_config = str( agent_dict.get( 'agent_config', 'missing' ) )
                    agent_id = str( agent_dict.get( 'id', 'unknown' ) )
                    name = str( agent_dict.get( 'name', 'unknown' ) )
                    description = str( agent_dict.get( 'description', '' ) )
                    version = str( agent_dict.get( 'version', 'unknown' ) )
                except Exception, e:
                    agent_config = str( e )
                    agent_id = 'unknown'
                    name = 'unknown'
            ...
            agents_root_folder = None
        return folder_id, agents_root_folder

    def build_agent_dependencies_folder( self, folder_id, agent_dependencies, label='Agent dependencies', missing=False,
                                        new_install=False, reinstalling=False ):
        """Return a folder hierarchy containing agent dependencies."""
        # When we're in Galaxy (not the agent shed) and the agent dependencies are not installed or are in an error state,
        # they are considered missing.  The agent dependency status will be displayed only if a record exists for the agent
        # dependency in the Galaxy database, but the agent dependency is not installed.  The value for new_install will be
        ...
        if agent_dependencies:
            agent_dependency_id = 0
            folder_id += 1
            agent_dependencies_root_folder = Folder( id=folder_id, key='root', label='root', parent=None )
            folder_id += 1
            folder = Folder( id=folder_id, key='agent_dependencies', label=label, parent=agent_dependencies_root_folder )
            if self.app.name == 'galaxy':
                if new_install or reinstalling:
                    folder.description = "repository agents require handling of these dependencies"
            ...
            agent_dependencies_root_folder.folders.append( folder )
            # Insert a header row.
            agent_dependency_id += 1
            if self.app.name == 'galaxy':
                agent_dependency = AgentDependency( id=agent_dependency_id,
                                                  name='Name',
                                                  version='Version',
                                                  type='Type',
                                                  readme=None,
                ...
                agent_dependency = AgentDependency( id=agent_dependency_id,
                                                  ...
                                                  repository_id=None,
                                                  agent_dependency_id=None )
            folder.agent_dependencies.append( agent_dependency )
                ...
                if dependency_key in [ 'set_environment' ]:
                    for set_environment_dict in requirements_dict:
                        try:
                            ...
                            td_id = set_environment_dict.get( 'agent_dependency_id', None )
                        except Exception, e:
                            name = str( e )
                            type = 'unknown'
                    ...
                    try:
                        ...
                        td_id = requirements_dict.get( 'agent_dependency_id', None )
                    except Exception, e:
                        name = str( e )
                        version = 'unknown'
                        type = 'unknown'
        ...
        return folder_id, agent_dependencies_root_folder
    ...
    def build_workflows_folder( self, folder_id, workflows, repository_metadata_id=None, repository_id=None,
        ...
        received workflows list.  When this method is called from the agent shed, repository_metadata_id
    ...
    def get_components_from_repository_dependency_for_installed_repository( self, repository_dependency ):
        ...
        if len( repository_dependency ) == 6:
            ...
            agent_shed_repository_id = repository_dependency[ 4 ]
            installation_status = repository_dependency[ 5 ]
            agent_shed, name, owner, changeset_revision = repository_dependency[ 0:4 ]
            repository_dependency = [ agent_shed, name, owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td ]
            ...
            agent_shed_repository_id = repository_dependency[ 5 ]
            installation_status = repository_dependency[ 6 ]
            agent_shed, name, owner, changeset_revision, prior_installation_required = repository_dependency[ 0:5 ]
            ...
            agent_shed_repository_id = repository_dependency[ 6 ]
        ...
        if agent_shed_repository_id:
            agent_shed_repository = suc.get_agent_shed_repository_by_id( self.app,
                                                                       self.app.security.encode_id( agent_shed_repository_id ) )
            if agent_shed_repository:
                if agent_shed_repository.missing_repository_dependencies:
                    ...
                    installation_status = '%s, missing agent dependencies' % installation_status
        return agent_shed_repository_id, installation_status, repository_dependency
    ...
    def handle_repository_dependencies_container_entry( self, repository_dependencies_folder, rd_key, rd_value, folder_id,
        ...
        agentshed, repository_name, repository_owner, changeset_revision = components_list[ 0:4 ]
        ...
        for repository_dependency in rd_value:
            if self.app.name == 'galaxy':
                agent_shed_repository_id, installation_status, repository_dependency = \
            ...
            if can_create_dependency:
                agentshed, repository_name, repository_owner, changeset_revision, prior_installation_required, only_if_compiling_contained_td = \
    ...
    def key_is_current_repositorys_key( self, repository_name, repository_owner, changeset_revision,
        ...
        components_list = suc.extract_components_from_tuple( repository_components_tuple )
        agentshed, key_name, key_owner, key_changeset_revision = components_list[ 0:4 ]
        # For backward compatibility to the 12/20/12 Galaxy release.
        if len( components_list ) == 4:
