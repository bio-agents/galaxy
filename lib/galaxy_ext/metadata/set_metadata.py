...
def set_meta_with_agent_provided( dataset_instance, file_dict, set_meta_kwds, datatypes_registry ):
    # This method is somewhat odd, in that we set the metadata attributes from agent,
    # then call set_meta, then set metadata attributes from agent again.
    ...
    # values provided by the agent as Truth.
...
def set_metadata():
    ...
    galaxy.datatypes.metadata.MetadataTempFile.tmp_dir = agent_job_working_directory = os.path.abspath(os.getcwd())
    ...
    for filenames in sys.argv[1:]:
        ...
        try:
            ...
            files_path = os.path.abspath(os.path.join( agent_job_working_directory, "dataset_%s_files" % (dataset.dataset.id) ))
            ...
            set_meta_with_agent_provided( dataset, file_dict, set_meta_kwds, datatypes_registry )
    ...
    for i, ( filename, file_dict ) in enumerate( new_job_metadata_dict.iteritems(), start=1 ):
        new_dataset = galaxy.model.Dataset( id=-i, external_filename=os.path.join( agent_job_working_directory, file_dict[ 'filename' ] ) )
        extra_files = file_dict.get( 'extra_files', None )
        if extra_files is not None:
            new_dataset._extra_files_path = os.path.join( agent_job_working_directory, extra_files )
        ...
        set_meta_with_agent_provided( new_dataset_instance, file_dict, set_meta_kwds, datatypes_registry )
