...
log = logging.getLogger( "agents.genomespace.genomespace_exporter" )
...
# (6 on reload) when a user loads the agent interface
...
def get_cookie_opener( gs_username, gs_token, gs_agentname=None ):
    ...
    cookie_opener.addheaders.append( ( 'gs-agentname', gs_agentname or DEFAULT_GENOMESPACE_TOOLNAME ) )
...
def get_genome_space_launch_apps( atm_url, url_opener, file_url, file_type ):
    gs_request = urllib2.Request( "%s/%s/webagent/descriptor" % ( atm_url, GENOMESPACE_API_VERSION_STRING ) )
    gs_request.get_method = lambda: 'GET'
    opened_gs_request = url_opener.open( gs_request )
    webagent_descriptors = json.loads( opened_gs_request.read() )
    webagents = []
    for webagent in webagent_descriptors:
        webagent_name = webagent.get( 'name' )
        base_url = webagent.get( 'baseUrl' )
        use_agent = False
        for param in webagent.get( 'fileParameters', [] ):
            ...
            if use_agent:
                ...
                webagents.append( ( launch_url, webagent_name ) )
                break
    return webagents
...
def galaxy_code_get_genomespace_folders( genomespace_site='prod', trans=None, value=None, base_url=None, **kwd ):
    ...
    def recurse_directory_dict( url_opener, cur_options, url ):
        ...
        try:
            ...
            log.debug( 'GenomeSpace export agent failed reading a directory "%s": %s' % ( url, e ) )
    ...
    if trans and trans.user:
        ...
        if None not in ( username, token ):
            ...
            # in the middle of interacting with agents, so we'll have several layers of caching by ids/values
            if trans.user in GENOMESPACE_DIRECTORIES_BY_USER:
                if username in GENOMESPACE_DIRECTORIES_BY_USER[ trans.user ]:
                    ...
                    del GENOMESPACE_DIRECTORIES_BY_USER[ trans.user ]
            if not rval:
                url_opener = get_cookie_opener( username, token, gs_agentname=os.environ.get( 'GENOMESPACE_TOOLNAME', None ) )
...
def send_file_to_genomespace( genomespace_site, username, token, source_filename, target_directory, target_filename, file_type, content_type, log_filename, gs_agentname ):
    target_filename = target_filename.replace( '/', '-' )  # Slashes no longer allowed in filenames
    url_opener = get_cookie_opener( username, token, gs_agentname=gs_agentname )
    genomespace_site_dict = get_genomespace_site_urls()[ genomespace_site ]
    dm_url = genomespace_site_dict['dmServer']
    ...
    web_agents = get_genome_space_launch_apps( genomespace_site_dict['atmServer'], url_opener, result_url, file_type )
    if log_filename:
...
if __name__ == '__main__':
    ...
    parser.add_option( '', '--genomespace_agentname', dest='genomespace_agentname', action='store', type="string", default=DEFAULT_GENOMESPACE_TOOLNAME, help='value to use for gs-agentname, used in GenomeSpace internal logging' )

    (options, args) = parser.parse_args()

    send_file_to_genomespace( options.genomespace_site, options.username, options.token, options.dataset, map( binascii.unhexlify, options.subdirectory ), binascii.unhexlify( options.filename ), options.file_type, options.content_type, options.log, options.genomespace_agentname )
