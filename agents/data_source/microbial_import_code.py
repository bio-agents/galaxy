...
from galaxy import datatypes, config, agents
from shutil import copyfile

def exec_after_process(app, inp_data, out_data, param_dict, agent, stdout, stderr):
    ...
    #workflow passes galaxy.agents.parameters.basic.UnvalidatedValue instead of values
    if isinstance( kingdom, agents.parameters.basic.UnvalidatedValue ):
        kingdom = kingdom.value
    if isinstance( org, agents.parameters.basic.UnvalidatedValue ):
        org = org.value
    
    GALAXY_DATA_INDEX_DIR = app.config.agent_data_path
