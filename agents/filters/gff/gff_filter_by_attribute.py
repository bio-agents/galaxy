#!/usr/bin/env python
# This agent takes a gff file as input and creates filters on attributes based on certain properties.
# The agent will skip over invalid lines within the file, informing the user about the number of lines skipped.
# TODO: much of this code is copied from the Filter1 agent (filtering.py in agents/stats/). The commonalities should be
# abstracted and leveraged in each filtering agent.
...
try:
    ...
    if str( e ).startswith( 'invalid syntax' ):
        valid_filter = False
        stop_err( 'Filter condition "%s" likely invalid. See agent tips, syntax and examples.' % cond_text )
    else:
        stop_err( str( e ) )
...
if valid_filter:
    ...
    if valid_lines > 0:
        ...
        print 'Possible invalid filter condition "%s" or non-existent column referenced. See agent tips, syntax and examples.' % cond_text
    if skipped_lines > 0:
