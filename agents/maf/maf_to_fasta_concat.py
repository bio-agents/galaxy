...
def __main__():
    try:
        ...
        maf_utilities.agent_fail( "Error determining species value: %s" % e )
    try:
        input_filename = sys.argv[2]
    except Exception, e:
        maf_utilities.agent_fail( "Error reading MAF filename: %s" % e )
    try:
        file_out = open( sys.argv[3], 'w' )
    except Exception, e:
        maf_utilities.agent_fail( "Error opening file for output: %s" % e )
    ...
    if not species:
        try:
            ...
            maf_utilities.agent_fail( "Error determining species in input MAF: %s" % e )
    ...
    for spec in species:
        ...
        try:
            ...
            maf_utilities.agent_fail( "Your MAF file appears to be malformed: %s" % e )
        file_out.write( "\n" )
    file_out.close()
