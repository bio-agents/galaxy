...
from galaxy.agents.util import maf_utilities
...
def __main__():
    ...
    if options.dbkey:
        ...
        dbkey = None
    if dbkey in [None, "?"]:
        maf_utilities.agent_fail( "You must specify a proper build in order to extract alignments. You can specify your genome build by clicking on the pencil icon associated with your interval file." )
    ...
    if options.chromCol:
        ...
        maf_utilities.agent_fail( "Chromosome column not set, click the pencil icon in the history item to set the metadata attributes." )

    if options.startCol:
        startCol = int( options.startCol ) - 1
    else:
        maf_utilities.agent_fail( "Start column not set, click the pencil icon in the history item to set the metadata attributes." )

    if options.endCol:
        endCol = int( options.endCol ) - 1
    else:
        maf_utilities.agent_fail( "End column not set, click the pencil icon in the history item to set the metadata attributes." )
    ...
    if options.interval_file:
        ...
        maf_utilities.agent_fail( "Input interval file has not been specified." )

    if options.output_file:
        output_file = options.output_file
    else:
        maf_utilities.agent_fail( "Output file has not been specified." )
    ...
    if options.mafType:
        ...
        if index is None:
            maf_utilities.agent_fail( "The MAF source specified (%s) appears to be invalid." % ( options.mafType ) )
        ...
        if index is None:
            maf_utilities.agent_fail( "Your MAF file appears to be malformed." )
    else:
        maf_utilities.agent_fail( "Desired source MAF type has not been specified." )
