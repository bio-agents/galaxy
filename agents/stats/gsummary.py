...
def main():
    ...
    if len( symbols ) == 1 and ',' in symbols:
        ...
        stop_err( "Invalid columns '%s': this agent requires a single column or expression" % expression )
    ...
    if skipped_lines == i + 1:
        stop_err( "Invalid column or column data values invalid for computation.  See agent tips and syntax for data requirements." )
    else:
        ...
        if skipped_lines:
            print "Skipped %d invalid lines beginning with line #%d.  See agent tips for data requirements." % ( skipped_lines, first_invalid_line )
