...
show_help() {
cat <<EOF
'${0##*/} (test_path)'              for testing all the agents in functional directory
'${0##*/} -id bbb'                  for testing one agent with id 'bbb' ('bbb' is the agent id)
'${0##*/} -sid ccc'                 for testing one section with sid 'ccc' ('ccc' is the string after 'section::')
'${0##*/} -list'                    for listing all the agent ids
'${0##*/} -api (test_path)'         for running all the test scripts in the ./test/api directory
'${0##*/} -agentshed (test_path)'    for running all the test scripts in the ./test/agent_shed/functional directory
'${0##*/} -workflow test.xml'       for running a workflow test case as defined by supplied workflow xml test file (experimental)
'${0##*/} -installed'               for running tests of Agent Shed installed agents
'${0##*/} -framework'               for running through example agent tests testing framework features in test/functional/agents"
'${0##*/} -framework -id agentid'    for testing one framework agent (in test/functional/agents/) with id 'agentid'
...
Run a specific API test:
    ./run_tests.sh -api test/api/test_agents.py:AgentsTestCase.test_map_over_with_output_format_actions


Extra options:
 --verbose_errors      Force some tests produce more verbose error reporting.
 --no_cleanup          Do not delete temp files for Python functional tests (-agentshed, -framework, etc...)
...
show_list() {
    ...
    echo "==========================================================================================================================================="
    echo "'${0##*/} -id bbb'               for testing one agent with id 'bbb' ('bbb' is the agent id)"
...
with_framework_test_agents_arg=""
...
do
    case "$1" in
      ...
      -s|-sid|--sid)
          ...
          ;;
    -a|-api|--api)
          with_framework_test_agents_arg="-with_framework_test_agents"
          test_script="./scripts/functional_tests.py"
          ...
          ;;
      -t|-agentshed|--agentshed)
          test_script="./test/agent_shed/functional_tests.py"
          report_file="run_agentshed_tests.html"
          if [ $# -gt 1 ]; then
              agentshed_script=$2
              shift 2
          else
              agentshed_script="./test/agent_shed/functional"
              shift 1
          fi
          ;;
      -with_framework_test_agents|--with_framework_test_agents)
          with_framework_test_agents_arg="-with_framework_test_agents"
          shift
          ;;
      -w|-workflow|--workflow)
...
if [ -n "$migrated_test" ] ; then
    [ -n "$test_id" ] && class=":TestForAgent_$test_id" || class=""
    extra_args="functional.test_agentbox$class -migrated"
elif [ -n "$installed_test" ] ; then
    [ -n "$test_id" ] && class=":TestForAgent_$test_id" || class=""
    extra_args="functional.test_agentbox$class -installed"
elif [ -n "$framework_test" ] ; then
    [ -n "$test_id" ] && class=":TestForAgent_$test_id" || class=""
    extra_args="functional.test_agentbox$class -framework"
elif [ -n "$data_managers_test" ] ; then
    [ -n "$test_id" ] && class=":TestForDataManagerAgent_$test_id" || class=""
...
elif [ -n "$agentshed_script" ]; then
    extra_args="$agentshed_script"
    ...
    extra_args=`python agent_list.py $section_id`
elif [ -n "$test_id" ]; then
    class=":TestForAgent_$test_id"
    extra_args="functional.test_agentbox$class"
...
if [ "$driver" = "python" ]; then
    ...
    python $test_script $coverage_arg -v --with-nosehtml --html-report-file $report_file $xunit_args $structured_data_args $with_framework_test_agents_arg $extra_args
else
    ensure_grunt
    if [ -n "$watch" ]; then
        grunt_task="watch"
