...
# what agents to run - not so pretty
agents = {
...
class Browser:
    ...
    def upload(self, input):
        self.get("/agent_runner/index?agent_id=upload1")
    ...
    def runagent(self):
        self.get("/agent_runner/index?agent_id=%s" % self.agent)
        for k, v in self.agent_opts.items():
            tc.fv("1", k, v)
        tc.submit("runagent_btn")
        tc.code(200)
    ...
    def wait(self):
        ...
        while count < maxiter:
            ...
            if not self.history_state_terminal:
                ...
                break
        if count == maxiter:
            raise Exception("Agent never finished")
    ...
    def diff(self):
        self.get("/datasets/%s/display?to_ext=%s" % (self.hda_id, self.agent_opts.get('out_format', 'fasta')))
        ...
        if filecmp.cmp(tmp[1], self.check_file):
            dprint("Agent output is as expected")
        else:
            if not debug:
                os.remove(tmp[1])
            raise Exception("Agent output differs from expected")
        if not debug:
            os.remove(tmp[1])
...
if __name__ == "__main__":
    ...
    for agent, params in agents.iteritems():
        ...
        # get all the agent run conditions
        for dict in params:
            for k, v in dict.items():
                if k == 'inputs':
                    ...
                    b.check_file = v
                elif k == 'agent_run_options':
                    b.agent_opts = v
                else:
                    raise Exception("Unknown key in agents dict: %s" % k)
