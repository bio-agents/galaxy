...
then
    ...
    TOOL_SHED_TEST_DBURI="postgres://root@localhost:5930/agentshed?client_encoding=utf8"
...
then
    ...
    TOOL_SHED_TEST_DBURI="mysql://galaxy:galaxy@localhost/agentshed?unix_socket=/var/run/mysqld/mysqld.sock"
elif [ "$GALAXY_TEST_DATABASE_TYPE" = "sqlite" ];
then
    GALAXY_TEST_INSTALL_DB_MERGED="true"
    GALAXY_TEST_DBURI="sqlite:////opt/galaxy/galaxy.sqlite"
    TOOL_SHED_TEST_DBURI="sqlite:////opt/galaxy/agentshed.sqlite"
else
...
then
    ...
    TOOL_SHED_CONFIG_FILE=${GALAXY_CONFIG_FILE:-config/agent_shed.ini.sample}
