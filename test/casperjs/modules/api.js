...
// =================================================================== TOOLS
var AgentsAPI = function AgentsAPI( api ){
    this.api = api;
};
AgentsAPI.prototype.toString = function toString(){
    return this.api + '.AgentsAPI';
};

// -------------------------------------------------------------------
AgentsAPI.prototype.urlTpls = {
    index   : '/api/agents',
    show    : '/api/agents/%s',
    create  : '/api/agents'
};

AgentsAPI.prototype.index = function index( in_panel, trackster ){
    this.api.spaceghost.info( 'agents.index: ' + [ in_panel, trackster ] );
    var data = {};
...
AgentsAPI.prototype.show = function show( id ){
    this.api.spaceghost.info( 'agents.show: ' + [ id ] );
    var data = {};
...
AgentsAPI.prototype.create = function create( payload ){
    this.api.spaceghost.info( 'agents.create: ' + [ this.api.spaceghost.jsonStr( payload ) ] );
...
//AgentsAPI.prototype.uploadByForm = function upload( historyId, options ){
//    this.api.spaceghost.debug( '-------------------------------------------------' );
//    this.api.spaceghost.info( 'agents.upload: ' + [ historyId, '(contents)', this.api.spaceghost.jsonStr( options ) ] );
...
AgentsAPI.prototype.uploadByPaste = function upload( historyId, options ){
    this.api.spaceghost.info( 'agents.upload: ' + [ historyId, this.api.spaceghost.jsonStr( options ) ] );
    this.api.ensureId( historyId );
    options = options || {};
...
/** post a file to the upload1 agent over ajax */
AgentsAPI.prototype.upload = function upload( historyId, options ){
    this.api.spaceghost.info( 'api.agents.upload: ' + [ historyId, this.api.spaceghost.jsonStr( options ) ] );
    this.api.ensureId( historyId );
    options = options || {};
    ...
    var response = this.api.spaceghost.evaluate( function( url, historyId, inputs ){
        ...
        formData.append( 'agent_id', 'upload1' );
...
AgentsAPI.prototype.DEFAULT_UPLOAD_TIMEOUT = 30 * 1000;

/** add two casperjs steps - upload a file, wait for the job to complete, and run 'then' when they are */
AgentsAPI.prototype.thenUpload = function thenUpload( historyId, options, then ){
    var spaceghost = this.api.spaceghost,
        uploadedId;

    // upload via the api
    spaceghost.then( function(){
        var returned = this.api.agents.upload( historyId, options );
    ...
    spaceghost.then( function(){
        ...
        this.waitFor(
            ...
            options.timeout || spaceghost.api.agents.DEFAULT_UPLOAD_TIMEOUT
        );
    });
    return spaceghost;
};
...
AgentsAPI.prototype.thenUploadMultiple = function thenUploadMultiple( historyId, optionsArray, then ){
    var spaceghost = this.api.spaceghost,
        uploadedIds = [];
    ...
    this.api.spaceghost.then( function(){
        ...
        optionsArray.forEach( function( options ){
            var returned = spaceghost.api.agents.upload( historyId, options );
    ...
    this.api.spaceghost.then( function(){
        ...
        this.waitFor(
            ...
            ( options.timeout || spaceghost.api.agents.DEFAULT_UPLOAD_TIMEOUT ) * optionsArray.length
        );
    });
    return spaceghost;
};
...
AgentsAPI.prototype.thenUploadToCurrent = function thenUploadToCurrent( options, then ){
    var spaceghost = this.api.spaceghost;
    return spaceghost.then( function(){
        var currentHistoryId = this.api.histories.index()[0].id;
        spaceghost.api.agents.thenUpload( currentHistoryId, options, then );
    });
};
