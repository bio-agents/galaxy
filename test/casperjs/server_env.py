...
class TestEnvironment( object ):
    ...
    def __init__( self, env_config_dict=None ):
        ...
        self.agent_shed_test_file = self._get_setting_from_config_or_env(
            'agent_shed_test_file', self.ENV_TOOL_SHED_TEST_FILE )
        self.shed_agents_dict = self._get_shed_agents_dict()
    ...
    def as_dict( self, attributes=None ):
        if not attributes:
            ...
            attributes = [ 'protocol', 'host', 'port', 'history_id', 'file_dir',
                           'agent_shed_test_file', 'shed_agents_dict', 'saved_output_dir', 'debug_these_tests' ]
        this_dict = {}
    ...
    def _get_shed_agents_dict( self ):
        """Read the shed agents from the agent shed test file if given,
        ...
        if self.agent_shed_test_file:
            try:
                ...
                text = f.read()
                f.close()
                shed_agents_dict = loads( text )
            except Exception, exc:
                log.error( 'Error reading agent shed test file "%s": %s', self.agent_shed_test_file, exc, exc_info=True )

        return shed_agents_dict
