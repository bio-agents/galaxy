...
filtering_repository_description = "Galaxy's filtering agent for test 0520"
filtering_repository_long_description = "Long description of Galaxy's filtering agent for test 0520"
freebayes_repository_name = 'freebayes_0520'
freebayes_repository_description = "Galaxy's freebayes agent"
freebayes_repository_long_description = "Long description of Galaxy's freebayes agent for test 0520"
...
1) Upload a capsule with 2 repositories that define simple repository dependencies on each other, resulting in circular
   dependencies to a agent shed.
2) Make sure each repository contains an invalid repository dependency after the capsule has been uploaded (they should
   be invalid because their agentshed and changeset_revision attributes could not be auto-populated).
...
class TestExportImportRepository( ShedTwillTestCase ):
    ...
    def test_0005_import_repository_capsule( self ):
        ...
        circular dependencies to a agent shed.
        '''
        self.logout()
        self.login( email=common.admin_email, username=common.admin_username )
    ...
    def test_0010_verify_invalid_dependency( self ):
        ...
        (they should be invalid because their agentshed and changeset_revision attributes could not be auto-populated).
        '''
