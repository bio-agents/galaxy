...
category_name = 'Test 1420 Agent dependency environment variable inheritance'
...
package_bzlib_repository_description = 'Contains a agent dependency definition that downloads and compiles version 1.0 of the bzlib library.'
package_boost_repository_description = 'Contains a agent dependency definition that downloads and compiles version 1.53 of the boost C++ libraries'
package_numpy_repository_description = 'Contains a agent dependency definition that downloads and compiles version 1.7 of the the python numpy package'
package_rdkit_repository_description = 'Contains a agent dependency definition that downloads and compiles version 2012-12 of the RDKit cheminformatics and machine-learning package.'
...
class TestEnvironmentInheritance( ShedTwillTestCase ):
    '''Test referencing environment variables that were defined in a separate agent dependency.'''
    ...
    def test_0005_create_lapack_repository( self ):
        ...
        All agent dependency definitions should download and extract a tarball containing precompiled binaries from the local
        ...
        repository = self.get_or_create_repository( name=package_lapack_repository_name,
                                                    ...
                                                    strings_displayed=[] )
        # Load the original agent dependency definition into memory, then fill in the __PATH__ placeholder with the
        # actual system path where the binary tarball is found.
        agent_dependency_path = self.generate_temp_path( '1420_agent_dependency', additional_paths=[ 'package_lapack_3_4_1420' ] )
        precompiled_binary_tarball = self.get_filename( '1420_files/binary_tarballs/lapack.tar' )
        edited_agent_dependency_filename = self.get_filename( filepath=agent_dependency_path, filename='agent_dependencies.xml' )
        original_agent_dependency = self.get_filename( '1420_files/package_lapack_3_4_1420/agent_dependencies.xml' )
        agent_dependency_definition = file( original_agent_dependency, 'r' ).read().replace( '__PATH__', precompiled_binary_tarball )
        file( edited_agent_dependency_filename, 'w' ).write( agent_dependency_definition )
        # Upload the edited agent dependency definition to the package_lapack_3_4_1420 repository.
        self.upload_file( repository,
                          filename='agent_dependencies.xml',
                          filepath=agent_dependency_path,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_lapack_3_4_1420 with agent dependency definitions.',
                          strings_displayed=[],
    ...
    def test_0010_create_atlas_repository( self ):
        ...
        # Load the original agent dependency definition into memory, then fill in the __PATH__ placeholder with the
        # actual system path where the binary tarball is found.
        agent_dependency_path = self.generate_temp_path( '1420_agent_dependency', additional_paths=[ 'package_atlas_3_10_1420' ] )
        precompiled_binary_tarball = self.get_filename( '1420_files/binary_tarballs/atlas.tar' )
        edited_agent_dependency_filename = self.get_filename( filepath=agent_dependency_path, filename='agent_dependencies.xml' )
        original_agent_dependency = self.get_filename( '1420_files/package_atlas_3_10_1420/agent_dependencies.xml' )
        agent_dependency_definition = file( original_agent_dependency, 'r' ).read().replace( '__PATH__', precompiled_binary_tarball )
        file( edited_agent_dependency_filename, 'w' ).write( agent_dependency_definition )
        # Upload the edited agent dependency definition to the package_atlas_3_10_1420 repository.
        self.upload_file( repository,
                          filename='agent_dependencies.xml',
                          filepath=agent_dependency_path,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_atlas_3_10_1420 with agent dependency definitions.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0015_create_bzlib_repository( self ):
        ...
        # Load the original agent dependency definition into memory, then fill in the __PATH__ placeholder with the
        # actual system path where the binary tarball is found.
        agent_dependency_path = self.generate_temp_path( '1420_agent_dependency', additional_paths=[ 'package_bzlib_1_0_1420' ] )
        precompiled_binary_tarball = self.get_filename( '1420_files/binary_tarballs/bzlib.tar' )
        edited_agent_dependency_filename = self.get_filename( filepath=agent_dependency_path, filename='agent_dependencies.xml' )
        original_agent_dependency = self.get_filename( '1420_files/package_bzlib_1_0_1420/agent_dependencies.xml' )
        agent_dependency_definition = file( original_agent_dependency, 'r' ).read().replace( '__PATH__', precompiled_binary_tarball )
        file( edited_agent_dependency_filename, 'w' ).write( agent_dependency_definition )
        # Upload the edited agent dependency definition to the package_bzlib_1_0_1420 repository.
        self.upload_file( repository,
                          filename='agent_dependencies.xml',
                          filepath=agent_dependency_path,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_bzlib_1_0_1420 with agent dependency definitions.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0020_create_boost_repository( self ):
        ...
        # Load the original agent dependency definition into memory, then fill in the __PATH__ placeholder with the
        # actual system path where the binary tarball is found.
        agent_dependency_path = self.generate_temp_path( '1420_agent_dependency', additional_paths=[ 'package_boost_1_53_1420' ] )
        precompiled_binary_tarball = self.get_filename( '1420_files/binary_tarballs/boost.tar' )
        edited_agent_dependency_filename = self.get_filename( filepath=agent_dependency_path, filename='agent_dependencies.xml' )
        original_agent_dependency = self.get_filename( '1420_files/package_boost_1_53_1420/agent_dependencies.xml' )
        agent_dependency_definition = file( original_agent_dependency, 'r' ).read().replace( '__PATH__', precompiled_binary_tarball )
        file( edited_agent_dependency_filename, 'w' ).write( agent_dependency_definition )
        # Upload the edited agent dependency definition to the package_boost_1_53_1420 repository.
        self.upload_file( repository,
                          filename='agent_dependencies.xml',
                          filepath=agent_dependency_path,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_boost_1_53_1420 with agent dependency definitions.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0025_create_numpy_repository( self ):
        ...
        # Load the original agent dependency definition into memory, then fill in the __PATH__ placeholder with the
        # actual system path where the binary tarball is found.
        agent_dependency_path = self.generate_temp_path( '1420_agent_dependency', additional_paths=[ 'package_numpy_1_7_1420' ] )
        precompiled_binary_tarball = self.get_filename( '1420_files/binary_tarballs/numpy.tar' )
        edited_agent_dependency_filename = self.get_filename( filepath=agent_dependency_path, filename='agent_dependencies.xml' )
        original_agent_dependency = self.get_filename( '1420_files/package_numpy_1_7_1420/agent_dependencies.xml' )
        agent_dependency_definition = file( original_agent_dependency, 'r' ).read().replace( '__PATH__', precompiled_binary_tarball )
        file( edited_agent_dependency_filename, 'w' ).write( agent_dependency_definition )
        # Upload the edited agent dependency definition to the package_numpy_1_7_1420 repository.
        self.upload_file( repository,
                          filename='agent_dependencies.xml',
                          filepath=agent_dependency_path,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_numpy_1_7_1420 with agent dependency definitions.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0030_create_rdkit_repository( self ):
        ...
        # Load the original agent dependency definition into memory, then fill in the __PATH__ placeholder with the
        # actual system path where the binary tarball is found.
        agent_dependency_path = self.generate_temp_path( '1420_agent_dependency', additional_paths=[ 'package_rdkit_2012_12_1420' ] )
        precompiled_binary_tarball = self.get_filename( '1420_files/binary_tarballs/rdkit.tar' )
        edited_agent_dependency_filename = self.get_filename( filepath=agent_dependency_path, filename='agent_dependencies.xml' )
        original_agent_dependency = self.get_filename( '1420_files/package_rdkit_2012_12_1420/agent_dependencies.xml' )
        agent_dependency_definition = file( original_agent_dependency, 'r' ).read().replace( '__PATH__', precompiled_binary_tarball )
        file( edited_agent_dependency_filename, 'w' ).write( agent_dependency_definition )
        # Upload the edited agent dependency definition to the package_rdkit_2012_12_1420 repository.
        self.upload_file( repository,
                          filename='agent_dependencies.xml',
                          filepath=agent_dependency_path,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_rdkit_2012_12_1420 with agent dependency definitions.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0035_install_rdkit_2012_12_repository( self ):
        ...
        Install package_rdkit_2012_12_1420 with agent dependencies selected to be installed. The result of this should be
    ...
    def test_0040_verify_env_sh_contents( self ):
        ...
        and package_boost_1_53_1420. Test for the numpy and boost agent dependency paths.
        '''
        ...
        rdkit_env_sh = self.get_env_sh_path( agent_dependency_name='rdkit',
                                             agent_dependency_version='2012_12_1',
                                             repository=package_rdkit_repository )
        numpy_agent_dependency_path = self.get_agent_dependency_path( agent_dependency_name='numpy',
                                                                    agent_dependency_version='1.7.1',
                                                                    repository=package_numpy_repository )
        boost_agent_dependency_path = self.get_agent_dependency_path( agent_dependency_name='boost',
                                                                    agent_dependency_version='1.53.0',
                                                                    repository=package_boost_repository )
        rdkit_env_file_contents = file( rdkit_env_sh, 'r' ).read()
        if numpy_agent_dependency_path not in rdkit_env_file_contents or boost_agent_dependency_path not in rdkit_env_file_contents:
            message = 'Environment file for package_rdkit_2012_12_1420 does not contain expected path.'
            message += '\nExpected:\n%s\n%s\nContents:\n%s' % ( numpy_agent_dependency_path, boost_agent_dependency_path, rdkit_env_file_contents )
