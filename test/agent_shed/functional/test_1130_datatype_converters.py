...
2) Make sure the page section to select a agent panel section is NOT displayed since the agent will not be displayed in the Galaxy agent panel.
3) Make sure the bed_to_gff_converter agent is not displayed in the agent panel.
'''
...
class TestDatatypeConverters( ShedTwillTestCase ):
    ...
    def test_0010_install_datatype_converter_to_galaxy( self ):
        ...
        Install bed_to_gff_converter_0130, checking that the option to select the agent panel section is *not* displayed.
        '''
        self.galaxy_logout()
        self.galaxy_login( email=common.admin_email, username=common.admin_username )
        ...
        strings_not_displayed = [ 'agent panel section' ]
    ...
    def test_0015_uninstall_and_verify_agent_panel_section( self ):
        '''Uninstall bed_to_gff_converter_0130 and verify that the saved agent_panel_section is None.'''
        '''
        We are at step 3 - Make sure the bed_to_gff_converter agent is not displayed in the agent panel.
        The previous agent panel section for a agent is only recorded in the metadata when a repository is uninstalled,
        so we have to uninstall it first, then verify that it was not assigned a agent panel section.
        '''
        ...
        self.verify_installed_repository_no_agent_panel_section( repository )
