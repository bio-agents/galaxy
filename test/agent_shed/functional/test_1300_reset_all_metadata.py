...
category_0010_name = 'Test 0010 Repository With Agent Dependencies'
...
1. Directly, by calling sh run_functional_tests.sh -agentshed test/agent_shed/functional/test_1300_reset_all_metadata.py.
 -OR-
...
class TestResetInstalledRepositoryMetadata( ShedTwillTestCase ):
    ...
    def test_0005_create_categories( self ):
        ...
        self.create_category( name=category_0010_name, description='Tests for a repository with agent dependencies.' )
    ...
    def test_0020_create_repositories_from_0020_series( self ):
        ...
        repository = self.get_or_create_repository( name='emboss_datatypes_0020',
                                                    description="Galaxy applicable data formats used by Emboss agents.",
                                                    long_description="Galaxy applicable data formats used by Emboss agents. This repository contains no agents.",
                                                    owner=common.test_user_1_name,
                                                    category_id=self.security.encode_id( category.id ),
                                                    strings_displayed=[] )
        if self.repository_is_new( repository ):
            ...
            repository = self.get_or_create_repository( name='emboss_0020',
                                                 description='Galaxy wrappers for Emboss version 5.0.0 agents',
                                                 long_description='Galaxy wrappers for Emboss version 5.0.0 agents',
                                                 owner=common.test_user_1_name,
                                                 category_id=self.security.encode_id( category.id ),
                                                 strings_displayed=[] )
            self.upload_file( repository,
    ...
    def test_0035_create_repositories_from_0050_series( self ):
        ...
        if self.repository_is_new( datatypes_repository ):
            emboss_repository = self.get_or_create_repository( name='emboss_0050',
                                                        description="Galaxy's emboss agent",
                                                        long_description="Long description of Galaxy's emboss agent",
                                                        owner=common.test_user_1_name,
                                                        category_id=self.security.encode_id( category.id ),
                                                        strings_displayed=[] )
    ...
    def test_9905_reset_metadata_on_all_repositories( self ):
        ...
        for repository in repositories:
            ...
            # When a repository with agents to be displayed in a agent panel section is deactivated and reinstalled,
            # the agent panel section remains in the repository metadata. However, when the repository's metadata
            # is subsequently reset, the agent panel section is removed from the repository metadata. While this
            # is normal and expected behavior, the functional tests assume that repository metadata will not change
            # in any way after a reset. A workaround is to remove the agent panel section from the stored repository
            # metadata dict, in order to eliminate the misleading detection of changed metadata.
            if 'agent_panel_section' in old_metadata and 'agent_panel_section' not in repository.metadata:
                del old_metadata[ 'agent_panel_section' ]
