...
category_description = 'Test 1070 for a repository with an invalid agent.'


class TestFreebayesRepository( ShedTwillTestCase ):
    '''Test repository with multiple revisions with invalid agents.'''
    ...
    def test_0005_ensure_existence_of_repository_and_category( self ):
        '''Create freebayes repository and upload only freebayes.xml. This should result in an error message and invalid agent.'''
    ...
    def test_0010_browse_agent_shed( self ):
        """Browse the available agent sheds in this Galaxy instance and preview the bismark repository."""
        self.galaxy_logout()
        self.galaxy_login( email=common.admin_email, username=common.admin_username )
        ...
        self.preview_repository_in_agent_shed( repository_name, common.test_user_1_name, strings_displayed=[ repository_name ] )

    def test_0015_install_freebayes_repository( self ):
        '''Install the test repository without installing agent dependencies.'''
        self.install_repository( repository_name,
                                 common.test_user_1_name,
                                 category_name,
                                 install_agent_dependencies=False,
                                 new_agent_panel_section_label='test_1070' )
        ...
        strings_displayed.extend( [ 'methylation extractor', 'Invalid agents' ] )
        ...
        assert 'invalid_agents' in installed_repository.metadata, 'No invalid agents were defined in %s.' % \
