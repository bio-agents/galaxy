...
3) Visit the view agent metadata page and make sure that "Display in agent panel" is False
'''
...
class TestDatatypeConverters( ShedTwillTestCase ):
    ...
    def test_0010_verify_agent_and_datatype( self ):
        '''Verify that a valid agent and datatype are contained within the repository.'''
        '''
        We are at step 2 - Visit the manage repository page and make sure there is the appropriate valid agent and datatype.
        There should be a 'Convert BED to GFF' agent and a 'galaxy.datatypes.interval:Bed' datatype with extension 'bed'
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        strings_displayed = [ 'Convert BED to GFF', 'galaxy.datatypes.interval:Bed', 'bed', 'Valid agents', 'Datatypes' ]
    ...
    def test_0015_verify_agent_panel_display( self ):
        '''Verify that the agent is configured not to be displayed in the agent panel.'''
        '''
        We are at step 3
        Datatype converters that are associated with a datatype should have display in agent panel = False in the agent metadata.
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        agent_metadata_strings_displayed = '<label>Display in agent panel:</label>\n                    False'
