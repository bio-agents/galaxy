...
repository_description = 'Galaxy filtering agent for test 0400'
repository_long_description = 'Long description of Galaxy filtering agent for test 0400'

'''
1. Create users.
2. Grant reviewer role to test_user_2.
3. Check that the review components that are to be tested are defined in this agent shed instance.
...
13. Review the agent dependencies component.
14. Check that only agent dependencies, repository dependencies, functional tests, datatypes, and readme display as reviewed.
15. Review the agents component.
16. Check that only agents, agent dependencies, repository dependencies, functional tests, datatypes, and readme display as reviewed.
...
25. Upload a new version of the agent.
...
class TestRepositoryComponentReviews( ShedTwillTestCase ):
    ...
    def test_0010_verify_repository_review_components( self ):
        ...
        strings_displayed = [ 'Data types', 'Functional tests', 'README', 'Repository dependencies', 'Agent dependencies', 'Agents', 'Workflows' ]
    ...
    def test_0025_verify_datatype_review( self ):
        ...
        strings_not_displayed = [ 'Functional tests', 'README', 'Repository dependencies', 'Agent dependencies', 'Agents', 'Workflows' ]
...
#        self.login( email=common.test_user_3_email, username=common.test_user_3_name )
    ...
    def test_0035_verify_functional_test_review( self ):
        ...
        strings_not_displayed = [ 'README', 'Repository dependencies', 'Agent dependencies', 'Agents', 'Workflows' ]
        self.verify_repository_reviews( repository, reviewer=user, strings_displayed=strings_displayed, strings_not_displayed=strings_not_displayed )
    ...
    def test_0045_verify_readme_review( self ):
        ...
        strings_not_displayed = [ 'Repository dependencies', 'Agent dependencies', 'Agents', 'Workflows' ]
        self.verify_repository_reviews( repository, reviewer=user, strings_displayed=strings_displayed, strings_not_displayed=strings_not_displayed )
    ...
    def test_0055_verify_repository_dependency_review( self ):
        ...
        strings_not_displayed = [ 'Agent dependencies', 'Agents', 'Workflows' ]
        self.verify_repository_reviews( repository, reviewer=user, strings_displayed=strings_displayed, strings_not_displayed=strings_not_displayed )

    def test_0060_review_agent_dependencies( self ):
        '''Review the agent dependencies component for the current tip revision.'''
        """
        We are at step 13.
        Log in as test_user_2 and update the review with the agent dependencies component marked as 'Not applicable'.
        # Review this revision:
        #    Data types (N/A)
        #    Functional tests (One star, comment 'functional tests missing')
        #    README (N/A)
        #    Repository dependencies (N/A)
        #    Agent dependencies (N/A)
        """
        self.logout()
        self.login( email=common.test_user_2_email, username=common.test_user_2_name )
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        ...
        #   private: yes/no
        # }
        review_contents_dict = { 'Agent dependencies': dict() }
        self.review_repository( repository, review_contents_dict, user )

    def test_0065_verify_agent_dependency_review( self ):
        '''Verify that the agent dependencies component review displays correctly.'''
        """
        We are at step 14.
        Log in as test_user_1 and verify that the repository component reviews now include a review
        for the agent dependencies component.
        """
        self.logout()
        self.login( email=common.test_user_1_email, username=common.test_user_1_name )
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        user = self.test_db_util.get_user( common.test_user_2_email )
        strings_displayed = [ 'Agent dependencies', 'not_applicable' ]
        strings_not_displayed = [ 'Agents', 'Workflows' ]
        self.verify_repository_reviews( repository, reviewer=user, strings_displayed=strings_displayed, strings_not_displayed=strings_not_displayed )

    def test_0070_review_agents( self ):
        '''Review the agents component for the current tip revision.'''
        """
        We are at step 15.
        Log in as test_user_2 and update the review with the agents component given
        ...
        #   private: yes/no
        # }
        review_contents_dict = { 'Agents': dict( rating=5, comment='Excellent agent, easy to use.', approved='yes', private='no' ) }
        self.review_repository( repository, review_contents_dict, user )

    def test_0075_verify_agents_review( self ):
        '''Verify that the agents component review displays correctly.'''
        """
        We are at step 16.
        Log in as test_user_1 and verify that the repository component reviews now include a review
        for the agents component. As before, check for the presence of the comment on this review.
        """
        self.logout()
        self.login( email=common.test_user_1_email, username=common.test_user_1_name )
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        user = self.test_db_util.get_user( common.test_user_2_email )
        strings_displayed = [ 'Agents', 'yes', 'Excellent agent, easy to use.' ]
    ...
    def test_0120_upload_new_agent_version( self ):
        '''Upload filtering 2.2.0 to the filtering repository.'''
        """
        We are at step 25.
        Log in as test_user_1 and upload a new version of the agent to the filtering repository. This will create
    ...
    def test_0125_review_new_changeset_functional_tests( self ):
        ...
        update the agents component review to refer to the new agent version.
        """
        self.logout()
        self.login( email=common.test_user_2_email, username=common.test_user_2_name )
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        ...
        #   private: yes/no
        # }
        review_contents_dict = { 'Agents': dict( rating=5, comment='Version 2.2.0 does the impossible and improves this agent.', approved='yes', private='yes' ) }
        self.create_repository_review( repository,
                                       review_contents_dict,
                                       changeset_revision=self.get_repository_tip( repository ),
                                       copy_from=( str( last_review.changeset_revision ), last_review.id ) )
    ...
    def test_0135_verify_review_for_new_version( self ):
        ...
        """
        We are at step 27.
        Log in as test_user_1 and check that the agents component review is for filtering 2.2.0, but that the other component
        ...
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        user = self.test_db_util.get_user( common.test_user_2_email )
        strings_displayed = [ 'Data types', 'Functional tests', 'yes', 'A good set of functional tests.', 'README', 'yes', 'Workflows', 'Agents' ]
        strings_displayed.extend( [ 'Clear and concise readme file, a true pleasure to read.', 'Agent dependencies', 'not_applicable' ] )
        strings_displayed.extend( [ 'Repository dependencies', 'Version 2.2.0 does the impossible and improves this agent.'  ] )
        self.verify_repository_reviews( repository, reviewer=user, strings_displayed=strings_displayed )
