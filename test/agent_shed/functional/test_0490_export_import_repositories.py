...
repository_description = "Galaxy's filtering agent for test 0490"
repository_long_description = "Long description of Galaxy's filtering agent for test 0490"
...
3. Export that repository. Check the capsule's contents, verify that changeset revision and agent shed are not set.
...
class TestExportImportRepository( ShedTwillTestCase ):
    ...
    def test_0015_verify_exported_capsule( self ):
        ...
        '''
        This is step 3 - Check the capsule's contents, verify that changeset revision and agent shed are not set.
        Extract the exported capsule tarball to a temporary path, and confirm that the manifest does not specify
        a agent shed or changeset revision.
        '''
        global capsule_filepath
