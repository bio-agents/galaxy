...
3) Simulate clicking the htseq_count agent button.
4) On the resulting display agent page, look for the image string similar to the following string where the encoded repository_id is previously determined:
...
class TestAgentHelpImages( ShedTwillTestCase ):
    ...
    def test_0010_load_agent_page( self ):
        '''Load the agent page and check for the image.'''
        '''
        We are at step 2
        Visit the manage_repository page and the agent page, and look for the image url
