...
6. Log into the Agent Shed as a user that is not the repository owner (e.g., user2) and make sure the repository name
   and description cannot be changed.

7. As user user1, add user user2 as a repository admin user.

8. Log into the Agent Shed as user user2 and make sure the repository name and description can now be changed.
'''
...
class TestRepositoryAdminRole( ShedTwillTestCase ):
    ...
    def test_0030_verify_access_denied( self ):
        ...
        '''
        This is step 6 - Log into the Agent Shed as a user that is not the repository owner (e.g., user2) and make sure the repository
    ...
    def test_0040_rename_repository_as_repository_admin( self ):
        ...
        '''
        This is step 8 - Log into the Agent Shed as user user2 and make sure the repository name and description can now be changed.
        '''
        self.logout()
        self.login( email=common.test_user_2_email, username=common.test_user_2_name )
        repository = self.test_db_util.get_repository_by_name_and_owner( 'renamed_filtering_0530', common.test_user_1_name )
