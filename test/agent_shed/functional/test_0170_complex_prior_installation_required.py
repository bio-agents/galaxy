...
matplotlib_repository_description = "Contains a agent dependency definition that downloads and compiles version 1.2.x of the the python matplotlib package."
...
numpy_repository_description = "Contains a agent dependency definition that downloads and compiles version 1.7 of the the python numpy package."
...
3. Verify that package_matplotlib_1_2_0170 now depends on package_numpy_1_7_0170, and that the inherited agent dependency displays correctly.
'''
...
class TestComplexPriorInstallation( ShedTwillTestCase ):
    ...
    def test_0005_create_matplotlib_repository( self ):
        ...
        self.upload_file( repository,
                          ...
                          commit_message='Uploaded matplotlib agent dependency tarball.',
    ...
    def test_0010_create_numpy_repository( self ):
        ...
        self.upload_file( repository,
                          ...
                          uncompress_file=True,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Uploaded numpy agent dependency tarball.',
                          strings_displayed=[ 'This repository currently contains a single file named <b>agent_dependencies.xml</b>' ],
                          strings_not_displayed=[] )
    ...
    def test_0015_create_complex_repository_dependency( self ):
        ...
        # it replaces any existing agent or repository dependency XML file with the generated contents. This is undesirable
        # in this case, because matplotlib already has an additional agent dependency definition that we don't want to
        # overwrite.
        new_xml = '    <package name="numpy" version="1.7">\n'
        new_xml += '        <repository agentshed="%s" name="%s" owner="%s" changeset_revision="%s" prior_installation_required="True" />\n'
        new_xml += '    </package>\n'
        url = self.url
        ...
        original_xml = file( self.get_filename( 'package_matplotlib/agent_dependencies.xml' ), 'r' ).read()
        dependency_xml_path = self.generate_temp_path( 'test_0170', additional_paths=[ 'matplotlib' ] )
        new_xml_file = os.path.join( dependency_xml_path, 'agent_dependencies.xml' )
    ...
    def test_0020_verify_generated_dependency( self ):
        '''Verify that matplotlib now has a package agent dependency and a complex repository dependency.'''
        '''
        This is step 3 - Verify that package_matplotlib_1_2_0170 now depends on package_numpy_1_7_0170, and that the inherited agent
                         dependency displays correctly.
        'Inhherited' in this case means that matplotlib should show a package agent dependency on numpy version 1.7, and a repository
