...
class TestInstallingCircularDependencies( ShedTwillTestCase ):
    ...
    def test_0025_install_freebayes_repository( self ):
        '''Install freebayes with blank agent panel section, without agent dependencies but with repository dependencies.'''
        self.galaxy_logout()
    ...
    def test_0030_uninstall_filtering_repository( self ):
        '''Deactivate filtering, verify agent panel section and missing repository dependency.'''
        ...
        self.check_galaxy_repository_agent_panel_section( installed_filtering_repository, '' )
        strings_displayed = [ 'Missing repository', 'filtering', 'freebayes_0040', 'user1', "Galaxy's freebayes agent for test 0040" ]
    ...
    def test_0035_reactivate_filtering_repository( self ):
        '''Reinstall filtering into 'filtering' agent panel section.'''
        ...
        self.reinstall_repository( installed_filtering_repository,
                                   install_agent_dependencies=False,
                                   install_repository_dependencies=True,
                                   new_agent_panel_section_label='filtering',
                                   no_changes=False )

    def test_0040_uninstall_freebayes_repository( self ):
        '''Deactivate freebayes, verify agent panel section and missing repository dependency.'''
        installed_freebayes_repository = self.test_db_util.get_installed_repository_by_name_owner( freebayes_repository_name, common.test_user_1_name )
        installed_filtering_repository = self.test_db_util.get_installed_repository_by_name_owner( filtering_repository_name, common.test_user_1_name )
        self.display_installed_repository_manage_page( installed_freebayes_repository )
        self.display_installed_repository_manage_page( installed_filtering_repository )
        ...
        self.check_galaxy_repository_agent_panel_section( installed_freebayes_repository, '' )
        strings_displayed = [ 'Missing repository', 'freebayes', 'filtering_0040', 'user1', "Galaxy's filtering agent for test 0040" ]
    ...
    def test_0045_deactivate_filtering_repository( self ):
        '''Deactivate filtering, verify agent panel section.'''
        installed_filtering_repository = self.test_db_util.get_installed_repository_by_name_owner( filtering_repository_name, common.test_user_1_name )
        installed_freebayes_repository = self.test_db_util.get_installed_repository_by_name_owner( freebayes_repository_name, common.test_user_1_name )
        self.display_installed_repository_manage_page( installed_freebayes_repository )
        self.display_installed_repository_manage_page( installed_filtering_repository )
