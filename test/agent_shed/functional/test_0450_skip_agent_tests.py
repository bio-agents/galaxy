...
category_name = 'Test 0450 Skip Agent Tests'
category_description = 'Test 0450 Skip Agent Tests'
repository_name = 'filtering_0450'
repository_description = "Galaxy's filtering agent for test 0450"
repository_long_description = "Long description of Galaxy's filtering agent for test 0450"
first_metadata_revision = None

'''
1)  Create and populate a repository so that it has a single metadata revision.

2)  Visit the Manage repository page and check the "Skip agent tests for this revision" checkbox
...
4)  Visit the Manage repository page again, the checked "Skip agent tests" checkbox and the reason
    should still be displayed since it should now be associated with this metadata revision.

5)  Upload a new version of the agent to the repository so that a new metadata changeset revision
    is created (there should now be 3 revisions with 2 of them having associated metadata).

6)  Visit the Manage repository page for the tip revision - the Skip agents tests checkbox should
...
7)  Visit the Manage repository page for the previous metadata revision (with the README file in it).
    The Skip agent tests checkbox should still be checked and the reason should be there.
...
class TestSkipAgentTestFeature( ShedTwillTestCase ):
    ...
    def test_0010_set_skip_agent_tests_flag( self ):
        '''Set the skip agent tests flag on filtering_0450.'''
        '''
        This is step 2 - Visit the Manage repository page and check the "Skip agent tests for this revision" checkbox and
        enter some text in the reason.
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        self.set_skip_agent_tsts_flag( repository=repository, flag_value=True, reason='Skip reason for first revision.' )
    ...
    def test_0015_upload_readme_file( self ):
        ...
        upward in the changelog. This should result in the skip_agent_tests setting being applied to the updated
        metadata revision.
        '''
    ...
    def test_0020_verify_skip_on_new_revision( self ):
        '''Check that the skip agent tests setting was updated.'''
        '''
        This is step 4 - Visit the Manage repository page again, the checked "Skip agent tests" checkbox and
    ...
    def test_0025_upload_new_agent_version( self ):
        '''Upload filtering 2.2.0.'''
        '''
        This is step 5 - Upload a new version of the agent to the repository so that a new metadata changeset
    ...
    def test_0030_verify_skip_not_set( self ):
        '''Verify that skip agent tests is not set for the new metadata revision.'''
        '''
        This is step 6 - Visit the Manage repository page for the tip revision - the Skip agents tests checkbox
    ...
    def test_0045_verify_previous_revision_skip_setting( self ):
        '''Check that the previous metadata revision is still set to skip agent tests.'''
        '''
        This is step 7 - Visit the Manage repository page for the previous metadata revision (with the README
        file in it). The Skip agent tests checkbox should still be checked and the reason should be there.
        '''
        global first_metadata_revision
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
    ...
    def test_0050_set_new_revision_not_to_be_tested( self ):
        ...
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        self.set_skip_agent_tsts_flag( repository=repository, flag_value=True, reason='Skip reason for second revision.' )
        self.load_checkable_revisions( strings_not_displayed=[ self.get_repository_tip( repository ) ] )
    ...
    def test_0070_unset_skip_setting_on_first_revision( self ):
        ...
        global first_metadata_revision
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        self.set_skip_agent_tsts_flag( repository=repository, flag_value=False, reason='', changeset_revision=first_metadata_revision )
        self.display_manage_repository_page( repository,
                                             changeset_revision=first_metadata_revision,
    ...
    def test_0075_unset_skip_setting_on_second_revision( self ):
        ...
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        self.set_skip_agent_tsts_flag( repository=repository, flag_value=False, reason='' )
        self.display_manage_repository_page( repository,
                                             strings_not_displayed=[ 'Skip reason for second revision.', 'Skip reason for first revision.' ] )
        self.load_checkable_revisions( strings_displayed=[ self.get_repository_tip( repository ) ] )
