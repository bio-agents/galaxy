...
repository_description = "Galaxy's filtering agent for test 0000"
repository_long_description = "Long description of Galaxy's filtering agent for test 0000"
...
class TestBasicRepositoryFeatures( ShedTwillTestCase ):
    ...
    def test_0005_create_repository_without_categories( self ):
        ...
        strings_displayed = [ 'No categories have been configured in this instance of the Galaxy Agent Shed' ]
    ...
    def test_0020_edit_repository( self ):
        ...
        new_description = "Edited filtering agent"
    ...
    def test_0040_verify_repository( self ):
        ...
        self.check_for_valid_agents( repository, strings_displayed=[ 'Filter1' ] )
        self.check_count_of_metadata_revisions_associated_with_repository( repository, metadata_count=1 )
        tip = self.get_repository_tip( repository )
        agent_guid = '%s/repos/user1/filtering_0000/Filter1/1.1.0' % self.url.replace( 'http://', '' ).rstrip( '/' )
        agent_metadata_strings_displayed = [ agent_guid,
                                            '1.1.0',  # The agent version.
                                            'Filter1',  # The agent ID.
                                            'Filter',  # The agent name.
                                            'data on any column using simple expressions' ]  # The agent description.
        agent_page_strings_displayed = [ 'Filter (version 1.1.0)' ]
    ...
    def test_0055_upload_filtering_txt_file( self ):
        '''Upload filtering.txt file associated with agent version 1.1.0.'''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        self.upload_file( repository,
    ...
    def test_0070_verify_filtering_repository( self ):
        '''Verify the new agent versions and repository metadata.'''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        tip = self.get_repository_tip( repository )
        self.check_for_valid_agents( repository )
        ...
        agent_guid = '%s/repos/user1/filtering_0000/Filter1/2.2.0' % self.url.replace( 'http://', '' ).rstrip( '/' )
        agent_metadata_strings_displayed = [ agent_guid,
                                            '2.2.0',  # The agent version.
                                            'Filter1',  # The agent ID.
                                            'Filter',  # The agent name.
                                            'data on any column using simple expressions' ]  # The agent description.
        agent_page_strings_displayed = [ 'Filter (version 2.2.0)' ]
        self.check_repository_agents_for_changeset_revision( repository,
                                                            tip,
                                                            agent_metadata_strings_displayed=agent_metadata_strings_displayed,
                                                            agent_page_strings_displayed=agent_page_strings_displayed )
    ...
    def test_0075_upload_readme_txt_file( self ):
        '''Upload readme.txt file associated with agent version 2.2.0.'''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        self.upload_file( repository,
                          filename='readme.txt',
                          filepath=None,
    ...
    def test_0085_search_for_valid_filter_agent( self ):
        '''Search for the filtering agent by agent ID, name, and version.'''
        repository = self.test_db_util.get_repository_by_name_and_owner( repository_name, common.test_user_1_name )
        tip_changeset = self.get_repository_tip( repository )
        search_fields = dict( agent_id='Filter1', agent_name='filter', agent_version='2.2.0' )
        self.search_for_valid_agents( search_fields=search_fields, strings_displayed=[ tip_changeset ], strings_not_displayed=[] )
    ...
    def test_0095_verify_reserved_repository_name_handling( self ):
        ...
        category = self.test_db_util.get_category_by_name( 'Test 0000 Basic Repository Features 1' )
        error_message = 'The term <b>repos</b> is a reserved word in the agent shed, so it cannot be used as a repository name.'
    ...
    def test_0100_verify_reserved_username_handling( self ):
        ...
        error_message = 'The term <b>repos</b> is a reserved word in the agent shed, so it cannot be used as a public user name.'
    ...
    def test_0105_contact_repository_owner( self ):
        ...
        any external network connection. The embedded agent shed server these tests are running against has been configured
    ...
    def test_0130_verify_handling_of_invalid_characters( self ):
        ...
        self.load_changeset_in_agent_shed( repository_id, changeset_revision, strings_displayed=strings_displayed )
