...
repository_description = "Galaxy's filtering agent for test 0060"
repository_long_description = "Long description of Galaxy's filtering agent for test 0060"
...
workflow_repository_description = "Workflow referencing the filtering agent for test 0060"
...
class AgentWithRepositoryDependencies( ShedTwillTestCase ):
    ...
    def test_0010_install_repository_with_workflow( self ):
        """Browse the available agent sheds in this Galaxy instance and preview the filtering agent."""
        self.preview_repository_in_agent_shed( repository_name,
                                              common.test_user_1_name,
                                              strings_displayed=[ repository_name, 'Valid agents', 'Workflows' ] )
        self.galaxy_logout()
        self.galaxy_login( email=common.admin_email, username=common.admin_username )
        self.install_repository( repository_name,
                                 ...
                                 new_agent_panel_section_label='test_1060' )
    ...
    def test_0020_create_filter_workflow_repository( self ):
        ...
        if self.repository_is_new( repository ):
            ...
            self.upload_file( repository,
                              ...
                              strings_displayed=[],
                              strings_not_displayed=[] )
            self.load_workflow_image_in_agent_shed( repository,
                                                   'New workflow for 0060_filter_workflow_repository',
                                                   strings_displayed=[ '#EBBCB2' ] )

    def test_0025_install_repository_with_workflow( self ):
        """Browse the available agent sheds in this Galaxy instance and preview the filtering workflow repository."""
        self.preview_repository_in_agent_shed( workflow_repository_name,
                                              common.test_user_1_name,
                                              strings_displayed=[ 'filtering_workflow_0060', 'Workflows' ],
                                              strings_not_displayed=[ 'Valid agents', 'Invalid agents' ] )
        self.galaxy_logout()
        self.galaxy_login( email=common.admin_email, username=common.admin_username )
