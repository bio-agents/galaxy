...
category_name = 'Test 1440 Agent dependency missing env.sh'
...
agent_repository_name = 'filter_1440'
package_repository_description = 'Repository that should result in an env.sh file, but does not.'
agent_repository_description = 'Galaxy filtering agent.'
package_repository_long_description = '%s: %s' % ( package_repository_name, package_repository_description )
agent_repository_long_description = '%s: %s' % ( agent_repository_name, agent_repository_description )

'''
1. Create a agent dependency type repository that reliably fails to install successfully. This repository should define
...
3. Attempt to install the second repository into a galaxy instance, verify that it is installed but missing agent dependencies.
...
class TestMissingEnvSh( ShedTwillTestCase ):
    ...
    def test_0005_create_package_repository( self ):
        ...
        Create and populate a repository that is designed to fail a agent dependency installation. This agent dependency should
        ...
        repository = self.get_or_create_repository( name=package_repository_name,
                                                    ...
                                                    strings_displayed=[] )
        # Upload the edited agent dependency definition to the package_lapack_3_4_1440 repository.
        self.upload_file( repository,
                          filename='1440_files/dependency_definition/agent_dependencies.xml',
                          filepath=None,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate package_env_sh_1_0_1440 with a broken agent dependency definition.',
                          strings_displayed=[],
    ...
    def test_0010_create_filter_repository( self ):
        ...
        self.upload_file( repository,
                          ...
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Populate filter_1440 with the filtering agent.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
        self.upload_file( repository,
                          filename='1440_files/complex_dependency/agent_dependencies.xml',
                          filepath=None,
                          valid_agents_only=True,
                          uncompress_file=False,
                          remove_repo_files_not_in_tar=False,
    ...
    def test_0020_verify_missing_agent_dependency( self ):
        '''Verify that the filter_1440 repository is installed and missing agent dependencies.'''
        repository = self.test_db_util.get_installed_repository_by_name_owner( 'filter_1440', common.test_user_1_name )
        strings_displayed = [ 'Missing agent dependencies' ]
        self.display_installed_repository_manage_page( repository, strings_displayed=strings_displayed )
        assert len( repository.missing_agent_dependencies ) == 1, 'filter_1440 should have a missing agent dependency, but does not.'
