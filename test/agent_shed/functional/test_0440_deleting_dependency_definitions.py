...
bwa_package_repository_long_description = "BWA repository with a package agent dependency defined for BWA 0.5.9."
...
bwa_agent_dependency_repository_name = 'bwa_agent_dependency_0440'
bwa_agent_dependency_repository_description = "BWA Base"
bwa_agent_dependency_repository_long_description = "NT space mapping with BWA"
...
1. Create and populate bwa_package_0440 so that it has a valid agent dependency definition and an installable revision 0.
2. Create and populate bwa_base_0440 so that it has an installable revision 0.
3. Add a valid complex repository dependency agent_dependencies.xml to bwa_base_0440 that points to the installable revision 0 of bwa_package_0440.
4. Make sure that bwa_base_0440 installable revision is now revision 1 instead of revision 0.
5. Delete agent_dependencies.xml from bwa_base_0440, and make sure bwa_base_0440 now has two installable revisions: 1 and 2

Agent dependencies:
1. Create and populate bwa_agent_dependency_0440 so that it has a valid agent dependency definition and an installable revision 0.
2. Delete agent_dependencies.xml from bwa_agent_dependency_0440, and make sure that bwa_agent_dependency_0440 still has
   a single installable revision 0.
3. Add the same agent_dependencies.xml file to bwa_agent_dependency_0440, and make sure that bwa_agent_dependency_0440
...
class TestDeletedDependencies( ShedTwillTestCase ):
    ...
    def test_0005_create_column_maker_repository( self ):
        ...
        self.upload_file( repository,
                          ...
                          commit_message='Uploaded column maker agent tarball.',
                          strings_displayed=[],
    ...
    def test_0010_create_convert_chars_repository( self ):
        ...
        self.upload_file( repository,
                          ...
                          uncompress_file=True,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Uploaded convert chars agent tarball.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0030_create_bwa_package_repository( self ):
        ...
        agent dependency definition and an installable revision 0.
        '''
        category = self.test_db_util.get_category_by_name( 'Test 0440 Deleted Dependency Definitions' )
        self.logout()
        self.login( email=common.test_user_1_email, username=common.test_user_1_name )
        ...
        self.upload_file( repository,
                          ...
                          uncompress_file=True,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Uploaded package agent dependency definition.',
                          strings_displayed=[],
                          strings_not_displayed=[] )
    ...
    def test_0035_create_bwa_base_repository( self ):
        ...
        This repository should contain a agent with a defined dependency that will be satisfied by the agent dependency defined in bwa_package_0440.
        '''
        category = self.test_db_util.get_category_by_name( 'Test 0440 Deleted Dependency Definitions' )
        self.logout()
        self.login( email=common.test_user_1_email, username=common.test_user_1_name )
        ...
        self.upload_file( repository,
                          ...
                          uncompress_file=True,
                          remove_repo_files_not_in_tar=False,
                          commit_message='Uploaded BWA nucleotide space mapping agent tarball.',
                          strings_displayed=[],
                          strings_not_displayed=[] )

    def test_0040_create_dependency_on_bwa_package_repository( self ):
        '''Create a complex repository dependency on bwa_package_0440 and upload it to bwa_agent_0440.'''
        '''
        We are at complex repository dependencies, step 3 - Add a valid complex repository dependency agent_dependencies.xml to
    ...
    def test_0050_delete_complex_repository_dependency( self ):
        '''Delete the agent_dependencies.xml from bwa_base_0440.'''
        '''
        We are at complex repository dependencies, step 5 - Delete agent_dependencies.xml from bwa_base_0440,
        and make sure bwa_base_0440 now has two installable revisions: 1 and 2
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( bwa_base_repository_name, common.test_user_1_name )
        # Record the current tip, so we can verify that it's still a downloadable revision after agent_dependencies.xml
        # is deleted and a new downloadable revision is created.
        old_changeset_revision = self.get_repository_tip( repository )
        self.delete_files_from_repository( repository, filenames=[ 'agent_dependencies.xml' ] )
        new_changeset_revision = self.get_repository_tip( repository )
        # Check that the old changeset revision is still downloadable.
        metadata_record = self.get_repository_metadata_by_changeset_revision( repository, old_changeset_revision )
        assert metadata_record.downloadable, 'The revision of %s that contains agent_dependencies.xml is no longer downloadable.' % \
            repository.name
        # Check that the new tip is also downloadable.
        metadata_record = self.get_repository_metadata_by_changeset_revision( repository, new_changeset_revision )
        assert metadata_record.downloadable, 'The revision of %s that does not contain agent_dependencies.xml is not downloadable.' % \
            repository.name
        # Verify that there are only two downloadable revisions.
        assert len( repository.downloadable_revisions ) == 2, 'Repository %s has %d downloadable revisions, expected 2.' % \
            ( repository.name, len( repository.downloadable_revisions ) )

    def test_0055_create_bwa_agent_dependency_repository( self ):
        '''Create and populate the bwa_agent_dependency_0440 repository.'''
        '''
        We are at agent dependencies, step 1 - Create and populate bwa_agent_dependency_0440 so that it has a valid agent
        dependency definition and an installable revision 0.
        '''
        category = self.test_db_util.get_category_by_name( 'Test 0440 Deleted Dependency Definitions' )
        self.logout()
        self.login( email=common.test_user_1_email, username=common.test_user_1_name )
        strings_displayed = [ "Repository <b>bwa_agent_dependency_0440</b> has been created"  ]
        repository = self.get_or_create_repository( name=bwa_agent_dependency_repository_name,
                                                    description=bwa_agent_dependency_repository_description,
                                                    long_description=bwa_agent_dependency_repository_long_description,
                                                    owner=common.test_user_1_name,
                                                    category_id=self.security.encode_id( category.id ),
                                                    strings_displayed=strings_displayed )
        self.upload_file( repository,
    ...
    def test_0060_delete_bwa_agent_dependency_definition( self ):
        '''Delete the agent_dependencies.xml file from bwa_agent_dependency_0440.'''
        '''
        We are at agent dependencies, step 2 - Delete agent_dependencies.xml from bwa_agent_dependency_0440.
        Make sure bwa_agent_dependency_0440 still has a downloadable changeset revision at the old tip.
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( bwa_agent_dependency_repository_name, common.test_user_1_name )
        # Record the current tip, so we can verify that it's still a downloadable revision after repository_dependencies.xml
        # is deleted and a new downloadable revision is created.
        old_changeset_revision = self.get_repository_tip( repository )
        self.delete_files_from_repository( repository, filenames=[ 'agent_dependencies.xml' ] )
    ...
    def test_0065_reupload_bwa_agent_dependency_definition( self ):
        '''Reupload the agent_dependencies.xml file to bwa_agent_dependency_0440.'''
        '''
        We are at agent dependencies, step 3 - Add the same agent_dependencies.xml file to bwa_agent_dependency_0440, and make sure
        that bwa_agent_dependency_0440 still has a single installable revision 0.
        '''
        repository = self.test_db_util.get_repository_by_name_and_owner( bwa_agent_dependency_repository_name, common.test_user_1_name )
        # Record the current tip, so we can verify that it's still not a downloadable revision after agent_dependencies.xml
        ...
        # Check that the old changeset revision is still downloadable.
        metadata_record = self.get_repository_metadata_by_changeset_revision( repository, old_changeset_revision )
        assert metadata_record is None, 'The revision of %s that does not contain agent_dependencies.xml should not be downloadable, but is.' % \
            repository.name
        # Check that the new tip is also downloadable.
        metadata_record = self.get_repository_metadata_by_changeset_revision( repository, new_changeset_revision )
        assert metadata_record.downloadable, 'The revision of %s that contains agent_dependencies.xml is not downloadable.' % \
            repository.name
        # Verify that there are only two downloadable revisions.
        assert len( repository.downloadable_revisions ) == 1, 'Repository %s has %d downloadable revisions, expected 1.' % \
            ( repository.name, len( repository.downloadable_revisions ) )
