...
emboss_repository_description = 'Galaxy wrappers for Emboss version 5.0.0 agents for test 0020'
emboss_repository_long_description = 'Galaxy wrappers for Emboss version 5.0.0 agents for test 0020'

base_datatypes_count = 0
repository_datatypes_count = 0


class AgentWithRepositoryDependencies( ShedTwillTestCase ):
    ...
    def test_0010_browse_agent_shed( self ):
        """Browse the available agent sheds in this Galaxy instance and preview the emboss agent."""
        global base_datatypes_count
        self.galaxy_logout()
        self.galaxy_login( email=common.admin_email, username=common.admin_username )
        base_datatypes_count = int( self.get_datatypes_count() )
        self.browse_agent_shed( url=self.url, strings_displayed=[ 'Test 0020 Basic Repository Dependencies' ] )
        ...
        self.preview_repository_in_agent_shed( 'emboss_0020', common.test_user_1_name, strings_displayed=[ 'emboss_0020', 'Valid agents' ] )

    def test_0015_install_emboss_repository( self ):
        '''Install the emboss repository without installing agent dependencies.'''
        global repository_datatypes_count
        global base_datatypes_count
        strings_displayed = [ 'Handle', 'Never installed', 'agent dependencies', 'emboss', '5.0.0', 'package' ]
        self.install_repository( 'emboss_0020',
                                 common.test_user_1_name,
                                 'Test 0020 Basic Repository Dependencies',
                                 strings_displayed=strings_displayed,
                                 install_agent_dependencies=False,
                                 new_agent_panel_section_label='test_1020' )
        ...
        strings_displayed = [ 'emboss_0020',
                              'Galaxy wrappers for Emboss version 5.0.0 agents for test 0020',
                              'user1',
                              self.url.replace( 'http://', '' ),
                              installed_repository.installed_changeset_revision ]
        self.display_galaxy_browse_repositories_page( strings_displayed=strings_displayed )
        strings_displayed.extend( [ 'Installed agent shed repository', 'Valid agents', 'antigenic' ] )
