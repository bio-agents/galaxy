...
import galaxy.model
import galaxy.model.agent_shed_install
import galaxy.webapps.agent_shed.model as model

from functional.database_contexts import agent_shed_context as sa_session
...
log = logging.getLogger( 'test.agent_shed.test_db_util' )
...
def get_all_installed_repositories( actually_installed=False ):
    if actually_installed:
        return install_session.query( galaxy.model.agent_shed_install.AgentShedRepository ) \
                              .filter( and_( galaxy.model.agent_shed_install.AgentShedRepository.table.c.deleted == false(),
                                             galaxy.model.agent_shed_install.AgentShedRepository.table.c.uninstalled == false(),
                                             galaxy.model.agent_shed_install.AgentShedRepository.table.c.status == galaxy.model.agent_shed_install.AgentShedRepository.installation_status.INSTALLED ) ) \
                              .all()
    else:
        return install_session.query( galaxy.model.agent_shed_install.AgentShedRepository ).all()
...
def get_galaxy_repository_by_name_owner_changeset_revision( repository_name, owner, changeset_revision ):
    return install_session.query( galaxy.model.agent_shed_install.AgentShedRepository ) \
                          .filter( and_( galaxy.model.agent_shed_install.AgentShedRepository.table.c.name == repository_name,
                                         galaxy.model.agent_shed_install.AgentShedRepository.table.c.owner == owner,
                                         galaxy.model.agent_shed_install.AgentShedRepository.table.c.changeset_revision == changeset_revision ) ) \
                          .first()


def get_installed_repository_by_id( repository_id ):
    return install_session.query( galaxy.model.agent_shed_install.AgentShedRepository ) \
                          .filter( galaxy.model.agent_shed_install.AgentShedRepository.table.c.id == repository_id ) \
                          .first()


def get_installed_repository_by_name_owner( repository_name, owner, return_multiple=False ):
    query = install_session.query( galaxy.model.agent_shed_install.AgentShedRepository ) \
                           .filter( and_( galaxy.model.agent_shed_install.AgentShedRepository.table.c.name == repository_name,
                                          galaxy.model.agent_shed_install.AgentShedRepository.table.c.owner == owner ) )
    if return_multiple:
        return query.all()
