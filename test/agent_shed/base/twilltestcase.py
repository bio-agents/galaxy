...
import galaxy.model.agent_shed_install as galaxy_model
import galaxy.util
import galaxy.webapps.agent_shed.util.hgweb_config
from base.agent_shed_util import repository_installation_timeout
...
class ShedTwillTestCase( TwillTestCase ):
    ...
    def setUp( self ):
        ...
        self.hgweb_config_manager = galaxy.webapps.agent_shed.util.hgweb_config.HgWebConfigManager()
        self.hgweb_config_manager.hgweb_config_dir = self.hgweb_config_dir
        self.agent_shed_test_tmp_dir = os.environ.get( 'TOOL_SHED_TEST_TMP_DIR', None)
        ...
        self.shed_agent_data_table_conf = os.environ.get( 'TOOL_SHED_TEST_TOOL_DATA_TABLE_CONF' )
        self.file_dir = os.environ.get( 'TOOL_SHED_TEST_FILE_DIR', None )
        self.agent_shed_test_file = None
        self.agent_data_path = os.environ.get( 'GALAXY_TEST_TOOL_DATA_PATH' )
        self.shed_agent_conf = os.environ.get( 'GALAXY_TEST_SHED_TOOL_CONF' )
        self.test_db_util = test_db_util
        # TODO: Figure out a way to alter these attributes during tests.
        self.galaxy_agent_dependency_dir = os.environ.get( 'GALAXY_TEST_TOOL_DEPENDENCY_DIR' )
        self.shed_agents_dict = {}
    ...
    def browse_agent_shed( self, url, strings_displayed=[], strings_not_displayed=[] ):
        self.visit_galaxy_url( '/admin_agentshed/browse_agent_shed?agent_shed_url=%s' % url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def browse_agent_dependencies( self, strings_displayed=[], strings_not_displayed=[] ):
        url = '/repository/browse_agent_dependencies'
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def browse_agents( self, strings_displayed=[], strings_not_displayed=[] ):
        url = '/repository/browse_agents'
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def check_exported_repository_dependency( self, dependency_filename, repository_name, repository_owner ):
        ...
        for elem in root.findall( 'repository' ):
            ...
            if 'agentshed' in elem:
                raise AssertionError( 'Exported repository %s with owner %s has a dependency with a defined agent shed.' %
                                      ( repository_name, repository_owner ) )

    def check_for_valid_agents( self, repository, strings_displayed=[], strings_not_displayed=[] ):
        strings_displayed.append( 'Valid agents' )
    ...
    def check_galaxy_repository_agent_panel_section( self, repository, expected_agent_panel_section ):
        metadata = repository.metadata
        assert 'agents' in metadata, 'Agents not found in repository metadata: %s' % metadata
        # If integrated_agent_panel.xml is to be tested, this test method will need to be enhanced to handle agents
        # from the same repository in different agent panel sections. Getting the first agent guid is ok, because
        # currently all agents contained in a single repository will be loaded into the same agent panel section.
        if repository.status in [ galaxy_model.AgentShedRepository.installation_status.UNINSTALLED,
                                  galaxy_model.AgentShedRepository.installation_status.DEACTIVATED ]:
            agent_panel_section = self.get_agent_panel_section_from_repository_metadata( metadata )
        else:
            agent_panel_section = self.get_agent_panel_section_from_api( metadata )
        assert agent_panel_section == expected_agent_panel_section, 'Expected to find agent panel section *%s*, but instead found *%s*\nMetadata: %s\n' % \
            ( expected_agent_panel_section, agent_panel_section, metadata )

    def check_installed_repository_agent_dependencies( self,
                                                      installed_repository,
                                                      strings_displayed=[],
                                                      strings_not_displayed=[],
                                                      dependencies_installed=False ):
        # Agent dependencies are not being installed in these functional tests. If this is changed, the test method will also need to be updated.
        if not dependencies_installed:
            strings_displayed.append( 'Missing agent dependencies' )
        else:
            strings_displayed.append( 'Agent dependencies' )
        if dependencies_installed:
            strings_displayed.append( 'Installed' )
        else:
            strings_displayed.append( 'Never installed' )
        url = '/admin_agentshed/manage_repository?id=%s' % self.security.encode_id( installed_repository.id )
    ...
    def check_manifest( self, manifest_filepath, owner=None ):
        ...
        for elem in root.findall( 'repository' ):
            ...
            agentshed = elem.get( 'agentshed' )
            changeset_revision = elem.get( 'changeset_revision' )
            assert agentshed is None, 'Repository definition %s has a agent shed attribute %s.' % ( repository_name, agentshed )
    ...
    def check_repository_agents_for_changeset_revision( self, repository, changeset_revision, agent_metadata_strings_displayed=[], agent_page_strings_displayed=[] ):
        '''
        Loop through each agent dictionary in the repository metadata associated with the received changeset_revision.
        For each of these, check for a agents attribute, and load the agent metadata page if it exists, then display that agent's page.
        '''
        ...
        if 'agents' not in metadata:
            raise AssertionError( 'No agents in %s revision %s.' % ( repository.name, changeset_revision ) )
        for agent_dict in metadata[ 'agents' ]:
            agent_id = agent_dict[ 'id' ]
            agent_xml = agent_dict[ 'agent_config' ]
            url = '/repository/view_agent_metadata?repository_id=%s&changeset_revision=%s&agent_id=%s' % \
                  ( self.security.encode_id( repository.id ), changeset_revision, agent_id )
            self.visit_url( url )
            self.check_for_strings( agent_metadata_strings_displayed )
            self.load_display_agent_page( repository, agent_xml_path=agent_xml,
                                         changeset_revision=changeset_revision,
                                         strings_displayed=agent_page_strings_displayed,
                                         strings_not_displayed=[] )

    def check_repository_invalid_agents_for_changeset_revision( self, repository, changeset_revision, strings_displayed=[], strings_not_displayed=[] ):
        '''Load the invalid agent page for each invalid agent associated with this changeset revision and verify the received error messages.'''
        repository_metadata = self.get_repository_metadata_by_changeset_revision( repository, changeset_revision )
        metadata = repository_metadata.metadata
        assert 'invalid_agents' in metadata, 'Metadata for changeset revision %s does not define invalid agents' % changeset_revision
        for agent_xml in metadata[ 'invalid_agents' ]:
            self.load_invalid_agent_page( repository,
                                         agent_xml=agent_xml,
                                         changeset_revision=changeset_revision,
    ...
    def create_repository_dependency( self,
        ...
        repository_names = []
        if complex:
            filename = 'agent_dependencies.xml'
            ...
            for agentshed_url, name, owner, changeset_revision in repository_tuples:
    ...
    def deactivate_repository( self, installed_repository, strings_displayed=[], strings_not_displayed=[] ):
        url = '/admin_agentshed/deactivate_or_uninstall_repository?id=%s' % self.security.encode_id( installed_repository.id )
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def display_galaxy_browse_repositories_page( self, strings_displayed=[], strings_not_displayed=[] ):
        url = '/admin_agentshed/browse_repositories'
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def display_installed_repository_manage_page( self, installed_repository, strings_displayed=[], strings_not_displayed=[] ):
        ...
        # Every place Galaxy's XXXX agent appears in attribute - need to quote.
    ...
    def display_installed_workflow_image( self, repository, workflow_name, strings_displayed=[], strings_not_displayed=[] ):
        url = '/admin_agentshed/generate_workflow_image?repository_id=%s&workflow_name=%s' % \
              ( self.security.encode_id( repository.id ), agent_shed_encode( workflow_name ) )
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def generate_complex_dependency_xml( self, filename, filepath, repository_tuples, package, version ):
        ...
        for agentshed_url, name, owner, changeset_revision in repository_tuples:
            dependency_entries.append( template.safe_substitute( agentshed_url=agentshed_url,
                                                                 owner=owner,
    ...
    def generate_temp_path( self, test_script_path, additional_paths=[] ):
        temp_path = os.path.join( self.agent_shed_test_tmp_dir, test_script_path, os.sep.join( additional_paths ) )
    ...
    def get_env_sh_path( self, agent_dependency_name, agent_dependency_version, repository ):
        '''Return the absolute path to an installed repository's env.sh file.'''
        env_sh_path = os.path.join( self.get_agent_dependency_path( agent_dependency_name, agent_dependency_version, repository ),
                                    'env.sh' )
        return env_sh_path
    ...
    def get_agent_dependency_path( self, agent_dependency_name, agent_dependency_version, repository ):
        '''Return the absolute path for an installed agent dependency.'''
        return os.path.join( self.galaxy_agent_dependency_dir,
    ...
    def get_agents_from_repository_metadata( self, repository, include_invalid=False ):
        '''Get a list of valid and (optionally) invalid agent dicts from the repository metadata.'''
        valid_agents = []
        invalid_agents = []
        for repository_metadata in repository.metadata_revisions:
            if 'agents' in repository_metadata.metadata:
                valid_agents.append( dict( agents=repository_metadata.metadata[ 'agents' ], changeset_revision=repository_metadata.changeset_revision ) )
            if include_invalid and 'invalid_agents' in repository_metadata.metadata:
                invalid_agents.append( dict( agents=repository_metadata.metadata[ 'invalid_agents' ], changeset_revision=repository_metadata.changeset_revision ) )
        return valid_agents, invalid_agents

    def get_agent_panel_section_from_api( self, metadata ):
        agent_metadata = metadata[ 'agents' ]
        agent_guid = urllib.quote_plus( agent_metadata[ 0 ][ 'guid' ], safe='' )
        api_url = '/%s' % '/'.join( [ 'api', 'agents', agent_guid ] )
        self.visit_galaxy_url( api_url )
        agent_dict = loads( self.last_page() )
        agent_panel_section = agent_dict[ 'panel_section_name' ]
        return agent_panel_section

    def get_agent_panel_section_from_repository_metadata( self, metadata ):
        agent_metadata = metadata[ 'agents' ]
        agent_guid = agent_metadata[ 0 ][ 'guid' ]
        assert 'agent_panel_section' in metadata, 'Agent panel section not found in metadata: %s' % metadata
        agent_panel_section_metadata = metadata[ 'agent_panel_section' ]
        # agent_section_dict = dict( agent_config=guids_and_configs[ guid ],
        #                           id=section_id,
        #                           name=section_name,
        #                           version=section_version )
        # This dict is appended to agent_panel_section_metadata[ agent_guid ]
        agent_panel_section = agent_panel_section_metadata[ agent_guid ][ 0 ][ 'name' ]
        return agent_panel_section
    ...
    def import_workflow( self, repository, workflow_name, strings_displayed=[], strings_not_displayed=[] ):
        url = '/admin_agentshed/import_workflow?repository_id=%s&workflow_name=%s' % \
            ( self.security.encode_id( repository.id ), agent_shed_encode( workflow_name ) )
        self.visit_galaxy_url( url )
    ...
    def initiate_installation_process( self,
        ...
        # installation process. This regex will return the agent shed repository IDs in group(1), the encoded_kwd parameter in
        # group(2), and the reinstalling flag in group(3) and pass them to the manage_repositories method in the Galaxy
        # admin_agentshed controller.
        ...
        if install_parameters:
            ...
            url = '/admin_agentshed/manage_repositories?operation=install&agent_shed_repository_ids=%s&encoded_kwd=%s&reinstalling=%s' % \
                ( ','.join( galaxy.util.listify( repository_ids ) ), encoded_kwd, reinstalling )
            self.visit_galaxy_url( url )
            return galaxy.util.listify( repository_ids )

    def install_repositories_from_search_results( self, repositories, install_agent_dependencies=False,
        ...
        in a browser, but Twill manages to lose the 'agentshedgalaxyurl' cookie between one page and the next, so it's necessary to work
        around this by explicitly visiting the prepare_for_install method on the Galaxy side.
        '''
        url = '/admin_agentshed/prepare_for_install?agent_shed_url=%s&repository_ids=%s&changeset_revisions=%s' % \
        ...
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
        if 'install_agent_dependencies' in self.last_page():
            form = tc.browser.get_form( 'select_agent_panel_section' )
            checkbox = form.find_control( id="install_agent_dependencies" )
            checkbox.disabled = False
            if install_agent_dependencies:
                checkbox.selected = True
                kwd[ 'install_agent_dependencies' ] = 'True'
            else:
                checkbox.selected = False
                kwd[ 'install_agent_dependencies' ] = 'False'
        self.submit_form( 1, 'select_agent_panel_section_button', **kwd )
        repository_ids = self.initiate_installation_process()
        self.wait_for_repository_installation( repository_ids )

    def install_repository( self, name, owner, category_name, install_agent_dependencies=False,
                            ...
                            post_submit_strings_displayed=[], new_agent_panel_section_label=None, includes_agents_for_display_in_agent_panel=True,
                            **kwd ):
        self.browse_agent_shed( url=self.url )
        self.browse_category( test_db_util.get_category_by_name( category_name ) )
        self.preview_repository_in_agent_shed( name, owner, strings_displayed=preview_strings_displayed )
        ...
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
        # This section is tricky, due to the way twill handles form submission. The agent dependency checkbox needs to
        # be hacked in through tc.browser, putting the form field in kwd doesn't work.
        form = tc.browser.get_form( 'select_agent_panel_section' )
        if form is None:
            form = tc.browser.get_form( 'select_shed_agent_panel_config' )
        assert form is not None, 'Could not find form select_shed_agent_panel_config or select_agent_panel_section.'
        kwd = self.set_form_value( form, kwd, 'install_agent_dependencies', install_agent_dependencies )
        kwd = self.set_form_value( form, kwd, 'install_repository_dependencies', install_repository_dependencies )
        kwd = self.set_form_value( form, kwd, 'shed_agent_conf', self.shed_agent_conf )
        if new_agent_panel_section_label is not None:
            kwd = self.set_form_value( form, kwd, 'new_agent_panel_section_label', new_agent_panel_section_label )
        ...
        repository_ids = self.initiate_installation_process( new_agent_panel_section_label=new_agent_panel_section_label )
    ...
    def load_changeset_in_agent_shed( self, repository_id, changeset_revision, strings_displayed=[], strings_not_displayed=[] ):
    ...
    def load_checkable_revisions( self, strings_displayed=[], strings_not_displayed=[] ):
        params = urllib.urlencode( dict( do_not_test='false',
                                         ...
                                         skip_agent_test='false' ) )
        api_url = '%s?%s' % ( '/'.join( [ self.url, 'api', 'repository_revisions' ] ), params )
        self.visit_url( api_url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def load_display_agent_page( self, repository, agent_xml_path, changeset_revision, strings_displayed=[], strings_not_displayed=[] ):
        url = '/repository/display_agent?repository_id=%s&agent_config=%s&changeset_revision=%s' % \
              ( self.security.encode_id( repository.id ), agent_xml_path, changeset_revision )
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def load_galaxy_agent_migrations_page( self, strings_displayed=[], strings_not_displayed=[] ):
        url = '/admin/review_agent_migration_stages'
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def load_invalid_agent_page( self, repository, agent_xml, changeset_revision, strings_displayed=[], strings_not_displayed=[] ):
        url = '/repository/load_invalid_agent?repository_id=%s&agent_config=%s&changeset_revision=%s' % \
              ( self.security.encode_id( repository.id ), agent_xml, changeset_revision )
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def load_page_for_installed_agent( self, agent_guid, strings_displayed=[], strings_not_displayed=[] ):
        url = '/agent_runner?agent_id=%s' % agent_guid
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def load_workflow_image_in_agent_shed( self, repository, workflow_name, changeset_revision=None, strings_displayed=[], strings_not_displayed=[] ):
        if not changeset_revision:
            changeset_revision = self.get_repository_tip( repository )
        ...
        url = '/repository/generate_workflow_image?repository_metadata_id=%s&workflow_name=%s' % \
              ( self.security.encode_id( metadata.id ), agent_shed_encode( workflow_name ) )
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def preview_repository_in_agent_shed( self, name, owner, changeset_revision=None, strings_displayed=[], strings_not_displayed=[] ):
        repository = test_db_util.get_repository_by_name_and_owner( name, owner )
        if not changeset_revision:
            changeset_revision = self.get_repository_tip( repository )
        self.visit_url( '/repository/preview_agents_in_changeset?repository_id=%s&changeset_revision=%s' %
                        ( self.security.encode_id( repository.id ), changeset_revision ) )
        self.check_for_strings( strings_displayed, strings_not_displayed )

    def preview_workflow_in_agent_shed( self, repository_name, owner, workflow_name, strings_displayed=[], strings_not_displayed=[] ):
        ...
        url = '/repository/view_workflow?workflow_name=%s&repository_metadata_id=%s' % \
              ( agent_shed_encode( workflow_name ), self.security.encode_id( metadata[0].id ) )
        self.visit_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def reinstall_repository( self,
                              ...
                              strings_displayed=[],
                              strings_not_displayed=[] ):
        url = '/admin_agentshed/reselect_agent_panel_section?id=%s' % self.security.encode_id( installed_repository.id )
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed=[] )
        # Build the url that will simulate a filled-out form being submitted. Due to a limitation in twill, the reselect_agent_panel_section
        # form doesn't get parsed correctly.
        encoded_repository_id = self.security.encode_id( installed_repository.id )
        params = dict( id=encoded_repository_id, no_changes=no_changes, new_agent_panel_section_label=new_agent_panel_section_label )
        doseq = False
        ...
        if install_agent_dependencies:
            params[ 'install_agent_dependencies' ] = [ 'True', 'True' ]
            doseq = True
        else:
            params[ 'install_agent_dependencies' ] = False
        url = '/admin_agentshed/reinstall_repository'
        ...
        repository_ids = self.initiate_installation_process( install_agent_dependencies,
    ...
    def reset_installed_repository_metadata( self, repository ):
        url = '/admin_agentshed/reset_repository_metadata?id=%s' % self.security.encode_id( repository.id )
        self.visit_galaxy_url( url )
        self.check_for_strings( [ 'Metadata has been reset' ] )

    def reset_metadata_on_selected_repositories( self, repository_ids ):
        self.visit_url( '/admin/reset_metadata_on_selected_repositories_in_agent_shed' )
    ...
    def reset_metadata_on_selected_installed_repositories( self, repository_ids ):
        self.visit_galaxy_url( '/admin_agentshed/reset_metadata_on_selected_installed_repositories' )
        kwd = dict( repository_ids=repository_ids )
        self.submit_form( form_no=1, button="reset_metadata_on_selected_repositories_button", **kwd )
    ...
    def repair_installed_repository( self, repository ):
        repository_id = self.security.encode_id( repository.id )
        url = '/admin_agentshed/repair_repository?id=%s' % repository_id
        self.visit_galaxy_url( url )
    ...
    def search_for_valid_agents( self, search_fields={}, exact_matches=False, strings_displayed=[], strings_not_displayed=[], from_galaxy=False ):
        if from_galaxy:
            galaxy_url = '?galaxy_url=%s' % self.galaxy_url
        else:
            galaxy_url = ''
        for field_name, search_string in search_fields.items():
            url = '/repository/find_agents%s' % galaxy_url
            self.visit_url( url )
    ...
    def set_skip_agent_tsts_flag( self, repository, flag_value, reason, changeset_revision=None ):
        if changeset_revision is None:
            changeset_revision = self.get_repository_tip( repository )
        self.display_manage_repository_page( repository, changeset_revision=changeset_revision )
        form = tc.browser.get_form( 'skip_agent_tests' )
        assert form is not None, 'Could not find form skip_agent_tests.'
        for control in form.controls:
            control_name = str( control.name )
            if control_name == 'skip_agent_tests' and control.type == 'checkbox':
                checkbox = control.get()
                checkbox.selected = flag_value
            elif control_name == 'skip_agent_tests_comment':
                tc.browser.clicked( form, control )
                tc.formvalue( 'skip_agent_tests', control_name, reason )
        kwd = dict()
        self.submit_form( 'skip_agent_tests', 'skip_agent_tests_button', **kwd )
        if flag_value is True:
            self.check_for_strings( strings_displayed=[ 'Agents in this revision will not be tested by the automated test framework' ] )
        else:
            self.check_for_strings( strings_displayed=[ 'Agents in this revision will be tested by the automated test framework' ] )
    ...
    def update_agent_shed_status( self ):
        url = '/admin_agentshed/update_agent_shed_status_for_installed_repository?all_installed_repositories=True'
        self.visit_galaxy_url( url )
    ...
    def verify_installed_repository_no_agent_panel_section( self, repository ):
        '''Verify that there is no 'agent_panel_section' entry in the repository metadata.'''
        metadata = repository.metadata
        assert 'agent_panel_section' not in metadata, 'Agent panel section incorrectly found in metadata: %s' % metadata
    ...
    def verify_installed_repository_data_table_entries( self, required_data_table_entries ):
        ...
        data_tables, error_message = xml_util.parse_xml( self.shed_agent_data_table_conf )
        found = False
        # With the agent shed, the "path" attribute that is hard-coded into the agent_data_tble_conf.xml
        # file is ignored.  This is because the agent shed requires the directory location to which this
        # path points to be empty except when a specific agent is loaded.  The default location for this
        # directory configured for the agent shed is <Galaxy root>/shed-agent-data.  When a agent is loaded
        # in the agent shed, all contained .loc.sample files are copied to this directory and the
        # AgentDataTableManager parses and loads the files in the same way that Galaxy does with a very
        # important exception.  When the agent shed loads a agent and parses and loads the copied ,loc.sample
        # files, the AgentDataTableManager is already instantiated, and so its add_new_entries_from_config_file()
        # method is called and the agent_data_path parameter is used to over-ride the hard-coded "agent-data"
        # directory that Galaxy always uses.
        #
        # Agent data table xml structure:
        # <tables>
        #     <table comment_char="#" name="sam_fa_indexes">
        #        <columns>line_type, value, path</columns>
        #        <file path="agent-data/sam_fa_indices.loc" />
        #     </table>
        # </tables>
        ...
        for table_elem in data_tables.findall( 'table' ):
            ...
            for required_data_table_entry in required_data_table_entries:
                ...
                if 'name' in table_elem.attrib and table_elem.attrib[ 'name' ] == required_data_table_entry:
                    ...
                    # We have something like: <file path="agent-data/sam_fa_indices.loc" />
                    # The "path" attribute of the "file" tag is the location that Galaxy always uses because the
                    # Galaxy AgentDataTableManager was implemented in such a way that the hard-coded path is used
                    # rather than allowing the location to be a configurable setting like the agent shed requires.
                    file_path = file_elem.get( 'path', None )
                    # The value of file_path will be something like: "agent-data/all_fasta.loc"
                    assert file_path is not None, 'The "path" attribute is missing for the %s entry.' % required_data_table_entry
                    # The following test is probably not necesary, but the agent-data directory should exist!
                    galaxy_agent_data_dir, loc_file_name = os.path.split( file_path )
                    assert galaxy_agent_data_dir is not None, 'The hard-coded Galaxy agent-data directory is missing for the %s entry.' % required_data_table_entry
                    assert os.path.exists( galaxy_agent_data_dir ), 'The Galaxy agent-data directory does not exist.'
                    # Make sure the loc_file_name was correctly copied into the configured directory location.
                    configured_file_location = os.path.join( self.agent_data_path, loc_file_name )
                    ...
                    # shed_agent_data_table_conf.xml, so all is well!
                    break
            if found:
                break
        ...
        assert found, 'No entry for %s in %s.' % ( required_data_table_entry, self.shed_agent_data_table_conf )
    ...
    def verify_repository_in_capsule( self, repository_archive, repository_name, repository_owner ):
        ...
        for root, dirs, files in os.walk( repository_extraction_dir ):
            for filename in files:
                if filename in [ 'agent_dependencies.xml', 'repository_dependencies.xml' ]:
    ...
    def verify_agent_metadata_for_installed_repository( self, installed_repository, strings_displayed=[], strings_not_displayed=[] ):
        repository_id = self.security.encode_id( installed_repository.id )
        for agent in installed_repository.metadata[ 'agents' ]:
            strings = list( strings_displayed )
            strings.extend( [ agent[ 'id' ], agent[ 'description' ], agent[ 'version' ], agent[ 'guid' ], agent[ 'name' ] ] )
            params = dict( repository_id=repository_id, agent_id=agent[ 'id' ] )
            url = '/admin_agentshed/view_agent_metadata'
            self.visit_galaxy_url( url, params )
    ...
    def view_installed_workflow( self, repository, workflow_name, strings_displayed=[], strings_not_displayed=[] ):
        url = '/admin_agentshed/view_workflow?repository_id=%s&workflow_name=%s' % \
            ( self.security.encode_id( repository.id ), agent_shed_encode( workflow_name ) )
        self.visit_galaxy_url( url )
        self.check_for_strings( strings_displayed, strings_not_displayed )
    ...
    def wait_for_repository_installation( self, repository_ids ):
        final_states = [ galaxy_model.AgentShedRepository.installation_status.ERROR,
                         galaxy_model.AgentShedRepository.installation_status.INSTALLED ]
