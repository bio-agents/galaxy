...
define([
    ...
    module( "Node unit test", {
        setup: function() {
            ...
            this.app = create_app();
            this.element = this.app.$newNodeElement( "agent", "newnode" );
    ...
    test( "make active", function() {
        ok( ! this.element.hasClass( "agentForm-active" ) );
    ...
    test( "error", function() {
        // Test body of agentFormBody div updated and workflow notified of change.
        var test = this;
        this.expect_workflow_node_changed( function() {
            test.node.error( "TOOL ERROR" );
            equal( $( test.$(".agentFormBody").children()[ 0 ] ).html(), "TOOL ERROR" );
        } );
    } );
    ...
    test( "init_field_data properties", function() {
        ...
        this.expect_workflow_node_changed( function( ) {
            var data = {
                ...
                agent_state: "ok",
                agent_errors: false,
                agenttip: "agent agenttip",
                annotation: "agent annotation",
                workflow_outputs: [ {"output_name": "out1"} ],
                label: "Cat that data.",
            };
            node.init_field_data( data );
            equal( node.type, "agent" );
            equal( node.name, "cat1" );
            equal( node.form_html, "<form>" );
            equal( node.agent_state, "ok" );
            equal( node.agenttip, "agent agenttip" );
            equal( node.annotation, "agent annotation" );
    ...
    test( "node added to workflow", function() {
        ...
        var node = this.app.workflow.create_node( "agent", "Cat Files", "cat1" );
    ...
    module( "Node view ", {
       setup: function() {
        ...
        set_for_node: function( node ) {
            var element = $("<div><div class='agentFormBody'></div></div>");
    ...
    test( "agent error styling", function() {
        this.set_for_node( { agent_errors: false } );
        this.view.render();
        ok( ! this.view.$el.hasClass( "agent-node-error" ) );
        this.set_for_node( { agent_errors: true } );
        this.view.render();
        ok( this.view.$el.hasClass( "agent-node-error" ) );
    } );
