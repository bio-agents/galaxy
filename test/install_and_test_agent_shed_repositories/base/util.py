...
sys.path[1:1] = [ os.path.join( galaxy_root, "lib" ),
                  ...
                  os.path.join( galaxy_root, 'lib', 'agent_shed', 'scripts', 'api' ) ]
...
import install_and_test_agent_shed_repositories.functional.test_install_repositories as test_install_repositories
...
from agent_shed.util.xml_util import parse_xml
...
# Create a blank shed_agent_conf.xml to define the installed repositories.
shed_agent_conf_xml_template = '''<?xml version="1.0"?>
<agentbox agent_path="${shed_agent_path}">
</agentbox>
'''

# Since we will be running functional tests we'll need the upload agent, but the rest can be omitted.
...
# Set up an empty shed_agent_data_table_conf.xml.
...
# agent shed's configuration file that includes the agent shed from which repositories will be installed.
agent_sheds_conf_xml = '''<?xml version="1.0"?>
<agent_sheds>
...
# If we have a agent_data_table_conf.test.xml, set it up to be loaded when the UniverseApplication is started.
# This allows one to specify a set of agent data that is used exclusively for testing, and not loaded into any
...
if os.path.exists( 'agent_data_table_conf.test.xml' ):
    additional_agent_data_tables = 'agent_data_table_conf.test.xml'
    additional_agent_data_path = os.environ.get( 'GALAXY_INSTALL_TEST_EXTRA_TOOL_DATA_PATH',
                                                os.path.join( 'test-data-repo', 'location' ) )
else:
    additional_agent_data_tables = None
    additional_agent_data_path = None

agent_data_table_conf = None
# Set up default agent data tables. If a non-sample version is in config/, use that. Otherwise iterate through lower
# priority versions.
for conf in [ 'agent_data_table_conf.test.xml',
              'config/agent_data_table_conf.xml',
              'config/agent_data_table_conf.xml.sample',
              'agent_data_table_conf.xml',
              'agent_data_table_conf.xml.sample' ]:
    if os.path.exists( conf ):
        agent_data_table_conf = conf
        break
...
# of the defaults, the GALAXY_INSTALL_TEST_TOOL_SHEDS_CONF must refer to a agent shed configuration file that contains
# a definition for that agent shed.
galaxy_agent_shed_url = os.environ.get( 'GALAXY_INSTALL_TEST_TOOL_SHED_URL', None )
agent_shed_api_key = os.environ.get( 'GALAXY_INSTALL_TEST_TOOL_SHED_API_KEY', None )
...
REPOSITORIES_WITH_TOOLS = 'repositories_with_agents'
TOOL_DEPENDENCY_DEFINITIONS = 'agent_dependency_definitions'
...
class ReportResults( Plugin ):
    ...
    def addSuccess( self, test ):
        '''Only record test IDs that correspond to agent functional tests.'''
        if 'TestForAgent' in test.id():
            test_id = test.id()
...
class RepositoryMetadataApplication( object ):
    """Application that enables updating repository_metadata table records in the Agent Shed."""
    ...
    def __init__( self, config ):
        ...
        self.model = galaxy.webapps.agent_shed.model.mapping.init( self.config.file_path,
...
def display_agent_dependencies_by_name( agent_dependency_tups ):
    """Group summary display by repository owner."""
    agent_dependency_tups_by_name = {}
    for agent_dependency_tup in agent_dependency_tups:
        name, type, version = agent_dependency_tup
        if name:
            if name in agent_dependency_tups_by_name:
                processed_agent_dependency_tups_by_name = agent_dependency_tups_by_name.get( name, [] )
                if agent_dependency_tup not in processed_agent_dependency_tups_by_name:
                    agent_dependency_tups_by_name[ name ].append( agent_dependency_tup )
            else:
                agent_dependency_tups_by_name[ name ] = [ agent_dependency_tup ]
    # Display grouped summary.
    for name, agent_dependency_tups in agent_dependency_tups_by_name.items():
        print "# "
        for agent_dependency_tup in agent_dependency_tups:
            name, type, version = agent_dependency_tup
...
def get_database_version( app ):
    ...
    instance's database migration version. If a agent shed UniverseApplication
    is provided, it returns the agent shed's database migration version.
    '''
...
def get_missing_agent_dependencies( repository, all_missing_agent_dependencies=None ):
    """
    Return the entire list of missing agent dependencies for the received repository.  The entire
    dependency tree will be inspected.
    """
    if all_missing_agent_dependencies is None:
        all_missing_agent_dependencies = []
    print 'Checking revision %s of repository %s owned by %s for missing agent dependencies.' % \
        ( repository.changeset_revision, repository.name, repository.owner )
    all_missing_agent_dependencies.extend( repository.missing_agent_dependencies )
    for missing_agent_dependency in repository.missing_agent_dependencies:
        print 'Agent dependency %s version %s has status %s.' % \
            ( missing_agent_dependency.name, missing_agent_dependency.version, missing_agent_dependency.status )
    for repository_dependency in repository.repository_dependencies:
        if repository_dependency.missing_agent_dependencies:
            all_missing_agent_dependencies.extend( get_missing_agent_dependencies( repository_dependency,
                                                                                 all_missing_agent_dependencies ) )
    return all_missing_agent_dependencies


def get_repositories_to_install( agent_shed_url, test_framework ):
    """
    ...
    [{ "changeset_revision": <revision>,
       "encoded_repository_id": <encoded repository id from the agent shed>,
       "name": <name>,
       "owner": <owner>,
       "agent_shed_url": <url> }]
    """
    ...
    if test_framework == REPOSITORIES_WITH_TOOLS:
        ...
        params = dict( do_not_test='false',
                       ...
                       malicious='false',
                       skip_agent_test='false' )
    api_url = get_api_url( base=agent_shed_url, parts=parts, params=params )
    ...
    for baseline_repository_dict in baseline_repository_dicts:
        # We need to get some details from the agent shed API, such as repository name and owner, to pass on to the
        # module that will generate the install methods.
        repository_dict, error_message = get_repository_dict( galaxy_agent_shed_url, baseline_repository_dict )
        if error_message:
            ...
            if not deprecated:
                ...
                if changeset_revision != hg_util.INITIAL_CHANGELOG_HASH:
                    # If testing repositories of type agent_dependency_definition, filter accordingly.
    ...
    if testing_single_repository_dict:
        ...
        return repository_dicts, error_message
    # Get a list of repositories to test from the agent shed specified in the GALAXY_INSTALL_TEST_TOOL_SHED_URL
    # environment variable.
    print "The Agent Shed's API url...\n%s" % api_url
...
def get_repository_dependencies_dicts( agent_shed_url, encoded_repository_metadata_id ):
    """
    Return the list of dictionaries that define all repository dependencies of the repository_metadata
    record associated with the received encoded_repository_metadata_id via the Agent Shed API.
    """
    error_message = ''
    parts = [ 'api', 'repository_revisions', encoded_repository_metadata_id, 'repository_dependencies' ]
    api_url = get_api_url( base=agent_shed_url, parts=parts )
...
def get_time_last_tested( agent_shed_url, encoded_repository_metadata_id ):
    """
    Return the datetime value stored in the Agent Shed's repository_metadata.time_last_tested column
    via the Agent Shed API.
    """
    error_message = ''
    ...
    if isinstance( repository_metadata_dict, dict ):
        # The agent_test_results used to be stored as a single dictionary rather than a list, but we currently
        # return a list.
...
def get_agent_test_results_dict( agent_test_results_dicts ):
    if agent_test_results_dicts:
        # Inspect the agent_test_results_dict for the last test run to make sure it contains only a test_environment
        # entry.  If it contains more entries, then the script ~/agent_shed/api/check_repositories_for_functional_tests.py
        ...
        agent_test_results_dict = {}
    return agent_test_results_dict


def get_agent_test_results_dicts( agent_shed_url, encoded_repository_metadata_id ):
    """
    Return the list of dictionaries contained in the Agent Shed's repository_metadata.agent_test_results
    column via the Agent Shed API.
    """
    error_message = ''
    parts = [ 'api', 'repository_revisions', encoded_repository_metadata_id ]
    api_url = get_api_url( base=agent_shed_url, parts=parts )
    ...
    if isinstance( repository_metadata_dict, dict ):
        # The agent_test_results used to be stored as a single dictionary rather than a list, but we currently
        # return a list.
        agent_test_results = listify( repository_metadata_dict.get( 'agent_test_results', [] ) )
        return agent_test_results, error_message
    else:
        error_message = 'The url %s returned the invalid repository_metadata_dict %s' % ( str( api_url ), str( repository_metadata_dict ) )
        return None, error_message
...
def initialize_install_and_test_statistics_dict():
    ...
    install_and_test_statistics_dict[ 'successful_agent_dependency_installations' ] = []
    install_and_test_statistics_dict[ 'repositories_with_installation_error' ] = []
    install_and_test_statistics_dict[ 'agent_dependencies_with_installation_error' ] = []
...
def initialize_agent_tests_results_dict( app, agent_test_results_dict ):
    test_environment_dict = agent_test_results_dict.get( 'test_environment', {} )
    if len( test_environment_dict ) == 0:
        # Set information about the agent shed to nothing since we cannot currently determine it from here.
        # We could eventually add an API method...
        test_environment_dict = dict( agent_shed_database_version='',
    ...
    # Initialize and populate the agent_test_results_dict.
    agent_test_results_dict[ 'test_environment' ] = test_environment_dict
    agent_test_results_dict[ 'passed_tests' ] = []
    agent_test_results_dict[ 'failed_tests' ] = []
    agent_test_results_dict[ 'installation_errors' ] = dict( current_repository=[],
                                                            repository_dependencies=[],
                                                            agent_dependencies=[] )
    agent_test_results_dict[ 'successful_installations' ] = dict( current_repository=[],
                                                                 repository_dependencies=[],
                                                                 agent_dependencies=[] )
    return agent_test_results_dict
...
def install_repository( app, repository_dict ):
    """Install a repository defined by the received repository_dict from the agent shed into Galaxy."""
    name = str( repository_dict.get( 'name', None ) )
    owner = str( repository_dict.get( 'owner', None ) )
    changeset_revision = str( repository_dict.get( 'changeset_revision', None ) )
    error_message = ''
    ...
    # embedded Galaxy application, with agent dependencies and repository dependencies, if any.
    ...
    # install the specified repository with agent and repository dependencies also selected for installation.
...
def is_excluded( exclude_list_dicts, name, owner, changeset_revision, encoded_repository_metadata_id ):
    ...
    for exclude_dict in exclude_list_dicts:
        ...
        repository_dependency_dicts, error_message = \
            get_repository_dependencies_dicts( galaxy_agent_shed_url, encoded_repository_metadata_id )
        if error_message:
...
def parse_exclude_list( xml_filename ):
    ...
    # <?xml version="1.0"?>
    # <blacklist>
    #    <repositories agent_shed="http://testagentshed.g2.bx.psu.edu">
    #        <reason>
    ...
    if error_message:
        ...
        return exclude_list
    agent_sheds = xml_tree.findall( 'repositories' )
    xml_element = []
    exclude_count = 0
    for agent_shed in agent_sheds:
        if galaxy_agent_shed_url != agent_shed.attrib[ 'agent_shed' ]:
            continue
        else:
            xml_element = agent_shed
...
def populate_dependency_install_containers( app, repository, repository_identifier_tup, install_and_test_statistics_dict,
    ...
    itself was successfully installed) immediate repository and agent dependencies.  The entire dependency
    ...
    agent_test_results_dict[ 'successful_installations' ][ 'current_repository' ].append( repository_identifier_dict )
    ...
    if repository.missing_repository_dependencies:
        ...
        for missing_repository_dependency in repository.missing_repository_dependencies:
            agent_shed = str( missing_repository_dependency.agent_shed )
            ...
            missing_repository_dependency_info_dict = dict( agent_shed=agent_shed,
                                                            name=name,
                                                            owner=owner,
                                                            changeset_revision=changeset_revision,
                                                            error_message=error_message )
            agent_test_results_dict[ 'installation_errors' ][ 'repository_dependencies' ].append( missing_repository_dependency_info_dict )
    if repository.missing_agent_dependencies:
        print 'The following agent dependencies for revision %s of repository %s owned by %s have installation errors:' % \
            ( repository_changeset_revision, repository_name, repository_owner )
        params[ 'test_install_error' ] = True
        # Keep statistics for this repository's agent dependencies that resulted in installation errors.
        for missing_agent_dependency in repository.missing_agent_dependencies:
            name = str( missing_agent_dependency.name )
            type = str( missing_agent_dependency.type )
            version = str( missing_agent_dependency.version )
            error_message = unicodify( missing_agent_dependency.error_message )
            print 'Version %s of agent dependency %s %s has the following installation error:' % ( version, type, name )
            # Use log.debug here instead of print because print will throw UnicodeEncodeError exceptions.
            log.debug( '%s' % error_message )
            identity_tup = ( name, type, version )
            processed_agent_dependencies_with_installation_error = \
                install_and_test_statistics_dict.get( 'agent_dependencies_with_installation_error', [] )
            if identity_tup not in processed_agent_dependencies_with_installation_error:
                install_and_test_statistics_dict[ 'agent_dependencies_with_installation_error' ].append( identity_tup )
            missing_agent_dependency_info_dict = dict( type=type,
                                                      name=name,
                                                      version=version,
                                                      error_message=error_message )
            agent_test_results_dict[ 'installation_errors' ][ 'agent_dependencies' ].append( missing_agent_dependency_info_dict )
    if repository.installed_repository_dependencies:
        print 'The following repository dependencies for revision %s of repository %s owned by %s are installed:' % \
            ( repository_changeset_revision, repository_name, repository_owner )
        # Keep statistics for this repository's agent dependencies that resulted in successful installations.
        for repository_dependency in repository.installed_repository_dependencies:
            agent_shed = str( repository_dependency.agent_shed )
            ...
            repository_dependency_info_dict = dict( agent_shed=agent_shed,
                                                    name=name,
                                                    owner=owner,
                                                    changeset_revision=changeset_revision )
            agent_test_results_dict[ 'successful_installations' ][ 'repository_dependencies' ].append( repository_dependency_info_dict )
    if repository.installed_agent_dependencies:
        print 'The following agent dependencies for revision %s of repository %s owned by %s are installed:' % \
            ( repository_changeset_revision, repository_name, repository_owner )
        # Keep statistics for this repository's agent dependencies that resulted in successful installations.
        for agent_dependency in repository.installed_agent_dependencies:
            name = str( agent_dependency.name )
            type = str( agent_dependency.type )
            version = str( agent_dependency.version )
            installation_directory = agent_dependency.installation_directory( app )
            print 'Version %s of agent dependency %s %s is installed in: %s' % ( version, type, name, installation_directory )
            identity_tup = ( name, type, version )
            processed_successful_agent_dependency_installations = \
                install_and_test_statistics_dict.get( 'successful_agent_dependency_installations', [] )
            if identity_tup not in processed_successful_agent_dependency_installations:
                install_and_test_statistics_dict[ 'successful_agent_dependency_installations' ].append( identity_tup )
            agent_dependency_info_dict = dict( type=type,
                                              name=name,
                                              version=version,
                                              installation_directory=installation_directory )
            agent_test_results_dict[ 'successful_installations' ][ 'agent_dependencies' ].append( agent_dependency_info_dict )
    return params, install_and_test_statistics_dict, agent_test_results_dict
...
def populate_install_containers_for_repository_dependencies( app, repository, repository_metadata_id, install_and_test_statistics_dict,
    ...
    agent_test_results dictionary must be inspected for each dependency to make sure installation containers
    ...
    print 'Potentially populating install containers for repository dependencies of revision %s of repository %s owned by %s.' % \
        ( repository_changeset_revision, repository_name, repository_owner )
    repository_dependencies_dicts, error_message = get_repository_dependencies_dicts( galaxy_agent_shed_url, repository_metadata_id )
    if error_message:
        ...
        for repository_dependencies_dict in repository_dependencies_dicts:
            ...
            # Get the current list of agent_test_results dictionaries associated with the repository_metadata
            # record in the agent shed.
            agent_test_results_dicts, error_message = get_agent_test_results_dicts( galaxy_agent_shed_url,
                                                                                  required_repository_metadata_id )
            if error_message:
                print 'Cannot check install container for version %s of repository dependency %s owned by %s ' % \
                    ( changeset_revision, name, owner )
                print 'due to the following error getting agent_test_results:\n%s' % str( error_message )
            else:
                # The assumption is that the Agent Shed's install and test framework is executed no more than once per 24 hour
                # period, so check the required repository's time_last_tested value to see if its agent_test_results column
                ...
                # datetime.isoformat().
                time_last_tested, error_message = get_time_last_tested( galaxy_agent_shed_url, required_repository_metadata_id )
                ...
                if time_last_tested is None:
                    ...
                    if already_populated:
                        ...
                        print 'Version %s of repository dependency %s owned by %s was last tested more than 20 hours ago.' % \
                            ( changeset_revision, name, owner )
                # Inspect the agent_test_results_dict for the last test run to see if it has not yet been populated.
                if len( agent_test_results_dicts ) == 0:
                    agent_test_results_dict = {}
                else:
                    agent_test_results_dict = agent_test_results_dicts[ 0 ]
                    if len( agent_test_results_dict ) <= 1:
                        ...
                        agent_test_results_dict = {}
                # Make sure all expected entries are available in the agent_test_results_dict.
                agent_test_results_dict = initialize_agent_tests_results_dict( app, agent_test_results_dict )
                # Get the installed repository record from the Galaxy database.
                cleaned_agent_shed_url = remove_protocol_from_agent_shed_url( galaxy_agent_shed_url )
                required_repository = \
                    suc.get_agent_shed_repository_by_shed_name_owner_changeset_revision( app,
                                                                                        cleaned_agent_shed_url,
                                                                                        name,
                                                                                        owner,
                                                                                        changeset_revision )
                if required_repository is not None:
                    ...
                    if required_repository.is_installed:
                        ...
                        params, install_and_test_statistics_dict, agent_test_results_dict = \
                            populate_dependency_install_containers( app,
                                                                    ...
                                                                    install_and_test_statistics_dict,
                                                                    agent_test_results_dict )
                        save_test_results_for_changeset_revision( galaxy_agent_shed_url,
                        ...
                        required_repository_installation_error_dict = dict( agent_shed=galaxy_agent_shed_url,
                                                                            name=name,
                                                                            owner=owner,
                                                                            changeset_revision=changeset_revision,
                                                                            error_message=required_repository.error_message )
                        agent_test_results_dict[ 'installation_errors' ][ 'repository_dependencies' ].append( required_repository_installation_error_dict )
                    ...
                    print 'Cannot retrieve revision %s of required repository %s owned by %s from the database ' % \
                        ( changeset_revision, name, owner )
                    print 'so agent_test_results cannot be saved at this time.'
                    print 'The attributes used to retrieve the record are:'
                    print 'agent_shed: %s name: %s owner: %s changeset_revision: %s' % \
                        ( cleaned_agent_shed_url, name, owner, changeset_revision )


def populate_shed_conf_file( shed_conf_file, agent_path, xml_elems=None ):
    """Populate the file defined by shed_conf_file with xml_elems or initialize it with a template string."""
    if xml_elems is None:
        agent_conf_template_parser = string.Template( shed_agent_conf_xml_template )
        xml_elems = agent_conf_template_parser.safe_substitute( shed_agent_path=agent_path )
    file( shed_conf_file, 'w' ).write( xml_elems )


def populate_galaxy_shed_agents_dict_file( galaxy_shed_agents_dict_file, shed_agents_dict=None ):
    """
    Populate the file defined by galaxy_shed_agents_dict_file with the contents of the shed_agents_dict
    dictionary.
    """
    if shed_agents_dict is None:
        shed_agents_dict = {}
    file( galaxy_shed_agents_dict_file, 'w' ).write( json.dumps( shed_agents_dict ) )
...
def print_install_and_test_results( install_stage_type, install_and_test_statistics_dict, error_message ):
    ...
    if error_message:
        ...
        successful_agent_dependency_installations = \
            install_and_test_statistics_dict.get( 'successful_agent_dependency_installations', None )
        agent_dependencies_with_installation_error = \
            install_and_test_statistics_dict.get( 'agent_dependencies_with_installation_error', None )
        ...
        if successful_agent_dependency_installations:
            print "# ----------------------------------------------------------------------------------"
            print "# The following %d agent dependencies were successfully installed:" % len( successful_agent_dependency_installations )
            display_agent_dependencies_by_name( successful_agent_dependency_installations )
        if agent_dependencies_with_installation_error:
            print "# ----------------------------------------------------------------------------------"
            print "# The following %d agent dependencies have installation errors:" % len( agent_dependencies_with_installation_error )
            display_agent_dependencies_by_name( agent_dependencies_with_installation_error )
        if all_tests_passed:
            print '# ----------------------------------------------------------------------------------'
...
def remove_protocol_from_agent_shed_url( base_url ):
    """Eliminate the protocol from the received base_url and return the possibly altered url."""
    # The agent_shed value stored in the agent_shed_repository record does not include the protocol, but does
    ...
    if base_url:
        if base_url.find( '://' ) > -1:
            try:
                ...
                log.debug( "Returning unchanged invalid base_url from remove_protocol_from_agent_shed_url: %s" % str( base_url ) )
                return base_url
            return base.rstrip( '/' )
        return base_url.rstrip( '/' )
    log.debug( "Returning base_url from remove_protocol_from_agent_shed_url: %s" % str( base_url ) )
    return base_url
...
def save_test_results_for_changeset_revision( url, agent_test_results_dicts, agent_test_results_dict, repository_dict,
                                              params, can_update_agent_shed ):
    """
    Update the repository metadata agent_test_results and appropriate flags using the Agent Shed API.  This method
    updates agent_test_results with the received agent_test_results_dict, sets the do_not_test and agents_functionally
    correct flags to the appropriate values and updates the time_last_tested field.
    """
    if can_update_agent_shed:
        ...
        if metadata_revision_id is not None:
            ...
            if name is None or owner is None or changeset_revision is None:
                ...
                # anything has changed here, we'll let the Agent Shed's repository registry handle the process of proper
                # categorization.  To enable this, we'll just remove entries from the Agent Shed's repository registry and
                # then add them back.  This will ensure proper categorization for this repository.
                registry_params = dict( agent_shed_url=galaxy_agent_shed_url, name=name, owner=owner )
                print "Removing entries for repository ", name, " owned by ", owner, "from the Agent Shed's repository registry."
                url = '%s' % ( common_util.url_join( galaxy_agent_shed_url, 'api', 'repositories', 'remove_repository_registry_entry' ) )
                response_dict = submit( url, registry_params, api_key=agent_shed_api_key, return_formatted=False )
                ...
                if status == 'error':
                    ...
                    print error_message
                print "Adding entries for repository ", name, " owned by ", owner, "into the Agent Shed's repository registry."
                url = '%s' % ( common_util.url_join( galaxy_agent_shed_url, 'api', 'repositories', 'add_repository_registry_entry' ) )
                response_dict = submit( url, registry_params, api_key=agent_shed_api_key, return_formatted=False )
                status = response_dict.get( 'status', 'ok' )
                if status == 'error':
                    ...
                    print error_message
                print '\n=============================================================\n'
                print 'Inserting the following into agent_test_results for revision %s of repository %s owned by %s:\n%s' % \
                    ( changeset_revision, name, owner, str( agent_test_results_dict ) )
                print 'Updating agent_test_results for repository_metadata id %s.' % metadata_revision_id
                agent_test_results_dicts.insert( 0, agent_test_results_dict )
                params[ 'agent_test_results' ] = agent_test_results_dicts
                # Set the time_last_tested entry so that the repository_metadata.time_last_tested will be set in the agent shed.
                params[ 'time_last_tested' ] = 'This entry will result in this value being set via the Agent Shed API.'
                url = '%s' % ( common_util.url_join( galaxy_agent_shed_url, 'api', 'repository_revisions', str( metadata_revision_id ) ) )
                print 'url: ', url
                print 'params: ', params
                try:
                    response_from_update = update( agent_shed_api_key, url, params, return_formatted=False )
                    print 'Result of inserting agent_test_results for revision %s of repository %s owned by %s:\n%s' % \
                        ( changeset_revision, name, owner, str( response_from_update ) )
                    print '\n=============================================================\n'
                except Exception, e:
                    log.exception( 'Error updating agent_test_results for repository_metadata id %s:\n%s' %
