...
def get_repository( repository_id ):
    return database_contexts.install_context.query( install_model.AgentShedRepository ) \
                                            .filter( install_model.AgentShedRepository.table.c.id == repository_id ) \
                                            .first()


def get_installed_repository_by_name_owner_changeset_revision( name, owner, changeset_revision ):
    return database_contexts.install_context.query( install_model.AgentShedRepository ) \
                                            .filter( and_( install_model.AgentShedRepository.table.c.name == name,
                                                           install_model.AgentShedRepository.table.c.owner == owner,
                                                           install_model.AgentShedRepository.table.c.installed_changeset_revision == changeset_revision ) ) \
                                            .one()
...
def get_agent_dependencies_for_installed_repository( repository_id, status=None, exclude_status=None ):
    if status is not None:
        return database_contexts.install_context.query( install_model.AgentDependency ) \
                                                .filter( and_( install_model.AgentDependency.table.c.agent_shed_repository_id == repository_id,
                                                               install_model.AgentDependency.table.c.status == status ) ) \
                                                .all()
    elif exclude_status is not None:
        return database_contexts.install_context.query( install_model.AgentDependency ) \
                                                .filter( and_( install_model.AgentDependency.table.c.agent_shed_repository_id == repository_id,
                                                               install_model.AgentDependency.table.c.status != exclude_status ) ) \
                                                .all()
    else:
        return database_contexts.install_context.query( install_model.AgentDependency ) \
                                                .filter( install_model.AgentDependency.table.c.agent_shed_repository_id == repository_id ) \
                                                .all()
