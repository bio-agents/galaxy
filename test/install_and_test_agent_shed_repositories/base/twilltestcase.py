...
import galaxy.model.agent_shed_install as install_model
import galaxy.util
...
class InstallTestRepository( TwillTestCase ):
    ...
    def setUp( self ):
        ...
        self.shed_agent_data_table_conf = os.environ.get( 'GALAXY_INSTALL_TEST_SHED_TOOL_DATA_TABLE_CONF' )
        self.file_dir = os.environ.get( 'GALAXY_INSTALL_TEST_FILE_DIR', None )
        self.agent_data_path = os.environ.get( 'GALAXY_INSTALL_TEST_TOOL_DATA_PATH' )
        self.shed_agent_conf = os.environ.get( 'GALAXY_INSTALL_TEST_SHED_TOOL_CONF' )
        # TODO: Figure out a way to alter these attributes during tests.
        self.galaxy_agent_dependency_dir = os.environ.get( 'GALAXY_INSTALL_TEST_TOOL_DEPENDENCY_DIR' )
        self.shed_agents_dict = {}

    def initiate_installation_process( self, install_agent_dependencies=False, install_repository_dependencies=True, no_changes=True,
    ...
    def install_repository( self, repository_info_dict, install_agent_dependencies=True, install_repository_dependencies=True,
                            strings_displayed=[], strings_not_displayed=[], preview_strings_displayed=[],
                            post_submit_strings_displayed=[], new_agent_panel_section_label=None, **kwd ):
        changeset_revision = repository_info_dict[ 'changeset_revision' ]
        encoded_repository_id = repository_info_dict[ 'repository_id' ]
        agent_shed_url = repository_info_dict[ 'agent_shed_url' ]
        # Pass galaxy_url to the agent shed in order to set cookies and redirects correctly.
        install_params = dict( repository_ids=encoded_repository_id,
                               changeset_revisions=changeset_revision,
                               galaxy_url=self.url )
        # If the agent shed does not have the same hostname as the Galaxy server being used for these tests,
        ...
        url = '%s/repository/install_repositories_by_revision' % agent_shed_url
        ...
        url = '/admin_agentshed/prepare_for_install'
    ...
    def wait_for_repository_installation( self, repository_ids ):
        final_states = [ install_model.AgentShedRepository.installation_status.ERROR,
                         install_model.AgentShedRepository.installation_status.INSTALLED ]
        ...
        if repository_ids:
            ...
            for repository_id in repository_ids:
                ...
                if not galaxy_repository.metadata:
                    ...
                    timeout_counter = 0
                    url = '/admin_agentshed/reset_repository_metadata?id=%s' % repository_id
                    self.visit_url( url )
