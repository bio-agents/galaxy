...
import install_and_test_agent_shed_repositories.base.test_db_util as test_db_util
from install_and_test_agent_shed_repositories.base.twilltestcase import InstallTestRepository
...
def generate_install_method( repository_dict=None ):
    ...
    for key, val in G.items():
        if key.startswith( 'TestInstallRepository_' ) or key.startswith( 'TestForAgent_' ):
            del G[ key ]
    agent_shed = str( repository_dict[ 'agent_shed_url' ] )
    ...
    test_method.__doc__ = "Installing revision %s of repository %s owned by %s from agent shed %s." % \
        ( changeset_revision, repository_name, repository_owner, agent_shed )
