...
class AgentsTestCase( api.ApiTestCase ):

    def setUp( self ):
        super( AgentsTestCase, self ).setUp( )
        self.dataset_populator = DatasetPopulator( self.galaxy_interactor )
        self.dataset_collection_populator = DatasetCollectionPopulator( self.galaxy_interactor )

    def test_index( self ):
        agent_ids = self.__agent_ids()
        assert "upload1" in agent_ids

    def test_no_panel_index( self ):
        index = self._get( "agents", data=dict( in_panel="false" ) )
        agents_index = index.json()
        # No need to flatten out sections, with in_panel=False, only agents are
        # returned.
        agent_ids = map( itemgetter( "id" ), agents_index )
        assert "upload1" in agent_ids

    @skip_without_agent( "cat1" )
    def test_show_repeat( self ):
        agent_info = self._show_valid_agent( "cat1" )
        parameters = agent_info[ "inputs" ]
    ...
    def test_show_conditional( self ):
        agent_info = self._show_valid_agent( "random_lines1" )

        cond_info = agent_info[ "inputs" ][ 2 ]
    ...
    def test_show_multi_data( self ):
        agent_info = self._show_valid_agent( "multi_data_param" )

        f1_info, f2_info = agent_info[ "inputs" ][ 0 ], agent_info[ "inputs" ][ 1 ]
    ...
    def _show_valid_agent( self, agent_id ):
        agent_show_response = self._get( "agents/%s" % agent_id, data=dict( io_details=True ) )
        self._assert_status_code_is( agent_show_response, 200 )
        agent_info = agent_show_response.json()
        self._assert_has_keys( agent_info, "inputs", "outputs", "panel_section_id" )
        return agent_info

    def test_upload1_paste( self ):
        history_id = self.dataset_populator.new_history()
        payload = self.dataset_populator.upload_payload( history_id, 'Hello World' )
        create_response = self._post( "agents", data=payload )
    ...
    @skip_without_agent( "multi_select" )
    ...
    @skip_without_agent( "library_data" )
    ...
    def test_run_cat1( self ):
        # Run simple non-upload agent with an input data parameter.
        history_id = self.dataset_populator.new_history()
    ...
    def test_run_by_versions( self ):
        for version in ["0.1", "0.2"]:
            ...
            history_id = self.dataset_populator.new_history()
            inputs = dict()
            outputs = self._run_and_get_outputs( agent_id="multiple_versions", history_id=history_id, inputs=inputs, agent_version=version )
            self.assertEquals( len( outputs ), 1 )
            output1 = outputs[ 0 ]
            output1_content = self.dataset_populator.get_history_dataset_content( history_id, dataset=output1 )
    ...
    @skip_without_agent( "column_param" )
    ...
    @skip_without_agent( "collection_creates_list" )
    ...
    @skip_without_agent( "collection_creates_list_2" )
    def test_list_collection_output_format_source( self ):
        # test using format_source with a agent
        history_id = self.dataset_populator.new_history()
        new_dataset1 = self.dataset_populator.new_dataset( history_id, content='#col1\tcol2' )
    ...
    def test_run_cat1_with_two_inputs( self ):
        # Run agent with an multiple data parameter and grouping (repeat)
        history_id = self.dataset_populator.new_history()
    ...
    @skip_without_agent( "output_action_change_format" )
    ...
    @skip_without_agent( "identifier_single" )
    ...
    @skip_without_agent( "identifier_multiple" )
    ...
    @skip_without_agent( "collection_mixed_param" )
    ...
    def _run_and_get_outputs( self, agent_id, history_id, inputs, agent_version=None ):
        return self._run_outputs( self._run( agent_id, history_id, inputs, agent_version=agent_version ) )
    ...
    def _run( self, agent_id, history_id, inputs, assert_ok=False, agent_version=None ):
        payload = self.dataset_populator.run_agent_payload(
            agent_id=agent_id,
            inputs=inputs,
    ...
    def __agent_ids( self ):
        index = self._get( "agents" )
        agents_index = index.json()
        # In panels by default, so flatten out sections...
        agents = []
        for agent_or_section in agents_index:
            if "elems" in agent_or_section:
                agents.extend( agent_or_section[ "elems" ] )
            else:
                agents.append( agent_or_section )

        agent_ids = map( itemgetter( "id" ), agents )
        return agent_ids
