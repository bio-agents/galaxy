...
class WorkflowExtractionApiTestCase( BaseWorkflowsApiTestCase ):
    ...
    @skip_without_agent( "random_lines1" )
    @skip_without_agent( "multi_data_param" )
    def test_extract_reduction_from_history( self ):
        ...
        implicit_hdca1, job_id1 = self._run_agent_get_collection_and_job_id( self.history_id, "random_lines1", inputs1 )
        inputs2 = {
            "f1": { "src": "hdca", "id": implicit_hdca1[ "id" ] },
            "f2": { "src": "hdca", "id": implicit_hdca1[ "id" ] },
        }
        reduction_run_output = self.dataset_populator.run_agent(
            agent_id="multi_data_param",
            inputs=inputs2,
        ...
        agent_steps = self._get_steps_of_type( downloaded_workflow, "agent", expected_len=2 )
        random_lines_map_step = agent_steps[ 0 ]
        reduction_step = agent_steps[ 1 ]
    ...
    @skip_without_agent( "collection_paired_test" )
...
test_data:
  text_input1:
    type: paired
""")
        job_id = self._job_id_for_agent( jobs_summary.jobs, "collection_paired_test" )
        downloaded_workflow = self._extract_and_download_workflow(
...
test_data:
  text_input1:
    type: "list:paired"
        """)
        job1_id = self._job_id_for_agent( jobs_summary.jobs, "cat1" )
        job2_id = self._job_id_for_agent( jobs_summary.jobs, "cat_collection" )
        downloaded_workflow = self._extract_and_download_workflow(
            dataset_collection_ids=[ jobs_summary.inputs["text_input1"]["hid"] ],
...
""")
        agent_ids = [ "cat1", "collection_split_on_column", "cat_list" ]
        job_ids = map( funcagents.partial(self._job_id_for_agent, jobs_summary.jobs ), agent_ids )
        downloaded_workflow = self._extract_and_download_workflow(
            dataset_ids=[ "1", "2" ],
            job_ids=job_ids,
        )
  ...
  text_input1:
    ...
    elements:
      ...
      - identifier: samp2
        content: "samp1\t30.0\nsamp2\t40.0\n"
""")
        agent_ids = [ "cat1", "collection_creates_pair", "cat_collection", "cat_list" ]
        job_ids = map( funcagents.partial(self._job_id_for_agent, jobs_summary.jobs ), agent_ids )
        downloaded_workflow = self._extract_and_download_workflow(
    ...
    def _job_id_for_agent( self, jobs, agent_id ):
        return self._job_for_agent( jobs, agent_id )[ "id" ]

    def _job_for_agent( self, jobs, agent_id ):
        agent_jobs = filter( lambda j: j["agent_id"] == agent_id, jobs )
        if not agent_jobs:
            assert False, "Failed to find job for agent %s" % agent_id
        # if len( agent_jobs ) > 1:
        #     assert False, "Found multiple jobs for agent %s" % agent_id
        return agent_jobs[ -1 ]
    ...
    def __run_random_lines_mapped_over_pair( self, history_id ):
        ...
        implicit_hdca1, job_id1 = self._run_agent_get_collection_and_job_id( history_id, "random_lines1", inputs1 )
        inputs2 = {
            "input": { "batch": True, "values": [ { "src": "hdca", "id": implicit_hdca1[ "id" ] } ] },
            "num_lines": 1
        }
        _, job_id2 = self._run_agent_get_collection_and_job_id( history_id, "random_lines1", inputs2 )
        return hdca, job_id1, job_id2

    def __assert_looks_like_randomlines_mapping_workflow( self, downloaded_workflow ):
        # Assert workflow is input connected to a agent step with one output
        # connected to another agent step.
        assert len( downloaded_workflow[ "steps" ] ) == 3
        collect_step_idx = self._assert_first_step_is_paired_input( downloaded_workflow )
        agent_steps = self._get_steps_of_type( downloaded_workflow, "agent", expected_len=2 )
        agent_step_idxs = []
        agent_input_step_idxs = []
        for agent_step in agent_steps:
            self._assert_has_key( agent_step[ "input_connections" ], "input" )
            input_step_idx = agent_step[ "input_connections" ][ "input" ][ "id" ]
            agent_step_idxs.append( agent_step[ "id" ] )
            agent_input_step_idxs.append( input_step_idx )

        assert collect_step_idx not in agent_step_idxs
        assert agent_input_step_idxs[ 0 ] == collect_step_idx
        assert agent_input_step_idxs[ 1 ] == agent_step_idxs[ 0 ]
    ...
    def __assert_looks_like_cat1_example_workflow( self, downloaded_workflow ):
        ...
        agent_step = self._get_steps_of_type( downloaded_workflow, "agent", expected_len=1 )[ 0 ]

        input1 = agent_step[ "input_connections" ][ "input1" ]
        input2 = agent_step[ "input_connections" ][ "queries_0|input2" ]
    ...
    def __cat_job_id( self, history_id ):
        data = dict( history_id=history_id, agent_id="cat1" )
        jobs_response = self._get( "jobs", data=data )
        self._assert_status_code_is( jobs_response, 200 )
        cat1_job_id = jobs_response.json()[ 0 ][ "id" ]
        return cat1_job_id

    def _run_agent_get_collection_and_job_id( self, history_id, agent_id, inputs ):
        run_output1 = self.dataset_populator.run_agent(
            agent_id=agent_id,
            inputs=inputs,
    ...
    def __check_workflow(
        ...
        if agent_ids is not None:
            agent_steps = self._get_steps_of_type( workflow, "agent" )
            found_steps = set(map(operator.itemgetter("agent_id"), agent_steps))
            expected_steps = set(agent_ids)
