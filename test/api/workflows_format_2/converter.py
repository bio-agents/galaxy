...
def transform_input(context, step, default_name):
    ...
    for attrib in ["collection_type", "parameter_type", "optional"]:
        if attrib in step:
            agent_state[attrib] = step[attrib]

    __populate_agent_state(step, agent_state)
...
def transform_agent(context, step):
    if "agent_id" not in step:
        raise Exception("Agent steps must define a agent_id.")
    ...
    agent_state = {
        # TODO: Galaxy should not require agent state actually specify a __page__.
        "__page__": 0,
    }
    ...
    if "state" in step:
        ...
        for key, value in step_state.iteritems():
            agent_state[key] = json.dumps(value)
        del step["state"]
...
def run_agent_to_step(conversion_context, step, run_action):
    agent_description = conversion_context.galaxy_interface.import_agent(
        run_action
    )
    step["type"] = "agent"
    step["agent_id"] = agent_description["agent_id"]
    step["agent_version"] = agent_description["agent_version"]
    step["agent_hash"] = agent_description["agent_hash"]
...
def __populate_agent_state(step, agent_state):
    step["agent_state"] = json.dumps(agent_state)
