...
class AgentDataApiTestCase( api.ApiTestCase ):

    def test_admin_only( self ):
        index_response = self._get( "agent_data", admin=False )
        self._assert_status_code_is( index_response, 403 )

    def test_list(self):
        index_response = self._get( "agent_data", admin=True )
    ...
    def test_show(self):
        show_response = self._get( "agent_data/testalpha", admin=True )
        ...
        assert first_entry[2].endswith("test/functional/agent-data/data1/entry.txt")

    def test_show_field(self):
        show_field_response = self._get( "agent_data/testalpha/fields/data1", admin=True )
    ...
    def test_download_field_file(self):
        show_field_response = self._get( "agent_data/testalpha/fields/data1/files/entry.txt", admin=True )
        self._assert_status_code_is( show_field_response, 200 )
    ...
    #     show_response = self._get( "agent_data/testbeta", admin=True )
    ...
    #     payload = dataset_populator.run_agent_payload(
    ...
    #     url = self._api_url( "agent_data/testbeta?key=%s" % self.galaxy_interactor.master_api_key )
