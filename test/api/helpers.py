...
def skip_without_agent( agent_id ):
    """ Decorate an API test method as requiring a specific agent,
    have nose skip the test case is the agent is unavailable.
    """

    def method_wrapper( method ):

        def get_agent_ids( api_test_case ):
            index = api_test_case.galaxy_interactor.get( "agents", data=dict(in_panel=False) )
            agents = index.json()
        ...
        def wrapped_method( api_test_case, *args, **kwargs ):
            if agent_id not in get_agent_ids( api_test_case ):
...
class TestsDatasets:
    ...
    def _run_agent_payload( self, agent_id, inputs, history_id, **kwds ):
        return DatasetPopulator( self.galaxy_interactor ).run_agent_payload( agent_id, inputs, history_id, **kwds )
...
class BaseDatasetPopulator( object ):
    ...
    def new_dataset( self, history_id, content='TestData123', wait=False, **kwds ):
        ...
        run_response = self._post( "agents", data=payload ).json()
        if wait:
    ...
    def run_agent_payload( self, agent_id, inputs, history_id, **kwds ):
    ...
    def run_agent( self, agent_id, inputs, history_id, **kwds ):
        payload = self.run_agent_payload( agent_id, inputs, history_id, **kwds )
        agent_response = self._post( "agents", data=payload )
        api_asserts.assert_status_code_is( agent_response, 200 )
...
class BaseWorkflowPopulator( object ):
    ...
    def load_workflow( self, name, content=workflow_str, add_pja=False ):
        ...
        workflow[ "name" ] = name
        if add_pja:
            agent_step = workflow[ "steps" ][ "2" ]
            agent_step[ "post_job_actions" ][ "RenameDatasetActionout_file1" ] = dict(
