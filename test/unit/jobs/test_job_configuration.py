...
class JobConfXmlParserTestCase( unittest.TestCase ):
    ...
    def test_load_agent( self ):
        self.__with_advanced_config()
        baz_agent = self.job_config.agents[ "baz" ][ 0 ]
        assert baz_agent.id == "baz"
        assert baz_agent.handler == "special_handlers"
        assert baz_agent.destination == "bigmem"

    def test_load_agent_params( self ):
        self.__with_advanced_config()
        foo_agent = self.job_config.agents[ "foo" ][ 0 ]
        assert foo_agent.params[ "source" ] == "trackster"
