...
from galaxy.agents import evaluation

from agents_support import UsesApp
# from agents_support import MockAgent

# from ..agents_and_jobs_helpers import MockApp
...
class BaseWrapperTestCase(UsesApp):
    ...
    def _prepared_wrapper(self):
        ...
        with _mock_agent_evaluator(MockEvaluator):
            wrapper.prepare()
            yield wrapper
...
class MockApp(object):
    ...
    def __init__(self, object_store, test_directory, model_objects):
        ...
        self.agentbox = MockAgentbox(MockAgent(self))
        self.config = Bunch(
...
class MockAgentbox(object):

    def __init__(self, test_agent):
        self.test_agent = test_agent

    def get(self, agent_id, default=None):
        assert agent_id == TEST_TOOL_ID
        return self.test_agent

    def get_agent( self, agent_id, agent_version, exact=False ):
...
# dependnecy.
@contextmanager
def _mock_agent_evaluator(mock_constructor):
    name = evaluation.AgentEvaluator.__name__
