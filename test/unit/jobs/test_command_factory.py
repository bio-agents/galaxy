...
MOCK_COMMAND_LINE = "/opt/galaxy/agents/bowtie /mnt/galaxyData/files/000/input000.dat"
...
class TestCommandFactory(TestCase):
    ...
    def test_shell_commands(self):
        self.include_work_dir_outputs = False
        dep_commands = [". /opt/galaxy/agents/bowtie/default/env.sh"]
    ...
    def test_shell_commands_external(self):
        ...
        dep_commands = [". /opt/galaxy/agents/bowtie/default/env.sh"]
        self.job_wrapper.dependency_shell_commands = dep_commands
        self.__assert_command_is( "%s/agent_script.sh" % self.job_wrapper.working_directory)
        self.__assert_agent_script_is( "#!/bin/sh\n%s; %s" % (dep_commands[0], MOCK_COMMAND_LINE) )
    ...
    def __assert_agent_script_is(self, expected_command):
        self.assertEqual(open(self.__agent_script, "r").read(), expected_command)

    @property
    def __agent_script(self):
        return os.path.join(self.job_dir, "agent_script.sh")
