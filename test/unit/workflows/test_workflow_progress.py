...
steps:
  - type: "data_input"
    agent_inputs: {"name": "outer_input"}
  - type: "subworkflow"
    subworkflow:
       steps:
          - type: "data_input"
            agent_inputs: {"name": "inner_input"}
          - type: "agent"
            agent_id: "cat1"
            input_connections:
            -  input_name: "input1"
...
class WorkflowProgressTestCase( unittest.TestCase ):
    ...
    def test_replacement_for_agent_input( self ):
        self._setup_workflow(TEST_WORKFLOW_YAML)
        hda = model.HistoryDatasetAssociation()
        ...
        replacement = progress.replacement_for_agent_input(self._step(2), MockInput(), "input1")
        assert replacement is hda

    def test_connect_agent_output( self ):
        self._setup_workflow(TEST_WORKFLOW_YAML)
        hda = model.HistoryDatasetAssociation()
    ...
    def test_remaining_steps_with_progress(self):
        ...
        replacement = progress.replacement_for_agent_input(self._step(4), MockInput(), "input1")
    ...
    def test_subworkflow_progress(self):
        ...
        assert hda is subworkflow_progress.replacement_for_agent_input(
