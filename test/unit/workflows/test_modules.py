...
def test_valid_new_agent_has_no_errors():
    trans = MockTrans()
    mock_agent = mock.Mock()
    trans.app.agentbox.agents[ "cat1" ] = mock_agent
    agent_module = modules.module_factory.new( trans, "agent", content_id="cat1" )
    assert not agent_module.get_errors()
...
def test_data_input_modified_state():
    module = __from_state( {
        "type": "data_input",
        "agent_state": json.dumps({ "name": "Cool Input" }),
    } )
...
def test_data_input_compute_runtime_state_args():
    ...
    agent_state = module.encode_runtime_state( module.trans, module.test_step.state )

    hda = model.HistoryDatasetAssociation()
    with mock.patch('galaxy.workflow.modules.check_param') as check_method:
        check_method.return_value = ( hda, None )
        state, errors = module.compute_runtime_state( module.trans, { 'input': 4, 'agent_state': agent_state } )
...
def test_data_input_collection_modified_state():
    module = __from_state( {
        ...
        "agent_state": json.dumps({ "name": "Cool Input Collection", "collection_type": "list:paired" }),
    } )
...
def test_cannot_create_agent_modules_for_missing_agents():
    trans = MockTrans()
    exception = False
    try:
        modules.module_factory.new( trans, "agent", content_id="cat1" )
    except Exception:
        exception = True
    assert exception


def test_updated_agent_version():
    trans = MockTrans()
    mock_agent = __mock_agent(id="cat1", version="0.9")
    trans.app.agentbox.agents[ "cat1" ] = mock_agent
    module = __from_step(
        trans=trans,
        type="agent",
    ...
    # Make sure there is a warnin with agent id, old version,
    # and new version.
    for val in "cat1", "0.7", "0.9":
        assert val in module.version_changes[0]


def test_agent_version_same():
    trans = MockTrans()
    mock_agent = __mock_agent(id="cat1", version="1.0")
    trans.app.agentbox.agents[ "cat1" ] = mock_agent
    module = __from_step(
        trans=trans,
        type="agent",
...
steps:
  - type: "data_input"
    label: "input1"
    agent_inputs: {"name": "input1"}
  - type: "data_collection_input"
    agent_inputs: {"name": "input2"}
  - type: "agent"
    agent_id: "cat1"
    input_connections:
    -  input_name: "input1"
