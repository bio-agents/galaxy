...
from galaxy.agents.parameters import basic
...
class BaseParameterTestCase( TestCase, agents_support.UsesApp ):

    def setUp(self):
        self.setup_app( mock_model=False )
        self.mock_agent = bunch.Bunch(
            app=self.app,
            agent_type="default",
        )
    ...
    def _parameter_for(self, **kwds):
        ...
        return basic.AgentParameter.build( self.mock_agent, param_xml )
...
class ParameterParsingTestCase( BaseParameterTestCase ):
    ...
    established by the agents. The docs tests in basic.py are better but
    ...
    be much better - since that is a public API of the the agents.
    """
    ...
    def test_select_options_from(self):
        ...
        assert param.options.missing_agent_data_table_name == "cow"
    ...
    def test_agent_collection(self):
        param = self._parameter_for(xml="""
