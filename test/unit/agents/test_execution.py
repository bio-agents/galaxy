""" Test Agent execution and state handling logic.
"""
...
BASE_REPEAT_TOOL_CONTENTS = '''<agent id="test_agent" name="Test Agent">
...
# Agent with a repeat parameter, to test state update.
...
class AgentExecutionTestCase( TestCase, agents_support.UsesApp, agents_support.UsesAgents ):
    ...
    def setUp(self):
        ...
        self.agent_action = MockAction( self.trans )

    def tearDown(self):
        self.tear_down_app()

    def test_state_new( self ):
        self._init_agent( agents_support.SIMPLE_TOOL_CONTENTS )
        vars = self.__handle_with_incoming( param1="moo" )
        state = self.__assert_rerenders_agent_without_errors( vars )
    ...
    def test_execute( self ):
        ...
        assert self.agent_action.execution_call_args[ 0 ][ "rerun_remap_job_id" ] is None

    def test_execute_exception( self ):
        self._init_agent( agents_support.SIMPLE_TOOL_CONTENTS )
        self.agent_action.raise_exception( )
        try:
            self.__handle_with_incoming( param1="moo" )
        except Exception, e:
            assert 'Error executing agent' in str( e )

    def test_execute_errors( self ):
        self._init_agent( agents_support.SIMPLE_TOOL_CONTENTS )
        self.agent_action.return_error( )
        try:
            self.__handle_with_incoming( param1="moo" )
        except Exception, e:
    ...
    def test_redirect( self ):
        self._init_agent( agents_support.SIMPLE_TOOL_CONTENTS )
        self.agent_action.expect_redirect = True
    ...
    def test_remap_job( self ):
        ...
        self.__assert_executed( vars )
        assert self.agent_action.execution_call_args[ 0 ][ "rerun_remap_job_id" ] == 123
    ...
    def test_data_param_execute( self ):
        self._init_agent( agents_support.SIMPLE_CAT_TOOL_CONTENTS )
        hda = self.__add_dataset( 1 )
        # Execute agent action
        vars = self.__handle_with_incoming( param1=1 )
        self.__assert_executed( vars )
        # Agent 'executed' once, with hda as param1
        assert len( self.agent_action.execution_call_args ) == 1
        assert self.agent_action.execution_call_args[ 0 ][ "incoming" ][ "param1" ] == hda
    ...
    def test_simple_multirun_state_update( self ):
        ...
        self.__assert_rerenders_agent_without_errors( vars )
    ...
    def __handle_with_incoming( self, previous_state=None, **kwds ):
        """ Execute agent.handle_input with incoming specified by kwds
        ...
        if previous_state:
            ...
            incoming = kwds
        return self.agent.handle_input( trans=self.trans, incoming=incoming )

    def __to_incoming( self, state, **kwds ):
        new_incoming = {}
        params_to_incoming( new_incoming, self.agent.inputs, state.inputs, self.app )
        ...
        new_incoming[ "agent_state" ] = self.__state_to_string( state )
    ...
    def __assert_rerenders_agent_without_errors( self, vars ):
        self.__assert_no_errors( vars )
        return self.agent_action.execution_call_args[ 0 ][ "incoming" ]
