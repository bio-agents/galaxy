...
# For MockAgent
from galaxy.agents.parameters import params_from_strings
from galaxy.agents.parser.output_objects import AgentOutput
from galaxy.agents.parameters.grouping import Repeat
from galaxy.agents.parameters.grouping import Conditional
from galaxy.agents.parameters.grouping import ConditionalWhen
...
from galaxy.agents.parameters.basic import DataAgentParameter


# Test fixtures for Galaxy infrastructure.
from agents_support import UsesApp

# To Test:
# - param_file handling.
TEST_TOOL_DIRECTORY = "/path/to/the/agent"


class AgentEvaluatorTestCase(TestCase, UsesApp):

    def setUp(self):
        self.setup_app()
        self.agent = MockAgent(self.app)
        self.job = Job()
        self.job.history = History()
        self.job.parameters = [ JobParameter( name="thresh", value="4" ) ]
        self.evaluator = AgentEvaluator( self.app, self.agent, self.job, self.test_directory )
    ...
    def test_repeat_evaluation( self ):
        repeat = Repeat()
        repeat.name = "r"
        repeat.inputs = { "thresh": self.agent.test_thresh_param() }
        self.agent.set_params( { "r": repeat } )
        self.job.parameters = [ JobParameter( name="r", value='''[{"thresh": 4, "__index__": 0},{"thresh": 5, "__index__": 1}]''' ) ]
        self.agent._command_line = "prog1 #for $r_i in $r # $r_i.thresh#end for#"
        self._set_compute_environment()
        command_line, extra_filenames, _ = self.evaluator.build( )
    ...
    def test_conditional_evaluation( self ):
        ...
        parameter = SelectAgentParameter( self.agent, select_xml )

        conditional = Conditional()
        conditional.name = "c"
        conditional.test_param = parameter
        when = ConditionalWhen()
        when.inputs = { "thresh": self.agent.test_thresh_param() }
        when.value = "true"
        conditional.cases = [ when ]
        self.agent.set_params( { "c": conditional } )
        self.job.parameters = [ JobParameter( name="c", value='''{"thresh": 4, "always_true": "true", "__current_case__": 0}''' ) ]
        self.agent._command_line = "prog1 --thresh=${c.thresh} --test_param=${c.always_true}"
        self._set_compute_environment()
        command_line, extra_filenames, _ = self.evaluator.build( )
    ...
    def test_configfiles_evaluation( self ):
        self.agent.config_files.append( ( "conf1", None, "$thresh" ) )
        self.agent._command_line = "prog1 $conf1"
        self._set_compute_environment()
        command_line, extra_filenames, _ = self.evaluator.build( )
    ...
    def __test_arbitrary_path_rewriting( self ):
        ...
        parameter = SelectAgentParameter( self.agent, xml )
        ...
        self.agent.set_params( {
            "index_path": parameter
        } )
        self.agent._command_line = "prog1 $index_path.fields.path"
    ...
    def _assert_template_property_is(self, expression, value):
        self.agent._command_line = "test.exe"
        self.agent.config_files.append( ( "conf1", None, """%s""" % expression) )
        self._set_compute_environment()
    ...
    def _set_compute_environment(self, **kwds):
        ...
        assert "exec_before_job" in self.agent.hooks_called
...
class MockAgent( object ):
    ...
    def test_thresh_param( self ):
        ...
        return IntegerAgentParameter( self, elem )
