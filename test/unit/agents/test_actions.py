...
from galaxy.agents.actions import on_text_for_names
from galaxy.agents.actions import determine_output_format
...
# this.
DATA_IN_LABEL_TOOL_CONTENTS = '''<agent id="test_agent" name="Test Agent">
...
# Agent with two outputs - used to verify all datasets within same job get same
# object store id.
TWO_OUTPUTS = '''<agent id="test_agent" name="Test Agent">
    <command>echo "$param1" &lt; $out1</command>
    <inputs>
...
class DefaultAgentActionTestCase( unittest.TestCase, agents_support.UsesApp, agents_support.UsesAgents ):
    ...
    def setUp( self ):
        ...
        self.action = DefaultAgentAction()
    ...
    def test_output_label_data( self ):
        ...
        job, output = self._simple_execute(
            agents_support.SIMPLE_CAT_TOOL_CONTENTS,
            incoming,
        )
        self.assertEquals( output[ "out1" ].name, "Test Agent on data 2 and data 1" )
    ...
    def _simple_execute( self, contents=None, incoming=None ):
        if contents is None:
            contents = agents_support.SIMPLE_TOOL_CONTENTS
        if incoming is None:
            incoming = dict(param1="moo")
        self._init_agent( contents )
...
def quick_output(format, format_source=None, change_format_xml=None):
    test_output = AgentOutput( "test_output" )
