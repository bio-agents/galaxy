...
from galaxy.agents.parser import output_collection_def
...
class CollectPrimaryDatasetsTestCase( unittest.TestCase, agents_support.UsesApp, agents_support.UsesAgents ):
    ...
    def _collect( self, job_working_directory=None ):
        ...
        return self.agent.collect_primary_datasets( self.outputs, job_working_directory, "txt" )

    def _replace_output_collectors( self, xml_str ):
        # Rewrite agent as if it had been created with output containing
        # supplied dataset_collector elem.
        elem = util.parse_xml_string( xml_str )
        self.agent.outputs[ DEFAULT_TOOL_OUTPUT ].dataset_collector_descriptions = output_collection_def.dataset_collector_descriptions_from_elem( elem )
