...
def main(argv=None):
    ...
    target = funcagents.partial(_run, args, gi, workflow_id, uuid)
    threads = []
...
def _workflow_struct_simple(args, input_uuid):
    workflow_struct = [
        ...
        {"agent_id": "cat1", "state": {"input1": _link(0)}}
    ]
    ...
    for i in range(workflow_depth):
        ...
        workflow_struct.append(
            {"agent_id": "cat1", "state": {"input1": _link(link)}}
        )
...
def _workflow_struct_two_outputs(args, input_uuid):
    workflow_struct = [
        {"type": "input_collection", "uuid": input_uuid},
        {"agent_id": "cat1", "state": {"input1": _link(0), "input2": _link(0)}}
    ]
    ...
    for i in range(workflow_depth):
        ...
        workflow_struct.append(
            {"agent_id": "cat1", "state": {"input1": _link(link1), "input2": _link(link2)}}
        )
    return workflow_struct
