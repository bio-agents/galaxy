...
def stage_data_in_history( galaxy_interactor, all_test_data, history, shed_agent_id=None ):
    # Upload any needed files
    upload_waits = []

    if UPLOAD_ASYNC:
        for test_data in all_test_data:
            upload_waits.append( galaxy_interactor.stage_data_async( test_data, history, shed_agent_id ) )
        for upload_wait in upload_waits:
            upload_wait()
    else:
        for test_data in all_test_data:
            upload_wait = galaxy_interactor.stage_data_async( test_data, history, shed_agent_id )
            upload_wait()
...
class GalaxyInteractorApi( object ):
    ...
    def verify_output( self, history_id, jobs, output_data, output_testdef, shed_agent_id, maxseconds ):
        ...
        self.verify_output_dataset( history_id=history_id, hda_id=hid, outfile=outfile, attributes=attributes, shed_agent_id=shed_agent_id )
        ...
        for designation, ( primary_outfile, primary_attributes ) in primary_datasets.iteritems():
            ...
            self.verify_output_dataset( history_id, primary_hda_id, primary_outfile, primary_attributes, shed_agent_id=shed_agent_id )

    def wait_for_jobs( self, history_id, jobs, maxseconds ):
        for job in jobs:
            self.wait_for_job( job[ 'id' ], history_id, maxseconds )

    def verify_output_dataset( self, history_id, hda_id, outfile, attributes, shed_agent_id ):
        fetcher = self.__dataset_fetcher( history_id )
        self.twill_test_case.verify_hid( outfile, hda_id=hda_id, attributes=attributes, dataset_fetcher=fetcher, shed_agent_id=shed_agent_id )
    ...
    def __output_id( self, output_data ):
        # Allow data structure coming out of agents API - {id: <id>, output_name: <name>, etc...}
    ...
    def stage_data_async( self, test_data, history_id, shed_agent_id, async=True ):
        ...
        for elem in test_data.get('metadata', []):
            agent_input["files_metadata|%s" % elem.get( 'name' )] = elem.get( 'value' )

        composite_data = test_data[ 'composite_data' ]
        if composite_data:
            files = {}
            for i, composite_file in enumerate( composite_data ):
                file_name = self.twill_test_case.get_filename( composite_file.get( 'value' ), shed_agent_id=shed_agent_id )
            ...
            file_name = self.twill_test_case.get_filename( fname, shed_agent_id=shed_agent_id )
        ...
        submit_response_object = self.__submit_agent( history_id, "upload1", agent_input, extra_data={"type": "upload_dataset"}, files=files )
    ...
    def run_agent( self, testdef, history_id ):
        # We need to handle the case where we've uploaded a valid compressed file since the upload
        # agent will have uncompressed it on the fly.
        ...
        submit_response = self.__submit_agent( history_id, agent_id=testdef.agent.id, agent_input=inputs_tree )
        ...
        try:
            ...
            message = "Error creating a job for these agent inputs - %s" % submit_response_object[ 'err_msg' ]
            raise RunAgentException( message, inputs_tree )
    ...
    def __submit_agent( self, history_id, agent_id, agent_input, extra_data={}, files=None ):
        data = dict(
            history_id=history_id,
            agent_id=agent_id,
            inputs=dumps( agent_input ),
            **extra_data
        )
        return self._post( "agents", files=files, data=data )
...
class RunAgentException(Exception):

    def __init__(self, message, inputs=None):
        super(RunAgentException, self).__init__(message)
        self.inputs = inputs
