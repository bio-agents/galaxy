...
class TwillTestCase( unittest.TestCase ):
    ...
    def setUp( self ):
        ...
        self.agent_shed_test_file = os.environ.get( 'GALAXY_TOOL_SHED_TEST_FILE', None )
        if self.agent_shed_test_file:
            f = open( self.agent_shed_test_file, 'r' )
            text = f.read()
            f.close()
            self.shed_agents_dict = loads( text )
        else:
    ...
    def convert_format( self, hda_id, target_type ):
        ...
        self.wait()  # wait for the format convert agent to finish before returning
    ...
    def get_filename( self, filename, shed_agent_id=None ):
        if shed_agent_id and self.shed_agents_dict:
            file_dir = self.shed_agents_dict[ shed_agent_id ]
            if file_dir:
    ...
    def library_wait( self, library_id, cntrller='library_admin', maxiter=90 ):
        """Waits for the agents to finish"""
        count = 0
        sleep_amount = 1
        while count < maxiter:
            count += 1
    ...
    def makeTfname(self, fname=None):
        """safe temp name - preserve the file extension for agents that interpret it"""
    ...
    def run_ucsc_main( self, track_params, output_params ):
        """Gets Data From UCSC"""
        agent_id = "ucsc_table_direct1"
        galaxy_url = "%s/agent_runner/index?" % self.url
        track_params.update( dict( GALAXY_URL=galaxy_url, hgta_compressType='none', agent_id=agent_id ) )
    ...
    def submit_form( self, form_no=0, button="runagent_btn", **kwd ):
        ...
        for i, f in enumerate( self.showforms() ):
            if i == form_no:
                break
        # To help with debugging a agent, print out the form controls when the test fails
            ...
            if hc_prefix not in str( control ):
                try:
                    ...
                    if hasattr( control, 'attrs' ) and 'refresh_on_change' in control.attrs.keys():
                        changed = False
                        # For DataAgentParameter, control.value is the HDA id, but kwd contains the filename.
        ...
        for control_name, control_value in kwd.items():
            ...
            if control.is_of_kind( "text" ):
                ...
                try:
                    if control.is_of_kind( "multilist" ):
                        ...
                        for elem in control_value:
                            try:
                                ...
                                try:
                                    # Galaxy truncates long file names in the dataset_collector in galaxy/agents/parameters/basic.py
                                    if len( elem ) > 30:
                    ...
                    errmsg += "control: %s\n" % str( control )
                    errmsg += "If the above control is a DataAgentparameter whose data type class does not include a sniff() method,\n"
    ...
    def upload_library_dataset( self, cntrller, library_id, folder_id, filename='', server_dir='', replace_id='',
        ...
        if upload_option == 'import_from_history':
            ...
            for check_str in strings_displayed:
                self.check_page_for_string( check_str )
            tc.submit( "runagent_btn" )
        # Give the files some time to finish uploading
        self.library_wait( library_id )

    def upload_file( self, filename, ftype='auto', dbkey='unspecified (?)', space_to_tab=False, metadata=None, composite_data=None, name=None, shed_agent_id=None, wait=True ):
        """
        Uploads a file.  If shed_agent_id has a value, we're testing agents migrated from the distribution to the agent shed,
        so the agent-data directory of test data files is contained in the installed agent shed repository.
        """
        self.visit_url( "%s/agent_runner?agent_id=upload1" % self.url )
        try:
            self.refresh_form( "file_type", ftype )  # Refresh, to support composite files
            tc.fv( "agent_form", "dbkey", dbkey )
            if metadata:
                for elem in metadata:
                    tc.fv( "agent_form", "files_metadata|%s" % elem.get( 'name' ), elem.get( 'value' ) )
            if composite_data:
                for i, composite_file in enumerate( composite_data ):
                    filename = self.get_filename( composite_file.get( 'value' ), shed_agent_id=shed_agent_id )
                    tc.formfile( "agent_form", "files_%i|file_data" % i, filename )
                    tc.fv( "agent_form", "files_%i|space_to_tab" % i, composite_file.get( 'space_to_tab', False ) )
            else:
                filename = self.get_filename( filename, shed_agent_id=shed_agent_id )
                tc.formfile( "agent_form", "file_data", filename )
                tc.fv( "agent_form", "space_to_tab", space_to_tab )
                if name:
                    # NAME is a hidden form element, so the following prop must
                    # set to use it.
                    tc.config("readonly_controls_writeable", 1)
                    tc.fv( "agent_form", "NAME", name )
            tc.submit( "runagent_btn" )
        except AssertionError, err:
    ...
    def upload_url_paste( self, url_paste, ftype='auto', dbkey='unspecified (?)' ):
        ...
        self.visit_url( "/agent_runner/index?agent_id=upload1" )
        try:
            self.refresh_form( "file_type", ftype )  # Refresh, to support composite files
            tc.fv( "agent_form", "dbkey", dbkey )
            tc.fv( "agent_form", "url_paste", url_paste )
            tc.submit( "runagent_btn" )
        except Exception, e:
    ...
    def verify_composite_datatype_file_content( self, file_name, hda_id, base_name=None, attributes=None, dataset_fetcher=None, shed_agent_id=None ):
        dataset_fetcher = dataset_fetcher or self.__default_dataset_fetcher()
        local_name = self.get_filename( file_name, shed_agent_id=shed_agent_id )
    ...
    def verify_dataset_correctness( self, filename, hid=None, wait=True, maxseconds=120, attributes=None, shed_agent_id=None ):
        ...
        if filename is not None and self.is_zipped( filename ):
            ...
            self.verify_hid( filename, hid=hid, hda_id=hda_id, attributes=attributes, shed_agent_id=shed_agent_id)

    def verify_extra_files_content( self, extra_files, hda_id, dataset_fetcher, shed_agent_id=None ):
        files_list = []
        for extra_type, extra_value, extra_name, extra_attributes in extra_files:
            if extra_type == 'file':
                ...
                for filename in os.listdir( self.get_filename( extra_value, shed_agent_id=shed_agent_id ) ):
        ...
        for filename, filepath, attributes in files_list:
            self.verify_composite_datatype_file_content( filepath, hda_id, base_name=filename, attributes=attributes, dataset_fetcher=dataset_fetcher, shed_agent_id=shed_agent_id )

    def verify_hid( self, filename, hda_id, attributes, shed_agent_id, hid="", dataset_fetcher=None):
        dataset_fetcher = dataset_fetcher or self.__default_dataset_fetcher()
        ...
        if filename is not None:
            local_name = self.get_filename( filename, shed_agent_id=shed_agent_id )
            ...
            try:
                ...
                if compare == 'diff':
                    ...
                    raise Exception( 'Unimplemented Compare type: %s' % compare )
                if extra_files:
                    self.verify_extra_files_content( extra_files, hda_id, shed_agent_id=shed_agent_id, dataset_fetcher=dataset_fetcher )
            except AssertionError, err:
    ...
    def wait_for( self, func, **kwd ):
        ...
        if exceeded:
            message = 'Agent test run exceeded walltime [total %s, max %s], terminating.' % (slept, walltime_exceeded)
            log.info(message)
    ...
    def _check_command(self, command, description):
        # TODO: also collect ``which samagents`` and ``samagents --version``
    ...
    def _bam_to_sam( self, local_name, temp_name ):
        ...
        os.close( fd )
        command = 'samagents view -h -o "%s" "%s"' % ( temp_local.name, local_name  )
        self._check_command( command, 'Converting local (test-data) bam to sam' )
        command = 'samagents view -h -o "%s" "%s"' % ( temp_temp, temp_name  )
