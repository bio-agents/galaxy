...
# Stats for Nate's runs of the Bowtie 2 agent installed from the Agent Shed (all
# versions):
% ./runtime_stats.py -c galaxy.ini -u nate@bx.psu.edu 'agentshed.g2.bx.psu.edu/repos/devteam/bowtie2/bowtie2/'

# Stats for all runs of the Bowtie 2 agent installed from the Agent Shed (version
# 0.4 only):
% ./runtime_stats.py -c galaxy.ini 'agentshed.g2.bx.psu.edu/repos/devteam/bowtie2/bowtie2/0.4'

# Stats for all runs of the Bowtie 2 agent installed from the Agent Shed but we
# don't feel like figuring out or typing the long ID (matches any agent with
...
def parse_arguments():
    ...
    parser.add_argument('agent_id', help='Agent (by ID) to collect stats about')
    parser.add_argument('--like',
                        action='store_true',
                        default=False,
                        help='Use SQL `LIKE` operator to find '
                             'a shed-installed agent using the agent\'s '
                             '"short" id')
    ...
    if args.like and '/' in args.agent_id:
        print('ERROR: Do not use --like with a agent shed agent id (the agent '
...
def query(agent_id=None, user=None, like=None, source='metrics',
    ...
    if like:
        query_agent_id = '%%/%s/%%' % agent_id
    elif '/' in agent_id and not re.match('\d+\.\d+', agent_id.split('/')[-1]):
        query_agent_id = '%s%%' % agent_id
        like = True
    else:
        query_agent_id = agent_id

    sql_args = [query_agent_id]

    if like:
        agent_clause = "AND j.agent_id LIKE %s"
    else:
        agent_clause = "AND j.agent_id = %s"
    ...
    sql = sql.format(agent_clause=agent_clause, user_clause=user_clause,
