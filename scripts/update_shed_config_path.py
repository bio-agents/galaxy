...
import galaxy.model.agent_shed_install.mapping as mapping


def main( opts, session, model ):
    '''
    Find all agent shed repositories with the bad path and update with the correct path.
    '''
    for row in session.query( model.AgentShedRepository ).all():
...
if __name__ == '__main__':
    ...
    parser.add_argument( '--from', dest='bad_filename', required=True, help="The old, invalid path to the shed_agent_conf.xml or migrated_agents_conf.xml file." )
    parser.add_argument( '--to', dest='good_filename', required=True, help="The updated path to the shed_agent_conf.xml or migrated_agents_conf.xml file." )
