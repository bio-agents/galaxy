...
<%def name="handle_refresh_frames()">
    ...
    %endif
    %if 'agents' in refresh_frames:
        if ( parent.frames && Galaxy.agentPanel ) {
            // FIXME: refreshing the agent menu does not work with new JS-based approach,
            // but refreshing the agent menu is not used right now, either.
