        ...
        %if input.name in param_values:
            ...
            %elif input.visible:
                ...
                if  hasattr( input, "label" ) and input.label:
                    ...
                    #value for label not required, fallback to input name (same as agent panel)
                    label = input.name
                %>
                <tr>
...
<table class="tabletip">
    ...
    <tbody>
        ...
        %if job:
            <tr><td>Galaxy Agent ID:</td><td>${ job.agent_id | h }</td></tr>
            <tr><td>Galaxy Agent Version:</td><td>${ job.agent_version | h }</td></tr>
        %endif
        <tr><td>Agent Version:</td><td>${hda.agent_version | h}</td></tr>
        <tr><td>Agent Standard Output:</td><td><a href="${h.url_for( controller='dataset', action='stdout', dataset_id=encoded_hda_id )}">stdout</a></td></tr>
        <tr><td>Agent Standard Error:</td><td><a href="${h.url_for( controller='dataset', action='stderr', dataset_id=encoded_hda_id )}">stderr</a></td></tr>
        %if job:
            <tr><td>Agent Exit Code:</td><td>${ job.exit_code | h }</td></tr>
        %endif
...
<table class="tabletip">
    ...
    <tbody>
        % if params_objects and agent:
            ${ inputs_recursive( agent.inputs, params_objects, depth=1, upgrade_messages=upgrade_messages ) }
