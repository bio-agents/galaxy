...
<html>
    ...
    <body>
        ...
        %if job:
            ...
            %if job.traceback:
                The Galaxy framework encountered the following error while attempting to run the agent:
                <pre>${ util.unicodify( job.traceback ) | h}</pre>
            %endif
            %if job.stderr or job.info:
                Agent execution generated the following error message:
                %if job.stderr:
                ...
                %endif
            %else:
                Agent execution did not generate any error messages.
            %endif
            %if job.stdout:
                The agent produced the following additional output:
                <pre>${ util.unicodify( job.stdout ) | h}</pre>
            %endif
        %else:
            The agent did not create any additional job / error info.
        %endif
        <%
            if trans.user:
        ...
        <div class="agentForm">
            <div class="agentFormTitle">Error Report</div>
