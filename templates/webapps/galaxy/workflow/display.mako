    ...
    <table class="annotated-item">
        ...
        %for i, step in enumerate( steps ):
            ...
            %if step.type == 'agent' or step.type is None:
              ...
              <div class="agentForm">
                %if agent:
                  <div class="agentFormTitle">Step ${int(step.order_index)+1}: ${agent.name | h}</div>
                  <div class="agentFormBody">
                    ${do_inputs( agent.inputs, step.state.inputs, "", step )}
                  </div>
                %else:
                  <div class="agentFormTitle">Step ${int(step.order_index)+1}: Unknown Agent with id '${step.agent_id | h}'</div>
                %endif
              </div>
            %else:
            ...
            <% module = step.module %>
              <div class="agentForm">
                  <div class="agentFormTitle">Step ${int(step.order_index)+1}: ${module.name | h}</div>
                  <div class="agentFormBody">
