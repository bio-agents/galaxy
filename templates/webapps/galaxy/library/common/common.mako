...
<%def name="render_upload_form( cntrller, upload_option, action, library_id, folder_id, replace_dataset, file_formats, dbkeys, space_to_tab, link_data_only, widgets, roles_select_list, history, show_deleted )">
    ...
    %if upload_option in [ 'upload_file', 'upload_directory', 'upload_paths' ]:
        <div class="agentForm" id="upload_library_dataset_agent_form">
            <%
                if upload_option == 'upload_directory':
                    agent_form_title = 'Upload a directory of files'
                elif upload_option == 'upload_paths':
                    agent_form_title = 'Upload files from filesystem paths'
                else:
                    agent_form_title = 'Upload files'
            %>
            <div class="agentFormTitle">${agent_form_title | h}</div>
            <div class="agentFormBody">
                <form name="upload_library_dataset" id="upload_library_dataset" action="${action}" enctype="multipart/form-data" method="post">
                    <input type="hidden" name="agent_id" value="upload1"/>
                    <input type="hidden" name="agent_state" value="None"/>
                    ...
                    %if upload_option in ( 'upload_directory', 'upload_paths' ):
                        <div class="form-row">
                            ...
                            <div class="agentParamHelp" style="clear: both;">
                                Normally data uploaded with this agent is copied into Galaxy's configured "file_path" location where Galaxy
                    ...
                    %endif
                    <div class="form-row">
                        <input type="submit" class="primary-button" name="runagent_btn" value="Upload to library"/>
                    </div>
                </form>
            </div>
        </div>
    %elif upload_option == 'import_from_history':
        <div class="agentForm">
            <div class="agentFormTitle">Active datasets in your current history (${ util.unicodify( history.name ) | h})</div>
            <div class="agentFormBody">
