...
<form name="move_library_item" action="${h.url_for( controller='library_common', action='move_library_item', cntrller=cntrller, item_type=item_type, make_target_current=make_target_current, use_panels=use_panels, show_deleted=show_deleted )}" method="post">
    <div class="agentForm" style="float: left; width: 45%; padding: 0px;">
    ...
    <div class="agentForm" style="float: right; width: 45%; padding: 0px;">
        %if target_library:
            <div class="agentFormTitle">Select folder within data library: ${h.truncate( target_library.name, 30 ) | h}</div>
        %else:
            <div class="agentFormTitle">Select a data library</div>
        %endif
        <div class="agentFormBody">
            %if target_library:
                <div class="form-row">
