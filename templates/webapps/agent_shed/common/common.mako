<%def name="common_misc_javascripts()">
    <script type="text/javascript">
        ...
        function checkAllInstalledAgentDependencyIdFields()
        {
        ...
        function checkAllUninstalledAgentDependencyIdFields()
        {
            var chkAll = document.getElementById( 'checkAllUninstalled' );
            var name = 'uninstalled_agent_dependency_ids';
            checkAllFields( chkAll, name );
        }
    </script>
</%def>
...
<%def name="render_deprecated_repository_dependencies_message( deprecated_repository_dependency_tups )">
    <div class="warningmessage">
        <%
            from agent_shed.util.common_util import parse_repository_dependency_tuple
            msg = '<ul>'
...
<%def name="render_multiple_heads_message( heads )">
    <div class="warningmessage">
        <%
            from agent_shed.util.hg_util import get_revision_label_from_ctx
            heads_str = ''
            for ctx in heads:
                heads_str += '%s<br/>' % get_revision_label_from_ctx( ctx, include_date=True )
        %>
        Contact the administrator of this Agent Shed as soon as possible and let them know that
