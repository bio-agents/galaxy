...
<%def name="render_agent_shed_repository_actions( repository, metadata=None, changeset_revision=None )">
    <%
        from agent_shed.util.review_util import can_browse_repository_reviews, changeset_revision_reviewed_by_user, get_review_by_repository_id_changeset_revision_user_id
...
<%def name="render_galaxy_repository_actions( repository=None )">
    ...
    <ul class="manage-table-actions">
        %if repository:
            ...
            <li><a class="action-button" target="galaxy_main" href="${h.url_for( controller='repository', action='preview_agents_in_changeset', repository_id=trans.security.encode_id( repository.id ), changeset_revision=changeset_revision )}">Browse repository</a></li>
            <li><a class="action-button" id="repository-${repository.id}-popup" class="menubutton">Agent Shed Actions</a></li>
            <div popupmenu="repository-${repository.id}-popup">
                ...
                <a class="action-button" target="galaxy_main" href="${h.url_for( controller='repository', action='find_agents' )}">Search for valid agents</a>
