...
/galaxy/software/agent-data/gatk
"
...
case $type in
	external*)
		...
		for sample in agent-data/*.sample; do
	...
	*)
		...
		for link in $LINKS; do
		    echo "Linking $link"
		    rm -f agent-data/`basename $link`
		    ln -sf $link agent-data
		done
		
		if [ -d "$JARS" ]; then
		    echo "Linking $JARS"
		    rm -f agent-data/shared/jars
		    ln -sf $JARS agent-data/shared/jars
		fi
		;;
esac
...
for build in hg17 hg18; do
    if [ -f "test-data/chrom/$build.len" ]; then
        echo "Copying test-data/chrom/$build.len to agent-data/shared/ucsc/chrom/"
        mkdir -p agent-data/shared/ucsc/chrom
        cp test-data/chrom/$build.len agent-data/shared/ucsc/chrom/$build.len
    fi
done
...
echo "Appending agent-data/shared/ucsc/builds.txt.buildbot to agent-data/shared/ucsc/builds.txt"
cat agent-data/shared/ucsc/builds.txt.buildbot >> agent-data/shared/ucsc/builds.txt
